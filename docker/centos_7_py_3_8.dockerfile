...
# Install struphy
RUN python3.8 -m pip install struphy

RUN scl enable devtoolset-8 'struphy compile'

# Create a new user and change ownership
RUN adduser centos_7_py_3_8_user \
    && chown -R centos_7_py_3_8_user:centos_7_py_3_8_user /usr/local/lib/python3.8/site-packages/struphy/

# Switch to non-root user
USER centos_7_py_3_8_user
