...
#==========================================================================================================
@external_call
@types('double[:,:](order=F)','int[:]','int[:,:](order=F)','int[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)')
def accumulation_step1(particles, p0, spans0, Nbase, T1, T2, T3, tt1, tt2, tt3, mapping, B_part, mat12, mat13, mat23):
                    ...
                    for il3 in range(p0_3 + 1):
                        ...
                        for il2 in range(p0_2 + 1):
                            ...
                            for il1 in range(p1_1 + 1):
                                ...
                                mat13[i1, i2, i3, j1, j2, j3] += bi1
                                
                                
        # add contribution to 23 component (NDN NND)   
        for jl3 in range(p1_3 + 1):
            j3  = (span1_3 - jl3)%Nbase[2]
            bj3 = temp23 * D3[p1_3 - jl3]
            for jl2 in range(p0_2 + 1):
                j2  = (span0_2 - jl2)%Nbase[1]
                bj2 = bj3 * N2[p0_2 - jl2]
                for jl1 in range(p0_1 + 1):
...
#==========================================================================================================
@external_call
@types('double[:,:](order=F)','int[:]','int[:,:](order=F)','int[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:,:,:,:](order=F)','double[:,:,:](order=F)','double[:,:,:](order=F)','double[:,:,:](order=F)')
def accumulation_step3(particles, p0, spans0, Nbase, T1, T2, T3, tt1, tt2, tt3, mapping, B_part, mat11, mat12, mat13, mat22, mat23, mat33, vec1, vec2, vec3):
    ...
    for ip in range(np):
        ...
        matrix_matrix(temp_mat1, DFinv, temp_mat_vec)
        matrix_vector(temp_mat_vec, v, temp_vec)
        ...
        for jl3 in range(p0_3 + 1):
            ...
            for jl2 in range(p0_2 + 1):
                ...
                for jl1 in range(p1_1 + 1):
                    ...
                    bj1 = bj2 * D1[p1_1 - jl1]
                    vec1[j1, j2, j3] += bj1 * temp1
                    for il3 in range(p0_3 + 1):
                        i3  = (span0_3 - il3)%Nbase[2]
                        bi3 = bj1 * N3[p0_3 - il3] * temp11
                        for il2 in range(p0_2 + 1):
                            i2  = (span0_2 - il2)%Nbase[1]
                            bi2 = bi3 * N2[p0_2 - il2] 
                            for il1 in range(p1_1 + 1):
        ...
        for jl3 in range(p0_3 + 1):
            ...
            for jl2 in range(p1_2 + 1):
                ...
                for jl1 in range(p0_1 + 1):
                    j1  = (span0_1 - jl1)%Nbase[0]
                    bj1 = bj2 * N1[p0_1 - jl1]
                    vec2[j1, j2, j3] += bj1 * temp2
                    for il3 in range(p0_3 + 1):
                        i3  = (span0_3 - il3)%Nbase[2]
                        bi3 = bj1 * N3[p0_3 - il3] * temp22 
                        for il2 in range(p1_2 + 1):
                            i2  = (span1_2 - il2)%Nbase[1]
                            bi2 = bi3 * D2[p1_2 - il2] 
                            for il1 in range(p0_1 + 1):
        ...
        for jl3 in range(p1_3 + 1):
            ...
            for jl2 in range(p0_2 + 1):
                ...
                for jl1 in range(p0_1 + 1):
                    j1  = (span0_1 - jl1)%Nbase[0]
                    bj1 = bj2 * N1[p0_1 - jl1]
                    vec3[j1, j2, j3] += bj1 * temp3
                    for il3 in range(p1_3 + 1):
                        i3  = (span1_3 - il3)%Nbase[2]
                        bi3 = bj1 * D3[p1_3 - il3] * temp33 
                        for il2 in range(p0_2 + 1):
                            i2  = (span0_2 - il2)%Nbase[1]
                            bi2 = bi3 * N2[p0_2 - il2] 
                            for il1 in range(p0_1 + 1):
