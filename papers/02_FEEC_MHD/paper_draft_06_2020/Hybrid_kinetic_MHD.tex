...
In this work we consider CC with linearized MHD. This is sufficient for describing the three fundamental types of waves in ideal MHD, that are, the slow and fast magnetosonic wave and the shear Alfv\'{e}n wave. Assuming that MHD waves are small perturbations (denoted by tildes) with respect to a pre-defined equilibrium state (denoted by the subscript "eq") satisfying the condition $\nabla p_\text{eq}=(\nabla\times \bo{B}_\text{eq}-\bo{J}_\text{h,eq})\times \bo{B}_\text{eq}$, we make the ansatzes $\rho=\rho_\text{eq} + \tilde{\rho}$, $\bo{U}=\tilde{\bo{U}}$ (zero-flow equilibrium), $p=p_\text{eq}+\tilde{p}$ and $\bo{B}=\bo{B}_\text{eq}+\tilde{\bo{B}}$ for the MHD variables, plug it in (\ref{eq_MHD}) and neglect all nonlinear terms expect for the ones involving the kinetic particles. Hence we keep nonlinear coupling terms between the fluid and kinetic species. The partially linearized model then reads
\begin{subequations}
...
\end{align}
Using the identity $M\bo{b}\times M\bo{c}=\text{det}(M) (M^{-1})^\top(\bo{b}\times\bo{c})$ for a invertible matrix $M\in\mathbb{R}^{3\times 3}$, these expressions allow us to reformulate (\ref{eq_MHD_linear}) in curvilinear coordinates:
...
\hline
$(32,64,2)$  &$2.75\cdot10^{-4}$ & &$1.50\cdot10^{-14}$ &$1.50\cdot10^{-5}$ & &$3.05\cdot10^{-14}$ \\
\hline
$(64,128,2)$ &$6.81\cdot10^{-5}$ &$2.01$ &$2.90\cdot10^{-14}$ &$1.56\cdot10^{-6}$ &$3.27$ &$5.62\cdot10^{-14}$ \\
\hline
$(128,256,2)$ &$1.70\cdot10^{-5}$ &$2.00$ &$5.08\cdot10^{-14}$ &$1.82\cdot10^{-7}$ &$3.10$ &$8.13\cdot10^{-14}$ \\
\hline
$(256,512,2)$ &$4.25\cdot10^{-6}$ &$2.00$ &$1.54\cdot10^{-13}$ &$2.21\cdot10^{-8}$ &$3.04$ &$2.74\cdot10^{-13}$\\
\hline
...
In summary, in order to go from time $t_n$ to $t_{n+1}$, we successively apply the six integrators, where it is important to note that the input of the next integrator must be the output of the previous integrator:
\begin{align}
...
\end{align}
This first-order composition is know as the Lie-Trotter splitting \cite{Trotter1959}. We remark, however, that higher-order compositions are available and can be found e.g. in \cite{McLachlanetal2012}.
...
\end{align}
together with Leibniz rule (\ref{ap_Leibniz}) and $\ast\ast=\text{id}$, we can derive the formal adjoint of the exterior derivative via
\begin{align}
\begin{split}
