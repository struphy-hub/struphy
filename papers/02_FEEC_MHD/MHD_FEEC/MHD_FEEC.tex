...
\end{align}
where $Z$ is the plasma dispersion function and $\xi\pm=(\omega-kv_0\pm\Omega_\mr{ch})/(kv_\mr{th})$. Compared to the pure MHD case, we now have to deal with a new linearized  momentum balance equation of the form
\begin{align}
&\rho_0\frac{\pa\mb{U}}{\pa t}=\frac{1}{\mu_0}(\nabla\times\mb{B}_0)\times\mb{B}+\frac{1}{\mu_0}(\nabla\times\mb{B})\times\mb{B}_0-\nabla p+q_\mr{h}n_\mr{h0}\mb{U}\times\mb{B}_\text{t}-\mb{j}_\mr{h}\times\mb{B}_\text{t}\\
...
\end{align}
where $\mb{B}_\text{t}$ denotes the total magnetic fiel, i.e. the background and fluctuation field. To descretize the equation, we assume a particle-like distribution function in physical space:
\begin{align}
f_\mr{h}=f_\mr{h}(t,\mb{x},\mb{v})\approx \sum_k w_k\delta(\mb{x}-\mb{x}_k(t))\delta(\mb{v}-\mb{v}_k(t)).
\end{align}
This means that the number density, current density and energy density in physical space are given by
\begin{align}
...
&\tilde{\epsilon}_\mr{h}(t,\mb{x})=\sum_{k}w_kv_k^2(t)\delta(\mb{x}-\mb{x}_k(t)).
\end{align}
Since there is no difference between vectors and forms in physical space, these expression are are as well the components of the 3-form number density, the 1-form current density and the 3-form energy density. To get the components on the logical domain we apply the transformation formulas for 3-forms, respectively 1-forms:
\begin{align}
...
&\epsilon_\mr{h,123}(t,\mb{q})=\sqrt{g}\tilde{\epsilon}_\mr{h}(t,F(\mb{q}))=\sum_k w_kv_k^2(t)\delta(\mb{q}-\mb{q}_k(t)),
\end{align}
where we made use of the transformation formula
\begin{align}
\delta(\mb{x}-\mb{x}_k(t))=\frac{1}{\sqrt{g}}\delta(\mb{q}-\mb{q}_k(t)).
\end{align}
Using these results in the weak fromulation of the momentum balance equation we get for the term involving the current density
\begin{align}
&\left(i_{\#j_\mr{h}^1}B^2,V^1\right)=\int_{\hat{\Omega}}(\hat{\mb{B}}\times G^{-1}\mb{j}_\mr{h})^\top G^{-1}\mb{V}\sqrt{g}\,\mr{d}^3q\approx q_\mr{h}\sum_k w_k\mb{V}_h^\top(\mb{q}_k)G^{-1}(\hat{\mb{B}}_h(\mb{q}_k)\times DF^{-1}\mb{v}_k), \\
&=q_\mr{h}\mb{v}^\top\mathbb{P}_1\mathbb{W}\tilde{G}^{-1}\hat{\mathbb{B}}\tilde{DF}^{-1}\mb{V}\qquad\forall\;\mb{v}\in\mathbb{R}^{3N}.
\end{align}
Here we defined the antisymmetric block matrix 
\begin{align}
...
\end{align}
where $(\mathbb{P}^2_{1/2/3})_{ik}=\Lambda^2_{1/2/3,i}(\mb{q}_k)\in\mathbb{R}^{N\times N_p}$ and $\mb{Q}\in\mathbb{R}^{3N_p}$ is the vector holding all particle positions. Moreover,
\begin{align}
...
\end{pmatrix}\in\mathbb{R}^{3N_p\times 3N_p}.
\end{align}
The second term involving the number density amounts to
\begin{align}
&q_\mr{h}\left((\ast n_\mr{h}^3)i_{\#U^1}B^2,V^1\right)=q_\mr{h}\int_{\hat{\Omega}}\left(\frac{1}{\sqrt{g}}n_\mr{h,123}\hat{\mb{B}}\times G^{-1}\mb{U}\right)^\top G^{-1}\mb{V}\sqrt{g}\,\mr{d}^3q\\ 
&\approx q_\mr{h}\sum_k w_k\mb{V}_h^\top(\mb{q}_k)G^{-1}(\hat{\mb{B}}_h(\mb{q}_k)\times G^{-1}\mb{U}_h(\mb{q}_k))=q_\mr{h}\mb{v}^\top\mathbb{P}_1\mathbb{W}\tilde{G}^{-1}\hat{\mathbb{B}}\tilde{G}^{-1}\mathbb{P}_1^\top\mb{u}\qquad\forall\;\mb{v}\in\mathbb{R}^{3N},
\end{align}
...
&=\frac{q_\mr{h}}{m_\mr{h}}\left(DF^{-\top}(\hat{\mb{B}}_h(\mb{q}_k)\times G^{-1}\mb{U}_h(\mb{q}_k))-DF^{-\top}(\hat{\mb{B}}_h(\mb{q}_k)\times DF^{-1}\mb{v}_k)\right),
\end{align}
where we used the identity $A\mb{b}\times A\mb{c}=\det(A)A^{-\top}(\mb{b}\times\mb{c})$ from vector algebra. Writing the above equation of motion in maxtrix-vector form for all particles yields
\begin{align}
&\frac{\mr{d}\mb{Q}}{\mr{d}t}=\tilde{DF}^{-1}\mb{V},\\
&\frac{\mr{d}\mb{V}}{\mr{d}t}=\frac{q_\mr{h}}{m_\mr{h}}\left(\tilde{DF}^{-\top}\hat{\mathbb{B}}\tilde{G}^{-1}\mathbb{P}^{1\top}\mb{u}-\tilde{DF}^{-\top}\hat{\mathbb{B}}\tilde{DF}^{-1}\mb{V}\right).
\end{align}
In the last two equations we have again negelcted all terms arising form the mapping. Next, let us introduce the discrete Hamiltonian in the case of incompressible shear Alfv\'{e}n waves
\small
\begin{align}
...
\Rightarrow\quad &\frac{\mr{d}}{\mr{d}t}\begin{pmatrix}
\mb{u} \\ \mb{b} \\ \mb{Q} \\ \mb{V}
\end{pmatrix}=\mathbb{J}\nabla\mathcal{H}=\mathbb{J}\begin{pmatrix}
...
\end{pmatrix},\\ &\mathbb{J}=
\begin{pmatrix}
\frac{q_\mr{h}}{\rho_0^2}\mathbb{M}^{-1}\mathbb{P}_1\mathbb{W}\tilde{G}^{-1}\hat{\mathbb{B}}\tilde{G}^{-1}\mathbb{P}_1^\top\mathbb{M}^{-1} &\frac{1}{\rho_0}\mathbb{M}^{-1}\mathcal{T}^\top\mathbb{C}^\top &0 &\frac{q_\mr{h}}{\rho_0m_\mr{h}}\mathbb{M}^{-1}\mathbb{P}_1\tilde{G}^{-1}\hat{\mathbb{B}}\tilde{DF}^{-1}\\
-\frac{1}{\rho_0}\mathbb{C}\mathcal{T}\mathbb{M}^{-1} &0 &0 &0 \\
0 &0 &0 &\frac{1}{\mr{m}_\mr{h}}\mathbb{W}^{-1} \\
\frac{q_\mr{h}}{\rho_0m_\mr{h}}\tilde{DF}^{-\top}\hat{\mathbb{B}}\tilde{G}^{-1}\mathbb{P}^{1\top}\mathbb{M}^{-1} &0 &-\frac{1}{m_\mr{h}}\mathbb{W}^{-1} &\frac{q_\mr{h}}{m_\mr{h}^2}\tilde{DF}^{-\top}\hat{\mathbb{B}}\tilde{DF}^{-1}\mathbb{W}^{-1}
\end{pmatrix}.
\end{align}
\normalsize
We end up with an antisymmetric Poisson matrix meaning that the discrete Hamiltonian is conserved. Assuming that the the Poisson matrix satisfies the Jacobi identity, we perform once more a Hamiltonian splitting to advance the system in time. Consequently, we split the Hamiltonian into the 7 parts
\begin{align*}
\mathcal{H}=&\mathcal{H}_{U1}+\mathcal{H}_{U2}+\mathcal{H}_{U3}+\mathcal{H}_{B}+\mathcal{H}_{\mr{h}x}+\mathcal{H}_{\mr{h}y}+\mathcal{H}_{\mr{h}z}\\
=&\frac{1}{2}\rho_0\sum_{\mu=1}^3\,\mb{u}_\mu^\top\mathbb{M}^1_{\mu\mu}\mb{u}_\mu+\frac{1}{2}+\frac{1}{2\mu_0}\mb{b}^\top\mathbb{M}^2\mb{b}+\frac{m_\mr{h}}{2}\sum_{\mu=1}^3\mb{V}_\mu^\top\mathbb{W}_\mu\mb{V}_\mu.
\end{align*}
This yields the following sub-systems: \\

\noindent \textbf{Problem 1}. For $t\in[0,\Delta t]$ and $\textbf{s}(t=0)=\textbf{s}^0$ we have,
\begin{align}
\frac{\mathrm{d} \textbf{s}}{\mathrm{d}t}=\mathbb{J}(\textbf{s})\nabla_{\textbf{s}}\mathcal{H}_{U1}(\textbf{s}),
\end{align}
where 
\begin{align}
\mathcal{H}_{U1}(\textbf{s})=\frac{1}{2}\rho_0\mb{u}_1^\top\mathbb{M}^1_{11}\mb{u}^1.
\end{align}
This can be solved analytically as
\begin{subequations}
\begin{alignat}{3}
    &\frac{\mathrm{d} \textbf{u}_1}{\mathrm{d}t}=0 &&\Rightarrow\quad \textbf{u}_1(\Delta t) = \textbf{u}_1^0,\\[1mm]
    &\frac{\mathrm{d} \textbf{u}_2}{\mathrm{d}t}=\frac{q_\mr{h}}{\rho_0}\mathbb{M}_{22}^{-1}\mathbb{P}^1_2\mathbb{W}_2\hat{\mathbb{B}}_{21}\mathbb{P}^{1\top}_1\mb{u}_1 &&\Rightarrow\quad \textbf{u}_2(\Delta t) = \textbf{u}_2^0+\Delta t\frac{q_\mr{h}}{\rho_0}\mathbb{M}_{22}^{-1}\mathbb{P}^1_2\mathbb{W}_2\hat{\mathbb{B}}_{21}\mathbb{P}^{1\top}_1\mb{u}_1^0,\\[1mm]
    &\frac{\mathrm{d} \textbf{u}_3}{\mathrm{d}t}=-\frac{q_\mr{h}}{\rho_0}\mathbb{M}_{33}^{-1}\mathbb{P}^1_3\mathbb{W}_3\hat{\mathbb{B}}_{31}\mathbb{P}^{1\top}\mb{u}_1 &&\Rightarrow\quad \textbf{u}_2(\Delta t) = \textbf{u}_2^0+\Delta t\frac{q_\mr{h}}{\rho_0}\mathbb{M}_{33}^{-1}\mathbb{P}^1_3\mathbb{W}_3\hat{\mathbb{B}}_{31}\mathbb{P}^{1\top}_1\mb{u}_1^0,\\[1mm]
\end{alignat}
\end{subequations}
The corresponding integrator is denoted by $\textbf{s}(\Delta t)=\Phi_{\Delta t}^{U1}(\textbf{s}^0)$.
