...
\end{align}
where $\gamma=5/3$ is the adiabatic exponent. The model can equivalently be written in terms of coordinate independent differential forms and for physical reasons we assume the mass density to be a 3-form ($\rho\rightarrow\rho^3$), the pressure to be a 0-form ($p\rightarrow p^0$), the magnetic field to be a 2-form ($\mb{B}\rightarrow B^2$) and the velocity to be a 1-form ($\mb{U}\rightarrow U^1$). The model then takes the form
\begin{align}
...
\end{align} 
We linearize this system about a known equilibrium ($\rho^3=\rho^3_0+\rho^3_1$, $p^0=p_0^0+p^0_1$, $B^2=B^2_0+B^2_1$, $U^1=U^1_1$) where we assume that there is no equilibrium flow. After relabeling (e.g. $\rho_1^3\rightarrow \rho^3$ and $\rho_0^3\rightarrow\rho_0$) this results in
\begin{align}
...
\end{align} 
Note that the wedge product with a 0-form is just multiplying with a scalar. 

\section{Discretization}
As a next step, we introduce finite element basis functions satisfying a discrete deRham complex. Thus, we write
\begin{alignat}{2}
...
We start with the discretization of the mass continuity equation which we shall keep in strong form. In order to stay in the correct polynomial space, we need to project the second term back into the subspace of 3-forms by applying the projector $\Pi_3$. We use the same symbol for actions on forms and the components of a form. In the latter case the projector thus returns a vector of coefficients in the new basis.
\begin{align}
...
\Leftrightarrow\quad&\frac{\pa\bo{\rho}}{\pa t}+\mathbb{D}\mathbb{P}_2^1\mb{u}=0
\end{align}
Note that we have used the commuting diagram property for exchanging projectors and differential operators. Furthermore, we have introduced the discrete divergence matrix $\mathbb{D}\in\mathbb{R}^{N\times 3N}$ and the projection matrix $\mathbb{P}_2^1\in\mathbb{R}^{3N\times3N}$, where the notation of the latter is supposed to indicate that the projection is performed on the space $V_2$ and the upper index is just a numbering. Explicitly, we have
\begin{align}
\mathbb{P}_2^1:=\begin{pmatrix}
\Pi_{2,1}\left[\rho_0G^{11}(\bo{\Lambda}_1^1)^\top\right] &\Pi_{2,1}\left[\rho_0G^{12}(\bo{\Lambda}_2^1)^\top\right] &\Pi_{2,1}\left[\rho_0G^{13}(\bo{\Lambda}_3^1)^\top\right] \\
\Pi_{2,2}\left[\rho_0G^{21}(\bo{\Lambda}_1^1)^\top\right] &\Pi_{2,2}\left[\rho_0G^{22}(\bo{\Lambda}_2^1)^\top\right] &\Pi_{2,2}\left[\rho_0G^{23}(\bo{\Lambda}_3^1)^\top\right] \\
...
\end{pmatrix}.\label{proj_matrix_p12}
\end{align}
As already indicated, each of the projections returns a vector of coefficients in the new basis. This means that the above expression amounts to
\begin{align}
\mathbb{P}_2^1=\begin{pmatrix}
...
\end{pmatrix}.
\end{align}
Here, e.g. $\mb{c}_{23,1}$ are the coefficients resulting from the projection of the basis function with the index 1 in the block 23 in the matrix (\ref{proj_matrix_p12}). Unfortunatley, this is a dense matrix, which is problematic from a memory consumption point of view. Therefore, we just save the right-hand sides, which defines a sparse matrix, and perform the final projection in every time step again. Denoting by $(\mathcal{I}_{2,1},\mathcal{I}_{2,2},\mathcal{I}_{2,3})$ the mixed interpolation-histopolation matrices and by $(\mr{vec}_{2,1}(f),\mr{vec}_{2,2}(f),\mr{vec}_{2,3}(f))$ the right-hand side vectors for some vector valued function with components $(f_1,f_2,f_3)$, we can write
\begin{align}
\mathbb{P}_2^1&=\begin{pmatrix}
\mathcal{I}_{2,1}^{-1} &0 &0 \\ 0 &\mathcal{I}_{2,2}^{-1} &0 \\ 0 &0 &\mathcal{I}_{2,3}^{-1} 
\end{pmatrix}
\begin{pmatrix}
\mr{vec}_{2,1}\left[\rho_0G^{11}(\bo{\Lambda}_1^1)^\top\right] &\mr{vec}_{2,1}\left[\rho_0G^{12}(\bo{\Lambda}_2^1)^\top\right] &\mr{vec}_{2,1}\left[\rho_0G^{13}(\bo{\Lambda}_3^1)^\top\right] \\
\mr{vec}_{2,2}\left[\rho_0G^{21}(\bo{\Lambda}_1^1)^\top\right] &\mr{vec}_{2,2}\left[\rho_0G^{22}(\bo{\Lambda}_2^1)^\top\right] &\mr{vec}_{2,2}\left[\rho_0G^{23}(\bo{\Lambda}_3^1)^\top\right] \\
...
\end{pmatrix}\\ &=:\mathcal{I}_2^{-1}\tilde{\mathbb{P}}_2^1.
\end{align}
Thus, we only precompute the sparse matrix $\mathbb{R}_2^1$ with entries defined by the mixed interpolation-histopolation problem
\begin{align}
\mr{vec}_{2,1}(f)=\int_{\xi_{i_2}}^{\xi_{i_2+1}}\int_{\xi_{i_3}}^{\xi_{i_3+1}}f_1(\xi_{i_2},q_2,q_3)\mr{d}q^2\mr{d}q^3, \\
\mr{vec}_{2,2}(f)=\int_{\xi_{i_1}}^{\xi_{i_1+1}}\int_{\xi_{i_3}}^{\xi_{i_3+1}}f_2(q_1,\xi_{i_2},q_3)\mr{d}q^1\mr{d}q^3, \\
\mr{vec}_{2,3}(f)=\int_{\xi_{i_1}}^{\xi_{i_1+1}}\int_{\xi_{i_2}}^{\xi_{i_2+1}}f_3(q_1, q_2, \xi_{i_3})\mr{d}q^1\mr{d}q^2.
\end{align}
The sparsity of $\mathrm{R}_2^1$ follows immediately from the local support of the basis functions.
...
\Leftrightarrow\quad &\frac{\pa\mb{b}}{\pa t}+\mathbb{C}\mathbb{P}_{1}^1\mb{u}=0.
\end{align}
Here, we have introduced the discrete curl matrix $\mathbb{C}\in\mathbb{R}^{3N\times 3N}$ and we have written the vector product of the background magnetic field with the velocity field in terms of a matrix-vector product by using the matrix
\begin{align}
\mathbb{B}_0:=\begin{pmatrix}
0 &-B_{0,12} &B_{0,31} \\ B_{0,12} &0 &-B_{0,23} \\ -B_{0,31} &B_{0,23} &0
\end{pmatrix}\in\mathbb{R}^{3\times 3}.
\end{align}
Following the same steps as for the continuity equation, we obtain for the projection matrix
\begin{align}
\mathbb{P}_1^1=:\bo{\mathcal{I}}_1^{-1}\mathbb{R}_1^1\in\mathbb{R}^{3N\times3N},
\end{align}
where
\begin{align}
\small
\mathbb{R}_1^1:=\begin{pmatrix}
\mr{vec}_{1,1}\left[(B_{0,31}G^{31}-B_{0,12}G^{21})(\bo{\Lambda}^1_1)^\top, (B_{0,31}G^{32}-B_{0,12}G^{22})(\bo{\Lambda}^1_2)^\top, (B_{0,31}G^{33}-B_{0,12}G^{23})(\bo{\Lambda}^1_3)^\top\right] \\
\mr{vec}_{1,2}\left[(B_{0,12}G^{11}-B_{0,23}G^{31})(\bo{\Lambda}^1_1)^\top, (B_{0,12}G^{12}-B_{0,23}G^{32})(\bo{\Lambda}^1_2)^\top, (B_{0,12}G^{13}-B_{0,23}G^{33})(\bo{\Lambda}^1_3)^\top\right] \\
...
\end{pmatrix}
\end{align}
and the the right-hand sides of the mixed interpolation-histopolation problem are defined by
\begin{align}
\mr{vec}_{1,1}(f)=\int_{\xi_{i_1}}^{\xi_{i_1+1}}f_1(q_1,\xi_{i_2},\xi_{i_3})\mr{d}q^1, \\
\mr{vec}_{1,2}(f)=\int_{\xi_{i_2}}^{\xi_{i_2+1}}f_2(\xi_{i_1},q_2,\xi_{i_3})\mr{d}q^2, \\
\mr{vec}_{1,3}(f)=\int_{\xi_{i_3}}^{\xi_{i_3+1}}f_3(\xi_{i_1},\xi_{i_2},q_3)\mr{d}q^3.
\end{align}
\subsection{Momentum equation}
Unlike the previous equations we choose a weak formulation for the momentum equation and consequently take the inner product with a test function $V^1\in H\Lambda^1(\Omega)$ to obtain
\begin{align}
...
\end{align}
We use the Galerkin approximation and compute each term separately:
\begin{align}
\left(\ast\rho_0\frac{\pa U^1}{\pa t},V^1\right)&=\int_{\hat{\Omega}}\ast\rho_0\dot{\mb{U}}^\top G^{-1}\mb{V}\sqrt{g}\mr{d}^3q\approx \int_{\hat{\Omega}}\Pi_1\left(\ast\rho_0\dot{\mb{U}}_h^\top\right)G^{-1}\mb{V}_h\sqrt{g}\mr{d}^3q\\&=\dot{\mb{u}}^\top(\mathbb{P}_1^2)^\top\int_{\hat{\Omega}}\bo{\Lambda}^1G^{-1}(\bo{\Lambda}^1)^\top\sqrt{g}\mr{d}^3q\mb{v}=\dot{\mb{u}}^\top(\mathbb{P}_1^2)^\top\mathbb{M}^1\mb{v}\quad\forall \mb{v}\in\mathbb{R}^{3N},
\end{align}
where $\mathbb{M}^1\in\mathbb{R}^{3N\times 3N}$ is the mass matrix in the space $V_1$. The projection matrix is
\begin{align}
\mathbb{P}_1^2=\bo{\mathcal{I}}_1^{-1}\mathbb{R}_1^2\in\mathbb{R}^{3N\times 3N},
\end{align}
where
\begin{align}
\mathbb{R}_1^2:=\begin{pmatrix}
...
\end{align}
where we have introduced the co-differential operator $\mr{d}^\ast\alpha^p=(-1)^p\ast\mr{d}\ast\alpha^p$. Applying the Green formula for differential forms and assuming that the boundary term vanishes yields
\begin{align}
\left(i_{\#\ast B_0}\mr{d}\ast B^2,V^1\right)&=\left(B^2,\mr{d}i_{\#V^1}B_0\right)=\int_{\hat{\Omega}}\frac{1}{g}\hat{\mb{B}}^\top G\left(\nabla\times(B_0\times G^{-1}\mb{V})\right)\sqrt{g}\mr{d}^3q\\
&\approx\mb{b}^\top\int_{\hat{\Omega}}\frac{1}{\sqrt{g}}\bo{\Lambda}^2G(\bo{\Lambda}^2)^\top\mr{d}^3q\mathbb{C}\Pi_1\left(\mathbb{B}_0G^{-1}(\bo{\Lambda}^1)^\top\right)\mb{v}=\mb{b}^\top\mathbb{M}^2\mathbb{C}\mathbb{P}_1^1\mb{v}\quad\forall\mb{v}\in\mathbb{R}^{3N}.
\end{align}
We recognize that we end up with the same projection matrix as in the induction equation. Furthermore, we have introduced the mass matrix $\mathbb{M}^2\in\mathrm{R}^{3N\times 3N}$ in the space $V_2$.
\subsection{Energy equation}
The appearance of the co-differential operator all terms of the energy equation already indicates that it is convenient to solve this equation once more weakly. Taking the inner product with a test function $r^0\in H\Lambda^0(\Omega)$ yields
\begin{align}
...
\begin{align}
\left(\frac{\pa p^0}{\pa t},r^0\right)=\int_{\hat{\Omega}}\dot{p}r\sqrt{g}\mr{d}^3q\approx\dot{\mb{p}}^\top\int_{\hat{\Omega}}\bo{\Lambda}^0(\bo{\Lambda}^0)^\top\sqrt{g}\mr{d}^3q\mb{r}=\dot{\mb{p}}^\top\mathbb{M}^0\mb{r},
\end{align}
where $\mathbb{M}^0$ is the mass matrix in the space $V_0$. The second term amounts to
\begin{align}
...
&=\mb{u}^\top(\mathbb{P}_1^3)^\top\int_{\hat{\Omega}}\bo{\Lambda}^1G^{-1}(\bo{\Lambda}^1)^\top\sqrt{g}\mr{d}^3q\mathbb{G}\mb{r}=\mb{u}^\top(\mathbb{P}_1^3)^\top\mathbb{M}^1\mathbb{G}\mb{r},
\end{align}
with $\mathbb{P}_1^3\in\mathbb{R}^{3N\times3N}$ being the projection matrix
\begin{align}
\mathbb{P}_1^3:=\mathcal{I}_1^{-1}\mathbb{R}_1^3=\mathcal{I}_1^{-1}\begin{pmatrix}
...
&=\mb{u}^\top\mathbb{M}^1\mathbb{G}\mathbb{P}_0^1\mb{r},
\end{align}
where $\mathbb{P}_0^1\in\mathbb{R}^{N\times N}$ is the projection matrix
\begin{align}
\mathbb{P}_0^1=\mathcal{I}_0^{-1}\mathbb{R}_0^1=\mathcal{I}_0^{-1}\mr{vec}_0\left[p_0(\bo{\Lambda}^0)^\top\right],
\end{align}
whose right-hand side follow from a pure interpolation problem defined by
\begin{align}
\mr{vec}_0(f)=f(\xi_{i_1},\xi_{i_2},\xi_{i_3}).
\end{align}
In summary, we obtain
\begin{align}
&\mb{r}^\top\mathbb{M}^0\dot{\mb{p}}-\mb{r}^\top\mathbb{G}^\top\mathbb{M}^1\mathbb{P}_1^3\mb{u}-(\gamma-1)\mb{r}^\top(\mathbb{P}_0^1)^\top\mathbb{G}^\top\mathbb{M}^1\mb{u}=0\quad\forall\mb{r}\in\mathbb{R}^N\\
\Leftrightarrow\quad&\mathbb{M}^0\dot{\mb{p}}-\mathbb{G}^\top\mathbb{M}^1\mathbb{P}_1^3\mb{u}-(\gamma-1)(\mathbb{P}_0^1)^\top\mathbb{G}^\top\mathbb{M}^1\mb{u}=0.
\end{align}
...
\begin{figure}
\includegraphics[scale=0.5]{N_odd.pdf}
\includegraphics[scale=0.5]{D_odd.pdf}\\
\includegraphics[scale=0.5]{N_even.pdf}
\includegraphics[scale=0.5]{D_even.pdf}
\end{figure} 
\section{Test case 1: Shear Alfv\'{e}n waves}
The most simple first test case is the simulation of shear Alfv\'{e}n waves in a homogeneous equilibrium and slab geometry (i.e. $(q_1,q_2,q_3)=(x,y,z)$ and $\Omega=\hat{\Omega}$). Due to the fact that these waves are pure transverse waves with perturbations in the magnetic field and the velocity, only the momentum and induction equation are needed. The model then takes the reduced form
\begin{align}
\frac{\pa}{\pa t}\begin{pmatrix}
\mb{u} \\ \mb{b}
\end{pmatrix}=\begin{pmatrix}
0 &(\mathbb{M}^1)^{-1}(\mathbb{P}_1^1)^\top\mathbb{C}^\top\mathbb{M}^2/(\rho_0\mu_0) \\
-\mathbb{C}\mathbb{P}_1^1 &0
\end{pmatrix}\begin{pmatrix}
\mb{u}\\ \mb{b}
...
\begin{align}
H=\frac{1}{2}\int_\Omega\rho_0U^2\mr{d}^3x+\frac{1}{2\mu_0}\int_\Omega B^2\mr{d}^3x\approx\frac{1}{2}\rho_0\mb{u}^\top\mathbb{M}^1\mb{u}+\frac{1}{2\mu_0}\mb{b}^\top\mathbb{M}^2\mb{b},
\end{align}
we can write the system in the canonical Hamiltonian form
\begin{align}
\frac{\pa \mb{S}}{\pa t}=\mathbb{J}\nabla_\mb{S}H\quad\Leftrightarrow\quad
\frac{\pa}{\pa t}\begin{pmatrix}
\mb{u} \\ \mb{b}
\end{pmatrix}=\begin{pmatrix}
0 &(\mathbb{M}^1)^{-1}(\mathbb{P}_1^1)^\top\mathbb{C}^\top/\rho_0 \\
-\mathbb{C}\mathbb{P}_1^1(\mathbb{M}^1)^{-1}/\rho_0 &0
\end{pmatrix}\begin{pmatrix}
\rho_0\mathbb{M}^1\mb{u}\\ \mathbb{M}^2\mb{b}/\mu_0
\end{pmatrix}.
\end{align} 
...
\begin{align}
\frac{\mr{d} H}{\mr{d}t}=\nabla_\mb{S}H^\top\frac{\pa\mb{S}}{\pa t}=\nabla_\mb{S}H^\top\mathbb{J}\nabla_\mb{S}H=-\nabla_\mb{S}H^\top\mathbb{J}\nabla_\mb{S}H=0.
\end{align}
\subsection{Time discretization}
Since we are dealing with a Hamiltonian system, there are some naturals choices for integrating the dynamical system in time: First, we shall look at the Hamiltonian splitting which consists of splitting the Hamiltonian in $H=H_U+H_B$ while keeping the full Poisson matrix. Second, we shall use a discrete gradient method which should yield exact energy conservation.
\subsubsection{Hamiltonian splitting}
Splitting the Hamiltonian in the aforementioned way leads to the two sub-systems
\begin{alignat}{2}
&H_U:\enspace\mathbb{J}\nabla_\mb{S}H_U=\begin{pmatrix}
0 \\ -\mathbb{C}\mathbb{P}_1^1\mb{u}
\end{pmatrix}\enspace&&\Rightarrow \enspace\mb{b}(t)=\mb{b}(t_0)-t\mathbb{C}\mathbb{P}_1^1\mb{u},\\
&H_B:\enspace\mathbb{J}\nabla_\mb{S}H_U=\begin{pmatrix}
(\mathbb{M}^1)^{-1}(\mathbb{P}_1^1)^\top\mathbb{C}^\top\mathbb{M}^2/(\rho_0\mu_0) \\ 0
\end{pmatrix}\enspace&&\Rightarrow \enspace\mb{u}(t)=\mb{u}(t_0)+t(\mathbb{M}^1)^{-1}(\mathbb{P}_1^1)^\top\mathbb{C}^\top\mathbb{M}^2/(\rho_0\mu_0)\mb{b},
\end{alignat}
...
\subsection{Linearized drift-kinetic equation}
...
\subsection{Linearized drift-kinetic density}
...
\end{align}
\subsection{Linearized magnetization}
...
\end{align}
\subsection{Linearized drift-kinetic current and forces}
...
\Leftrightarrow\quad&-\frac{1}{\rho_0}(\mb{j}_\mr{h0}^\mr{D}\times\mb{B}_1+\mb{j}_\mr{h1}^\mr{D}\times\mb{B}_0)=-\frac{\nu_\mr{h}v_0^2}{B_0}\pa_z\mb{B}_1+\nu_\mr{h}\Omega_\mr{ci}\mb{e}_z\times\mb{U}_{1\perp}-\frac{m_\mr{h}\tilde{p}_\parallel}{q_\mr{h}B_0^2\rho_0}\mb{e}_z\times\Delta\mb{B}_1\\
&-\frac{\nu_\mr{h}}{2\Omega_\mr{ci}}(v_\mr{th}^2+2v_0^2)\mb{e}_z\times\pa_z^2\mb{U}_{1\perp}-2\nu_\mr{h}v_0\pa_z\mb{U}_\perp,
\end{align}
where we introduced the ratio between the number densities of the hot and bulk species $\nu_\mr{h}=n_\mr{h0}/n_0$, the ion cyclotron frequency $\Omega_\mr{ci}=q_\mr{i}B_0/m_\mr{i}$ and the hot particle's thermal velocity $v_\mr{th}^2=2T_\parallel/m_\mr{h}$. Moreover, we assumed hot ions, i.e. $m_\mr{h}=m_\mr{i}$.
...
\begin{align}
&-i\omega\hat{\mb{U}}+i\frac{v_\mr{A}^2k^2}{\omega}\hat{\mb{U}}_\perp+i\frac{c_\mr{S}^2k^2}{\omega}\mb{e}_z\hat{U}_\parallel=i\frac{\nu_\mr{h}v_0^2k^2}{\omega}\hat{\mb{U}}_\perp-\frac{\nu_\mr{h}k^3}{2\omega\Omega_\mr{ci}}(3v_0v_\mr{th}^2+v_0^3)\mb{e}_z\times\hat{\mb{U}}_\perp\\
&+\frac{\nu_\mr{h}k^2}{2\Omega_\mr{ci}}(v_\mr{th}^2+2v_0^2)\mb{e}_z\times\hat{\mb{U}}_\perp-i2\nu_\mr{h}v_0k\hat{\mb{U}}_\perp.
\end{align}
...
\begin{align}
\left[(\omega^2-v_\mr{A}^2k^2+\nu_\mr{h}v_0^2k^2-2\omega\nu_\mr{h}v_0k)\mathbb{I}_2+i\left(\frac{\omega\nu_\mr{h}v_\mr{th}^2}{2\Omega_\mr{ci}}k^2+\frac{\omega\nu_\mr{h}v_0^2}{\Omega_\mr{ci}}k^2-\frac{3\nu_\mr{h}v_0v_\mr{th}^2}{2\Omega_\mr{ci}}k^3-\frac{\nu_\mr{h}v_0^3}{\Omega_\mr{ci}}k^3\right)\mathbb{J}_2\right]\hat{\mb{U}}_\perp=0,
\end{align} 
...
\begin{align}
(\omega^2-v_\mr{A}^2k^2+\nu_\mr{h}v_0^2k^2-2\omega\nu_\mr{h}v_0k)^2-\left[\frac{\omega\nu_\mr{h}v_\mr{th}^2}{2\Omega_\mr{ci}}k^2+\frac{\omega\nu_\mr{h}v_0^2}{\Omega_\mr{ci}}k^2-\frac{3\nu_\mr{h}v_0v_\mr{th}^2}{2\Omega_\mr{ci}}k^3-\frac{\nu_\mr{h}v_0^3}{\Omega_\mr{ci}}k^3\right]^2=0.
\end{align}
We consider the two roots
\begin{align}
D_\mr{R/L}(k,\omega)=\omega^2-v_\mr{A}^2k^2+\nu_\mr{h}v_0^2k^2-2\omega\nu_\mr{h}v_0k+p_\mr{R/L}\left[\frac{\omega\nu_\mr{h}k^2}{2\Omega_\mr{ci}}(v_\mr{th}^2+2v_0^2)-\frac{\nu_hk^3}{2\Omega_\mr{ci}}(3v_0v_\mr{th}^2+2v_0^3)\right]=0,
\end{align}
...
\begin{align}
&\omega_{\mr{R/L},\pm}(k)=\nu_\mr{h}v_0k-p_\mr{R/L}\frac{\nu_\mr{h}k^2}{4\Omega_\mr{ci}}(v_\mr{th}^2+2v_0^2)\\
&\pm k\sqrt{v_\mr{A}^2-v_0^2\nu_\mr{h}(1-\nu_\mr{h})+p_\mr{R/L}\frac{\nu_\mr{h}k}{2\Omega_\mr{ci}}(3v_0v_\mr{th}^2+2v_0^3-\nu_\mr{h}v_0v_\mr{th}^2-2\nu_\mr{h}v_0^3)+\frac{\nu_\mr{h}^2k^2}{16\Omega_\mr{ci}^2}(v_\mr{th}^2+2v_0^2)^2}.
\end{align}
In order to have wave growth or damping we need the argument of the square root to be negative. We immediately see that this is not possible in the limit $v_0\rightarrow0$. Let us use the normlaizations $\omega^\prime=\omega/\Omega_\mr{ci}$, $v_\mr{th}^\prime=v_\mr{th}/v_\mr{A}$, $v_0^\prime v_0/v_\mr{A}$ and $k^\prime=kv_\mr{A}/\Omega_\mr{ci}$ to get the scaled dispersion relation
\begin{align}
&\omega^\prime(k^\prime)=\nu_\mr{h}v_0^\prime k^\prime-p_\mr{R/L}\frac{1}{4}\nu_\mr{h}k^{\prime 2}(v_\mr{th}^{\prime 2}+2v_0^{\prime 2})\\
&\pm k^\prime\sqrt{1-v_0^{\prime 2}\nu_\mr{h}(1-\nu_\mr{h})+p_\mr{R/L}\frac{1}{2}\nu_\mr{h}k^\prime(3v_0^\prime v_\mr{th}^2+2v_0^3-\nu_\mr{h}v_0v_\mr{th}^2-2\nu_\mr{h}v_0^3)+\frac{\nu_\mr{h}^2k^2}{16\Omega_\mr{ci}^2}(v_\mr{th}^2+2v_0^2)^2}.
\end{align}
\end{document}