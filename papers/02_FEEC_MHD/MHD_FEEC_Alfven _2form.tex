...
(\ast\rho_\text{eq}^3)\wedge&\frac{\pa\tilde{U}^2}{\pa t}=\ast i_{\lozenge B^2_\text{eq}}\mr{d}\ast \tilde{B}^2+\ast i_{\lozenge\tilde{B}^2}\mr{d}\ast B^2_\text{eq}-(\ast\rho^3_\text{h})\wedge \ast i_{\lozenge\tilde{U}^2}B^2+\ast i_{\lozenge j_\text{h}^2}B^2,\\
...
\begin{alignat}{2}
&\tilde{p}^0(\mb{q})\approx \tilde{p}_h^0(\mb{q})=\sum_{\mathbf{i}}p_\mb{i}\Lambda^0_\mb{i}(\mb{q}), &&\mb{p}^\top:=(p_0,\ldots,p_{N-1})\in\mathbb{R}^N,\\
&\tilde{U}^2(\mb{q})\approx \tilde{U}_h^2(\mb{q})=\sum_{\mathbf{i}}\sum_{\mu = 1}^3u_{\mu,\mathbf{i}}\Lambda^2_{\mu,\mathbf{i}}(\mb{q})(\mathrm{d}q^\alpha\wedge\mr{d}q^\beta)_\mu, \qquad &&\mb{u}^\top:=(\mb{u}_1^\top,\mb{u}_2^\top,\mb{u}_3^\top)\in\mathbb{R}^{3N},\\
...
&\tilde{\rho}^3(\mb{q})\approx \tilde{\rho}_h^3(\mb{q})=\sum_{\mathbf{i}}\rho_\mb{i}\Lambda^3_\mb{i}(\mb{q})\,\text{d}q^1\wedge\text{d}q^2\wedge\text{d}q^3, &&\boldsymbol{\rho}^\top:=(\rho_0,\ldots,\rho_{N-1})\in\mathbb{R}^N,
\end{alignat}
...
&\mb{j}_\mr{h}(t,\mb{q})=\sqrt{g}DF^{-1}\mathring{\mb{j}}_\mr{h}(t, F(\mb{q}))=DF^{-1}\sum_k w_k\delta(\mb{q}-\mb{q}_k(t))\mb{v}_k(t),\\
...
\end{align}
Let us use these results to derive an energy conserving semi-discrete system for the finite element coefficients of $\tilde{p}_h^0$, $\tilde{U}_h^2$, $\tilde{B}_h^2$ and $\tilde{\rho}^3_h$ and the particle's positions $(\mb{q}_k)_{k=1,\ldots,N_\text{p}}$ and velocities $(\mb{v}_k)_{k=1,\ldots,N_\text{p}}$.
...
\begin{align}
&\frac{\pa\tilde{\rho}_h}{\pa t}+\hat{\nabla}\cdot\left\lbrace\Pi_2(i_{\lozenge\tilde{U}^2}\rho^3_\text{eq})\right\rbrace=0\\
