...
(\ast\rho_\text{eq}^3)\wedge&\frac{\pa\tilde{U}^1}{\pa t}=i_{\lozenge B^2_\text{eq}}\mr{d}\ast \tilde{B}^2+i_{\lozenge\tilde{B}^2}\mr{d}\ast B^2_\text{eq}-(\ast\rho^3_\text{h})\wedge i_{\lozenge\tilde{U}^2}B^2+i_{\lozenge j_\text{h}^2}B^2-d\tilde{p}^0,\\
...
&\tilde{U}^1(t,\boldsymbol{\xi})\approx \tilde{U}_h^1(t,\boldsymbol{\xi})=\sum_{\mathbf{i}}\sum_{\mu = 1}^3u_{\mu,\mathbf{i}}(t)\Lambda^1_{\mu,\mathbf{i}}(\boldsymbol{\xi})\mathrm{d}\xi^\mu \qquad &&\mb{u}^\top:=(\mb{u}_1^\top(t),\mb{u}_2^\top(t),\mb{u}_3^\top(t))\in\mathbb{R}^{3N},\\
...
\end{align}
Let us use these results to derive an energy conserving semi-discrete system for the finite element coefficients of $\tilde{p}_h^0$, $\tilde{U}_h^1$, $\tilde{B}_h^2$ and $\tilde{\rho}^3_h$ and the particle's positions $(\boldsymbol{\xi}_k)_{k=1,\ldots,N_\text{p}}$ and velocities $(\mb{v}_k)_{k=1,\ldots,N_\text{p}}$.
...
\begin{align}
\left((\ast\rho_\text{eq}^3)\wedge\frac{\pa\tilde{U}^1}{\pa t},C^1\right)&=\int_{\hat{\Omega}}\frac{1}{\sqrt{g}}\rho_\text{eq,123}\left(\frac{\pa\tilde{\mb{U}}}{\pa t}\right)^\top G^{-1}\mb{C}\sqrt{g}\,\mr{d}^3\xi\\
&\approx\int_{\hat{\Omega}}\Pi_1\left[\frac{1}{\sqrt{g}}\rho_{\text{eq},123}\left(\frac{\pa\tilde{\mb{U}}_h}{\pa t}\right)^\top\right]G^{-1} \mb{C}_h\sqrt{g}\,\mr{d}^3\xi\\&=\dot{\mb{u}}^\top\mathcal{W}^\top\underbrace{\int_{\hat{\Omega}}\mathbb{\Lambda}^1G^{-1}(\mathbb{\Lambda}^1)^\top\sqrt{g}\,\mr{d}^3\xi}_{=:\mathbb{M}^1}\,\mb{c}=\dot{\mb{u}}^\top\mathcal{W}^\top\mathbb{M}^1\mb{c}\quad\forall\;\mb{c}\in\mathbb{R}^{3N},
\end{align}
...
\begin{align}
\left(i_{\lozenge B^2_\text{eq}}\mr{d}\ast \tilde{B}^2,C^1\right)&=\left(\tilde{B}^2,\mr{d}i_{\# C^1}B_\text{eq}^2\right)=\int_{\hat{\Omega}}\frac{1}{g}\tilde{\mb{B}}^\top G\left[\hat{\nabla}\times\left(\mb{B}_\text{eq}\times G^{-1}\mb{C}\right)\right]\sqrt{g}\,\mr{d}^3\xi\\
&\approx\mb{b}^\top\underbrace{\int_{\hat{\Omega}}\frac{1}{\sqrt{g}}\mathbb{\Lambda}^2G(\mathbb{\Lambda}^2)^\top\mr{d}^3\xi}_{=:\mathbb{M}^2}\,\mathbb{C}\hat{\Pi}_1\left[\mathbb{B}_\text{eq}G^{-1}(\mathbb{\Lambda}^1)^\top\right]\mb{c}=\mb{b}^\top\mathbb{M}^2\mathbb{C}\mathcal{T}\mb{c}\quad\forall\,\mb{c}\in\mathbb{R}^{3N},
\end{align}
...
\begin{align}
\left(i_{\lozenge\tilde{B}^2}\mr{d}\ast B^2_\text{eq},C^1\right)&=\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left[\left(\hat{\nabla}\times\frac{1}{\sqrt{g}}G\mb{B}_\text{eq}\right)\times\frac{1}{\sqrt{g}}\tilde{\mb{B}}\right]\sqrt{g}\,\mr{d}^3\xi\\
&\hspace{-2cm}\approx\mb{c}^\top\int_{\hat{\Omega}}\mathbb{\Lambda}^1 G^{-1}(\mathbb{\Lambda}^1)^\top\mr{d}^3\xi\,\hat{\Pi}_1\left[\frac{1}{\sqrt{g}}\mathbb{B}_\text{eq}^{\nabla\times}(\mathbb{\Lambda}^2)^\top\right]\mb{b}=\mb{c}^\top\mathbb{M}^1\mathcal{P}\,\mb{b}\quad\forall \,\mb{c}\in\mathbb{R}^{3N}.
\end{align}
...
\begin{align}
(\text{d}\tilde{p}^0,C^1)=(\text{d}\tilde{p}^0,C^1)&=\int_{\hat{\Omega}}\hat{\nabla}\tilde{p}^\top\,G^{-1}\mb{C}\sqrt{g}\,\text{d}^3\xi\\
&\hspace{-2cm}\approx \mb{p}^\top\mathbb{G}^\top\int_{\hat{\Omega}}\mathbb{\Lambda}^1G^{-1}(\mathbb{\Lambda}^1)^\top\,\text{d}^3\xi\,\mb{c}=\mb{p}^\top\mathbb{G}^\top\mathbb{M}^1\mb{c}\quad\forall \,\mb{c}\in\mathbb{R}^{3N},
\end{align}
...
\begin{align}
&\left((\ast\rho^3_\text{h})\wedge i_{\#\tilde{U}^1}B^2,C^1\right)=\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left(\mathring{\rho_\mr{h}}\mb{B}\times G^{-1}\tilde{\mb{U}}\right)\sqrt{g}\,\mr{d}^3\xi\\ 
=&\int_{\hat{\Omega}}\mb{C}^\top G^{-1} \left[\int(\mathring{f}_\mr{h}^0-\mathring{M}_0)\,\mr{d}^3v\,\mb{B}\times G^{-1}\tilde{\mb{U}}\right]\sqrt{g}\,\mr{d}^3\xi+\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left(\int \mathring{M}_0\,\mr{d}^3v\,\mb{B}\times G^{-1}\tilde{\mb{U}}\right)\sqrt{g}\,\mr{d}^3\xi\\
=&\int_{\hat{\Omega}}\int\left\lbrace\mb{C}^\top G^{-1}\left[\left(\frac{f_\mr{h}^0-M_0}{g_\text{h}}\right)\,\mb{B}\times G^{-1}\tilde{\mb{U}}\right]\right\rbrace g_\text{h}\,\mr{d}^3v\,\mr{d}^3\xi+\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left(\mathring{\rho}_{\text{h0}}\,\mb{B}\times G^{-1}\tilde{\mb{U}}\right)\,\mr{d}^3\xi.
\end{align}
...
\approx &\sum_k\underbrace{\frac{1}{N_k}\left(\frac{f_\mr{h}^0(\boldsymbol{\xi}_k^0,\mb{v}_k^0)}{g_\mr{h}^0(\boldsymbol{\xi}_k^0,\mb{v}_k^0)}-\frac{M_0(\boldsymbol{\xi_k},\mb{v}_k)}{g_\mr{h}^0(\boldsymbol{\xi}_k^0,\mb{v}_k^0)}\right)}_{=:w_k(t)}\mb{C}_h^\top(\boldsymbol{\xi}) G^{-1}(\bo{\xi}_k)(\mb{B}_h(\boldsymbol{\xi}_k)\times G^{-1}(\bo{\xi}_k)\tilde{\mb{U}}_h(\boldsymbol{\xi}_k))\\
&\hspace{8cm}+\mb{c}^\top\int_{\hat{\Omega}}\mathbb{\Lambda}^1\mathring{\rho}_{\text{h0}}\,\mathbb{B}(\mathbb{\Lambda}^1)^\top\,\mr{d}^3\xi\,\mb{u}\\
= &\sum_k w_k\mb{C}_h^\top(\boldsymbol{\xi}_k) G^{-1}(\bo{\xi}_k)(\mb{B}_h(\boldsymbol{\xi}_k)\times G^{-1}(\bo{\xi}_k)\tilde{\mb{U}}_h(\boldsymbol{\xi}_k))+\mb{c}^\top \mathbb{X}(\mb{b})\mb{u}\\
...
\end{align}
where $N_\text{p}$ is the number of particles and $\boldsymbol{\Xi}\in\mathbb{R}^{3N_\text{p}}$ is a vector holding all particle positions. Moreover, we define the following matrices:
\begin{itemize}
\item $\mathbb{P}^1_{\mu,ik}=\Lambda^1_{\mu,i}(\boldsymbol{\xi}_k)\in\mathbb{R}^{N\times N_p}$,\qquad\qquad\qquad $\mathbb{P}^2_{\mu,ik}=\Lambda^2_{\mu,i}(\boldsymbol{\xi}_k)\in\mathbb{R}^{N\times N_p}$,
...
\item $(\bar{G}^{-1})_{ab}=\text{diag}\left[(G^{-1})_{ab}(\boldsymbol{\Xi})\right]$\quad $1\leq a,b\leq3$
\item $(\bar{DF}^{-1})_{ab}=\text{diag}\left[(DF^{-1})_{ab}(\boldsymbol{\Xi})\right]$\quad $1\leq a,b\leq3$
\end{itemize}
A similar procedure for the term involving the hot ion current density yields
\begin{align}
&\left(i_{\lozenge j_\mr{h}^2}B^2,C^1\right)=\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left(\mb{B}\times \frac{1}{\sqrt{g}}\mb{j}_\mr{h}\right)\sqrt{g}\,\mr{d}^3\xi\\
=&\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left[\mb{B}\times DF^{-1}\int (\mathring{f}_\mr{h}-\mathring{M}_0)\mb{v}\,\mr{d}^3v\right]\sqrt{g}\,\mr{d}^3\xi\\
&\hspace{5cm}+\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left[\mb{B}\times DF^{-1}\int \mathring{M}_0\mb{v}\,\mr{d}^3v\right]\sqrt{g}\,\mr{d}^3\xi\\
=&\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left[\mb{B}\times DF^{-1}\left(\frac{f_\mr{h}-M_0}{g_\mr{h}}\right)\mb{v}\right]g_\mr{h}\,\mr{d}^3v\,\mr{d}^3\xi+\int_{\hat{\Omega}}\mb{C}^\top G^{-1}\left[\mb{B}\times DF^{-1}\mathring{\mb{j}}_{\text{h0}}\right]\sqrt{g}\,\mr{d}^3\xi\\
\approx&\sum_k w_k\mb{C}^\top_h(\boldsymbol{\xi}_k)\bar{G}^{-1}(\bo{\xi}_k)(\mb{B}_h(\boldsymbol{\xi}_k)\times DF^{-1}(\boldsymbol{\xi}_k)\mb{v}_k)+\mb{c}^\top\int_{\hat{\Omega}}\mathbb{\Lambda}^1\left(\mb{B}_h\times DF^{-1}\mathring{\mb{j}}_\mr{h0}\right)\sqrt{g}\,\mr{d}^3\xi\\
...
&=\mb{u}^\top\hat{\Pi}_1\left[p_\text{eq}\mathbb{\Lambda}^1\right]\int_{\hat{\Omega}}\mathbb{\Lambda}^1G^{-1}(\mathbb{\Lambda}^1)^\top\sqrt{g}\,\text{d}^3\xi\,\mathbb{G}\mb{r}=\mb{u}^\top\mathcal{S}^\top\mathbb{M}^1\mathbb{G}\mb{r}\quad\forall\,\mb{r}\in\mathbb{R}^N,
\end{align} 
...
&=\mb{u}^\top\int_{\hat{\Omega}}\mathbb{\Lambda}^1G^{-1}(\mathbb{\Lambda}^1)^\top\sqrt{g}\,\text{d}^3\xi\,\mathbb{G}\,\hat{\Pi}_0\left[p_\text{eq}(\boldsymbol{\Lambda}^0)^\top\right]\mb{r}\\
&=\mb{u}^\top\mathbb{M}^1\mathbb{G}\mathcal{K}\mb{r}\quad\forall\,\mb{r}\in\mathbb{R}^N.
\end{align} 
...
As a last step, we derive the equations of motion for a single particle with logical coordinate $\mb{\xi}_k$ and physical velocity $\mb{v}_k$. For the latter, we have to transform the forms $B^2$ and $\tilde{U}^1$ to physical coordinates:
\begin{align}
\frac{\mr{d}\bo{\xi}_k}{\mr{d}t}&=DF^{-1}(\bo{\xi}_k)\mb{v}_k,\\
\frac{\mr{d}\mb{v}_k}{\mr{d}t}&=\frac{1}{\sqrt{g(\bo{\xi}_k)}}DF(\bo{\xi}_k)\mb{B}_h(\bo{\xi}_k)\times DF^{-\top}(\bo{\xi}_k)\tilde{\mb{U}}_h(\bo{\xi}_k)-\frac{1}{\sqrt{g(\bo{\xi}_k)}}DF(\bo{\xi}_k)\mb{B}_h(\bo{\xi}_k)\times\mb{v}_k.\\
&=DF^{-\top}(\bo{\xi}_k)(\mb{B}_h(\bo{\xi}_k)\times G^{-1}(\bo{\xi}_k)\mb{U}_h(\bo{\xi}_k))-DF^{-\top}(\bo{\xi}_k)(\mb{B}_h(\bo{\xi}_k)\times DF^{-1}(\bo{\xi}_k)\mb{v}_k),
\end{align}
...
\begin{align}
&\frac{\mr{d}\bo{\Xi}}{\mr{d}t}=\bar{DF}^{-1}\mb{V},\\
&\frac{\mr{d}\mb{V}}{\mr{d}t}=\bar{DF}^{-\top}\bar{\mathbb{B}}\bar{G}^{-1}\mathbb{P}^{1\top}\mb{u}-\bar{DF}^{-\top}\bar{\mathbb{B}}\bar{DF}^{-1}\mb{V}.
\end{align}
...
\begin{align}
\mathcal{H}_h:=\,\,&\frac{1}{2}\left((\ast\rho_\text{eq}^3)\wedge\tilde{U}_h^1,\tilde{U}_h^1\right)+\frac{1}{2}(\tilde{B}_h^2,\tilde{B}_h^2)+\frac{1}{\gamma-1}(\tilde{p}^0,1)+\frac{1}{2}\int_{\hat{\Omega}}\epsilon_{\text{h},123}\,\text{d}^3\xi\\
...
\mathcal{A}^{-1}\mathbb{M}^1\mathcal{P}\mb{b}-\mathcal{A}^{-1}\mathbb{M}^1\mathbb{G}\mb{p}-\mathcal{A}^{-1}\mathbb{X}\mb{u}+\mathcal{A}^{-1}\mb{x}\\
0\\
(\mathbb{M}^0)^{-1}\mathbb{G}^\top\mathbb{M}^1\mathcal{S}\mb{u}+(\gamma-1)(\mathbb{M}^0)^{-1}\mathcal{K}^\top\mathbb{G}^\top\mathbb{M}^1\mb{u}\\
0\\
0
\end{pmatrix}.
\end{align}
...
\begin{align}
&\textcolor{red}{\mathbb{J}_{11}(\mb{b},\bo{\Xi})}=-\mathcal{A}^{-1}\mathbb{P}^1(\bo{\Xi})\mathbb{W}\bar{G}^{-1}(\bo{\Xi})\bar{\mathbb{B}}(\mb{b},\bo{\Xi})\bar{G}^{-1}(\bo{\Xi})\mathbb{P}^{1\top}(\bo{\Xi})\mathcal{A}^{-1},\\
&\textcolor{blue}{\mathbb{J}_{12}}=\mathcal{A}^{-1}\mathcal{T}^\top\mathbb{C}^\top,\\
&\textcolor{green}{\mathbb{J}_{14}(\mb{b},\bo{\Xi})}=\mathcal{A}^{-1}\mathbb{P}^1(\bo{\Xi})\tilde{G}^{-1}(\bo{\Xi})\bar{\mathbb{B}}(\mb{b},\bo{\Xi})\tilde{DF}^{-1}(\bo{\Xi}),\\
&\textcolor{magenta}{\mathbb{J}_{34}(\bo{\Xi})}=\bar{DF}^{-1}(\bo{\Xi})\mathbb{W}^{-1},\\
&\mathbb{J}_{44}(\mb{b},\bo{\Xi})=-\tilde{DF}^{-\top}(\bo{\Xi})\bar{\mathbb{B}}(\mb{b},\bo{\Xi})\tilde{DF}^{-1}(\bo{\Xi})\mathbb{W}^{-1}.
\end{align}
