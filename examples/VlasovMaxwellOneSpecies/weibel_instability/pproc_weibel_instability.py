...
def main():
    ...
    extract_field_energy_axes = lambda field: [
        xp.array([field_energy(getattr(pdata.spline_values.em_fields, field).data[t][i]) for t in Nt]) for i in range(3)
    ]
    ...
    def plot_phaseSpace(bin, bin_name):
        ...
        for i in range(nrows):
            for j in range(ncols):
                ...
                color_mapped = getattr(getattr(pdata.f.kinetic_ions, bin_name), bin)[time_idx].T
