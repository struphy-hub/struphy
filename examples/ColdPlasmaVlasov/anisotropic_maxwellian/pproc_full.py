...
SIM_PATH = os.path.abspath("thesis_run1_full")
...
def main():
    ...
    print(
        f"Loaded {len(t_energy)} energy time points"
    )
    ...
    ax.set_title(
        "Energy evolution: geometric ColdPlasmaVlasov Run 1"
    )
    ...
    relative_energy_error = (
        np.abs(en_tot - en_tot[0])
        / np.abs(en_tot[0])
    )

    fig, ax = plt.subplots(figsize=(9, 5))

    positive_error = np.maximum(
        relative_energy_error,
        np.finfo(float).tiny,
    )
    ...
    filename = os.path.join(
        out,
        "relative_total_energy_error.png",
    )
