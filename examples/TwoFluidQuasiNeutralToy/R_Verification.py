...
>>>>>>> Stashed changes
        ...
        for label, spline, src_fn, comp in [
            ...
            im0 = axes[0].contourf(x_phys, y_phys, vals_proj, levels=50)
            axes[0].set_title("projected (FE)")
            plt.colorbar(im0, ax=axes[0])
            _add_domain_boundary(axes[0])
            im1 = axes[1].contourf(x_phys, y_phys, vals_ref, levels=50)
            axes[1].set_title("reference (analytical)")
            plt.colorbar(im1, ax=axes[1])
            _add_domain_boundary(axes[1])
            fig.suptitle(label)
        ...
        for t in simdata.spline_values.ions.u_log.data.keys():
            ...
            for num, mms, lbl in [
                ...
                im0 = axes[0].contourf(x_phys, y_phys, num, levels=50)
                axes[0].set_title("numerical")
                plt.colorbar(im0, ax=axes[0])
                _add_domain_boundary(axes[0])
                im1 = axes[1].contourf(x_phys, y_phys, mms, levels=50)
                axes[1].set_title("MMS")
                plt.colorbar(im1, ax=axes[1])
                _add_domain_boundary(axes[1])
                im2 = axes[2].contourf(x_phys, y_phys, num - mms, levels=50)
                axes[2].set_title("difference")
                plt.colorbar(im2, ax=axes[2])
                _add_domain_boundary(axes[2])
