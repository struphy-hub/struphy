...
name = "Default ColdPlasmaVlasov"
description = """
This is the default simulation for the model ColdPlasmaVlasov. 
...
# Model instance
model = ColdPlasmaVlasov(base_units=base_units)
...
model.hot_elec.set_markers(loading_params=loading_params,
...
model.thermal_elec.current.add_background(FieldsBackground())

# Perturbations for (some) FEEC variables
model.thermal_elec.current.add_perturbation(perturbations.TorusModesCos(given_in_basis='v', comp=0))
model.thermal_elec.current.add_perturbation(perturbations.TorusModesCos(given_in_basis='v', comp=1))
model.thermal_elec.current.add_perturbation(perturbations.TorusModesCos(given_in_basis='v', comp=2))
...
model.hot_elec.var.add_background(background)
...
model.hot_elec.var.add_initial_condition(init)
