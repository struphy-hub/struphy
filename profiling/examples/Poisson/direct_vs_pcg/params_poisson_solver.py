...
exactly the situation feectools.linalg.solvers.DirectSolver targets: the original PCG
solver re-derives an iterative solution from scratch every step regardless, while
DirectSolver factorizes the (unchanging) operator once, lazily, on the first call, and
reuses that factorization -- a small, fixed number of kernel launches -- for every call
after that.

`--repeats` controls how many identical time steps to run (default 20); more repeats
make DirectSolver's one-time factorization cost matter less and less relative to PCG's
cost, which is paid in full on every single call.
"""
...
parser.add_argument(
    ...
    nargs=3,
    default=[16, 16, 16],
    help="Grid resolution (default: 32 32 32). DirectSolver is single-rank only, so this case never scales MPI ranks.",
)
parser.add_argument(
    "--repeats",
    type=int,
    default=20,
    help="Number of identical time steps to run, i.e. repeated solves of the same system (default: 20).",
)
...
solver_params = SolverParameters(tol=1e-12, maxiter=3000, info=True, recycle=True)
