...
if __name__ == "__main__":
    ...
    if sim.rank == 0:
        ...
        # Deliberately plain NumPy from here on, not xp/cunumpy: everything below reads
        # already-saved (host, h5py) or already-postprocessed (PlottingData, host-only
        # regardless of ARRAY_BACKEND) data purely to hand it to matplotlib, so there is
        # nothing left to dispatch onto the active backend -- using xp on data that is
        # already NumPy would only risk exactly the array-type/active-backend mismatches
        # this branch's CuPy port spent a lot of effort finding and fixing elsewhere.
        ...
        phi_phy = np.asarray(sim.spline_values.em_fields.phi_phy.data[Tend_saved][0])
        X, Y, Z = (np.asarray(g) for g in sim.grids_phy)
        R = np.sqrt(X**2 + Y**2)
        ...
        delta_f_final = np.asarray(density_data.delta_f_binned)[-1]
        eta1_grid, eta2_grid = np.meshgrid(
            np.asarray(density_data.grid_e1),
            np.asarray(density_data.grid_e2),
            indexing="ij",
        )
