...
`--ranks 1 2 4` (the default) all fit on a single Booster node (4 GPUs/node), so this only
exercises intra-node scaling; ranks beyond `GPUS_PER_NODE` would spill onto a second node,
still one GPU per rank, but multi-node network overhead is untested here.

Uses `params_GuidingCenter_scaling.py`, not `params_GuidingCenter.py` (the single-GPU
NumPy-vs-CuPy comparison case): its much larger default Np (10,000,000 vs. 200,000) is
needed for a scaling study specifically because at the smaller size, per-rank compute
between marker exchanges is too small to outweigh the exchange cost -- adding ranks there
measured *slower*, not faster (see that file's docstring for the numbers). Whether the
larger problem here still scales, and how far, is what this case measures.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[1, 2, 4],
        help="MPI rank counts to run with, one GPU per rank (default: 1 2 4).",
    )
    ...
    profiling_case = ProfilingCase(
        ...
        description="5D guiding-centre test particles (Np=10,000,000) in a homogeneous slab on a 3D cube, run with the CuPy array backend at increasing MPI rank counts (one GPU per rank) to measure strong-scaling speedup and verify the CUDA-ported kernels work correctly under MPI.",
