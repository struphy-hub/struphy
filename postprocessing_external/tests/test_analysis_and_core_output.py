...
def test_reductions_are_available_from_external_helpers_and_the_accessor(run):
    product = run.evaluate(F)
    moments = velocity_moments(product)
...
def test_physical_time_units_are_not_converted_twice(tmp_path):
    ...
    np.testing.assert_allclose(physical.to_si(F).t, physical.evaluate(F).t)
