...
def test_divB():
    ...
    test_case = 1
    # Which B-field to use. To be implemented. 1: (0,0,1), 2:(1,1,1), 3: (1,2,3), 4: GVEC's B-field.
    kind_B_field = 3
    ...
    print('Set tensor space operators and derivatives done.')
    ...
    # print('Max diff', np.max(np.abs(cx - ncx)))
    ...
    eta1, eta2, eta3 = np.linspace(0,1,3), np.linspace(0,1,3), np.linspace(0,1,3)
    # mesh = np.meshgrid(eta1, eta2, eta3, indexing='ij')

    if use_B_components:
        B_orig = (B[0](eta1, eta2, eta3), B[1](eta1, eta2, eta3), B[2](eta1, eta2, eta3))
    else:
        B_orig = B(eta1, eta2, eta3)
    # print('Original B-field evaluated at B{}: {}'.format((eta1, eta2, eta3), B_orig))

    # Pull B(\eta) -> (Bx,By,Bz) to its 2-form representation.
    B2_1 = domain.pull(B, eta1, eta2, eta3, '2_form_1')
    B2_2 = domain.pull(B, eta1, eta2, eta3, '2_form_2')
    B2_3 = domain.pull(B, eta1, eta2, eta3, '2_form_3')
    # print('B Pulled to 2 form:', (B2_1, B2_2, B2_3))
    # B2_1 = domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_1')
    # B2_2 = domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_2')
    # B2_3 = domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_3')
    # print('B Pulled to 2 form:', B2_1, B2_2, B2_3)

    hat_B2 = [B2_1, B2_2, B2_3]

    # Then push back to Cartesian to see if we get the same thing.
    pullpushed_Bx = domain.push(hat_B2, eta1, eta2, eta3, '2_form_1')
    pullpushed_By = domain.push(hat_B2, eta1, eta2, eta3, '2_form_2')
    pullpushed_Bz = domain.push(hat_B2, eta1, eta2, eta3, '2_form_3')
    # print('Push-pulled B(x,y,z):', (pullpushed_Bx, pullpushed_By, pullpushed_Bz))
    # print('Should be equal to the original B-field.') # TODO: Add assert
    ...
    def hat_B2_fun1(eta1, eta2, eta3):
        return domain.pull(B, eta1, eta2, eta3, '2_form_1')
    def hat_B2_fun2(eta1, eta2, eta3):
        return domain.pull(B, eta1, eta2, eta3, '2_form_2')
    def hat_B2_fun3(eta1, eta2, eta3):
        return domain.pull(B, eta1, eta2, eta3, '2_form_3')
    # def hat_B2_fun1(eta1, eta2, eta3):
    #     return domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_1')
    # def hat_B2_fun2(eta1, eta2, eta3):
    #     return domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_2')
    # def hat_B2_fun3(eta1, eta2, eta3):
    #     return domain.pull([Bx, By, Bz], eta1, eta2, eta3, '2_form_3')

    hat_B2 = [hat_B2_fun1, hat_B2_fun2, hat_B2_fun3]
    ...
    # Do the Pi2 projection.
    b2_coeff = proj_3d.PI_2(hat_B2_fun1, hat_B2_fun2, hat_B2_fun3) # Coefficients of projected 2-form.
    print('Pi2 projection done.')
    ...
    # TODO: Show Div B = 0.
    concat = np.concatenate((b2_coeff[0].flatten(), b2_coeff[1].flatten(), b2_coeff[2].flatten()))
    div = tensor_space_FEM.D.dot(concat)
    print('Maximum error', np.max(np.abs(div)))

    assert b2_coeff[0].flatten().size + b2_coeff[1].flatten().size + b2_coeff[2].flatten().size == tensor_space_FEM.D.shape[1], 'Matrix size should match'



    # ============================================================
    # Visualize in ParaView Div B and components of B.
    # ============================================================
