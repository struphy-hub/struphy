...
# ==========================================================================================          
@types('int','int','int','int','int','int','int','int','int','double[:,:]','double[:,:]','double[:,:,:,:]','double[:,:,:,:]','int[:,:]','int[:,:]','double[:,:,:]','double[:,:,:,:,:]')
def kernel_inner(nel1, nel2, n3, p1, p2, nq1, nq2, ni1, ni2, w1, w2, bi1, bi2, ind_base1, ind_base2, mat, mat_fun):
    
    mat[:, :, :] = 0.
    
    #$ omp parallel
    #$ omp do reduction ( + : mat) private (ie1, ie2, ie3, il1, il2, value, q1, q2, wvol, bi)
    for ie1 in range(nel1):
        for ie2 in range(nel2):
            for ie3 in range(n3):
...
def kernel_l2error(nel, p, nq, w1, w2, ni, nj, bi1, bi2, bj1, bj2, ind_basei1, ind_basei2, ind_basej1, ind_basej2, error, mat_f1, mat_f2, c1, c2, mat_map):
    
    
    #$ omp parallel
    #$ omp do private (ie1, ie2, q1, q2, wvol, bi, bj, il1, il2, jl1, jl2)
...
# ==========================================================================================
@types('int[:]','int','int[:]','int[:]','int[:]','double[:,:,:]','int[:,:]','int[:,:]','double[:,:,:,:]','double[:,:,:,:]','double[:,:,:,:,:]')        
def kernel_evaluate_2form(nel, n3, p, ns, nq, b_coeff, ind_base1, ind_base2, bi1, bi2, b_eva):
    
    b_eva[:, :, :, :, :] = 0.
    
    #$ omp parallel
    #$ omp do private (ie1, ie2, ie3, q1, q2, il1, il2)
    for ie1 in range(nel[0]):
        for ie2 in range(nel[1]):
            for ie3 in range(n3):
