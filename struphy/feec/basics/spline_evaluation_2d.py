...
def evaluation_kernel_2d(p1 : int, p2 : int, basis1 : 'float[:]', basis2 : 'float[:]', ind1 : 'int[:]', ind2 : 'int[:]', coeff : 'float[:,:]') -> float:
    ...
    ----------
        ...
        basis1, basis2 : array[float]           
            The p+1 values of non-zero basis splines at one point (eta1, eta2) from 'basis_funs' of shape.
...
# =============================================================================
def evaluate(kind1 : int, kind2 : int, t1 : 'float[:]', t2 : 'float[:]', p1 : int, p2 : int, ind1 : 'int[:,:]', ind2 : 'int[:,:]', coeff : 'float[:,:]', eta1 : float, eta2 : float) -> float:
    """
    ...
    spline_value = evaluation_kernel_2d(p1, p2, b1, b2, ind1[span1 - p1, :], ind2[span2 - p2, :], coeff)

    return spline_value


# =============================================================================
def evaluate_tensor_product(t1 : 'float[:]', t2 : 'float[:]', p1 : int, p2 : int, ind1 : 'int[:,:]', ind2 : 'int[:,:]', coeff : 'float[:,:]', eta1 : 'float[:]', eta2 : 'float[:]', spline_values : 'float[:,:]', kind : int):
    """
    ...
    for i1 in range(len(eta1)):
        for i2 in range(len(eta2)):
            ...
            if   kind == 0:
                spline_values[i1, i2] = evaluate(1, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])
            elif kind == 11:
                spline_values[i1, i2] = evaluate(2, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])
            elif kind == 12:
                spline_values[i1, i2] = evaluate(1, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])
            elif kind == 2:
                spline_values[i1, i2] = evaluate(2, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])           
            elif kind == 31:
                spline_values[i1, i2] = evaluate(3, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])
            elif kind == 32:
                spline_values[i1, i2] = evaluate(1, 3, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1], eta2[i2])
...
def evaluate_matrix(t1 : 'float[:]', t2 : 'float[:]', p1 : int,  p2 : int, ind1 : 'int[:,:]', ind2 : 'int[:,:]', coeff : 'float[:,:]', eta1 : 'float[:,:]', eta2 : 'float[:,:]', spline_values : 'float[:,:]', kind : int):
    ...
    for i1 in range(n1):
        for i2 in range(n2):
            ...
            if   kind == 0:
                spline_values[i1, i2] = evaluate(1, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])
            elif kind == 11:
                spline_values[i1, i2] = evaluate(2, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])
            elif kind == 12:
                spline_values[i1, i2] = evaluate(1, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])
            elif kind == 2:
                spline_values[i1, i2] = evaluate(2, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])     
            elif kind == 31:
                spline_values[i1, i2] = evaluate(3, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])
            elif kind == 32:
                spline_values[i1, i2] = evaluate(1, 3, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, i2], eta2[i1, i2])
...
def evaluate_sparse(t1 : 'float[:]', t2 : 'float[:]', p1 : int, p2 : int, ind1 : 'int[:,:]', ind2 : 'int[:,:]', coeff : 'float[:,:]', eta1 : 'float[:,:]', eta2 : 'float[:,:]', spline_values : 'float[:,:]', kind : int):
    ...
    for i1 in range(n1):
        for i2 in range(n2):
            ...
            if   kind == 0:
                spline_values[i1, i2] = evaluate(1, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])
            elif kind == 11:
                spline_values[i1, i2] = evaluate(2, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])
            elif kind == 12:
                spline_values[i1, i2] = evaluate(1, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])
            elif kind == 2:
                spline_values[i1, i2] = evaluate(2, 2, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])     
            elif kind == 31:
                spline_values[i1, i2] = evaluate(3, 1, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])
            elif kind == 32:
                spline_values[i1, i2] = evaluate(1, 3, t1, t2, p1, p2, ind1, ind2, coeff, eta1[i1, 0], eta2[0, i2])
