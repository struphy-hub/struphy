...
def create_equal_random_arrays(V, seed):
    '''Creates equal two equal random arrays, where one array is a numpy array and the other one a distributed psydac array.
    ...
    if isinstance(V, TensorFemSpace):
        ...
        arr_psy[s[0]:e[0] + 1, s[1]:e[1] + 1, s[2]:e[2] + 1] = arr.reshape(V.spaces[0].nbasis, V.spaces[1].nbasis, V.spaces[2].nbasis)[s[0]:e[0] + 1, s[1]:e[1] + 1, s[2]:e[2] + 1]
        
    elif isinstance(V, ProductFemSpace):
        
        arr_psy = BlockVector(V.vector_space)
        
        arrs = np.split(arr, [V.spaces[0].nbasis, V.spaces[0].nbasis + V.spaces[1].nbasis])
        ...
        for d, block in enumerate(arr_psy.blocks):
            ...
            arr_psy[d][s[0]:e[0] + 1, s[1]:e[1] + 1, s[2]:e[2] + 1] = arrs[d].reshape(V.spaces[d].spaces[0].nbasis, V.spaces[d].spaces[1].nbasis, V.spaces[d].spaces[2].nbasis)[s[0]:e[0] + 1, s[1]:e[1] + 1, s[2]:e[2] + 1]
...
def compare_arrays(arr_psy, arr, rank, atol=1e-14):
    ...
    if isinstance(arr_psy, StencilVector):
        ...
        if arr.ndim == 3:
            ...
            tmp2 = arr.reshape(arr_psy.space.npts[0], arr_psy.space.npts[1], arr_psy.space.npts[2])[s[0]: e[0] + 1, s[1]: e[1] + 1, s[2]: e[2] + 1]
        ...
        if not (isinstance(arr, tuple) or isinstance(arr, list)):
            arrs = np.split(arr, [arr_psy.blocks[0].shape[0], arr_psy.blocks[0].shape[0] + arr_psy.blocks[1].shape[0]])
        else:
            arrs = arr
        ...
        for vec_psy, vec in zip(arr_psy, arrs):
            ...
            if vec.ndim == 3:
                ...
                tmp2 = vec.reshape(vec_psy.space.npts[0], vec_psy.space.npts[1], vec_psy.space.npts[2])[s[0]: e[0] + 1, s[1]: e[1] + 1, s[2]: e[2] + 1]
