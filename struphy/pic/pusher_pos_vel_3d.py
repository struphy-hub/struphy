...
# ==========================================================================================================
@types(                      'double[:,:]','double','int[:]','double[:]','double[:]','double[:]','int[:,:]','int[:,:]','int[:,:]','int[:,:]','int[:,:]','int[:,:]','double[:]','double[:]','double[:]','double[:]','double[:]','double[:]','int','int[:]','int[:]','double[:]','double[:]')
def pusher_x_v_static_efield(particles,    dt,      p,       t1,         t2,         t3,         indN1,     indN2,     indN3,     indD1,     indD2,     indD3,     loc1,       loc2,       loc3,       weight1,    weight2,    weight3,    np,   nbase_n, nbase_d, e0_coeffs,  eps        ):
    """
    ...
    ------------
        particles : array
            shape(6, np), contains the values for the positions [:3,], velocities [3:6,], and weights [6,]
    ...
    #$ omp parallel
    #$ omp do private(particles, ip, k, m, n, temp, eta1, eta2, eta3, v1, v2, v3, eta1_curr, eta2_curr, eta3_curr, v1_curr, v2_curr, v3_curr, eta1_next, eta2_next, eta3_next, v1_next, v2_next, v3_next, quad_pos1, quad_pos2, quad_pos3, span1, span2, span3, ie1, ie2, ie3, bn1, bn2, bn3, bd1, bd2, bd3, i1, i2, i3, il1, il2, il3)
    for ip in range(np):
        ...
        while abs(eta1_next - eta1_curr) > eps_pos or abs(eta2_next - eta2_curr) > eps_pos or abs(eta3_next - eta3_curr) > eps_pos or abs(v1_next - v1_curr) > eps_vel or abs(v2_next - v2_curr) > eps_vel or abs(v3_next - v3_curr) > eps_vel:
            ...
            eta1_next = eta1 + dt/2. * (v1_curr + v1)
            eta2_next = eta2 + dt/2. * (v2_curr + v2)
            eta3_next = eta3 + dt/2. * (v3_curr + v3)
            ...
            for k in range(n_quad1):
                ...
                quad_pos1 = eta1 + (loc1[k]+1)/2 * (eta1_curr - eta1)
                quad_pos2 = eta2 + (loc1[k]+1)/2 * (eta2_curr - eta2)
                quad_pos3 = eta3 + (loc1[k]+1)/2 * (eta3_curr - eta3)
                ...
                bsp.basis_funs_slim(t1, pn1, quad_pos1, span1, bn1, bd1)
                bsp.basis_funs_slim(t2, pn2, quad_pos2, span2, bn2, bd2)
                bsp.basis_funs_slim(t3, pn3, quad_pos3, span3, bn3, bd3)
            ...
            v1_next = v1 + dt * temp / 2.

            # ======================================================================================
            # update velocity in direction 2
            temp = 0.
            for m in range(n_quad2):

                # find position for quadrature formula; x1 + (1/2 * tau_i + 1/2) * ( x1_curr - x1 )
                quad_pos1 = eta1 + (loc2[m]+1)/2 * (eta1_curr - eta1)
                quad_pos2 = eta2 + (loc2[m]+1)/2 * (eta2_curr - eta2)
                quad_pos3 = eta3 + (loc2[m]+1)/2 * (eta3_curr - eta3)
            ...
            for n in range(n_quad3):
                ...
                # find position for quadrature formula; x1 + (1/2 * tau_i + 1/2) * ( x1_curr - x1 )
                quad_pos1 = eta1 + (loc3[n]+1)/2 * (eta1_curr - eta1)
                quad_pos2 = eta2 + (loc3[n]+1)/2 * (eta2_curr - eta2)
                quad_pos3 = eta3 + (loc3[n]+1)/2 * (eta3_curr - eta3)
            ...
            v3_next = v3 + dt * temp / 2.

        # write the results in the particles array and impose periodic boundary conditions on the particles by taking modulo 1
        particles[0,ip] = eta1_next%1
        particles[1,ip] = eta2_next%1
        particles[2,ip] = eta3_next%1
        particles[3,ip] = v1_next
        particles[4,ip] = v2_next
        particles[5,ip] = v3_next