...
# ==========================================================================================================
def pusher_v_mhd_electric(particles : 'float[:,:]', dt : float, t1 : 'float[:]', t2 : 'float[:]', p : 'int[:]', ind_n1 : 'int[:,:]', ind_n2 : 'int[:,:]', ind_d1 : 'int[:,:]', ind_d2 : 'int[:,:]', b_eq_1 : 'float[:,:,:]', b_eq_2 : 'float[:,:,:]', b_eq_3 : 'float[:,:,:]', b_p_1 : 'float[:,:,:]', b_p_2 : 'float[:,:,:]', b_p_3 : 'float[:,:,:]', b_norm : 'float[:,:,:]', u1 : 'float[:,:,:]', u2 : 'float[:,:,:]', u3 : 'float[:,:,:]', basis_u : int, kind_map : int, params_map : 'float[:]', tf1 : 'float[:]', tf2 : 'float[:]', tf3 : 'float[:]', pf : 'int[:]', ind1f : 'int[:,:]', ind2f : 'int[:,:]', ind3f : 'int[:,:]', cx : 'float[:,:,:]', cy : 'float[:,:,:]', cz : 'float[:,:,:]', mu : 'float[:]', n_tor : int):
    ...
    for ip in range(np):
        ...
        if basis_u == 0:
            ...
            for i in range(2):
                ...
                u[0] += evaluation_kernel_2d(pn1, pn2, bn1, bn2, span1, span2, ind_n1, ind_n2, u1[:, :, i])*cs[i]
                u[1] += evaluation_kernel_2d(pn1, pn2, bn1, bn2, span1, span2, ind_n1, ind_n2, u2[:, :, i])*cs[i]
                u[2] += evaluation_kernel_2d(pn1, pn2, bn1, bn2, span1, span2, ind_n1, ind_n2, u3[:, :, i])*cs[i]
        ...
        # velocity field (1-form, push forward with df^(-T))
            ...
            for i in range(2):
                ...
                u[0] += evaluation_kernel_2d(pd1, pn2, bd1, bn2, span1 - 1, span2 - 0, ind_d1, ind_n2, u1[:, :, i])*cs[i]
                u[1] += evaluation_kernel_2d(pn1, pd2, bn1, bd2, span1 - 0, span2 - 1, ind_n1, ind_d2, u2[:, :, i])*cs[i]
                u[2] += evaluation_kernel_2d(pn1, pn2, bn1, bn2, span1 - 0, span2 - 0, ind_n1, ind_n2, u3[:, :, i])*cs[i]
        ...
        # velocity field (2-form, push forward with df/|det df|)
            ...
            for i in range(2):
                ...
                u[0] += evaluation_kernel_2d(pn1, pd2, bn1, bd2, span1 - 0, span2 - 1, ind_n1, ind_d2, u1[:, :, i])*cs[i]
                u[1] += evaluation_kernel_2d(pd1, pn2, bd1, bn2, span1 - 1, span2 - 0, ind_d1, ind_n2, u2[:, :, i])*cs[i]
                u[2] += evaluation_kernel_2d(pd1, pd2, bd1, bd2, span1 - 1, span2 - 1, ind_d1, ind_d2, u3[:, :, i])*cs[i]
        ...
        b[0] = evaluation_kernel_2d(pn1, pd2, bn1, bd2, span1 - 0, span2 - 1, ind_n1, ind_d2, b_eq_1[:, :, 0])
        b[1] = evaluation_kernel_2d(pd1, pn2, bd1, bn2, span1 - 1, span2 - 0, ind_d1, ind_n2, b_eq_2[:, :, 0])
        b[2] = evaluation_kernel_2d(pd1, pd2, bd1, bd2, span1 - 1, span2 - 1, ind_d1, ind_d2, b_eq_3[:, :, 0])

        # perturbed magnetic field (2-form)
        for i in range(2):
            
            b[0] += evaluation_kernel_2d(pn1, pd2, bn1, bd2, span1 - 0, span2 - 1, ind_n1, ind_d2, b_p_1[:, :, i])*cs[i]
            b[1] += evaluation_kernel_2d(pd1, pn2, bd1, bn2, span1 - 1, span2 - 0, ind_d1, ind_n2, b_p_2[:, :, i])*cs[i]
            b[2] += evaluation_kernel_2d(pd1, pd2, bd1, bd2, span1 - 1, span2 - 1, ind_d1, ind_d2, b_p_3[:, :, i])*cs[i]
        ...
        b_grad[0] = evaluation_kernel_2d(pn1, pn2, der1, bn2, span1, span2, ind_n1, ind_n2, b_norm[:, :, 0])
        b_grad[1] = evaluation_kernel_2d(pn1, pn2, bn1, der2, span1, span2, ind_n1, ind_n2, b_norm[:, :, 0])
        b_grad[2] = 0.
...
# ==========================================================================================================
def pusher_vxb(particles : 'float[:,:]', dt : float, t1 : 'float[:]', t2 : 'float[:]', p : 'int[:]', ind_n1 : 'int[:,:]', ind_n2 : 'int[:,:]', ind_d1 : 'int[:,:]', ind_d2 : 'int[:,:]', b_eq_1 : 'float[:,:,:]', b_eq_2 : 'float[:,:,:]', b_eq_3 : 'float[:,:,:]', b_p_1 : 'float[:,:,:]', b_p_2 : 'float[:,:,:]', b_p_3 : 'float[:,:,:]', kind_map : int, params_map : 'float[:]', tf1 : 'float[:]', tf2 : 'float[:]', tf3 : 'float[:]', pf : 'int[:]', ind1f : 'int[:,:]', ind2f : 'int[:,:]', ind3f : 'int[:,:]', cx : 'float[:,:,:]', cy : 'float[:,:,:]', cz : 'float[:,:,:]', n_tor : int):
