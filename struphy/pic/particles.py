...
class Particles6D:
    ...
    def __init__(self, name, params_markers, domain, domain_array, mpi_comm):
        ...
        self.mpi_comm.Allgather(np.array(params_markers['ppc']*n_cells), n_k_load)
        ...
        self._n_k_loc_all = round(n_k_load[self.mpi_rank]*(1 + 1/np.sqrt(n_k_load[self.mpi_rank]) + 0.25))
        ...
        if self.params['loading']['type'] == 'external':
            ...
            if self.mpi_rank == 0:
                ...
                self._markers[:, :n_k_load_cum_sum[0]] = file['particles'][:, :n_k_load_cum_sum[0]]
        ...
        # ----------- load fresh particles ---------------------------------
            ...
            # 3. symmetric sobol numbers in all 6 dimensions with skip of first 1000 numbers
                ...
                sampling.set_particles_symmetric_6d(temp_markers, self._markers, self.n_k_loc_all)
        ...
        self._markers[7, :n_k_load[self.mpi_rank]] = self.s0(self.markers[0, :n_k_load[self.mpi_rank]], self.markers[1, :n_k_load[self.mpi_rank]], self.markers[2, :n_k_load[self.mpi_rank]], self.markers[3, :n_k_load[self.mpi_rank]], self.markers[4, :n_k_load[self.mpi_rank]], self.markers[5, :n_k_load[self.mpi_rank]])
        ...
        assert not (np.any(self._markers[:3, :n_k_load[self.mpi_rank]] >= 1.) or np.any(self._markers[:3, :n_k_load[self.mpi_rank]] <= 0.))
    ...
    # sampling distribution s0 (0-form) on logical domain
    ...
    def send_recv_markers(self):
        ...
        markers_to_be_sent, holes = sendrecv_determine_mtbs(self._markers, self.domain_array, self.mpi_rank)
        
        # determine where to send markers_to_be_sent
        send_info, send_list = sendrecv_get_destinations(markers_to_be_sent, self.domain_array, self.mpi_size)
        
        # transpose send_info
        recv_info = sendrecv_all_to_all(send_info, self.mpi_comm)
        
        # send and receive markers
        sendrecv_markers(send_list, recv_info, holes, self._markers, self.mpi_comm)
    ...
    def show_logical(self, save_dir=None):
        ...
        plt.scatter(self.markers[0, true_markers], self.markers[1, true_markers], s=1, color='b')
        
        # plot domain decomposition
        for i in range(self.mpi_size):
            
            e1 = np.linspace(self.domain_array[i, 0], self.domain_array[i, 1], int(self.domain_array[i, 2]) + 1)
            e2 = np.linspace(self.domain_array[i, 3], self.domain_array[i, 4], int(self.domain_array[i, 5]) + 1)
    ...
    def show_physical(self, save_dir=None):
        ...
        X = self.domain.evaluate(self.markers[0, true_markers], self.markers[1, true_markers], np.zeros(self.markers[1, true_markers].size, dtype=float), 'x', 'flat')
        Y = self.domain.evaluate(self.markers[0, true_markers], self.markers[1, true_markers], np.zeros(self.markers[1, true_markers].size, dtype=float), 'y', 'flat')
    ...
    def initialize_weights(self, init, params):
        """
        ...
        kinetic_init = KineticInit6D(self.domain, init, params)
        
        # compute w0
        self._markers[8] = KINETIC_INIT.f0(self.markers[0], self.markers[1], self.markers[2], self.markers[3], self.markers[4], self.markers[5]) / self.markers[7]
    ...
    def update_weights(self, kinetic_background, use_control):
        ...
        if use_control:
            self._markers[6] = self._markers[8] - kinetic_background.fh0_eq(self.markers[0], self.markers[1], self.markers[2], self.markers[3], self.markers[4], self.markers[5])
        else:
             self._markers[6] = self.markers[8]
...
class Particles5D:
    ...
    def __init__(self, name, params_markers, domain, mpi_comm):

        self._name = name
        self._params = params_markers
        self._domain = domain
        self._n_markers = params_markers['n_markers']

        self._mpi_comm = mpi_comm
        self._mpi_size = mpi_comm.Get_size()
        self._mpi_rank = mpi_comm.Get_rank()

        self._n_markers_loc = self.n_markers//self.mpi_size
        ...
        for i, req in enumerate(reqs):
            if req is None:
                ...
                if req.Test():
                    ...
                    markers[:, holes[first_hole[i] + np.arange(recv_info[i])]] = recvbufs[i].T
