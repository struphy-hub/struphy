...
class Initialize_mhd_psydac:
    '''
    Initialize MHD variables (see Initilaize_mhd above) when using Psydac API.
    ...
    nq_pr : list
        Number of quadrature points for histopolation in each direction.
    ...
    def __init__(self, DOMAIN, DERHAM, nq_pr, general_init, params_init):
        ...
        # Space dimensions
        N_dof_0form = self.DERHAM.V0.nbasis
        N_dof_1form = sum([space.nbasis for space in DERHAM.V1.spaces])
        N_dof_2form = sum([space.nbasis for space in DERHAM.V2.spaces])
        N_dof_3form = self.DERHAM.V3.nbasis

        # Initialize unknowns as stencil vectors  
        self.r3 = StencilVector(DERHAM.V3.vector_space)
        self.b2 = BlockVector(DERHAM.V2.vector_space)

        if   self.basis_p == 0:
            self.pp = StencilVector(DERHAM.V0.vector_space)
        elif self.basis_p == 3:
            self.pp = StencilVector(DERHAM.V3.vector_space)

        if self.basis_u == 1:
            self.up = BlockVector(DERHAM.V1.vector_space)
        elif self.basis_u == 2:
            self.up = BlockVector(DERHAM.V2.vector_space)

        # Type of intial condition
        if self.init_type == 'modes_k':
            self.r3[:] = P3(self.r3_ini).coeffs[:]
            self.b2[0][:] = P2((self.b2_ini_1, self.b2_ini_2, self.b2_ini_3)).coeffs[0][:]
            self.b2[1][:] = P2((self.b2_ini_1, self.b2_ini_2, self.b2_ini_3)).coeffs[1][:]
            self.b2[2][:] = P2((self.b2_ini_1, self.b2_ini_2, self.b2_ini_3)).coeffs[2][:]

            if self.basis_p == 0:
                self.pp[:] = P0(self.p0_ini).coeffs[:]
            elif self.basis_p == 3:
                self.pp[:] = P3(self.p3_ini).coeffs[:]

            if self.basis_u == 1:
                self.up[0][:] = P1([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[0][:]
                self.up[1][:] = P1([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[1][:]
                self.up[2][:] = P1([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[2][:]
            elif self.basis_u == 2:
                self.up[0][:] = P2([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[0][:]
                self.up[1][:] = P2([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[1][:]
                self.up[2][:] = P2([self.u1_ini_1, self.u1_ini_2, self.u1_ini_3]).coeffs[2][:]

        elif self.init_type == 'noise':
            np.random.seed(1607)

            assert all(DERHAM.V0.periodic)
            assert all(DERHAM.V1.spaces[0].periodic)
            assert all(DERHAM.V1.spaces[1].periodic)
            assert all(DERHAM.V1.spaces[2].periodic)
            assert all(DERHAM.V2.spaces[0].periodic)
            assert all(DERHAM.V2.spaces[1].periodic)
            assert all(DERHAM.V2.spaces[2].periodic)
            assert all(DERHAM.V3.periodic)

            if self.plane == 'xy':
                shape_2d = self.pp[:, :, 0].shape # all fields have the same shape because of periodicity
                size_1d  = self.pp[0, 0, :].size
                amps = np.random.rand(8, *shape_2d) 
                start = self.pp.starts[2]
                end   = self.pp.ends[2] 
            ...
            if not 'p'  in self.target: self.pp[:] = 0.
            if not 'r'  in self.target: self.r3[:] = 0.
            if not 'u1' in self.target: self.up[0][:] = 0.
            if not 'u2' in self.target: self.up[1][:] = 0.
            if not 'u3' in self.target: self.up[2][:] = 0.
            if not 'b1' in self.target: self.b2[0][:] = 0.
            if not 'b2' in self.target: self.b2[1][:] = 0.
            if not 'b3' in self.target: self.b2[2][:] = 0.

        elif self.init_type == 'modes_mn' or 'eigfun':
            raise NotImplementedError

        print('density'.ljust(16) + 'initialized as 3-form of size', self.r3.shape)
        print('mhd velocity'.ljust(16) + 'initialized as ' + str(self.basis_u) + '-form of size', self.up[0].shape, self.up[1].shape, self.up[2].shape)
        print('magnetic field'.ljust(16) + 'initialized as 2-form of size', self.b2[0].shape, self.b2[1].shape, self.b2[2].shape)
        print('pressure'.ljust(16) + 'initialized as ' + str(self.basis_p) + '-form of size', self.pp.shape)
