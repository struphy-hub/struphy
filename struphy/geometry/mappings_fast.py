...
def spline_2d_torus(eta1: float, eta2: float, eta3: float,
    ...
    where :math:`c^{R(y)}_{ij}` are the control points in the :math:`\eta_1-\eta_2`-plane, independent of :math:`\eta_3`.
    ...
    # Evaluate mapping
    f_out[0] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * cos(2*pi*eta3)
    f_out[1] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cy)
    f_out[2] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * sin(2*pi*eta3)

    # TODO: explanation
    if eta1 == 0. and cx[0, 0] == cx[0, 1]:
        f_out[0] = cx[0, 0]*cos(2*pi*eta3)
...
def spline_2d_torus_df(eta1: float, eta2: float, eta3: float,
    ...
    df_out[0, 0] = eva_2d.evaluation_kernel_2d(p[0], p[1], der1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * cos(2*pi*eta3)
    df_out[0, 1] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, der2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * cos(2*pi*eta3)
    df_out[0, 2] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * sin(2*pi*eta3) * (-2*pi)
    df_out[1, 0] = eva_2d.evaluation_kernel_2d(p[0], p[1], der1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cy)
    df_out[1, 1] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, der2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cy)
    df_out[1, 2] = 0.
    df_out[2, 0] = eva_2d.evaluation_kernel_2d(p[0], p[1], der1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * sin(2*pi*eta3)
    df_out[2, 1] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, der2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * sin(2*pi*eta3)
    df_out[2, 2] = eva_2d.evaluation_kernel_2d(p[0], p[1], b1, b2, ind1[span1 - p[0], :], ind2[span2 - p[1], :], cx) * cos(2*pi*eta3) * 2*pi
...
def hollow_cyl(eta1: float, eta2: float, eta3: float,
    ...
    ----------
        ...
        r0 : float
            Offset of pole in x-direction (major radius).
...
def rotated_ellipse(eta1: float, eta2: float, eta3: float,
    ...
    ----------
        ...
        th : float
            Rotation angle in [0, 1).

        f_out : array[float]
            Output: (x, y, z) = F(eta1, eta2, eta3).
    '''

    f_out[0] = x0 + (eta1 * r1) * cos(2*pi*th) * cos(2*pi*eta2) - \
        (eta1 * r2) * sin(2*pi*th) * sin(2*pi*eta2)
    f_out[1] = y0 + (eta1 * r1) * sin(2*pi*th) * cos(2*pi*eta2) + \
        (eta1 * r2) * cos(2*pi*th) * sin(2*pi*eta2)
    f_out[2] = z0 + (eta3 * lz)
...
def powered_ellipse(eta1: float, eta2: float, eta3: float,
    ...
    .. math::
        F_x &= x_0+r_x\,\eta_1^s\cos(2\pi\,\eta_2)\,, 

        F_y &= y_0+r_y\,\eta_1^s\sin(2\pi\,\eta_2)\,, 

        F_z &= z_0+L_z\,\eta_3\,.

    Note
    ----
        Example with paramters :math:`x_0=y_0=z_0=0\,,r_x=1\,,r_y=2,s=0.5` and :math:`L_z=1`:
    ...
    f_out[0] = x0 + (eta1**s) * rx * cos(2*pi*eta2)
    f_out[1] = y0 + (eta1**s) * ry * sin(2*pi*eta2)
    f_out[2] = z0 + (eta3 * lz)
...
def shafranov_dshaped(eta1: float, eta2: float, eta3: float,
    ...
    f_out[0] = x0 + r0 * (1 + (1 - eta1**2) * dx + eg *
...
def shafranov_eta3dep(eta1: float, eta2: float, eta3: float,
    ...
    ----------
        ...
        xi : float
            Xi: Strength of dependence on eta3.

        f_out : array[float]
            Output: (x, y, z) = F(eta1, eta2, eta3).
    '''

    f_out[0] = x0 + r0 * (1 + xi * cos(2*pi*eta3)) * (1 + (1 - eta1**2) * dx + eg *      eta1 * cos(2*pi*eta2 + arcsin(dg)*eta1*sin(2*pi*eta2)))
    f_out[1] = y0 + r0 * (1 - xi * cos(2*pi*eta3)) * (    (1 - eta1**2) * dy + eg * kg * eta1 * sin(2*pi*eta2))
    f_out[2] = z0 + (eta3 * lz)
...
def shafranov_eta3dep_df(eta1: float, eta2: float, eta3: float,
    ...
    df_out[0, 1] = - r0 * eg * eta1 * (1 + xi * cos(2*pi*eta3)) * (2*pi*eta1 * cos(2*pi*eta2) * arcsin(dg) + 2*pi) * sin(eta1 * sin(2*pi*eta2) * arcsin(dg) + 2*pi*eta2)
    df_out[0, 2] = - 2 * pi * r0 * xi * (dx * (1 - eta1**2) + eg * eta1 * cos(eta1 * sin(2*pi*eta2) * arcsin(dg) + 2*pi*eta2) + 1) * sin(2*pi*eta3)
    df_out[1, 0] = r0 * (- 2 * dy * eta1 + eg * kg * sin(2*pi*eta2)) * (1 - xi * cos(2*pi*eta3))
    df_out[1, 1] = 2 * pi * r0 * eg * eta1 * kg * (1 - xi * cos(2*pi*eta3)) * cos(2*pi*eta2)
    df_out[1, 2] = 2 * pi * r0 * xi * (dy * (1 - eta1**2) + eg * eta1 * kg * sin(2*pi*eta2)) * sin(2*pi*eta3)
    df_out[2, 0] = 0.
    df_out[2, 1] = 0.
    df_out[2, 2] = lz
