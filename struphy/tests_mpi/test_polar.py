...
@pytest.mark.parametrize('p', [[3, 2, 1], [4, 3, 2]])
@pytest.mark.parametrize('spl_kind', [[False, True, True], [False, True, False]])
def test_extraction_and_linear_ops(Nel, p, spl_kind):
    ...
    c1_blocks = PolarExtractionBlocksC1(domain.cx[:, :, 0], domain.cy[:, :, 0], spl_kind[2], derham.nbasis_v0[2])
    ...
    # ==================== test DOF extraction operators ===================
    if rank == 0:
        print('----------- Test DOF extraction operators ---------')
    
    # test DOF extraction operator
    ...
    assert np.allclose(r0_pol.toarray(True), space.projectors.P0.dot(f0_tp_leg))
    assert np.allclose(r1_pol.toarray(True), space.projectors.P1.dot(e1_tp_leg))
    assert np.allclose(r2_pol.toarray(True), space.projectors.P2.dot(b2_tp_leg))
    assert np.allclose(r3_pol.toarray(True), space.projectors.P3.dot(p3_tp_leg))
    ...
    compare_arrays(r0, space.projectors.P0.T.dot(f0_pol_leg), rank)
    compare_arrays(r1, space.projectors.P1.T.dot(e1_pol_leg), rank)
    compare_arrays(r2, space.projectors.P2.T.dot(b2_pol_leg), rank)
    compare_arrays(r3, space.projectors.P3.T.dot(p3_pol_leg), rank)
...
if __name__ == '__main__':
    ...
    #test_extraction_and_linear_ops([6, 9, 4], [2, 2, 2], [False, True, False])