...
def test_lin_mhd_ops(Nel, p, spl_kind):
    ...
    elapsed = time()
    OPS_PSY = MHD_ops(DERHAM_PSY, n_quad, EQ_MHD_L, F, mpi_comm=MPI_COMM)
    OPS_PSY.assemble_K1()
    ...
    assert_ops(mpi_rank, dof_PSY, dof_STR, res_PSY, res_STR, verbose=True)
    print(f'Rank {mpi_rank} | Asserting MHD operator K1 (as identity operator).')
    # assert_ops(mpi_rank, x3_st, res_STR, x3_st, res_STR, verbose=True)
    # assert_ops(mpi_rank, x3_st, res_PSY, x3_st, res_PSY, verbose=True)
    ...
    #assert_ops(mpi_rank, x0_st, res_STR, x0_st, res_PSY, verbose=False)
    print(f'Rank {mpi_rank} | Assertion passed.')
    ...
    res_STR, dof_STR_0, dof_STR_1, dof_STR_2 = OPS_STR.Q1_dot(
        np.concatenate((x1[0].flatten(), x1[1].flatten(), x1[2].flatten())))
    res_STR_0, res_STR_1, res_STR_2 = SPACES.extract_2(res_STR)
    dof_STR = np.concatenate(
        (dof_STR_0.flatten(), dof_STR_1.flatten(), dof_STR_2.flatten()))
    ...
    res_STR, dof_STR_0, dof_STR_1, dof_STR_2 = OPS_STR.W1_dot(
        np.concatenate((x1[0].flatten(), x1[1].flatten(), x1[2].flatten())))
    ...
    res_STR, dof_STR_0, dof_STR_1, dof_STR_2 = OPS_STR.Q2_dot(
...
def assert_ops(mpi_rank, dof_PSY, dof_STR, res_PSY, res_STR, verbose=False, MPI_COMM=None):
    ...
    if verbose:
        ...
        print(f'Rank {mpi_rank} | Maximum absolute diference (DOFs):\n', np.max(np.abs(
            dof_PSY[
                res_PSY.starts[0]: res_PSY.ends[0] + 1,
                res_PSY.starts[1]: res_PSY.ends[1] + 1,
                res_PSY.starts[2]: res_PSY.ends[2] + 1,
        ...
        # print(f'Rank {mpi_rank} | Elements where DOF error > 1e-4:\n', np.where( np.abs(
        #     dof_PSY[
        #         res_PSY.starts[0] : res_PSY.ends[0] + 1,
        #         res_PSY.starts[1] : res_PSY.ends[1] + 1,
        #         res_PSY.starts[2] : res_PSY.ends[2] + 1,
...
if __name__ == '__main__':
    test_lin_mhd_ops(Nel=[8, 9, 8], p=[2, 2, 2], spl_kind=[False, True, True])
