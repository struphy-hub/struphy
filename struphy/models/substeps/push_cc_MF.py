...
class Current_coupling:
    ...
    def step_jh(self, up, particles, b2_eq, b2, print_info=False):
        ...
        rhs = self.MHD_OPS_MF.A(up) - self.dts[0]**2/4 * mat.dot(up) + self.dts[0]*vec
        
        # LHS of linear system
        LHS = spa.linalg.LinearOperator(self.MHD_OPS_MF.A.shape, lambda x : self.MHD_OPS_MF.A(x) + self.dts[0]**2/4 * mat.dot(x))
    ...
    def step_rhoh(self, up, particles, b2_eq, b2, print_info=False):
        ...
        # rhs of linear system
        rhs = self.MHD_OPS_MF.A(up) + self.dts[1]/2 * mat.dot(up)
        
        # LHS of linear system
        LHS = spa.linalg.LinearOperator(self.MHD_OPS_MF.A.shape, lambda x : self.MHD_OPS_MF.A(x) - self.dts[1]/2 * mat.dot(x))
            
        # solve linear system with gmres method and values from last time step as initial guess 
        up[:], info = spa.linalg.gmres(LHS, rhs, x0=up, tol=self.params_sol['tol1'], 
                                                     maxiter=self.params_sol['maxiter1'], M=self.MHD_OPS_MF.A_inv, callback=count_iters
                                                     )
