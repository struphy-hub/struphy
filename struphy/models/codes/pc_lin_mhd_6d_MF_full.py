...
from struphy.feec.projectors.pro_global import mhd_operators_MF as mhd_ops_MF
from struphy.pic.pc_lin_mhd_6d          import accumulation
from struphy.models.substeps            import push_linear_mhd_MF
from struphy.models.substeps            import push_markers
from struphy.models.substeps            import push_pc_MF


def execute(file_in, path_out, restart):
    '''Executes the code pc_lin_mhd_6d_MF_full.
    ...
    print()
    print('Starting code "pc_lin_mhd_6d_MF_full" ...')
    print()
    ...
    # =========================================================================================
    SPACES.set_projectors()
    MHD_OPS_MF = mhd_ops_MF.projectors_dot_x(SPACES, EQ_MHD_L, DOMAIN, params['mhd_init']['general']['basis_u'], 
    ...
    time_series['en_U'][0] = 1/2*MHD.up.dot(MHD_OPS_MF.A(MHD.up))
    ...
    UPDATE_MHD = push_linear_mhd_MF.Linear_mhd(dts_mhd, SPACES, MHD_OPS_MF, params['solvers'], 
    ...
    UPDATE_PC = push_pc_MF.Pressure_coupling(dts_pc, DOMAIN, SPACES, MHD_OPS_MF, ACCUM, MPI_COMM, KIN.Np, KIN.Np_loc, 
                                                                params['solvers'], params['kinetic_equilibrium']['general'],
                                                                params['mhd_init']['general']['basis_u'])
    print('Preussre coupling time stepping available.')
    print()
    ...
    def update():
        ...
        if params['time']['split_algo'] == 'LieTrotter':
            ...
            UPDATE_PC.step_ph_full(MHD.up, KIN.particles_loc, print_info=True)
            MPI_COMM.Bcast(MHD.up, root=0)

            UPDATE_MARKERS.step_eta_RK4_pc_full(KIN.particles_loc, MHD.up, params['mhd_init']['general']['basis_u'], print_info=True)
