...
def test_gvec_equil():
    ...
    if comm.Get_rank() == 0:
        ...
        assert rel_err(
            gvec.det_df(e1, e2, e3),
            mhd_equil.domain.jacobian_det(e1, e2, e3),
        ) < 4e-3
        ...
        assert rel_err(
            gvec.b_cart(e1, e2, e3)[0][0],
            mhd_equil.b_cart(e1, e2, e3)[0][0],
        ) < 4e-3
        # print(rel_err(gvec.b_cart(e1, e2, e3)[0][1], mhd_equil.b_cart_2(e1, e2, e3)[0]))
        assert rel_err(
            gvec.b_cart(e1, e2, e3)[0][1],
            mhd_equil.b_cart(e1, e2, e3)[0][1],
        ) < 3e-3
        # print(rel_err(gvec.b_cart(e1, e2, e3)[0][2], mhd_equil.b_cart_3(e1, e2, e3)[0]))
        assert rel_err(
            gvec.b_cart(e1, e2, e3)[0][2],
            mhd_equil.b_cart(e1, e2, e3)[0][2],
        ) < 6e-3
