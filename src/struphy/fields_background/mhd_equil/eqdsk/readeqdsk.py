...
class Geqdsk:
    ...
    def openFile(self, filename, data_type=0):
        """
        ...
        self.data['psirz'] = numpy.reshape(
            data[4*nw:4*nw+nw*nh], (nh, nw),
        ), "Poloidal flux in Weber / rad on the rectangular grid points"
        self.data['qpsi']  = numpy.array(
            data[4*nw+nw*nh:5*nw+nw*nh],
