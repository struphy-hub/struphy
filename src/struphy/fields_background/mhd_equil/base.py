...
class MHDequilibrium(metaclass=ABCMeta):
    ...
    def a1(self, *etas, squeeze_out=False):
        ...
        assert (
            self.__class__.__name__ in avail_list
    ...
    def j_cart(self, *etas, squeeze_out=False):
        ...
        j_out = self.domain.push(
            self.jv(*etas, squeeze_out=False), *etas, kind="v", a_kwargs={"squeeze_out": False}, squeeze_out=squeeze_out
        )
    ...
    def show(self, n1=16, n2=33, n3=21, n_planes=5):
        ...
        for n in range(n_planes):
            ...
            for i in range(pc1.shape[0]):
                for j in range(pc1.shape[1] - 1):
                    if i < pc1.shape[0] - 1:
                        ax.plot([pc1[i, j], pc1[i + 1, j]], [pc2[i, j], pc2[i + 1, j]], "b", linewidth=0.6)
                    if j < pc1.shape[1] - 1:
                        ax.plot([pc1[i, j], pc1[i, j + 1]], [pc2[i, j], pc2[i, j + 1]], "b", linewidth=0.6)
        ...
        for m in range(2):
            ...
            for i in range(tc1.shape[0]):
                for j in range(tc1.shape[1] - 1):
                    if i < tc1.shape[0] - 1:
                        ax.plot([tc1[i, j], tc1[i + 1, j]], [tc2[i, j], tc2[i + 1, j]], "b", linewidth=0.6)
                    if j < tc1.shape[1] - 1:
                        if i == 0:
                            ax.plot([tc1[i, j], tc1[i, j + 1]], [tc2[i, j], tc2[i, j + 1]], "r", linewidth=1)
                        else:
                            ax.plot([tc1[i, j], tc1[i, j + 1]], [tc2[i, j], tc2[i, j + 1]], "b", linewidth=0.6)
            ax.set_xlabel(l1)
            ax.set_ylabel(l2)
            ax.axis("equal")
        ...
        for n in range(n_planes):
            ...
            ax.set_ylabel(l2)
            ax.axis("equal")
            ax.set_title("Equilibrium density at $\\eta_3$={0:4.3f}".format(e3[int(n * jump)]))
            fig.colorbar(map, ax=ax, location="right")
...
class AxisymmMHDequilibrium(CartesianMHDequilibrium):
    ...
    def gradB_xyz(self, x, y, z):
        ...
        gradBR = (
            -RabsB / R**2
            + (self.psi(R, Z, dZ=1) * self.psi(R, Z, dR=1, dZ=1) + self.psi(R, Z, dR=1) * self.psi(R, Z, dR=2))
            / RabsB
            / R
        )
        gradBP = 0.0
