...
class ODEsolverFEEC():
    ...
    def __init__(self, vector_field: dict, algo: str = 'rk4',):
        ...
        for vec, f in vector_field.items():
            ...
            assert callable(f)
            sig = signature(f)
            assert len(sig.parameters) == len(vector_field) + 2, 'Number of arguments of each callable must match the number of unknows plus two (for time and out).'
