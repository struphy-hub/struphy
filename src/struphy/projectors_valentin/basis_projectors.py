...
from struphy.projectors_valentin import basis_projection_kernels
...
class BasisProjectionOperator_V(LinearOperator):
    """
    ...
    def __init__(self, P, V, fun, transposed=False, preproc_grid=None, dof_mat=None):
        ...
        BasisProjectionOperator_V.assemble_mat(
    ...
    def transpose(self, conjugate=False):
        ...
        if self.transposed:
            return BasisProjectionOperator_V(self._P, self._V, self._fun, not self.transposed, preproc_grid=self._preproc_grid, dof_mat=self._dof_operator.transpose())
        else : 
            return BasisProjectionOperator_V(self._P, self._V, self._fun, not self.transposed, preproc_grid=self._preproc_grid, dof_mat=self._dof_operator)
    ...
    def assemble_mat(P, V, fun, dof_operator, preproc_grid=None):
        ...
        for Wspace, W1d, nq, fun_line in zip(_Wspaces, _W1ds, _nqs, fun):
            ...
            for Vspace, V1d, f in zip(_Vspaces, _V1ds, fun_line):
                ...
                dofs_mat.set_backend(backend=PSYDAC_BACKEND_GPYCCEL, precompiled=False)
                j+=1
            i+=1
