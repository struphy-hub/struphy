...
def setup_domain_and_equil(params: dict, units: dict = None):
    ...
    if "mhd_equilibrium" in params:
        ...
        if mhd_type in ("EQDSKequilibrium", "GVECequilibrium", "DESCequilibrium"):
            equil = mhd_class(units=units, **params["mhd_equilibrium"][mhd_type])
        else:
        ...
        equil = br_eq_class(**params["braginskii_equilibrium"][br_eq_type])
        equil.domain = domain
...
def setup_derham(params_grid, comm, inter_comm=None, domain=None, mpi_dims_mask=None):
    ...
    if comm_world_rank == 0:
        ...
        print("GL quad pts (L2):".ljust(25), nq_el)
        print("GL quad pts (hist):".ljust(25), nq_pr)
        print("MPI proc. per dir.:".ljust(25), derham.domain_decomposition.nprocs)
...
def setup_domain_cloning(comm, params, Nclones):
    ...
    if size % Nclones != 0:
        if rank == 0:
            print(f"Total number of ranks ({size}) is not divisible by the number of clones ({Nclones}).")
    ...
    clone_info = comm.gather((rank, clone_color, local_rank, inter_comm.Get_rank()), root=0)
    ...
    if "kinetic" in params:
        for species_name, species_data in params["kinetic"].items():
            ...
            for marker_key in marker_keys:
                ...
                clone_particle_info[current_rank][species_name][marker_key + "_original"] = marker_value
                if marker_value is not None and "grid" in params and params["grid"].get("Nclones"):
                    ...
                    clone_particle_info[current_rank][species_name][marker_key] = task_marker_value
                    params["kinetic"][species_name]["markers"][marker_key] = task_marker_value
...
def pre_processing(
    ...
    if mpi_rank == 0:
        ...
        with open(path_out + "/meta.txt", "w") as f:
            f.write("date of simulation: ".ljust(30) + str(datetime.datetime.now()) + "\n")
        ...
        if isinstance(val, list):
            ...
            if d_default is None:
                if len(keys) == 0:
                    ...
                    raise ValueError(f"Depth of options dictionary must not exceed 3, but is {len(keys) + 1}.")
