...
def plot_time_vs_duration(
    ...
    unique_regions = set(
        region_name for rank_data in profiling_data.values() for region_name in rank_data
    )
    ...
    for rank_name, rank_data in profiling_data.items():
        for region_name, info in rank_data.items():
            if region_name == 'main' or 'setup' in region_name or 'initialize' in region_name:
                continue
...
def plot_gantt_chart(
    ...
    rank_names = list(profiling_data.keys())
    ...
    region_spacing = 0.05  # Vertical spacing between groups for different regions
    ...
    for region_idx, region_name in enumerate(region_names):
        ...
        for rank_idx, (rank_name, rank_data) in enumerate(profiling_data.items()):
