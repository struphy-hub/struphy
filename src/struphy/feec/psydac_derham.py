...
class Derham:
    ...
    def _get_domain_array(self):
        ...
        dom_arr = xp.zeros(xp.oc * 9, dtype=float)
        ...
        return dom_arr.reshape(xp.oc, 9)
    ...
    def _get_index_array(self, decomposition):
        ...
        # main array (receive buffers)
        ind_arr = xp.zeros(xp.oc * 6, dtype=int)
        ...
        return ind_arr.reshape(xp.oc, 6)
...
class SplineFunction:
    ...
    def _tmp_noise_for_mpi(self, *shapes, direction="e3", amp=0.0001, seed=None):
        ...
        if self.derham.comm is not None:
            ...
            rank = self.derham.comm.Get_rank()
            xp.ocs = self.derham.domain_decomposition.xp.ocs
        else:
            comm_size = 1
            rank = 0
            xp.ocs = [1, 1, 1]
        ...
        already_drawn = xp.zeros(xp.ocs) == 1.0

        # 1d mid point arrays in each direction
        mid_points = []
        for xp. in xp.ocs:
            delta = 1.0 / xp.
            mid_points_i = xp.zeros(xp.)
            for n in range(xp.):
                mid_points_i[n] = delta * (n + 1 / 2)
            mid_points += [mid_points_i]

        if direction == "e1":
            tmp_arrays = xp.zeros(xp.ocs[0]).tolist()
        elif direction == "e2":
            tmp_arrays = xp.zeros(xp.ocs[1]).tolist()
        elif direction == "e3":
            tmp_arrays = xp.zeros(xp.ocs[2]).tolist()
        elif direction == "e1e2":
            tmp_arrays = xp.zeros((xp.ocs[0], xp.ocs[1])).tolist()
            Warning, f"2d noise in the directions {direction} is not correctly initilaized for MPI !!"
        elif direction == "e1e3":
            tmp_arrays = xp.zeros((xp.ocs[0], xp.ocs[2])).tolist()
            Warning, f"2d noise in the directions {direction} is not correctly initilaized for MPI !!"
        elif direction == "e2e3":
            tmp_arrays = xp.zeros((xp.ocs[1], xp.ocs[2])).tolist()
            Warning, f"2d noise in the directions {direction} is not correctly initilaized for MPI !!"
        elif direction == "e1e2e3":
            tmp_arrays = xp.zeros((xp.ocs[0], xp.ocs[1], xp.ocs[2])).tolist()
