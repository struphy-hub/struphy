            ...
            for kk in range(span3.shape[0]):
                ...
                for iq in range(nq1):
                    for jq in range(nq2):
                        for kq in range(nq3):
                            funval = (
                                fun_q[nq1 * ii + iq, nq2 * jj + jq, nq3 * kk + kq]
                                * wts1[ii, iq]
                                * wts2[jj, jq]
                                * wts3[kk, kq]
                            )
...
@stack_array("shp")
def get_dofs_local_1_form_e1_component(f1: "float[:,:,:]", p1: int, wts: "float[:]", f_eval_aux: "float[:,:,:]"):
...
@stack_array("shp")
def get_dofs_local_1_form_e2_component(f2: "float[:,:,:]", p2: int, wts: "float[:]", f_eval_aux: "float[:,:,:]"):
...
@stack_array("shp")
def get_dofs_local_1_form_e3_component(f3: "float[:,:,:]", p3: int, wts: "float[:]", f_eval_aux: "float[:,:,:]"):
...
def get_dofs_local_2_form_e1_component(
    f1: "float[:,:,:]", p2: int, p3: int, GLweightsx: "float[:,:]", f_eval_aux: "float[:,:,:]"
):
    ...
    for i in range(shp[0]):
        for j in range(shp[1]):
            for k in range(shp[2]):
                ...
                for jj in range(p2):
                    for kk in range(p3):
                        f_eval_aux[i, j, k] += f1[i, in_start_2 + jj, in_start_3 + kk] * GLweightsx[jj, kk]


@stack_array("shp")
def get_dofs_local_2_form_e2_component(
    f2: "float[:,:,:]", p1: int, p3: int, GLweightsy: "float[:,:]", f_eval_aux: "float[:,:,:]"
):
    ...
    for i in range(shp[0]):
        for j in range(shp[1]):
            for k in range(shp[2]):
                ...
                for ii in range(p1):
                    for kk in range(p3):
                        f_eval_aux[i, j, k] += f2[in_start_1 + ii, j, in_start_3 + kk] * GLweightsy[ii, kk]


@stack_array("shp")
def get_dofs_local_2_form_e3_component(
    f3: "float[:,:,:]", p1: int, p2: int, GLweightsz: "float[:,:]", f_eval_aux: "float[:,:,:]"
):
    ...
    for i in range(shp[0]):
        for j in range(shp[1]):
            for k in range(shp[2]):
                ...
                for ii in range(p1):
                    for jj in range(p2):
                        f_eval_aux[i, j, k] += f3[in_start_1 + ii, in_start_2 + jj, k] * GLweightsz[ii, jj]


@stack_array("shp")
def get_dofs_local_3_form(
    faux: "float[:,:,:]", p1: int, p2: int, p3: int, GLweights: "float[:,:,:]", f_eval: "float[:,:,:]"
):
            ...
            for k in range(shp[2]):
                ...
                for ii in range(p1):
                    for jj in range(p2):
                        for kk in range(p3):
                            f_eval[i, j, k] += (
                                faux[in_start_1 + ii, in_start_2 + jj, in_start_3 + kk] * GLweights[ii, jj, kk]
                            )
    ...
    for i0 in range(starts[0], ends[0] + 1):
        ...
        for i1 in range(starts[1], ends[1] + 1):
            ...
            for i2 in range(starts[2], ends[2] + 1):
                L123 = 0.0
                startj1, endj1 = select_quasi_points(i0, p1, npts[0], periodic[0])
                startj2, endj2 = select_quasi_points(i1, p2, npts[1], periodic[1])
                startj3, endj3 = select_quasi_points(i2, p3, npts[2], periodic[2])
                for j1 in range(2 * p1 - 1):
                    ...
                    for j2 in range(2 * p2 - 1):
                        ...
                        if startj2 + j2 < original_size2:
                            ...
                            pos2 = index_translation2[int(startj2 + j2 - 2 * npts[1])]
                        auxL3 = 0.0
                        for j3 in range(2 * p3 - 1):
                            ...
                            if startj3 + j3 < original_size3:
                                ...
                                pos3 = index_translation3[int(startj3 + j3 - 2 * npts[2])]
...
def solve_local_1_form(
    original_pts_sizex: "int[:]",
    original_pts_sizey: "int[:]",
    original_pts_sizez: "int[:]",
    index_translationx0: "int[:]",
    index_translationx1: "int[:]",
    index_translationx2: "int[:]",
    index_translationy0: "int[:]",
    index_translationy1: "int[:]",
    index_translationy2: "int[:]",
    index_translationz0: "int[:]",
    index_translationz1: "int[:]",
    index_translationz2: "int[:]",
    nsp: int,
    starts: "int[:,:]",
    ends: "int[:,:]",
    pds: "int[:,:]",
            ...
            for i0 in range(starts[h][0], ends[h][0] + 1):
                ...
                for i1 in range(starts[h][1], ends[h][1] + 1):
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        startj1, endj1 = select_quasi_points(i0, p1, npts[1][0], periodic[0][0])
                        startj2, endj2 = select_quasi_points(i1, p2, npts[0][1], periodic[0][1])
                        startj3, endj3 = select_quasi_points(i2, p3, npts[0][2], periodic[0][2])
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizex[0]:
                                ...
                                pos1 = index_translationx0[int(startj1 + j1 + shift1)]
                            if whij0[i0][j1] != 0.0:
                                auxL2 = 0.0
                                for j2 in range(lenj2):
                                    ...
                                    if startj2 + j2 < original_pts_sizex[1]:
                                        ...
                                        pos2 = index_translationx1[int(startj2 + j2 - 2 * npts[0][1])]
                                    auxL3 = 0.0
                                    for j3 in range(lenj3):
                                        ...
                                        if startj3 + j3 < original_pts_sizex[2]:
                                            ...
                                            pos3 = index_translationx2[int(startj3 + j3 - 2 * npts[0][2])]
                                        auxL3 += wij2[i2][j3] * rhs0[pos1, pos2, pos3]
                                    auxL2 += wij1[i1][j2] * auxL3
                                L123 += whij0[i0][j1] * auxL2
                        out0[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
                    counteri1 += 1
                counteri0 += 1
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizey[0]:
                                ...
                                pos1 = index_translationy0[int(startj1 + j1 - 2 * npts[1][0])]
                            auxL2 = 0.0
                            for j2 in range(lenj2):
                                # position 2 to evaluate rhs
                                if startj2 + j2 < original_pts_sizey[1]:
                                    ...
                                    pos2 = index_translationy1[int(startj2 + j2 + shift2)]
                                if whij1[i1][j2] != 0.0:
                                    auxL3 = 0.0
                                    for j3 in range(lenj3):
                                        # position 3 to evaluate rhs
                                        if startj3 + j3 < original_pts_sizey[2]:
                                            ...
                                            pos3 = index_translationy2[int(startj3 + j3 - 2 * npts[0][2])]
                                        auxL3 += wij2[i2][j3] * rhs1[pos1, pos2, pos3]
                                    auxL2 += whij1[i1][j2] * auxL3
                            L123 += wij0[i0][j1] * auxL2
                        out1[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
                    counteri1 += 1
                counteri0 += 1
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizez[0]:
                                ...
                                pos1 = index_translationz0[int(startj1 + j1 - 2 * npts[1][0])]
                            auxL2 = 0.0
                            for j2 in range(lenj2):
                                # position 2 to evaluate rhs
                                if startj2 + j2 < original_pts_sizez[1]:
                                    ...
                                    pos2 = index_translationz1[int(startj2 + j2 - 2 * npts[0][1])]
                                auxL3 = 0.0
                                for j3 in range(lenj3):
                                    # position 3 to evaluate rhs
                                    if startj3 + j3 < original_pts_sizez[2]:
                                        ...
                                        pos3 = index_translationz2[int(startj3 + j3 + shift3)]
                                    if whij2[i2][j3] != 0.0:
                                        auxL3 += whij2[i2][j3] * rhs2[pos1, pos2, pos3]
                                auxL2 += wij1[i1][j2] * auxL3
                            L123 += wij0[i0][j1] * auxL2
                        out2[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
                    counteri1 += 1
                counteri0 += 1
            ...
            for i0 in range(starts[h][0], ends[h][0] + 1):
                ...
                for i1 in range(starts[h][1], ends[h][1] + 1):
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        L123 = 0.0
                        # For the third input I need the number of B-splines
                        startj1, endj1 = select_quasi_points(i0, p1, npts[0][0], periodic[0][0])
                        startj2, endj2 = select_quasi_points(i1, p2, npts[1][1], periodic[0][1])
                        startj3, endj3 = select_quasi_points(i2, p3, npts[2][2], periodic[0][2])
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizex[0]:
                                ...
                                pos1 = index_translationx0[int(startj1 + j1 - 2 * npts[0][0])]
                            auxL2 = 0.0
                            for j2 in range(lenj2):
                                # position 2 to evaluate rhs
                                if startj2 + j2 < original_pts_sizex[1]:
                                    ...
                                    pos2 = index_translationx1[int(startj2 + j2 + shift2)]
                                if whij1[i1][j2] != 0.0:
                                    auxL3 = 0.0
                                    for j3 in range(lenj3):
                                        # position 3 to evaluate rhs
                                        if startj3 + j3 < original_pts_sizex[2]:
                                            ...
                                            pos3 = index_translationx2[int(startj3 + j3 + shift3)]
                                        if whij2[i2][j3] != 0.0:
                                            auxL3 += whij2[i2][j3] * rhs0[pos1, pos2, pos3]
                                    auxL2 += whij1[i1][j2] * auxL3
                            L123 += wij0[i0][j1] * auxL2
                        out0[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizey[0]:
                                ...
                                pos1 = index_translationy0[int(startj1 + j1 + shift1)]
                            if whij0[i0][j1] != 0.0:
                                auxL2 = 0.0
                                for j2 in range(lenj2):
                                    # position 2 to evaluate rhs
                                    if startj2 + j2 < original_pts_sizey[1]:
                                        ...
                                        pos2 = index_translationy1[int(startj2 + j2 - 2 * npts[1][1])]
                                    auxL3 = 0.0
                                    for j3 in range(lenj3):
                                        # position 3 to evaluate rhs
                                        if startj3 + j3 < original_pts_sizey[2]:
                                            ...
                                            pos3 = index_translationy2[int(startj3 + j3 + shift3)]
                                        if whij2[i2][j3] != 0.0:
                                            auxL3 += whij2[i2][j3] * rhs1[pos1, pos2, pos3]
                                    auxL2 += wij1[i1][j2] * auxL3
                                L123 += whij0[i0][j1] * auxL2
                        out1[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizez[0]:
                                ...
                                pos1 = index_translationz0[int(startj1 + j1 + shift1)]
                            if whij0[i0][j1] != 0.0:
                                auxL2 = 0.0
                                for j2 in range(lenj2):
                                    # position 2 to evaluate rhs
                                    if startj2 + j2 < original_pts_sizez[1]:
                                        ...
                                        pos2 = index_translationz1[int(startj2 + j2 + shift2)]
                                    if whij1[i1][j2] != 0.0:
                                        auxL3 = 0.0
                                        for j3 in range(lenj3):
                                            # position 3 to evaluate rhs
                                            if startj3 + j3 < original_pts_sizez[2]:
                                                ...
                                                pos3 = index_translationz2[int(startj3 + j3 - 2 * npts[2][2])]
                                            auxL3 += wij2[i2][j3] * rhs2[pos1, pos2, pos3]
                                        auxL2 += whij1[i1][j2] * auxL3
                                L123 += whij0[i0][j1] * auxL2
                        out2[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
                        counteri2 += 1
...
def solve_local_3_form(
    ...
    for i0 in range(starts[0], ends[0] + 1):
        ...
        for i1 in range(starts[1], ends[1] + 1):
            ...
            for i2 in range(starts[2], ends[2] + 1):
                L123 = 0.0
                startj1, endj1 = select_quasi_points(i0, p1, npts[0] + 1, periodic[0])
                startj2, endj2 = select_quasi_points(i1, p2, npts[1] + 1, periodic[1])
                startj3, endj3 = select_quasi_points(i2, p3, npts[2] + 1, periodic[2])
                    ...
                    if whij0[i0][j1] != 0.0:
                        ...
                        for j2 in range(2 * p2):
                            ...
                            if startj2 + j2 < original_size2:
                                ...
                                pos2 = index_translation2[int(startj2 + j2 + shift2)]
                            if whij1[i1][j2] != 0.0:
                                auxL3 = 0.0
                                for j3 in range(2 * p3):
                                    ...
                                    if startj3 + j3 < original_size3:
                                        ...
                                        pos3 = index_translation3[int(startj3 + j3 + shift3)]
                                    if whij2[i2][j3] != 0.0:
                                        auxL3 += whij2[i2][j3] * rhs[pos1, pos2, pos3]
                                auxL2 += whij1[i1][j2] * auxL3
                        L123 += whij0[i0][j1] * auxL2
                out[pds[0] + counteri0, pds[1] + counteri1, pds[2] + counteri2] = L123
                counteri2 += 1
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizey[0]:
                                ...
                                pos1 = index_translationy0[int(startj1 + j1 - 2 * npts[0][0])]
                            auxL2 = 0.0
                            for j2 in range(lenj2):
                                # position 2 to evaluate rhs
                                if startj2 + j2 < original_pts_sizey[1]:
                                    ...
                                    pos2 = index_translationy1[int(startj2 + j2 - 2 * npts[0][1])]
                                auxL3 = 0.0
                                for j3 in range(lenj3):
                                    # position 3 to evaluate rhs
                                    if startj3 + j3 < original_pts_sizey[2]:
                    ...
                    for i2 in range(starts[h][2], ends[h][2] + 1):
                        ...
                        for j1 in range(lenj1):
                            ...
                            if startj1 + j1 < original_pts_sizez[0]:
                                ...
                                pos1 = index_translationz0[int(startj1 + j1 - 2 * npts[0][0])]
                            auxL2 = 0.0
                            for j2 in range(lenj2):
                                # position 2 to evaluate rhs
                                if startj2 + j2 < original_pts_sizez[1]:
                                ...
                                for j3 in range(lenj3):
                                    ...
                                    if startj3 + j3 < original_pts_sizez[2]:
                                        ...
                                        pos3 = index_translationz2[int(startj3 + j3 - 2 * npts[0][2])]
                                    auxL3 += wij2[i2][j3] * rhs2[pos1, pos2, pos3]
                                auxL2 += wij1[i1][j2] * auxL3
                            L123 += wij0[i0][j1] * auxL2
                        out2[pds[h][0] + counteri0, pds[h][1] + counteri1, pds[h][2] + counteri2] = L123
