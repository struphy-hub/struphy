...
def test_bckgr_init_const(Nel, p, spl_kind, spaces, vec_comps):
    ...
    bckgr_params = {"type": "LogicalConst", "LogicalConst": {"comps": {}}}
    ...
    for i, (space, val) in enumerate(zip(spaces, vals)):
        field = derham.create_field("name_" + str(i), space, bckgr_params=bckgr_params)
...
def test_bckgr_init_mhd(Nel, p, spl_kind, with_desc, show_plot=False):
    ...
    # background parameters
    bckgr_params = {"type": "MHD", "MHD": {"comps": {}}}
    ...
    for key, val in inspect.getmembers(equils):
        if inspect.isclass(val) and "MHDequilibrium" not in key:
            ...
            field_0 = derham.create_field("name_0", "H1", bckgr_params=bckgr_params)
            field_1 = derham.create_field("name_1", "Hcurl", bckgr_params=bckgr_params)
            field_2 = derham.create_field("name_2", "Hdiv", bckgr_params=bckgr_params)
            field_3 = derham.create_field("name_3", "L2", bckgr_params=bckgr_params)
            field_4 = derham.create_field("name_4", "H1vec", bckgr_params=bckgr_params)
            ...
            assert (
                np.max(np.abs(field_0(*meshgrids) - mhd_equil.absB0(*meshgrids)))
                / np.max(np.abs(mhd_equil.absB0(*meshgrids)))
                < 0.057
            )
            assert (
                np.max(np.abs(field_3(*meshgrids) - mhd_equil.p3(*meshgrids)))
            ...
            if np.max(np.abs(ref[0])) < 1e-11:
                ...
                denom = np.max(np.abs(ref[0]))
            print(f"{np.max(np.abs(field_2(*meshgrids)[0] - ref[0])) / denom = }")
            assert np.max(np.abs(field_2(*meshgrids)[0] - ref[0])) / denom < 0.86
            if np.max(np.abs(ref[1])) < 1e-11:
                denom = 1.0
            else:
                denom = np.max(np.abs(ref[1]))
            print(f"{np.max(np.abs(field_2(*meshgrids)[1] - ref[1])) / denom = }")
            assert np.max(np.abs(field_2(*meshgrids)[1] - ref[1])) / denom < 0.4
            if np.max(np.abs(ref[2])) < 1e-11:
                denom = 1.0
            else:
                denom = np.max(np.abs(ref[2]))
            print(f"{np.max(np.abs(field_2(*meshgrids)[2] - ref[2])) / denom = }")
            ...
            if np.max(np.abs(ref[0])) < 1e-11:
                ...
                denom = np.max(np.abs(ref[0]))
            print(f"{np.max(np.abs(field_4(*meshgrids)[0] - ref[0])) / denom = }")
            assert np.max(np.abs(field_4(*meshgrids)[0] - ref[0])) / denom < 0.55
            if np.max(np.abs(ref[1])) < 1e-11:
                denom = 1.0
            else:
                denom = np.max(np.abs(ref[1]))
            print(f"{np.max(np.abs(field_4(*meshgrids)[1] - ref[1])) / denom = }")
            assert np.max(np.abs(field_4(*meshgrids)[1] - ref[1])) / denom < 0.2
            if np.max(np.abs(ref[2])) < 1e-11:
                denom = 1.0
            else:
                denom = np.max(np.abs(ref[2]))
            print(f"{np.max(np.abs(field_4(*meshgrids)[2] - ref[2])) / denom = }")
            assert np.max(np.abs(field_4(*meshgrids)[2] - ref[2])) / denom < 0.04
            ...
            if show_plot and rank == 0:
                ...
                plt.figure(f"0/3-forms poloidal, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"1-forms top, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"1-forms poloidal, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"2-forms top, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"2-forms poloidal, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"vector-fields top, {mhd_equil = }", figsize=(24, 16))
                plt.figure(f"vector-fields poloidal, {mhd_equil = }", figsize=(24, 16))
                ...
                if "Slab" in key or "Pinch" in key:
                    plt.contourf(x[:, 0, :], z[:, 0, :], absB0_h[:, 0, :], levels=levels)
                    plt.contourf(
                        x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], absB0_h[:, Nel[1] // 2, :], levels=levels
                    )
                    plt.xlabel("x")
                    plt.ylabel("z")
                else:
                    plt.contourf(x[:, 0, :], y[:, 0, :], absB0_h[:, 0, :], levels=levels)
                    plt.contourf(
                        x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], absB0_h[:, Nel[1] // 2, :], levels=levels
                    )
                    plt.xlabel("x")
                    plt.ylabel("y")
                plt.axis("equal")
                ...
                plt.subplot(2, 3, 3 + 1)
                if "Slab" in key or "Pinch" in key:
                    plt.contourf(x[:, 0, :], z[:, 0, :], absB0[:, 0, :], levels=levels)
                    plt.contourf(
                        x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], absB0[:, Nel[1] // 2, :], levels=levels
                    )
                    plt.xlabel("x")
                    plt.ylabel("z")
                else:
                    plt.contourf(x[:, 0, :], y[:, 0, :], absB0[:, 0, :], levels=levels)
                    plt.contourf(
                        x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], absB0[:, Nel[1] // 2, :], levels=levels
                    )
                    plt.xlabel("x")
                    plt.ylabel("y")
                plt.axis("equal")
                ...
                plt.subplot(2, 3, 2)
                if "Slab" in key or "Pinch" in key:
                    plt.contourf(x[:, 0, :], z[:, 0, :], p3_h[:, 0, :], levels=levels)
                    plt.contourf(x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], p3_h[:, Nel[1] // 2, :], levels=levels)
                    plt.xlabel("x")
                    plt.ylabel("z")
                else:
                    plt.contourf(x[:, 0, :], y[:, 0, :], p3_h[:, 0, :], levels=levels)
                    plt.contourf(x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], p3_h[:, Nel[1] // 2, :], levels=levels)
                    plt.xlabel("x")
                    plt.ylabel("y")
                plt.axis("equal")
                plt.colorbar()
                ...
                plt.subplot(2, 3, 3 + 2)
                if "Slab" in key or "Pinch" in key:
                    plt.contourf(x[:, 0, :], z[:, 0, :], p3[:, 0, :], levels=levels)
                    plt.contourf(x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], p3[:, Nel[1] // 2, :], levels=levels)
                    plt.xlabel("x")
                    plt.ylabel("z")
                else:
                    plt.contourf(x[:, 0, :], y[:, 0, :], p3[:, 0, :], levels=levels)
                    plt.contourf(x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], p3[:, Nel[1] // 2, :], levels=levels)
                    plt.xlabel("x")
                    plt.ylabel("y")
                plt.axis("equal")
                plt.colorbar()
                ...
                b1h = mhd_equil.domain.push(field_1(*meshgrids), *meshgrids, kind="1")
                b1 = mhd_equil.domain.push([*mhd_equil.b1(*meshgrids)], *meshgrids, kind="1")
                ...
                for i, (bh, b) in enumerate(zip(b1h, b1)):
                    ...
                    plt.subplot(2, 3, 1 + i)
                    if "Slab" in key or "Pinch" in key:
                        plt.contourf(x[:, 0, :], z[:, 0, :], bh[:, 0, :], levels=levels)
                        plt.contourf(
                            x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], bh[:, Nel[1] // 2, :], levels=levels
                        )
                        plt.xlabel("x")
                        plt.ylabel("z")
                    else:
                        plt.contourf(x[:, 0, :], y[:, 0, :], bh[:, 0, :], levels=levels)
                        plt.contourf(
                            x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], bh[:, Nel[1] // 2, :], levels=levels
                        )
                        plt.xlabel("x")
                        plt.ylabel("y")
                    plt.axis("equal")
                    ...
                    if "Slab" in key or "Pinch" in key:
                        plt.contourf(x[:, 0, :], z[:, 0, :], b[:, 0, :], levels=levels)
                        plt.contourf(
                            x[:, Nel[1] // 2, :], z[:, Nel[1] // 2 - 1, :], b[:, Nel[1] // 2, :], levels=levels
                        )
                        plt.xlabel("x")
                        plt.ylabel("z")
                    else:
                        plt.contourf(x[:, 0, :], y[:, 0, :], b[:, 0, :], levels=levels)
                        plt.contourf(
                            x[:, Nel[1] // 2, :], y[:, Nel[1] // 2 - 1, :], b[:, Nel[1] // 2, :], levels=levels
                        )
                        plt.xlabel("x")
                        plt.ylabel("y")
                    plt.axis("equal")
                    ...
                    plt.axis("equal")
                    plt.colorbar()
                    plt.title(f"Equilibrium $B_{i + 1}$, poloidal view (e1-e2)")
                    plt.subplot(2, 3, 3 + 1 + i)
                    if "Slab" in key or "Pinch" in key:
                        plt.contourf(x[:, :, 0], y[:, :, 0], b[:, :, 0], levels=levels)
                        plt.xlabel("x")
                ...
                b2h = mhd_equil.domain.push(field_2(*meshgrids), *meshgrids, kind="2")
                b2 = mhd_equil.domain.push([*mhd_equil.b2(*meshgrids)], *meshgrids, kind="2")
                ...
                bvh = mhd_equil.domain.push(field_4(*meshgrids), *meshgrids, kind="v")
                bv = mhd_equil.domain.push([*mhd_equil.bv(*meshgrids)], *meshgrids, kind="v")
...
def test_sincos_init_const(Nel, p, spl_kind, show_plot=False):
    ...
    bckgr_params = {
        "type": "LogicalConst",
        "LogicalConst": {"comps": {"name_0": avg_0, "name_1": avg_1, "name_2": avg_2}},
    }
    ...
    pert_params = {
        ...
        "ModesSin": {
            "comps": {"name_0": "0", "name_1": ["1", None, "1"]},
            "ms": {"name_0": ms_s, "name_1": [ms_s, None, ms_s]},
            "ns": {"name_0": ns_s, "name_1": [ns_s, None, ns_s]},
            "amps": {"name_0": amps, "name_1": [amps, None, amps]},
        },
        "ModesCos": {
            "comps": {"name_0": "0", "name_1": ["1", "1", None], "name_2": [None, "2", None]},
            "ms": {"name_0": ms_c, "name_1": [ms_c, ms_c, None], "name_2": [None, ms_c, None]},
            "ns": {"name_0": ns_c, "name_1": [ns_c, ns_c, None], "name_2": [None, ns_c, None]},
            "amps": {"name_0": amps, "name_1": [amps, amps, None], "name_2": [None, amps, None]},
        },
    }
    ...
    field_0 = derham.create_field("name_0", "H1", bckgr_params=bckgr_params, pert_params=pert_params)
    field_1 = derham.create_field("name_1", "Hcurl", bckgr_params=bckgr_params, pert_params=pert_params)
    field_2 = derham.create_field("name_2", "Hdiv", bckgr_params=bckgr_params, pert_params=pert_params)
    ...
    fun_2 = [avg_2[0] + 0.0 * meshgrids[0], avg_2[1] + f_cos(*meshgrids), avg_2[2] + 0.0 * meshgrids[0]]
...
if __name__ == "__main__":
    ...
    test_bckgr_init_mhd([18, 24, 12], [1, 2, 1], [False, True, True], show_plot=False)
