...
def test_noise_init(Nel, p, spl_kind, space, direction):
    ...
    field_np = derham_xp.create_spline_function("field", space)
    ...
    field_xp.initialize_coeffs(pert_params=pert_params)

    # print('#'*80)
    # print(f'npts={field.vector[0].space.npts}, npts_np={field_xp.vector[0].space.npts}')
    # print(f'rank={rank}: nprocs={derham.domain_array[rank]}')
    # print(f'rank={rank}, field={field.vector[0].toarray_local().shape}, field_np={field_xp.vector[0].toarray_local().shape}')
    # print(f'rank={rank}: \ncomp{0}={field.vector[0].toarray_local()}, \ncomp{0}_np={field_xp.vector[0].toarray_local()}')

    compare_arrays(
        field.vector,
        [field_xp.vector[n].toarray_local() for n in range(3)],
        rank,
    )
