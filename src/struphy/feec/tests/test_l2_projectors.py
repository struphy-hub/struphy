...
def test_l2_projectors_mappings(Nel, p, spl_kind, array_input, do_plot=False):
    ...
    for key, val in inspect.getmembers(domains):
        if inspect.isclass(val) and key != 'Domain':
            dom_types += [key]
            dom_classes += [val]
...
@pytest.mark.parametrize('pi', [1, 2, 3])
...
def test_l2_projectors_convergence(direction, pi, spl_kindi, do_plot=False):
    ...
    for n, Neli in enumerate(Nels):
        ...
        # create domain object
        dom_type = 'Cuboid'
        dom_params = {'l1': 0., 'r1': 1., 'l2': 0., 'r2': 1., 'l3': 0., 'r3': 1.}
