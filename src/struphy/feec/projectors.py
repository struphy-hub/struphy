...
class CommutingProjectorLocal:
    ...
    @property
    def pts(self):
        """3D (4D for BlockVectors) list of 2D array with the quasi-interpolation points (or Gauss-Legendre quadrature points for histopolation). In format (ns, nb, xp. = (spatial direction, B-spline index, point) for StencilVector spaces or (nv,ns, nb, xp. = (vector entry,spatial direction, B-spline index, point) for BlockVector spaces."""
        return self._pts

    @property
    def wts(self):
        """3D (4D for BlockVectors) list of 2D array with the Gauss-Legendre quadrature points (full of ones for interpolation). In format (ns, nb, xp. = (spatial direction, B-spline index, point) for StencilVector spaces or (nv,ns, nb, xp. = (vector entry,spatial direction, B-spline index, point) for BlockVector spaces."""
        return self._wts

    @property
    def wij(self):
        r"""List of 2D arrays for the coefficients :math:`\omega_j^i` obtained by inverting the local collocation matrix. Use for obtaining the FE coefficients of a function via interpolation. In format (ns, nb, xp. = (spatial direction, B-spline index, point)."""
        return self._wij

    @property
    def whij(self):
        r"""List of 2D arrays for the coefficients :math:`\hat{\omega}_j^i` obtained from the :math:`\omega_j^i`. Use for obtaining the FE coefficients of a function via histopolation. In format (ns, nb, xp. = (spatial direction, D-spline index, point)."""
        return self._whij
