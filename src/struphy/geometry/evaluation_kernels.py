...
def f(eta1: float, eta2: float, eta3: float, args: "DomainArguments", f_out: "float[:]"):
    """
    Point-wise evaluation of (x, y, z) = F(eta1, eta2, eta3).
    """

    if args.kind_map == 0:
        mappings_kernels.spline_3d(eta1, eta2, eta3, args.p, args.ind1, args.ind2, args.ind3, args, f_out)
    elif args.kind_map == 1:
        mappings_kernels.spline_2d_straight(eta1, eta2, eta3, args.p, args.ind1, args.ind2, args, args.params[0], f_out)
    elif args.kind_map == 2:
        mappings_kernels.spline_2d_torus(eta1, eta2, eta3, args.p, args.ind1, args.ind2, args, args.params[0], f_out)
        ...
        mappings_kernels.hollow_cyl(eta1, eta2, eta3, args.params[0], args.params[1], args.params[2], f_out)
        ...
        mappings_kernels.hollow_torus(
            eta1, eta2, eta3, args.params[0], args.params[1], args.params[2], args.params[3], args.params[4], f_out
        )
...
@stack_array("tmp1", "tmp2", "tmp3")
def df(eta1: float, eta2: float, eta3: float, args: "DomainArguments", df_out: "float[:,:]"):
    """
    Point-wise evaluation of the Jacobian matrix DF = (dF_i/deta_j)_(i,j=1,2,3).
    """

    if args.kind_map == 0:
        mappings_kernels.spline_3d_df(eta1, eta2, eta3, args.p, args.ind1, args.ind2, args.ind3, args, df_out)
    elif args.kind_map == 1:
        mappings_kernels.spline_2d_straight_df(eta1, eta2, args.p, args.ind1, args.ind2, args, args.params[0], df_out)
    elif args.kind_map == 2:
        mappings_kernels.spline_2d_torus_df(
            eta1, eta2, eta3, args.p, args.ind1, args.ind2, args, args.params[0], df_out
        )
    elif args.kind_map == 10:
        mappings_kernels.cuboid_df(
            args.params[0], args.params[1], args.params[2], args.params[3], args.params[4], args.params[5], df_out
        )
    elif args.kind_map == 11:
        ...
        mappings_kernels.colella_df(eta1, eta2, args.params[0], args.params[1], args.params[2], args.params[3], df_out)
    elif args.kind_map == 20:
...
@stack_array("df_mat")
def det_df(eta1: float, eta2: float, eta3: float, args: "DomainArguments") -> float:
    """
...
@stack_array("df_mat")
def df_inv(eta1: float, eta2: float, eta3: float, args: "DomainArguments", dfinv_out: "float[:,:]"):
    """
...
def g(eta1: float, eta2: float, eta3: float, args: "DomainArguments", g_out: "float[:,:]"):
    """
...
@stack_array("g_mat")
def g_inv(eta1: float, eta2: float, eta3: float, args: "DomainArguments", ginv_out: "float[:,:]"):
    """
...
@stack_array("tmp1")
def select_fun(eta1: float, eta2: float, eta3: float, args: "DomainArguments", mat_f: "float[:,:]", kind_coeff: int):
    """
    ...
    # Jacobian determinant det(dF)
        ...
        mat_f[0, 0] = det_df(eta1, eta2, eta3, args)
...
@stack_array("np", "countere1", "e2", "e3", "tmp1")
def kernel_evaluate_pic(
    markers: "float[:,:]", kind_coeff: int, args: "DomainArguments", mat_f: "float[:,:,:]", remove_outside: bool
) -> int:
    """
...
def kernel_evaluate_pic_flat(
    ...
    for i in range(np):
        ...
        if False and (
            e1 < 0.0 or e1 > 1.0 or e2 < 0.0 or e2 > 1.0 or e3 < 0.0 or e3 > 1.0
        ):  # TODO (Mati) !!! REMOVE FALSE AFTER REFLECT BC FIX!!!
            if remove_outside:
                continue
            else:
                if kind_coeff >= 0:
