...
import struphy.bsplines.evaluation_kernels_2d as evaluation_kernels_2d_mod
...
def spline_3d(
    ...
    # mapping spans
    span1 = bsplines_kernels_mod.find_span(args.t1, int(p[0]), eta1)
    span2 = bsplines_kernels_mod.find_span(args.t2, int(p[1]), eta2)
    span3 = bsplines_kernels_mod.find_span(args.t3, int(p[2]), eta3)
    ...
    bsplines_kernels_mod.b_splines_slim(args.t1, int(p[0]), eta1, span1, b1)
    bsplines_kernels_mod.b_splines_slim(args.t2, int(p[1]), eta2, span2, b2)
    bsplines_kernels_mod.b_splines_slim(args.t3, int(p[2]), eta3, span3, b3)
    ...
    f_out[0] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, b2, b3, tmp1, tmp2, tmp3, args.cx
    )
    f_out[1] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, b2, b3, tmp1, tmp2, tmp3, args.cy
    )
    f_out[2] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
...
def spline_3d_df(
    ...
    bsplines_kernels_mod.b_der_splines_slim(args.t1, int(p[0]), eta1, span1, b1, der1)
    bsplines_kernels_mod.b_der_splines_slim(args.t2, int(p[1]), eta2, span2, b2, der2)
    bsplines_kernels_mod.b_der_splines_slim(args.t3, int(p[2]), eta3, span3, b3, der3)
    ...
    df_out[0, 0] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), der1, b2, b3, tmp1, tmp2, tmp3, args.cx
    )
    df_out[0, 1] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, der2, b3, tmp1, tmp2, tmp3, args.cx
    )
    df_out[0, 2] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, b2, der3, tmp1, tmp2, tmp3, args.cx
    )
    df_out[1, 0] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), der1, b2, b3, tmp1, tmp2, tmp3, args.cy
    )
    df_out[1, 1] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, der2, b3, tmp1, tmp2, tmp3, args.cy
    )
    df_out[1, 2] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, b2, der3, tmp1, tmp2, tmp3, args.cy
    )
    df_out[2, 0] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), der1, b2, b3, tmp1, tmp2, tmp3, args.cz
    )
    df_out[2, 1] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
        int(p[0]), int(p[1]), int(p[2]), b1, der2, b3, tmp1, tmp2, tmp3, args.cz
    )
    df_out[2, 2] = evaluation_kernels_3d_mod.evaluation_kernel_3d(
...
def spline_2d_straight(
    ...
    f_out[0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cx)
    f_out[1] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cy)
    f_out[2] = lz * eta3
...
def spline_2d_straight_df(
    ...
    df_out[0, 0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), der1, b2, tmp1, tmp2, cx)
    df_out[0, 1] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, der2, tmp1, tmp2, cx)
    df_out[0, 2] = 0.0
    df_out[1, 0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), der1, b2, tmp1, tmp2, cy)
    df_out[1, 1] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, der2, tmp1, tmp2, cy)
    df_out[1, 2] = 0.0
    df_out[2, 0] = 0.0
    df_out[2, 1] = 0.0
    df_out[2, 2] = lz
...
def spline_2d_torus(
    ...
    f_out[0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cx) * cos(
        2 * pi * eta3 / tor_period
    )
    f_out[1] = (
        evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cx)
        * (-1)
        * sin(2 * pi * eta3 / tor_period)
    )
    f_out[2] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cy)
...
def spline_2d_torus_df(
    ...
    df_out[0, 0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), der1, b2, tmp1, tmp2, cx) * cos(
        2 * pi * eta3 / tor_period
    )
    df_out[0, 1] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, der2, tmp1, tmp2, cx) * cos(
        2 * pi * eta3 / tor_period
    )
    df_out[0, 2] = (
        evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, b2, tmp1, tmp2, cx)
    ...
    df_out[1, 0] = (
        evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), der1, b2, tmp1, tmp2, cx)
        * (-1)
        * sin(2 * pi * eta3 / tor_period)
    )
    df_out[1, 1] = (
        evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, der2, tmp1, tmp2, cx)
        * (-1)
        * sin(2 * pi * eta3 / tor_period)
    )
    df_out[1, 2] = (
    ...
    df_out[2, 0] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), der1, b2, tmp1, tmp2, cy)
    df_out[2, 1] = evaluation_kernels_2d_mod.evaluation_kernel_2d(int(p[0]), int(p[1]), b1, der2, tmp1, tmp2, cy)
    df_out[2, 2] = 0.0
