def run(n_procs):
    ...
    subprocess.run(
        [
            ...
            "LinearMHDVlasovPC",
            "-i",
            os.path.join(libpath, "io/inp/longer_examples/params_hybridmhdvlasovpc.yml"),
            "-o",
            out_name,
            "--mpi",
            str(n_procs),
    ...
    subprocess.run(["struphy", "pproc", "-d", out_name], check=True)
...
def diagnostics():
    ...
    f = np.load(
        os.path.join(out_path, "post_processing/kinetic_data/energetic_ions/distribution_function/v3/f_binned.npy")
    )
    vz = np.load(
        os.path.join(out_path, "post_processing/kinetic_data/energetic_ions/distribution_function/v3/grid_v3.npy")
    )
    ...
    plt.arrow(51, 2.8e-2, 7.0, 0.0, head_width=0.01, head_length=0.5000)
    ...
    plt.arrow(3.3, 0.03, -0.5, 0.0, head_width=0.002, head_length=0.15)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mpi", type=int, metavar="N", help="number of MPI processes used to run the model (default=1)", default=1
    )
