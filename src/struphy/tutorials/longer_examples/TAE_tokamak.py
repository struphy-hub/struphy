def run(n_procs):
    ...
    subprocess.run(["struphy", "pproc", "-d", out_name, "-s", "500", "--celldivide", "5"], check=True)
...
def diagnostics():
    ...
    for m in range(2, 4 + 1):
        ...
        plt.plot(
            domain(grids_log[0], 0.0, 0.0)[0] - mhd_params["R0"],
            spec_calc(domain(grids_log[0], 0.0, 0.0)[0] - mhd_params["R0"], m, -2)["shear_Alfvén"] ** 2 / omegaA**2,
            "k--",
            linewidth=0.5,
        )
    ...
    plt.arrow(0.44, 0.5, 0.0, -0.30, head_width=0.02)
    plt.text(0.39, 0.55, "TAE")
    plt.plot(0.1 * np.ones(11), np.linspace(0.0, 1.0, 11), "k--")

    # plot U2_1(t=0) on mapped grid
    plt.subplot(2, 2, 3)

    plt.contourf(grids_phy[0][:, :, 0], grids_phy[2][:, :, 0], u_field_log[0.0][0][:, :, 8], levels=51, cmap="coolwarm")
