...
def struphy_test(group, mpi=2, fast=False, with_desc=False, verbose=False, monitor=False, n=None, Tend=None):
    ...
    if "unit" in group:
        ...
        if with_desc:
            subprocess.run(cmd + ["--with-desc"], check=True, cwd=libpath)
        else:
            subprocess.run(cmd, check=True, cwd=libpath)
        ...
        if fast:
            subprocess.run(cmd + ["--fast"], check=True, cwd=libpath)
        else:
            subprocess.run(cmd, check=True, cwd=libpath)
        ...
        for obj in objs:
            try:
                ...
                if fast:
                    print(f"Fast is enabled, mappings other than Cuboid are skipped ...")
                else:
...
def pymon_html_json(group, verbose=False):
    ...
    filename = os.path.join(pymon_abs, "pymon_" + group + "_contexts_" + language + ".html")
    ...
    filename = os.path.join(pymon_abs, "pymon_" + group + "_sessions_" + language + ".html")
    with open(filename, "r+") as f:
        content = f.read()
        f.seek(0, 0)
    ...
    filename = os.path.join(pymon_abs, "pymon_" + group + "_metrics_" + language + ".html")
    with open(filename, "r+") as f:
        content = f.read()
        f.seek(0, 0)
        f.write("<TABLE BORDER=1 WIDTH=1200>".rstrip("\r\n") + "\n" + content)
