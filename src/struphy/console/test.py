    ...
    if "unit" in group:
        ...
        for obj in objs:
            try:
                ...
                if fast:
                    print(
                        "Fast is enabled, mappings other than Cuboid are skipped ...",
                    )
                else:
                    obj(
...
def pymon_html_json(group, verbose=False):
    ...
    filename = os.path.join(
        pymon_abs,
        "pymon_" + group + "_contexts_" + language + ".html",
    )
    ...
    filename = os.path.join(
        pymon_abs,
        "pymon_" + group + "_sessions_" + language + ".html",
    )
    with open(filename, "r+") as f:
        content = f.read()
        f.seek(0, 0)
    ...
    filename = os.path.join(
        pymon_abs,
        "pymon_" + group + "_metrics_" + language + ".html",
    )
    with open(filename, "r+") as f:
        content = f.read()
        f.seek(0, 0)
