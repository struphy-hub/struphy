...
def struphy():
    ...
    parser.add_argument(
        "-p", "--path", action="version", version=path_message, help="default installations and i/o paths"
    )
    ...
    subparsers = parser.add_subparsers(title="available commands", metavar="COMMAND", dest="command")
    ...
    parser_compile.add_argument(
        "--language", type=str, metavar="LANGUAGE", help='either "c" (default) or "fortran"', default="c"
    )
    ...
    parser_compile.add_argument("--omp-pic", help="compile PIC kernels with OpenMP", action="store_true")

    parser_compile.add_argument("--omp-feec", help="compile FEEC kernels with OpenMP", action="store_true")

    parser_compile.add_argument(
        "-d", "--delete", help="remove .f90/.c and .so files (for running pure Python code)", action="store_true"
    )

    parser_compile.add_argument(
        "-s", "--status", help="print current Struphy compilation status on screen", action="store_true"
    )

    parser_compile.add_argument(
        "-v", "--verbose", help="call pyccel with --verbose compiler option", action="store_true"
    )
    ...
    parser_compile.add_argument("-y", "--yes", help="say yes to prompt when changing the language", action="store_true")
    ...
    parser_params.add_argument("-o", "--options", help="show model options", action="store_true")

    parser_params.add_argument("-y", "--yes", help="Say yes on prompt to overwrite .yml FILE", action="store_true")
    ...
    parser_profile.add_argument("dirs", type=str, nargs="+", metavar="DIR", help="simulation ouput folders")
    ...
    parser_test.add_argument("-f", "--fast", help="test model(s) just in slab geometry (Cuboid)", action="store_true")

    parser_test.add_argument(
        "--with-desc", help="include DESC equilibrium in tests (mem consuming)", action="store_true"
    )

    parser_test.add_argument("-v", "--verbose", help="print timings to screen", action="store_true")

    parser_test.add_argument("--monitor", help="use pytest-monitor in tests", action="store_true")

    parser_test.add_argument("-n", type=int, help="specific tutorial simulation to run (int, optional)", default=None)
    ...
    if args.refresh_models:
        ...
        with open(os.path.join(libpath, "models", "models_message"), "wb") as fp:
            pickle.dump([model_message, fluid_message, kinetic_message, hybrid_message, toy_message], fp)
...
def recursive_get_files(path, contains=(".yml", ".yaml"), out=[], prefix=[]):
    ...
    for name in all_names:
        if any([cont in name for cont in contains]):
            ...
            recursive_get_files(os.path.join(path, name), contains=contains, out=out, prefix=prefix)
