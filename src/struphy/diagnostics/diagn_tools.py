...
def power_spectrum_2d(
    ...
    grids : 3-tuple
        1d logical grids in each eta-direction with Nel[i]*xp.s_per_cell[i] + 1 entries in each direction.
...
def plot_distr_fun(
    ...
    for folder in os.listdir(path):
        ...
        for filename in os.listdir(subpath):
            ...
            if filename == "f_binned.xp.":
                f = xp.load(filepath)

            # load delta f
            elif filename == "delta_f_binned.xp.":
...
def get_slices_grids_directions_and_df_data(plot_full_f, grid_slices, data_path, slice_name, background_params=None):
    ...
    for direction in directions:
        grids[direction] = xp.load(
            os.path.join(data_path, "grid_" + direction + ".xp."),
        )
