...
def power_spectrum_2d(
    ...
    if do_plot:
        ...
        disp_plot = ax.contourf(
            K, W, dispersion**2 / (dispersion**2).max(), cmap=colormap, norm=colors.LogNorm(), levels=lvls
        )
...
def plot_scalars(
    ...
    if "en_tot" in scalar_quantities.keys():
        ...
        plt.plot(time[1:], np.divide(np.abs(en_tot[1:] - en_tot[0]), en_tot[0]))
    ...
    for key, plot_quantity in plot_quantities.items():
        ...
        if do_log:
            ...
            if inds_exs is not None:
                # do the fitting
                coeffs = np.polyfit(times_extrema, np.log(quantity_extrema), deg=degree)
                plt.plot(times_extrema, quantity_extrema, "r*", label="local extrema")
                plt.plot(
                    time_cut,
            ...
            if inds_exs is not None:
                # do the fitting
                coeffs = np.polyfit(times_extrema, quantity_extrema, deg=degree)
...
def plot_distr_fun(path, time_idx, grid_slices, save_plot=False, savepath=None, file_format="png"):
    ...
    for folder in os.listdir(path):
        ...
        for direction in directions:
            grids += [np.load(os.path.join(subpath, "grid_" + direction + ".npy"))]

        # Get indices of where to plot in other directions
        grid_idxs = {}
        for k in range(f.ndim - 1):
            grid_idxs[directions[k]] = np.argmin(np.abs(grids[k] - grid_slices[directions[k]]))
        ...
        for k in range(f.ndim - 1):
            ...
            if delta_f is not None:
                ...
                if save_plot:
                    ...
                    savename = os.path.join(savepath, species + "_delta_f_" + directions[k] + "." + file_format)
            ...
            if f is not None:
                ...
                if save_plot:
                    assert savepath is not None, "When wanting to save the plot a path has to be given!"
                    savename = os.path.join(savepath, species + "_f_" + directions[k] + "." + file_format)
                    plt.savefig(savename)
                else:
                    plt.show()
                plt.close()
...
def phase_space_video(t_grid, grid_slices, slice_name, marker_type, species, path, model_name, background_params=None):
    ...
    data_path = os.path.join(path, "post_processing", "kinetic_data", species, "distribution_function", slice_name)
    ...
    print(f"Found {len(slices_2d)} 2D slicing(s) for {species}, proceeding to generate images")

    # Load all the grids
    grids = []
    for direction in directions:
        grids += [np.load(os.path.join(data_path, "grid_" + direction + ".npy"))]
    ...
    if marker_type == "full_f":
        ...
        if background_type in background_params.keys():
            background_function = getattr(maxwellians, background_params["type"])(background_params[background_type])
        else:
            background_function = getattr(maxwellians, background_params["type"])()
        ...
        df_data = np.load(os.path.join(data_path, "f_binned.npy")) - background_data[None, :, :, :, :, :, :].squeeze()
    elif marker_type in ["control_variate", "delta_f"]:
        df_data = np.load(os.path.join(data_path, "delta_f_binned.npy"))
    else:
        raise NotImplementedError(f"Making a video for marker type {marker_type} is not implemented!")
    ...
    for slc in slices_2d:
        ...
        eta_grid = np.load(os.path.join(data_path, "grid_" + slc[:2] + ".npy"))
        v_grid = np.load(os.path.join(data_path, "grid_" + slc[-2:] + ".npy"))
        ...
        try:
            ...
            yn = input("It seems like cv2 is not installed. Would you like to install it now (Y/n)?")
        ...
        images = [img for img in sorted(os.listdir(imgs_folder)) if img.endswith(".png")]
...
def phase_space_plots(t_grid, eta_grid, v_grid, df_binned, save_path, model_name, eta_label=None, v_label=None):
    ...
    for n in tqdm(range(nt)):
        ...
        plt.savefig(os.path.join(save_path, "step_{0:0{1}d}.png".format(n, log_nt)), bbox_inches="tight")
        plt.clf()
