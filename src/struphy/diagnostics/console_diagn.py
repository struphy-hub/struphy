...
def main():
    ...
    parser.add_argument(
        "-f", nargs=1, type=str, default=["sim_1"], help="name of the folder for the simulation data (in io/out)"
    )
    parser.add_argument(
        "-scalars", nargs="+", action="append", default=[], help="(for plot_scalars) which quantities to plot"
    )
    parser.add_argument("--log", action="store_true", help="(for plot_scalars) if logarithmic y-axis should be used")
    parser.add_argument("--show", action="store_true", help="(for plot_scalars) if the plot should be shown")
    parser.add_argument("--nosave", action="store_true", help="(for plot_scalars) if the plot should not be displayed")
    parser.add_argument("--fit", action="store_true", help="(for plot_scalars) if a fit should be done (using maxima)")
    parser.add_argument("--minfit", action="store_true", help="(for plot_scalars) if a fit should be done using minima")
    parser.add_argument(
        "-degree", nargs=1, type=int, default=[1], help="(for plot_scalars --fit) the degree of the fit curve"
    )
    parser.add_argument(
        "-extrema",
        nargs=1,
    ...
    parser.add_argument(
        "-e1", nargs=1, type=float, default=[0.5], help="(for plot_distr) at which position in eta1 direction to plot"
    )
    parser.add_argument(
        "-e2", nargs=1, type=float, default=[0.5], help="(for plot_distr) at which position in eta2 direction to plot"
    )
    parser.add_argument(
        "-e3", nargs=1, type=float, default=[0.5], help="(for plot_distr) at which position in eta3 direction to plot"
    )
    parser.add_argument(
        "-v1", nargs=1, type=float, default=[None], help="(for plot_distr) at which point in v1 direction to plot"
    )
    parser.add_argument(
        "-v2", nargs=1, type=float, default=[None], help="(for plot_distr) at which point in v2 direction to plot"
    )
    parser.add_argument(
        "-v3", nargs=1, type=float, default=[None], help="(for plot_distr) at which point in v3 direction to plot"
    )
    ...
    if "plot_distr" or "phase_space" in actions:
        for species in params["kinetic"].keys():
            ...
            if marker_type == "full_f":
                for k in range(1, 4):
                    ...
                    if grid_slices["v" + str(k)] is None:
                        ...
                        if isinstance(bckgr_param, dict):
                            grid_slices["v" + str(k)] = bckgr_param["u0" + str(k)]
                        else:
                ...
                raise NotImplementedError(f"Diagnostics for markers of type {marker_type} are not implemented!")
            ...
            if "plot_distr" in actions:
                ...
                plot_distr_fun(
                    path=os.path.join(path, "post_processing", "kinetic_data", species),
                    time_idx=time_idx,
            ...
            if "phase_space" in actions:
                for slice_name in os.listdir(
                    os.path.join(path, "post_processing", "kinetic_data", species, "distribution_function")
                ):
                    phase_space_video(
                        t_grid=saved_time,
                        grid_slices=grid_slices,
