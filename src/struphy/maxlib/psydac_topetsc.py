...
def get_cache_key(vec):
    """Generate a unique key for the vector based on its properties."""
    # Create a hash of the vector's properties to use as a unique key
    key = {
        'shape': vec._data.shape,
        'pads': vec.pads,
        'shifts': vec.space.shifts,
        'starts': vec.space.starts,
        'dtype': vec.space.dtype
    }
    return sha256(pickle.dumps(key)).hexdigest()

# @profile
# def flatten_vec_new( vec ):
...
#         vec = coo_matrix((vec._data[idx].flatten(), (indices, indices)),
...
#         vecs = [flatten_vec_new(b) for b in vec.blocks]
...
def flatten_vec_new(vec):
    ...
    if isinstance(vec, StencilVector):
        ...
        multi_indices = np.indices(shape).reshape(len(shape), -1).T

        # Calculate flattened indices
        flattened_indices = np.empty(multi_indices.shape[0], dtype=int)
        for i, idx in enumerate(multi_indices):
            flattened_indices[i] = np.ravel_multi_index([s + x for s, x in zip(starts, idx)], dims=npts, order='C')

        data = vec._data[idx].flatten()

        # Debugging information
        print(f"Length of flattened indices: {len(flattened_indices)}")
        print(f"Length of data: {len(data)}")

        if len(flattened_indices) != len(data):
            print("Mismatch detected!")
            print("Sample of flattened_indices:", flattened_indices[:10])
            print("Sample of data:", data[:10])
            raise ValueError("Mismatch between lengths of indices and data arrays.")

        vec = coo_matrix((data, (flattened_indices, flattened_indices)),
                         shape=[vec.space.dimension, vec.space.dimension],
                         dtype=vec.space.dtype)
        ...
        blocks = [[None]*len(vecs) for _ in vecs]
        for i, v in enumerate(vecs):
            blocks[i][i] = v
        ...
        raise TypeError(f"Expected StencilVector or BlockVector, found instead {type(vec)}")
...
def vec_topetsc( vec ):
    ...
    indices, data = flatten_vec_new(vec)
