...
class Maxwell(Propagator):
    ...
    def __init__(self, e: BlockVector, b: BlockVector, *, solver: dict = options(default=True)["solver"]):
...
class JxBCold(Propagator):
    ...
    def __init__(self, j: BlockVector, *, epsilon: float = 1.0, solver: dict = options(default=True)["solver"]):
...
class ShearAlfven(Propagator):
    ...
    def __init__(self, u: BlockVector, b: BlockVector, *, u_space: str, solver: dict = options(default=True)["solver"]):
...
class Hall(Propagator):
    ...
    def options(default=False):
        dct = {}
        dct["solver"] = {
            "type": [("pbicgstab", "MassMatrixPreconditioner"), ("bicgstab", None)],
            "tol": 1.0e-8,
            "maxiter": 3000,
            "info": False,
            "verbose": False,
    ...
    def __init__(self, b: BlockVector, *, epsilon: float = 1.0, solver: dict = options(default=True)["solver"]):
...
class Magnetosonic(Propagator):
    ...
    def __call__(self, dt):
        ...
        # write new coeffs into self.feec_vars
        max_dn, max_du, max_dp = self.feec_vars_update(nn1, un1, pn1)
...
class MagnetosonicUniform(Propagator):
    ...
    def __init__(
        self, n: StencilVector, u: BlockVector, p: StencilVector, *, solver: dict = options(default=True)["solver"]
    ):
...
class FaradayExtended(Propagator):
    ...
    def __init__(self, a, **params):
        ...
        self._pts_x = 1.0 / (2.0 * self.derham.Nel[0]) * np.polynomial.legendre.leggauss(self._nqs[0])[0] + 1.0 / (
            2.0 * self.derham.Nel[0]
        )
        self._pts_y = 1.0 / (2.0 * self.derham.Nel[1]) * np.polynomial.legendre.leggauss(self._nqs[1])[0] + 1.0 / (
            2.0 * self.derham.Nel[1]
        )
        self._pts_z = 1.0 / (2.0 * self.derham.Nel[2]) * np.polynomial.legendre.leggauss(self._nqs[2])[0] + 1.0 / (
    ...
    def __call__(self, dt):
        ...
        self._RHS = -(self._L2.dot(self._a)) - dt * (
            self._Minv.dot(self._accum_potential._vectors[0] - self.derham.curl.T @ self._M2).dot(self._beq)
        )
        ...
        for _ in range(10):
            ...
            curla_mid = self.derham.curl.dot(0.5 * (self._a_old + self._a)) + self._beq
            ...
            self._weight = [
                [0.0 * self._weight_pre[0], 0.0 * self._weight_pre[2], 0.0 * self._weight_pre[1]],
                [0.0 * self._weight_pre[2], 0.0 * self._weight_pre[1], 0.0 * self._weight_pre[0]],
                [0.0 * self._weight_pre[1], 0.0 * self._weight_pre[0], 0.0 * self._weight_pre[2]],
            ]
...
class ShearAlfvenCurrentCoupling5D(Propagator):
    ...
    def __call__(self, dt):
        ...
        self._ACC(self._unit_b1[0]._data, self._unit_b1[1]._data, self._unit_b1[2]._data, self._scale_vec, 0.0)
                ...
                for n in range(3):
                    fun[-1] += [
                        lambda e1, e2, e3, m=m, n=n: (
                            rot_B(e1, e2, e3)
                            @ self.domain.metric_inv(e1, e2, e3, change_out_order=True, squeeze_out=False)
                        )[:, :, :, m, n]
                    ]
...
class VariationalMomentumAdvection(Propagator):
    ...
    def options(default=False):
        ...
        dct["lin_solver"] = {
            "tol": 1e-12,
            "maxiter": 500,
            "type": [("pcg", "MassMatrixDiagonalPreconditioner"), ("cg", None)],
            "verbose": False,
        }
        dct["nonlin_solver"] = {"tol": 1e-8, "maxiter": 100, "type": ["Newton", "Picard"], "info": False}
        if default:
            dct = descend_options_dict(dct, [])
        return dct
    ...
    def __call_picard(self, dt):
        ...
        for it in range(self._nonlin_solver["maxiter"]):
            ...
            weak_diff = self._Mrhoinv.dot(self._tmp_diff, out=self._tmp_weak_diff)
    ...
    def _initialize_mass(self):
        ...
        if self._lin_solver["type"][1] is None:
            ...
            pc_class = getattr(preconditioner, self._lin_solver["type"][1])
        ...
        self._inv_Mv = inverse(self.mass_ops.Mv, "pcg", pc=self.pc_Mv, tol=1e-16, maxiter=1000, verbose=False)

    def _get_error_newton(self, mn_diff):
        weak_un_diff = self._inv_Mv.dot(self.derham.boundary_ops["v"].dot(mn_diff), out=self._tmp_weak_diff)
...
class VariationalDensityEvolve(Propagator):
    ...
    def __call_newton(self, dt):
        ...
        if self._implicit_transport:
            ...
            rhon_diff -= rhon
            weak_rhon_diff = self.mass_ops.M3.dot(rhon_diff, out=self._tmp_rhon_weak_diff)
        ...
        for it in range(self._nonlin_solver["maxiter"]):
            ...
            advection = self.divPirhoT.dot(self._linear_form_dl_drho, out=self._tmp_advection)
            advection *= dt

            rho_advection = self.divPirho.dot(un12, out=self._tmp_rho_advection)
            rho_advection *= dt
    ...
    def _initialize_projectors_and_mass(self):
        ...
        self._inv_Mv = inverse(
            self.mass_ops.Mv, "pcg", pc=self.pc_Mv, tol=1e-16, maxiter=1000, verbose=False, recycle=True
        )
        ...
        self.integration_grid_spans, self.integration_grid_bn, self.integration_grid_bd = (
            self.derham.prepare_eval_tp_fixed(integration_grid)
        )
        ...
        metric = self.domain.metric(*integration_grid) * self.domain.jacobian_det(*integration_grid)
        ...
        self._Jacobian[0, 0] = self._Mrho + self._dt2_pc_divPirhoT @ self._M_un
        ...
        if self._model == "full":
            ...
            gam = self._gamma
            metric = np.power(self.domain.jacobian_det(*integration_grid), 2 - gam)
            self._proj_rho2_metric_term = deepcopy(metric)

            metric = np.power(self.domain.jacobian_det(*integration_grid), 1 - gam)
    ...
    def _update_Pirho(
        ...
        self.Pirho.update_weights([[rhof0_values, None, None], [None, rhof1_values, None], [None, None, rhof2_values]])

        self.PirhoT.update_weights([[rhof0_values, None, None], [None, rhof1_values, None], [None, None, rhof2_values]])
    ...
    def _update_Piu(
        ...
        uf0_values = self.uf.eval_tp_fixed_loc(self.hist_grid_0_spans, [self.hist_grid_0_bn] * 3, out=self._uf_0_values)
        uf1_values = self.uf.eval_tp_fixed_loc(self.hist_grid_1_spans, [self.hist_grid_1_bn] * 3, out=self._uf_1_values)
        uf2_values = self.uf.eval_tp_fixed_loc(self.hist_grid_2_spans, [self.hist_grid_2_bn] * 3, out=self._uf_2_values)

        self.Piu.update_weights([[uf0_values[0]], [uf1_values[1]], [uf2_values[2]]])

    def _update_weighted_MM(
        self,
    ):
        ...
        self._Mrho.assemble(
            [
                ...
                [self._full_term_mass[1, 0], self._full_term_mass[1, 1], self._full_term_mass[1, 2]],
                [self._full_term_mass[2, 0], self._full_term_mass[2, 1], self._full_term_mass[2, 2]],
            ],
            verbose=False,
        )

    def _update_linear_form_u2(
        self,
    ):
        """Update the linearform representing integration in V3 against kynetic energy"""

        uf_values = self.uf.eval_tp_fixed_loc(
            self.integration_grid_spans, [self.integration_grid_bn] * 3, out=self._uf_values
        )
        uf1_values = self.uf1.eval_tp_fixed_loc(
            self.integration_grid_spans, [self.integration_grid_bn] * 3, out=self._uf1_values
        )
    ...
    def _get_jacobian(self, dt):
        ...
        self._M_un.assemble([[self._Guf_values[0], self._Guf_values[1], self._Guf_values[2]]], verbose=False)
        self._M_un1.assemble([[self._Guf1_values[0]], [self._Guf1_values[1]], [self._Guf1_values[2]]], verbose=False)
    ...
    def _get_error_newton(self, mn_diff, rhon_diff):
        ...
        weak_un_diff = self._inv_Mv.dot(self.derham.boundary_ops["v"].dot(mn_diff), out=self._tmp_un_weak_diff)
        weak_rhon_diff = self.mass_ops.M3.dot(rhon_diff, out=self._tmp_rhon_weak_diff)
        err_rho = weak_rhon_diff.dot(rhon_diff)
        err_u = weak_un_diff.dot(mn_diff)
    ...
    def _get_error_picard(self, un_diff, rhon_diff):
        ...
        weak_un_diff = self.mass_ops.Mv.dot(un_diff, out=self._tmp_un_weak_diff)
        weak_rhon_diff = self.mass_ops.M3.dot(rhon_diff, out=self._tmp_rhon_weak_diff)
        err_rho = weak_rhon_diff.dot(rhon_diff)
...
class VariationalEntropyEvolve(Propagator):
    ...
    def __call_newton(self, dt):
        ...
        if self._implicit_transport:
            ...
            sn_diff -= sn
            weak_sn_diff = self.mass_ops.M3.dot(sn_diff, out=self._tmp_sn_weak_diff)
        ...
        for it in range(self._nonlin_solver["maxiter"]):
            ...
            # Compute the advection terms
            advection = self.divPisT.dot(self._linear_form_dl_ds, out=self._tmp_advection)
            advection *= dt

            s_advection = self.divPis.dot(un12, out=self._tmp_s_advection)
            s_advection *= dt
    ...
    def _update_Pis(
        ...
        sf0_values = self.sf1.eval_tp_fixed_loc(self.hist_grid_0_spans, self.hist_grid_0_bd, out=self._sf_0_values)
        sf1_values = self.sf1.eval_tp_fixed_loc(self.hist_grid_1_spans, self.hist_grid_1_bd, out=self._sf_1_values)
        sf2_values = self.sf1.eval_tp_fixed_loc(self.hist_grid_2_spans, self.hist_grid_2_bd, out=self._sf_2_values)

        self.Pis.update_weights([[sf0_values, None, None], [None, sf1_values, None], [None, None, sf2_values]])

        self.PisT.update_weights([[sf0_values, None, None], [None, sf1_values, None], [None, None, sf2_values]])
...
class VariationalMagFieldEvolve(Propagator):
    ...
    def __call_newton(self, dt):
        ...
        if self._implicit_transport:
            ...
            bn_diff -= bn
            weak_bn_diff = self.mass_ops.M2.dot(bn_diff, out=self._tmp_bn_weak_diff)
        ...
        for it in range(self._nonlin_solver["maxiter"]):
            ...
            # Compute the advection terms
            advection = self.curlPibT.dot(self._linear_form_dl_db, out=self._tmp_advection)
            advection *= dt

            b_advection = self.curlPib.dot(un12, out=self._tmp_b_advection)
            b_advection *= dt
    ...
    def _update_Piu(
        ...
        uf0_values = self.uf.eval_tp_fixed_loc(self.hist_grid_0_spans, [self.hist_grid_0_bn] * 3, out=self._uf0_values)
        uf1_values = self.uf.eval_tp_fixed_loc(self.hist_grid_1_spans, [self.hist_grid_1_bn] * 3, out=self._uf1_values)
        uf2_values = self.uf.eval_tp_fixed_loc(self.hist_grid_2_spans, [self.hist_grid_2_bn] * 3, out=self._uf2_values)

        self.Piu.update_weights(
            [
                [None, uf0_values[2], -uf0_values[1]],
...
class VariationalViscosity(Propagator):
    ...
    def options(default=False):
        ...
        dct["nonlin_solver"] = {"tol": 1e-8, "maxiter": 100, "type": ["Newton"], "info": False}
        dct["physics"] = {"gamma": 1.66666666667, "mu": 0.0, "mu_a": 0.0}
    ...
    def __call_newton(self, dt):
        ...
        sn = self.feec_vars[0]
        un = self.feec_vars[1]
        if self._mu < 1.0e-15 and self._mu_a < 1.0e-15:
    ...
    def _initialize_projectors_and_mass(self):
        ...
        Pcoord0 = CoordinateProjector(0, self.derham.Vh_pol["v"], self.derham.Vh_pol["0"])
        Pcoord1 = CoordinateProjector(1, self.derham.Vh_pol["v"], self.derham.Vh_pol["0"])
        Pcoord2 = CoordinateProjector(2, self.derham.Vh_pol["v"], self.derham.Vh_pol["0"])
        ...
        self._inv_M3 = inverse(self.mass_ops.M3, "pcg", pc=self.pc_M3, tol=1e-16, maxiter=1000, verbose=False)
        ...
        self.integration_grid_gradient = [
            ...
            [self.integration_grid_bn[0], self.integration_grid_bd[1], self.integration_grid_bn[2]],
        ...
        metric = np.power(self.domain.jacobian_det(*integration_grid), -gam)
        self._mass_metric_term = deepcopy(metric)

        metric = np.power(self.domain.jacobian_det(*integration_grid), 1 - gam)
        self._energy_metric = deepcopy(metric)

        metric = np.power(self.domain.jacobian_det(*integration_grid), 1)
...
class VariationalResistivity(Propagator):
    ...
    def options(default=False):
        ...
        dct["nonlin_solver"] = {"tol": 1e-8, "maxiter": 100, "type": ["Newton"], "info": False}
        dct["physics"] = {"eta": 0.0, "eta_a": 0.0, "gamma": 5 / 3}
    ...
    def __call_newton(self, dt):
        ...
        if self._linearize_current:
            bn1 = self.evol_op.dot(
                bn + dt * self._eta * self.derham.curl.dot(self.Tcurl.dot(self.projected_mhd_equil.b2)),
                out=self._tmp_bn1,
            )
        else:
    ...
    def _initialize_projectors_and_mass(self):
        ...
        inv_M1 = inverse(self.mass_ops.M1, "pcg", pc=pc_M1, tol=1e-16, maxiter=1000, verbose=False)

        pc_M3 = preconditioner.MassMatrixDiagonalPreconditioner(self.mass_ops.M3)
        self._inv_M3 = inverse(self.mass_ops.M3, "pcg", pc=pc_M3, tol=1e-16, maxiter=1000, verbose=False)
        ...
        self.M1_cb = WeightedMassOperator(
            ...
            V_extraction_op=self.derham.extraction_ops["1"],
            W_extraction_op=self.derham.extraction_ops["1"],
            weights_info=[[g, None, None], [None, g, None], [None, None, g]],
        )
        ...
        metric = self.domain.metric_inv(*integration_grid)
...
class TimeDependentSource(Propagator):
    ...
    * :math:`h(\omega t) = \sin(\omega t)`
    ...
    def __init__(
        self, c: StencilVector, *, omega: float = options()["omega"], hfun: str = options(default=True)["hfun"]
    ):
