...
def _mms_1d(bc_type):
    ...
    if bc_type == "periodic":
        ...
        def phi(x, y, z):
            return xp.sin(2 * pi * x), zeros_like(x), zeros_like(x)

        def u(x, y, z):
            return xp.sin(2 * pi * x) + 1, zeros_like(x), zeros_like(x)
        ...
        def u(x, y, z):
            return xp.sin(2 * pi * x) + x + 1, zeros_like(x), zeros_like(x)

        def ue(x, y, z):
            return xp.sin(2 * pi * x) + x, zeros_like(x), zeros_like(x)
...
def _mms_2d(bc_type):
    ...
    if bc_type == "periodic":
        ...
        def phi(x, y, z):
            return xp.cos(2 * pi * x) + xp.sin(2 * pi * y), zeros_like(x), zeros_like(x)

        def u(x, y, z):
            return -xp.sin(2 * pi * x) * xp.sin(2 * pi * y), -xp.cos(2 * pi * x) * xp.cos(2 * pi * y), zeros_like(x)

        def ue(x, y, z):
            return -xp.sin(4 * pi * x) * xp.sin(4 * pi * y), -xp.cos(4 * pi * x) * xp.cos(4 * pi * y), zeros_like(x)
    else:  # hom_dirichlet
        ...
        def u(x, y, z):
            return -xp.sin(2 * pi * x) * xp.cos(2 * pi * y), xp.cos(2 * pi * x) * xp.sin(2 * pi * y), zeros_like(x)

        def ue(x, y, z):
            return -xp.sin(4 * pi * x) * xp.cos(4 * pi * y), xp.cos(4 * pi * x) * xp.sin(4 * pi * y), zeros_like(x)
...
def test_mms_2d(bc_type):
    ...
    if bc_type == "periodic":
        ...
        derham_opts = DerhamOptions(
            degree=(2, 2, 1), bcs=(("dirichlet", "dirichlet"), ("dirichlet", "dirichlet"), None)
        )
