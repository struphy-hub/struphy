...
def convergence_test_1d(
    ...
    for p in range(1, pmax + 1):
        ...
        plt.plot(h_vec, [(h**(p+1))/(h_vec[0]**(p+1))*errors[0] for h in h_vec], 'k--', label=f"Theoretical error, rate = p + 1")
        plt.xscale("log")
        plt.yscale("log")
    ...
    for p in range(1, pmax + 1):
        ...
        for n, Nel in enumerate(Nels):
            ...
            if direction == "1":
                ...
                plt.pcolormesh(y[0, :, :],z[0, :, :], E_difference[0][0, :, :], vmin=0.0,vmax=1.0, label=f"{Nel}x{Nel}")
                plt.colorbar()
            
            elif direction == "2":

                plt.pcolormesh(z[:, 0, :],x[:, 0, :], E_difference[1][ :,0, :], vmin=0.0,vmax=1.0, label=f"{Nel}x{Nel}")
                plt.colorbar()
            
            elif direction == "3":

                plt.pcolormesh(x[:, :, 0],y[:, :, 0], E_difference[2][:, :, 0], vmin=0.0,vmax=1.0, label=f"{Nel}x{Nel}")
                plt.colorbar()
