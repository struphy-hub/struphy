...
def inner_prod_V0(tensor_space_FEM, domain, fun):
    ...
    if callable(fun):
        ...
        mat_f[:, :] = fun(quad_mesh[0], quad_mesh[1], 0.)
    else:
        mat_f[:, :] = fun
    ...
    ker.kernel_inner(
        Nel[0], Nel[1], p[0], p[1], n_quad[0], n_quad[1], 0, 0, wts[0],
...
def inner_prod_V1(tensor_space_FEM, domain, fun):
    ...
    for a in range(3):
        ...
        for b in range(3):
            ...
            if callable(fun[b]):
                mat_f += fun[b](quad_mesh[0], quad_mesh[1], 0.) * g_inv[a, b]
            else:
        ...
        ker.kernel_inner(
            Nel[0], Nel[1], p[0], p[1], n_quad[0], n_quad[1], ns[a][0], ns[a][1],
            wts[0], wts[1], basis[a][0], basis[a][1], ind[a][0], ind[a][1], F[a], mat_f*det_df,
        )
...
def inner_prod_V2(tensor_space_FEM, domain, fun):
    ...
    for a in range(3):
        ...
        for b in range(3):
            ...
            if callable(fun[b]):
                mat_f += fun[b](quad_mesh[0], quad_mesh[1], 0.) * g[a, b]
            else:
                mat_f += fun[b] * g[a, b]
...
def inner_prod_V3(tensor_space_FEM, domain, fun):
    ...
    ker.kernel_inner(
        Nel[0], Nel[1], p[0], p[1], n_quad[0], n_quad[1], 1, 1, wts[0],
