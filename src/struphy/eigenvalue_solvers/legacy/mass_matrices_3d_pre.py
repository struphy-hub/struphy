...
def get_M0_PRE(tensor_space_FEM, domain):
    ...
    Nel_pre      = [
        ...
        tensor_space_FEM.spaces[1].NbaseN, tensor_space_FEM.spaces[2].NbaseN,
    ]
    spl_kind_pre = [True, True, True]
    spaces_pre   = [
        spl.Spline_space_1d(Nel, p, spl_kind, nq_el) for Nel, p, spl_kind, nq_el in zip(
            Nel_pre, tensor_space_FEM.p, spl_kind_pre, tensor_space_FEM.n_quad,
        )
    ]
...
def get_M1_PRE(tensor_space_FEM, domain):
    ...
    spaces_pre[0].assemble_M0(
        lambda eta : (domain.params_map[1] - domain.params_map[0])
        * np.ones(eta.shape, dtype=float),
    )
    spaces_pre[1].assemble_M0(
        lambda eta : (domain.params_map[3] - domain.params_map[2])
        * np.ones(eta.shape, dtype=float),
    )
    spaces_pre[2].assemble_M0(
        lambda eta : (domain.params_map[5] - domain.params_map[4])
        * np.ones(eta.shape, dtype=float),
    )

    spaces_pre[0].assemble_M1(
        lambda eta : 1/(domain.params_map[1] - domain.params_map[0])
        * np.ones(eta.shape, dtype=float),
    )
    spaces_pre[1].assemble_M1(
        lambda eta : 1/(domain.params_map[3] - domain.params_map[2])
        * np.ones(eta.shape, dtype=float),
    )
    spaces_pre[2].assemble_M1(
        lambda eta : 1/(domain.params_map[5] - domain.params_map[4])
        * np.ones(eta.shape, dtype=float),
    )
...
def get_M1_PRE_3(tensor_space_FEM, mats_pol=None):
    ...
    if mats_pol == None:
        mat = [
            tensor_space_FEM.B1_pol.dot(
                tensor_space_FEM.M1_pol_mat[0].dot(
                    tensor_space_FEM.B1_pol.T,
                ),
...
def get_M2_PRE_3(tensor_space_FEM, mats_pol=None):
    ...
    if mats_pol == None:
        mat = [
            tensor_space_FEM.B2_pol.dot(
                tensor_space_FEM.M2_pol_mat[0].dot(
                    tensor_space_FEM.B2_pol.T,
                ),
