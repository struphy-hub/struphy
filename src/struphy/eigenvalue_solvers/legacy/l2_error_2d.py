...
def l2_error_V0(tensor_space_FEM, domain, f0, c0, method='standard'):
    ...
    det_df = abs(domain.jacobian_det(pts[0].flatten(), pts[1].flatten(), 0.))

    # evaluation of exact 0-form at quadrature points
    if callable(f0):
        quad_mesh = np.meshgrid(pts[0].flatten(), pts[1].flatten(), indexing='ij')
        f0 = f0(quad_mesh[0], quad_mesh[1], 0.)

    if method == 'standard':

        # evaluation of discrete 0-form at quadrature points
        f0_h = tensor_space_FEM.evaluate_NN(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c0, 'V0')[:, :, 0]
        ...
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [0, 0], [0, 0], basisN[0], basisN[1], basisN[0], basisN[1], indN[0], indN[1], indN[0], indN[1], error, f0.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f0.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c0, c0, det_df.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )
...
def l2_error_V1(tensor_space_FEM, domain, f1, c1, method='standard'):
    ...
    metric_coeffs = domain.metric_inv(pts[0].flatten(), pts[1].flatten(), 0.)
    metric_coeffs *= abs(domain.jacobian_det(pts[0].flatten(), pts[1].flatten(), 0.))

    # evaluation of exact 1-form components at quadrature points
    if callable(f1[0]):
        quad_mesh = np.meshgrid(pts[0].flatten(), pts[1].flatten(), indexing='ij')
        f1_1 = f1[0](quad_mesh[0], quad_mesh[1], 0.)
        f1_2 = f1[1](quad_mesh[0], quad_mesh[1], 0.)
        f1_3 = f1[2](quad_mesh[0], quad_mesh[1], 0.)

    if method == 'standard':

        # evaluation of discrete 1-form components at quadrature points
        f1_h_1 = tensor_space_FEM.evaluate_DN(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c1_1, 'V1')[:, :, 0]
        f1_h_2 = tensor_space_FEM.evaluate_ND(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c1_2, 'V1')[:, :, 0]
        f1_h_3 = tensor_space_FEM.evaluate_NN(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c1_3, 'V1')[:, :, 0]
        ...
        # 1 * d_f1 * G^11 * |det(DF)| * d_f1
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [1, 0], [1, 0], basisD[0], basisN[1], basisD[0], basisN[1], indD[0], indN[1], indD[0], indN[1], error, f1_1.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f1_1.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c1_1, c1_1, 1*metric_coeffs[0, 0].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 2 * d_f1 * G^12 * |det(DF)| * d_f2
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [1, 0], [0, 1], basisD[0], basisN[1], basisN[0], basisD[1], indD[0], indN[1], indN[0], indD[1], error, f1_1.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f1_2.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c1_1, c1_2, 2*metric_coeffs[0, 1].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 1 * d_f2 * G^22 * |det(DF)| * d_f2
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [0, 1], [0, 1], basisN[0], basisD[1], basisN[0], basisD[1], indN[0], indD[1], indN[0], indD[1], error, f1_2.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f1_2.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c1_2, c1_2, 1*metric_coeffs[1, 1].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 1 * d_f3 * G^33 * |det(DF)| * d_f3
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [0, 0], [0, 0], basisN[0], basisN[1], basisN[0], basisN[1], indN[0], indN[1], indN[0], indN[1], error, f1_3.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f1_3.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c1_3, c1_3, 1*metric_coeffs[2, 2].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )
...
def l2_error_V2(tensor_space_FEM, domain, f2, c2, method='standard'):
    ...
    metric_coeffs = domain.metric(pts[0].flatten(), pts[1].flatten(), 0.)
    metric_coeffs /= abs(domain.jacobian_det(pts[0].flatten(), pts[1].flatten(), 0.))

    # evaluation of exact 2-form components at quadrature points
    if callable(f2[0]):
        quad_mesh = np.meshgrid(pts[0].flatten(), pts[1].flatten(), indexing='ij')
        f2_1 = f2[0](quad_mesh[0], quad_mesh[1], 0.)
        f2_2 = f2[1](quad_mesh[0], quad_mesh[1], 0.)
        f2_3 = f2[2](quad_mesh[0], quad_mesh[1], 0.)

    if method == 'standard':

        # evaluation of discrete 2-form components at quadrature points
        f2_h_1 = tensor_space_FEM.evaluate_ND(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c2_1, 'V2')[:, :, 0]
        f2_h_2 = tensor_space_FEM.evaluate_DN(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c2_2, 'V2')[:, :, 0]
        f2_h_3 = tensor_space_FEM.evaluate_DD(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c2_3, 'V2')[:, :, 0]
        ...
        # 1 * d_f1 * G_11 / |det(DF)| * d_f1
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [0, 1], [0, 1], basisN[0], basisD[1], basisN[0], basisD[1], indN[0], indD[1], indN[0], indD[1], error, f2_1.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f2_1.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c2_1, c2_1, 1*metric_coeffs[0, 0].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 2 * d_f1 * G_12 / |det(DF)| * d_f2
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [0, 1], [1, 0], basisN[0], basisD[1], basisD[0], basisN[1], indN[0], indD[1], indD[0], indN[1], error, f2_1.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f2_2.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c2_1, c2_2, 2*metric_coeffs[0, 1].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 1 * d_f2 * G_22 / |det(DF)| * d_f2
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [1, 0], [1, 0], basisD[0], basisN[1], basisD[0], basisN[1], indD[0], indN[1], indD[0], indN[1], error, f2_2.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f2_2.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c2_2, c2_2, 1*metric_coeffs[1, 1].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )

        # 1 * d_f3 * G_33 / |det(DF)| * d_f3
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [1, 1], [1, 1], basisD[0], basisD[1], basisD[0], basisD[1], indD[0], indD[1], indD[0], indD[1], error, f2_3.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f2_3.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c2_3, c2_3, 1*metric_coeffs[2, 2].reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )
...
def l2_error_V3(tensor_space_FEM, domain, f3, c3, method='standard'):
    ...
    if callable(f3):
        quad_mesh = np.meshgrid(pts[0].flatten(), pts[1].flatten(), indexing='ij')
        f3 = f3(quad_mesh[0], quad_mesh[1], 0.)

    if method == 'standard':

        # evaluation of discrete 3-form at quadrature points
        f3_h = tensor_space_FEM.evaluate_DD(pts[0].flatten(), pts[1].flatten(), np.array([0.]), c3, 'V3')[:, :, 0]
        ...
        ker.kernel_l2error(
            Nel, p, n_quad, wts[0], wts[1], [1, 1], [1, 1], basisD[0], basisD[1], basisD[0], basisD[1], indD[0], indD[1], indD[0], indD[1], error, f3.reshape(
                Nel[0], n_quad[0], Nel[1], n_quad[1],
            ), f3.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]), c3, c3, 1/det_df.reshape(Nel[0], n_quad[0], Nel[1], n_quad[1]),
        )
