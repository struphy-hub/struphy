...
def solve_ev_problem_FEEC(Rho, B_phi, dB_phi, B_z, dB_z, P, gamma, a, R0, n, m, num_params):
    ...
    b2_eq_z = np.append(np.array([0.0]), np.linalg.solve(proj.D.toarray()[1:, 1:], proj.rhs_1(B2_z)[1:]))
    ...
    rho3_eq = np.append(np.array([0.0]), np.linalg.solve(proj.D.toarray()[1:, 1:], proj.rhs_1(Rho3)[1:]))
    p3_eq = np.append(np.array([0.0]), np.linalg.solve(proj.D.toarray()[1:, 1:], proj.rhs_1(P3)[1:]))
    ...
    for j in range(splines.NbaseD):
        ...
        integrand2 = (
            lambda eta: splines.evaluate_D(eta, cD) / J(eta) * 2 * np.pi * a * (B_phi(r(eta)) + r(eta) * dB_phi(r(eta)))
        )
    ...
    for j in range(splines.NbaseN):
        ...
        integrand2 = (
            lambda eta: splines.evaluate_N(eta, cN) / J(eta) * 2 * np.pi * a * (B_phi(r(eta)) + r(eta) * dB_phi(r(eta)))
        )
    ...
    ker.rhs1_1d(
        ...
        pi1_D_i[1],
        subs,
        np.append(0, np.cumsum(subs - 1)[:-1]),
        wts,
        basis_his_D,
    ...
    A_all = np.block(
        [
            ...
            [np.zeros((A_2.shape[0], A_1.shape[1])), A_2, np.zeros((A_2.shape[0], A_3.shape[1]))],
    ...
    LHS = np.block(
        [
            ...
            [
                np.zeros((A_all.shape[0], A_all.shape[1])),
                np.identity(A_all.shape[0]),
                np.zeros((A_all.shape[0], len(p3) - 1)),
            ],
            [
                np.zeros((len(p3) - 1, A_all.shape[1])),
                np.zeros((len(p3) - 1, A_all.shape[1])),
                np.identity(len(p3) - 1),
            ],
        ]
    )
    ...
    RHS = np.block(
        [
            ...
            [I_all, np.zeros((I_all.shape[0], MB_b_all.shape[1])), np.zeros((I_all.shape[0], MB_p_all.shape[1]))],
    ...
    for n in range(Nt):
        ...
        u2_r_all[n + 1, :] = np.array([0.0] + list(unew[: (splines.NbaseN - 2)]) + [0.0])
        u2_phi_all[n + 1, :] = unew[(splines.NbaseN - 2) : (splines.NbaseN - 2 + splines.NbaseD)]
        u2_z_all[n + 1, :] = np.array([0.0] + list(unew[(splines.NbaseN - 2 + splines.NbaseD) :]))

        b2_r_all[n + 1, :] = np.array([0.0] + list(bnew[: (splines.NbaseN - 2)]) + [0.0])
        b2_phi_all[n + 1, :] = bnew[(splines.NbaseN - 2) : (splines.NbaseN - 2 + splines.NbaseD)]
        b2_z_all[n + 1, :] = np.array([0.0] + list(bnew[(splines.NbaseN - 2 + splines.NbaseD) :]))

        p3_all[n + 1, :] = np.array([0.0] + list(pnew))
