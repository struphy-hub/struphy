...
class projectors_local_mhd:
    ...
    def __init__(self, tensor_space, n_quad):
        ...
        self.x_int = [
            np.zeros((n_lambda_int, n_int), dtype=float)
        ...
        self.x_int_indices  = [
            np.zeros((n_lambda_int, n_int), dtype=int)
            for n_lambda_int, n_int in zip(n_lambda_int, self.n_int)
        ]
            ...
            if self.bc[a] == False:
                ...
                for i in range(n_lambda_int[a]):
                    ...
                    if self.p[a] == 1:
                        ...
                        self.x_int_indices[a][i] = (
                            np.arange(self.n_int[a]) + 2 *
                            (i - (self.p[a] - 1))
                        ) % (2*self.Nel[a])
        ...
        self.x_his = [
            np.zeros((n_lambda_his, n_his + 1), dtype=float)
        ...
        self.x_his_indices  = [
            np.zeros((n_lambda_his, n_his), dtype=int)
            for n_lambda_his, n_his in zip(n_lambda_his, self.n_his)
        ]
        ...
        for a in range(3):
            if self.bc[a] == False:
                ...
                # quadrature points and weights
                self.pts[a], self.wts[a] = bsp.quadrature_grid(
                    np.append(np.unique(self.x_his[a].flatten() % 1.), 1.), self.pts_loc[a], self.wts_loc[a],
                )

        # evaluate N basis functions at interpolation and quadrature points
        self.basisN_int = [
            bsp.collocation_matrix(T, p, x_int, bc)
            for T, p, x_int, bc in zip(self.T, self.p, self.x_int, self.bc)
        ]

        self.basisN_his = [
            bsp.collocation_matrix(T, p, pts.flatten(), bc).reshape(
                pts[:, 0].size, pts[0, :].size, NbaseN,
    ...
    def projection_Q_0form(self, domain):
        ...
        Q11 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.n_int_nvcof_N[0],
                self.n_his_nvcof_N[1], self.n_his_nvcof_N[2],
            ), dtype=float,
        )
        Q22 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.n_his_nvcof_N[0],
                self.n_int_nvcof_N[1], self.n_his_nvcof_N[2],
            ), dtype=float,
        )
        Q33 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.n_his_nvcof_N[0],
                self.n_his_nvcof_N[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.pts[2].flatten(), mat_eq, 11, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.pts[2].flatten(), mat_eq, 11, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.pts[1].flatten(), self.x_int[2], mat_eq, 11, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_int_nvcof_N[0], self.n_his_nvcof_N[1], self.n_his_nvcof_N[2],
        ))
        ...
        Q11 = spa.csc_matrix(
            (Q11.flatten(), (row.flatten(), col.flatten())), shape=(
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_his_nvcof_N[0], self.n_int_nvcof_N[1], self.n_his_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        Q22 = spa.csc_matrix(
            (Q22.flatten(), (row.flatten(), col.flatten())), shape=(
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_his_nvcof_N[0], self.n_his_nvcof_N[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        Q33 = spa.csc_matrix(
            (Q33.flatten(), (row.flatten(), col.flatten())), shape=(
    ...
    def projection_Q_2form(self, domain):
        ...
        Q11 = np.empty(
            (
                self.NbaseN[0], self.NbaseD[1], self.NbaseD[2], self.n_int_nvcof_N[0],
                self.n_his_nvcof_D[1], self.n_his_nvcof_D[2],
            ), dtype=float,
        )
        Q22 = np.empty(
            (
                self.NbaseD[0], self.NbaseN[1], self.NbaseD[2], self.n_his_nvcof_D[0],
                self.n_int_nvcof_N[1], self.n_his_nvcof_D[2],
            ), dtype=float,
        )
        Q33 = np.empty(
            (
                self.NbaseD[0], self.NbaseD[1], self.NbaseN[2], self.n_his_nvcof_D[0],
                self.n_his_nvcof_D[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        ...
        indices = np.indices((
            ...
            self.n_int_nvcof_N[0], self.n_his_nvcof_D[1], self.n_his_nvcof_D[2],
        ))
        ...
        indices = np.indices((
            ...
            self.n_his_nvcof_D[0], self.n_int_nvcof_N[1], self.n_his_nvcof_D[2],
        ))
        ...
        indices = np.indices((
            ...
            self.n_his_nvcof_D[0], self.n_his_nvcof_D[1], self.n_int_nvcof_N[2],
        ))
    ...
    def projection_W_0form(self, domain):
        ...
        W1 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.n_int_nvcof_N[0],
                self.n_int_nvcof_N[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.x_int[1], self.x_int[2], mat_eq, 12, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )

        ker_loc.kernel_pi0(
            self.NbaseN, self.n_int, self.n_int_locbf_N, self.int_global_N[0], self.int_global_N[1], self.int_global_N[2], self.int_loccof_N[0], self.int_loccof_N[1], self.int_loccof_N[2], self.coeff_i[0], self.coeff_i[
                1
            ], self.coeff_i[2], self.coeffi_indices[0], self.coeffi_indices[1], self.coeffi_indices[2], self.basisN_int[0], self.basisN_int[1], self.basisN_int[2], self.x_int_indices[0], self.x_int_indices[1], self.x_int_indices[2], W1, mat_eq,
        )
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_int_nvcof_N[0], self.n_int_nvcof_N[1], self.n_int_nvcof_N[2],
        ))
        ...
        W1 = spa.csc_matrix(
            (W1.flatten(), (row.flatten(), col.flatten())), shape=(
    ...
    def projection_T_0form(self, domain):
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.x_int[2], mat_eq, 23, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.x_int[2], mat_eq, 22, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.x_int[2], mat_eq, 23, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.x_int[2], mat_eq, 21, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.x_int[1], self.pts[2].flatten(), mat_eq, 22, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.x_int[1], self.pts[2].flatten(), mat_eq, 21, domain.kind_map, domain.params_map,
            domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_his_nvcof_N[0], self.n_int_nvcof_N[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        T12 = spa.csc_matrix(
            (T12.flatten(), (row.flatten(), col.flatten())), shape=(
        ...
        T13 = spa.csc_matrix(
            (T13.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseD[0]*self.NbaseN[1]*self.NbaseN[2],
            ),
        )
        T13.eliminate_zeros()
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_int_nvcof_N[0], self.n_his_nvcof_N[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        T21 = spa.csc_matrix(
            (T21.flatten(), (row.flatten(), col.flatten())), shape=(
        ...
        T23 = spa.csc_matrix(
            (T23.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseN[0]*self.NbaseD[1]*self.NbaseN[2],
            ),
        )
        T23.eliminate_zeros()
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseN[2],
            self.n_int_nvcof_N[0], self.n_int_nvcof_N[1], self.n_his_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        T31 = spa.csc_matrix(
            (T31.flatten(), (row.flatten(), col.flatten())), shape=(
        ...
        T32 = spa.csc_matrix(
            (T32.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseN[0]*self.NbaseN[1]*self.NbaseD[2],
            ),
        )
        T32.eliminate_zeros()
    ...
    def projection_T_1form(self, domain):
        ...
        T12 = np.empty(
            (
                self.NbaseN[0], self.NbaseD[1], self.NbaseN[2], self.n_his_nvcof_N[0],
                self.n_int_nvcof_D[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        T13 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseD[2], self.n_his_nvcof_N[0],
                self.n_int_nvcof_N[1], self.n_int_nvcof_D[2],
            ), dtype=float,
        )

        T21 = np.empty(
            (
                self.NbaseD[0], self.NbaseN[1], self.NbaseN[2], self.n_int_nvcof_D[0],
                self.n_his_nvcof_N[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        T23 = np.empty(
            (
                self.NbaseN[0], self.NbaseN[1], self.NbaseD[2], self.n_int_nvcof_N[0],
                self.n_his_nvcof_N[1], self.n_int_nvcof_D[2],
            ), dtype=float,
        )
        ...
        T32 = np.empty(
            (
                self.NbaseN[0], self.NbaseD[1], self.NbaseN[2], self.n_int_nvcof_N[0],
                self.n_int_nvcof_D[1], self.n_his_nvcof_N[2],
            ), dtype=float,
        )
        ...
        indices = np.indices((
            ...
            self.n_his_nvcof_N[0], self.n_int_nvcof_D[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseD[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            ...
            self.n_his_nvcof_N[0], self.n_int_nvcof_N[1], self.n_int_nvcof_D[2],
        ))
        row     = self.NbaseN[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            ...
            self.n_int_nvcof_D[0], self.n_his_nvcof_N[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseN[1], self.NbaseD[2],
            self.n_int_nvcof_N[0], self.n_his_nvcof_N[1], self.n_int_nvcof_D[2],
        ))
        row     = self.NbaseN[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseD[0], self.NbaseN[1], self.NbaseN[2],
            self.n_int_nvcof_D[0], self.n_int_nvcof_N[1], self.n_his_nvcof_N[2],
        ))
        row     = self.NbaseN[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseD[1], self.NbaseN[2],
            self.n_int_nvcof_N[0], self.n_int_nvcof_D[1], self.n_his_nvcof_N[2],
        ))
        row     = self.NbaseD[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
    ...
    def projection_T_2form(self, domain):
        ...
        T21 = np.empty(
            (
                self.NbaseN[0], self.NbaseD[1], self.NbaseD[2], self.n_int_nvcof_N[0],
                self.n_his_nvcof_D[1], self.n_int_nvcof_D[2],
            ), dtype=float,
        )
        T23 = np.empty(
            (
                self.NbaseD[0], self.NbaseD[1], self.NbaseN[2], self.n_int_nvcof_D[0],
                self.n_his_nvcof_D[1], self.n_int_nvcof_N[2],
            ), dtype=float,
        )
        ...
        T31 = np.empty(
            (
                self.NbaseN[0], self.NbaseD[1], self.NbaseD[2], self.n_int_nvcof_N[0],
                self.n_int_nvcof_D[1], self.n_his_nvcof_D[2],
            ), dtype=float,
        )
        T32 = np.empty(
            (
                self.NbaseD[0], self.NbaseN[1], self.NbaseD[2], self.n_int_nvcof_D[0],
                self.n_int_nvcof_N[1], self.n_his_nvcof_D[2],
            ), dtype=float,
        )
        ...
        indices = np.indices((
            self.NbaseD[0], self.NbaseN[1], self.NbaseD[2],
            self.n_his_nvcof_D[0], self.n_int_nvcof_N[1], self.n_int_nvcof_D[2],
        ))
        row     = self.NbaseN[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseD[0], self.NbaseD[1], self.NbaseN[2],
            self.n_his_nvcof_D[0], self.n_int_nvcof_D[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseD[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseD[1], self.NbaseD[2],
            self.n_int_nvcof_N[0], self.n_his_nvcof_D[1], self.n_int_nvcof_D[2],
        ))
        row     = self.NbaseD[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseD[0], self.NbaseD[1], self.NbaseN[2],
            self.n_int_nvcof_D[0], self.n_his_nvcof_D[1], self.n_int_nvcof_N[2],
        ))
        row     = self.NbaseD[1]*self.NbaseN[2]*indices[0] + self.NbaseN[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseN[0], self.NbaseD[1], self.NbaseD[2],
            self.n_int_nvcof_N[0], self.n_int_nvcof_D[1], self.n_his_nvcof_D[2],
        ))
        row     = self.NbaseD[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
        ...
        indices = np.indices((
            self.NbaseD[0], self.NbaseN[1], self.NbaseD[2],
            self.n_int_nvcof_D[0], self.n_int_nvcof_N[1], self.n_his_nvcof_D[2],
        ))
        row     = self.NbaseN[1]*self.NbaseD[2]*indices[0] + self.NbaseD[2]*indices[1] + indices[2]
    ...
    def projection_S_0form(self, domain):
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.pts[2].flatten(), mat_eq, 31, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.pts[2].flatten(), mat_eq, 31, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.pts[1].flatten(), self.x_int[2], mat_eq, 31, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        S11 = spa.csc_matrix(
            (S11.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseN[0]*self.NbaseD[1]*self.NbaseD[2],
            ),
        )
        S11.eliminate_zeros()
        ...
        S22 = spa.csc_matrix(
            (S22.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseD[0]*self.NbaseN[1]*self.NbaseD[2],
            ),
        )
        S22.eliminate_zeros()
        ...
        S33 = spa.csc_matrix(
            (S33.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseD[0]*self.NbaseD[1]*self.NbaseN[2],
            ),
        )
        S33.eliminate_zeros()
    ...
    def projection_K_3form(self, domain):
        ...
        K = np.zeros(
            (
                self.NbaseD[0], self.NbaseD[1], self.NbaseD[2], self.n_his_nvcof_D[0],
                self.n_his_nvcof_D[1], self.n_his_nvcof_D[2],
            ), dtype=float,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.pts[1].flatten(), self.pts[2].flatten(), mat_eq, 41, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )

        # assembly of K
        ker_loc.kernel_pi3(
            self.NbaseD, self.n_quad, self.n_his, self.n_his_locbf_D, self.his_global_D[0], self.his_global_D[1], self.his_global_D[2], self.his_loccof_D[0], self.his_loccof_D[1], self.his_loccof_D[2], self.coeff_h[0], self.coeff_h[1], self.coeff_h[2], self.coeffh_indices[0], self.coeffh_indices[1], self.coeffh_indices[
                2
            ], self.basisD_his[0], self.basisD_his[1], self.basisD_his[2], self.x_his_indices[0], self.x_his_indices[1], self.x_his_indices[2], self.wts[0], self.wts[1], self.wts[2], K, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, self.pts[1][:, 0].size, self.pts[1][0, :].size, self.pts[2][:, 0].size, self.pts[2][0, :].size),
        )

        # conversion to sparse matrix
        indices = np.indices((
            self.NbaseD[0], self.NbaseD[1], self.NbaseD[2],
            self.n_his_nvcof_D[0], self.n_his_nvcof_D[1], self.n_his_nvcof_D[2],
        ))
        ...
        K = spa.csc_matrix(
            (K.flatten(), (row.flatten(), col.flatten())), shape=(
    ...
    def projection_N_0form(self, domain):
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.pts[2].flatten(), mat_eq, 51, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.pts[2].flatten(), mat_eq, 51, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.pts[1].flatten(), self.x_int[2], mat_eq, 51, domain.kind_map,
            domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        N11 = spa.csc_matrix(
            (N11.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseN[0]*self.NbaseD[1]*self.NbaseD[2],
            ),
        )
        N11.eliminate_zeros()
        ...
        N22 = spa.csc_matrix(
            (N22.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseD[0]*self.NbaseN[1]*self.NbaseD[2],
            ),
        )
        N22.eliminate_zeros()
        ...
        N33 = spa.csc_matrix(
            (N33.flatten(), (row.flatten(), col.flatten())), shape=(
                self.NbaseN[0]*self.NbaseN[1]*self.NbaseN[2], self.NbaseD[0]*self.NbaseD[1]*self.NbaseN[2],
            ),
        )
        N33.eliminate_zeros()
    ...
    def projection_N_2form(self, domain):
        ...
        ker_eva.kernel_eva(
            self.x_int[0], self.pts[1].flatten(), self.pts[2].flatten(),
            mat_eq, 51, kind_map=kind_map, params_map=params_map,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.x_int[1], self.pts[2].flatten(),
            mat_eq, 51, kind_map=kind_map, params_map=params_map,
        )
        ...
        ker_eva.kernel_eva(
            self.pts[0].flatten(), self.pts[1].flatten(), self.x_int[2],
            mat_eq, 51, kind_map=kind_map, params_map=params_map,
        )
    ...
    def RHS6(self, u, p, b, dt):
        ...
        out = self.A(u) + dt**2/4*self.N.T.dot(
            self.tensor_space.DIV.T.dot(self.tensor_space.M3.dot(self.L(u))),
...
class term_curl_beq:
    ...
    def __init__(self, tensor_space, mapping, kind_map=None, params_map=None, tensor_space_F=None, cx=None, cy=None, cz=None):
        ...
        self.mat_curl_beq_1 = np.empty(
            (
                self.Nel[0], self.Nel[1], self.Nel[2],
                self.n_quad[0], self.n_quad[1], self.n_quad[2],
            ), dtype=float,
        )
        ...
        if mapping == 0:
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad,
                self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_1, 61, kind_map, params_map,
            )
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad,
                self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_2, 62, kind_map, params_map,
            )
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad,
                self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_3, 63, kind_map, params_map,
            )
        elif mapping == 1:
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad, self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_1,
                61, self.T_F[0], self.T_F[1], self.T_F[2], self.p_F, self.NbaseN_F, cx, cy, cz,
            )
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad, self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_2,
                62, self.T_F[0], self.T_F[1], self.T_F[2], self.p_F, self.NbaseN_F, cx, cy, cz,
            )
            ker_eva.kernel_eva_quad(
                self.Nel, self.n_quad, self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_3,
        ...
        self.B1 = np.empty(
            (
                self.Nel[0], self.Nel[1], self.Nel[2], self.n_quad[0],
    ...
    def inner_curl_beq(self, b1, b2, b3):
        ...
        ker_loc_3d.kernel_evaluate_2form(
            self.Nel, self.p, [0, 1, 1], self.n_quad, b1, [
                self.NbaseN[0], self.NbaseD[1], self.NbaseD[2],
            ], self.basisN[0], self.basisD[1], self.basisD[2], self.B1,
        )
        ker_loc_3d.kernel_evaluate_2form(
            self.Nel, self.p, [1, 0, 1], self.n_quad, b2, [
                self.NbaseD[0], self.NbaseN[1], self.NbaseD[2],
            ], self.basisD[0], self.basisN[1], self.basisD[2], self.B2,
        )
        ker_loc_3d.kernel_evaluate_2form(
            self.Nel, self.p, [1, 1, 0], self.n_quad, b3, [
                self.NbaseD[0], self.NbaseD[1], self.NbaseN[2],
            ], self.basisD[0], self.basisD[1], self.basisN[2], self.B3,
        )

        # assembly of F (1 - component)
        ker_loc_3d.kernel_inner_2(
            self.Nel[0], self.Nel[1], self.Nel[2], self.p[0], self.p[1], self.p[2], self.n_quad[0], self.n_quad[1], self.n_quad[2], 0, 0, 0, self.wts[0], self.wts[1],
            self.wts[2], self.basisN[0], self.basisN[1], self.basisN[2], self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.F1, self.mat_curl_beq_2 *
            self.B3 - self.mat_curl_beq_3*self.B2,
        )
        ...
        ker_loc_3d.kernel_inner_2(
            self.Nel[0], self.Nel[1], self.Nel[2], self.p[0], self.p[1], self.p[2], self.n_quad[0], self.n_quad[1], self.n_quad[2], 0, 0, 0, self.wts[0], self.wts[1],
            self.wts[2], self.basisN[0], self.basisN[1], self.basisN[2], self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.F2, self.mat_curl_beq_3 *
        ...
        ker_loc_3d.kernel_inner_2(
            self.Nel[0], self.Nel[1], self.Nel[2], self.p[0], self.p[1], self.p[2], self.n_quad[0], self.n_quad[1], self.n_quad[2], 0, 0, 0, self.wts[0], self.wts[1],
            self.wts[2], self.basisN[0], self.basisN[1], self.basisN[2], self.NbaseN[0], self.NbaseN[1], self.NbaseN[2], self.F3, self.mat_curl_beq_1 *
            self.B2 - self.mat_curl_beq_2*self.B1,
        )
...
def mass_curl(tensor_space, kind_map, params_map):
    ...
    M = [
        np.zeros((Nbi1, Nbi2, Nbi3, 2*p[0] + 1, 2*p[1] + 1, 2*p[2] + 1), dtype=float)
        for Nbi1, Nbi2, Nbi3 in zip(Nbi1, Nbi2, Nbi3)
    ]

    # assembly of block 12
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 1, 0, 1, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisD[0], basisN[1], basisD[2], NbaseN[0], NbaseN[1], NbaseN[2], M[0], mat_curl_beq_3,
    )

    # assembly of block 13
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 1, 1, 0, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisD[0], basisD[1], basisN[2], NbaseN[0], NbaseN[1], NbaseN[2], M[1], mat_curl_beq_2,
    )

    # assembly of block 21
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 0, 1, 1, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisN[0], basisD[1], basisD[2], NbaseN[0], NbaseN[1], NbaseN[2], M[2], mat_curl_beq_3,
    )

    # assembly of block 23
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 1, 1, 0, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisD[0], basisD[1], basisN[2], NbaseN[0], NbaseN[1], NbaseN[2], M[3], mat_curl_beq_1,
    )

    # assembly of block 31
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 0, 1, 1, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisN[0], basisD[1], basisD[2], NbaseN[0], NbaseN[1], NbaseN[2], M[4], mat_curl_beq_2,
    )

    # assembly of block 32
    ker_loc_3d.kernel_mass(
        Nel[0], Nel[1], Nel[2], p[0], p[1], p[2], n_quad[0], n_quad[1], n_quad[2], 0, 0, 0, 1, 0, 1, wts[0], wts[1],
        wts[2], basisN[0], basisN[1], basisN[2], basisD[0], basisN[1], basisD[2], NbaseN[0], NbaseN[1], NbaseN[2], M[5], mat_curl_beq_1,
    )
    ...
    for i in range(6):
        ...
        M[counter] = spa.csc_matrix(
            (M[counter].flatten(), (row, col.flatten())), shape=(
