...
class projectors_local_mhd:
    ...
    def projection_Q_0form(self, domain):
        ...
        ker_loc.kernel_pi2_1(
            ...
            [self.n_int[0],
             self.n_his[1],
             self.n_his[2]],
            [self.n_int_locbf_N[0],
             self.n_his_locbf_N[1],
            ...
            self.wts[1],
            self.wts[2],
            Q11, mat_eq.reshape(n_unique1[0],
                                self.pts[1][:, 0].size, self.pts[1][0, :].size, self.pts[2][:, 0].size, self.pts[2][0, :].size))
        ...
        ker_loc.kernel_pi2_2(
            ...
            [self.n_his[0],
             self.n_int[1],
             self.n_his[2]],
            [self.n_his_locbf_N[0],
             self.n_int_locbf_N[1],
             self.n_his_locbf_N[2]],
            self.his_global_N[0],
            self.int_global_N[1],
            self.his_global_N[2],
            ...
            self.wts[0],
            self.wts[2],
            Q22, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, n_unique2[1],
    ...
    def projection_T_0form(self, domain):
        ...
        ker_loc.kernel_pi1_1(
            ...
            self.x_int_indices[2],
            self.wts[0],
            T12, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, n_unique1[1],
                                n_unique1[2]))
        ...
        ker_loc.kernel_pi1_1(
            ...
            self.x_int_indices[2],
            self.wts[0],
            T13, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, n_unique1[1],
                                n_unique1[2]))
        ...
        ker_loc.kernel_pi1_2(
            ...
            self.x_int_indices[2],
            self.wts[1],
            T21, mat_eq.reshape(n_unique2[0],
                                self.pts[1][:, 0].size, self.pts[1][0, :].size, n_unique2[2]))
        ...
        ker_loc.kernel_pi1_2(
            ...
            self.x_int_indices[2],
            self.wts[1],
            T23, mat_eq.reshape(n_unique2[0],
                                self.pts[1][:, 0].size, self.pts[1][0, :].size, n_unique2[2]))
        ...
        ker_loc.kernel_pi1_3(
            ...
            self.x_his_indices[2],
            self.wts[2],
            T31, mat_eq.reshape(n_unique3[0],
                                n_unique3[1],
                                self.pts[2][:, 0].size, self.pts[2][0, :].size))
        ...
        ker_loc.kernel_pi1_3(
            ...
            self.x_his_indices[2],
            self.wts[2],
            T32, mat_eq.reshape(n_unique3[0],
                                n_unique3[1],
                                self.pts[2][:, 0].size, self.pts[2][0, :].size))
    ...
    def projection_S_0form(self, domain):
        ...
        ker_loc.kernel_pi2_1(
            ...
            self.wts[1],
            self.wts[2],
            S11, mat_eq.reshape(n_unique1[0],
                                self.pts[1][:, 0].size, self.pts[1][0, :].size, self.pts[2][:, 0].size, self.pts[2][0, :].size))
        ...
        ker_loc.kernel_pi2_2(
            ...
            self.wts[0],
            self.wts[2],
            S22, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, n_unique2[1],
                                self.pts[2][:, 0].size, self.pts[2][0, :].size))
    ...
    def projection_K_3form(self, domain):
        ...
        # assembly of K
        ker_loc.kernel_pi3(
            self.NbaseD, self.n_quad, self.n_his, self.n_his_locbf_D, self.his_global_D[0],
            self.his_global_D[1],
            self.his_global_D[2],
            self.his_loccof_D[0],
            self.his_loccof_D[1],
    ...
    def projection_N_0form(self, domain):
        ...
        ker_loc.kernel_pi2_1(
            ...
            self.wts[1],
            self.wts[2],
            N11, mat_eq.reshape(n_unique1[0],
                                self.pts[1][:, 0].size, self.pts[1][0, :].size, self.pts[2][:, 0].size, self.pts[2][0, :].size))
        ...
        ker_loc.kernel_pi2_2(
            ...
            self.wts[0],
            self.wts[2],
            N22, mat_eq.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, n_unique2[1],
                                self.pts[2][:, 0].size, self.pts[2][0, :].size))
    ...
    def RHS6(self, u, p, b, dt):
        ...
        out = self.A(u) + dt**2/4*self.N.T.dot(self.tensor_space.DIV.T.dot(self.tensor_space.M3.dot(self.L(u)))
...
class term_curl_beq:
    ...
    def __init__(self, tensor_space, mapping, kind_map=None, params_map=None, tensor_space_F=None, cx=None, cy=None, cz=None):
        ...
        if mapping == 0:
            ...
            ker_eva.kernel_eva_quad(self.Nel, self.n_quad, self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_2,
                                    62, self.T_F[0], self.T_F[1], self.T_F[2], self.p_F, self.NbaseN_F, cx, cy, cz)
            ker_eva.kernel_eva_quad(self.Nel, self.n_quad, self.pts[0], self.pts[1], self.pts[2], self.mat_curl_beq_3,
    ...
    def inner_curl_beq(self, b1, b2, b3):
        ...
        ker_loc_3d.kernel_evaluate_2form(self.Nel, self.p, [0, 1, 1], self.n_quad, b1, [self.NbaseN[0], self.NbaseD[1],
                                         self.NbaseD[2]], self.basisN[0], self.basisD[1], self.basisD[2], self.B1)
        ker_loc_3d.kernel_evaluate_2form(self.Nel, self.p, [1, 0, 1], self.n_quad, b2, [self.NbaseD[0], self.NbaseN[1],
                                         self.NbaseD[2]], self.basisD[0], self.basisN[1], self.basisD[2], self.B2)
        ker_loc_3d.kernel_evaluate_2form(self.Nel, self.p, [1, 1, 0], self.n_quad, b3, [self.NbaseD[0], self.NbaseD[1],
                                         self.NbaseN[2]], self.basisD[0], self.basisD[1], self.basisN[2], self.B3)
