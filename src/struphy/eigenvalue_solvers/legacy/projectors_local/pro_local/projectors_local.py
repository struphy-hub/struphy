...
class projectors_local_1d:
    ...
    def __init__(self, spline_space, n_quad):
        ...
        if self.p == 1:
            ...
            # number of non-vanishing N bf in interpolation interval (2, 3, 5, 7, ...)
            self.n_int_locbf_N = 2*self.p - 1
            # number of non-vanishing D bf in interpolation interval (1, 2, 4, 6, ...)
            self.n_int_locbf_D = 2*self.p - 2

        # interpolation points for each coeff.
        self.x_int          = np.zeros((n_lambda_int, self.n_int)        , dtype=float)
        ...
        self.int_loccof_N   = np.zeros(
            (n_lambda_int, self.n_int_locbf_N),
            dtype=int,
        )   # index of non-vanishing coeff. (N)
        self.int_loccof_D   = np.zeros(
            (n_lambda_int, self.n_int_locbf_D),
            dtype=int,
        )   # index of non-vanishing coeff. (D)
        ...
        # number of non-vanishing N bf in histopolation interval (2, 4, 6, 8, ...)
        self.n_his_locbf_N  = 2*self.p
        # number of non-vanishing D bf in histopolation interval (2, 4, 6, 8, ...)
        self.n_his_locbf_D  = 2*self.p - 1
        ...
        if self.bc == False:
            ...
            self.pts, self.wts = bsp.quadrature_grid(
                np.append(np.unique(self.x_his.flatten() % 1.), 1.), self.pts_loc, self.wts_loc,
            )
...
class projectors_local_3d:
    ...
    def pi_0(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        ker_loc.kernel_pi0_3d(
            self.NbaseN, self.p, self.coeff_i[0], self.coeff_i[1], self.coeff_i[2], self.coeffi_indices[0],
            self.coeffi_indices[1], self.coeffi_indices[2], self.x_int_indices[0], self.x_int_indices[1], self.x_int_indices[2], mat_f, lambdas,
        )
    ...
    def pi_3(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        mat_f = np.empty(
            (
                self.pts[0].flatten().size, self.pts[1].flatten().size,
                self.pts[2].flatten().size,
            ), dtype=float,
        )
        ...
        if callable(fun):
            ...
            if eval_kind == 'meshgrid':
                pts1, pts2, pts3 = np.meshgrid(
                    self.pts[0].flatten(
                    ), self.pts[1].flatten(), self.pts[2].flatten(), indexing='ij',
                )
                mat_f[:, :, :]   = fun(pts1, pts2, pts3)
            ...
            # point-wise evaluation
                ...
                for i1 in range(self.pts[0].size):
                    for i2 in range(self.pts[1].size):
                        for i3 in range(self.pts[2].size):
                            mat_f[i1, i2, i3] = fun(
                                self.pts[0].flatten()[i1], self.pts[1].flatten()[
                                    i2
        ...
        ker_loc.kernel_pi3_3d(
            self.NbaseD, self.p, self.n_quad, self.coeff_h[0], self.coeff_h[1], self.coeff_h[2], self.coeffh_indices[0], self.coeffh_indices[1], self.coeffh_indices[2], self.x_his_indices[0], self.x_his_indices[
                1
            ], self.x_his_indices[2], self.wts[0], self.wts[1], self.wts[2], mat_f.reshape(self.pts[0].shape[0], self.pts[0].shape[1], self.pts[1].shape[0], self.pts[1].shape[1], self.pts[2].shape[0], self.pts[2].shape[1]), lambdas,
        )
