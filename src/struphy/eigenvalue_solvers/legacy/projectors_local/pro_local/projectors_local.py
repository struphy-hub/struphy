...
class projectors_local_1d:
    ...
    def __init__(self, spline_space, n_quad):
        ...
        self.int_global_N   = np.zeros((n_lambda_int, self.n_int_locbf_N),
                                       dtype=int)   # global indices of non-vanishing N bf
        self.int_global_D   = np.zeros((n_lambda_int, self.n_int_locbf_D),
                                       dtype=int)   # global indices of non-vanishing D bf

        self.int_loccof_N   = np.zeros((n_lambda_int, self.n_int_locbf_N),
                                       dtype=int)   # index of non-vanishing coeff. (N)
        self.int_loccof_D   = np.zeros((n_lambda_int, self.n_int_locbf_D),
                                       dtype=int)   # index of non-vanishing coeff. (D)
...
class projectors_local_3d:
    ...
    def pi_0(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        ker_loc.kernel_pi0_3d(
            self.NbaseN, self.p, self.coeff_i[0],
            self.coeff_i[1],
            self.coeff_i[2],
    ...
    def pi_1(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        ker_loc.kernel_pi11_3d(
            [self.NbaseD[0],
             self.NbaseN[1],
             self.NbaseN[2]],
            self.p, self.n_quad, self.coeff_h[0],
            self.coeff_i[1],
            self.coeff_i[2],
            ...
            self.x_int_indices[2],
            self.wts[0],
            mat_f.reshape(self.pts[0].shape[0],
                          self.pts[0].shape[1],
                          x_int2.size, x_int3.size),
            lambdas1)
        ...
        ker_loc.kernel_pi12_3d(
            [self.NbaseN[0],
             self.NbaseD[1],
             self.NbaseN[2]],
            self.p, self.n_quad, self.coeff_i[0],
            self.coeff_h[1],
            self.coeff_i[2],
            self.coeffi_indices[0],
            ...
            self.x_int_indices[2],
            self.wts[1],
            mat_f.reshape(x_int1.size, self.pts[1].shape[0],
                          self.pts[1].shape[1],
                          x_int3.size),
            lambdas2)
        ...
        ker_loc.kernel_pi13_3d(
            ...
            self.wts[2],
            mat_f.reshape(x_int1.size, x_int2.size, self.pts[2][:, 0].shape[0],
                          self.pts[2].shape[1]),
            lambdas3)
    ...
    def pi_2(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        ker_loc.kernel_pi22_3d(
            ...
            mat_f.reshape(self.pts[0].shape[0],
                          self.pts[0].shape[1],
                          x_int2.size, self.pts[2].shape[0],
                          self.pts[2].shape[1]),
            lambdas2)
    ...
    def pi_3(self, fun, include_bc=True, eval_kind='meshgrid'):
        ...
        mat_f = np.empty(
            (self.pts[0].flatten().size,
             self.pts[1].flatten().size,
        ...
        if callable(fun):
            ...
            # point-wise evaluation
                ...
                for i1 in range(self.pts[0].size):
                    for i2 in range(self.pts[1].size):
                        for i3 in range(self.pts[2].size):
                            mat_f[i1, i2, i3] = fun(self.pts[0].flatten()[i1], self.pts[1].flatten()[
                                                    i2], self.pts[2].flatten()[i3])
