...
def kernel_1_form(idnx, idny, idnz, iddx, iddy, iddz, right1, right2, right3, bn1, bn2, bn3, bd1, bd2, bd3, pts1, pts2, pts3, wts1, wts2, wts3, Np, quad, p, Nel, p_shape, p_size, particle, lambdas_11, lambdas_12, lambdas_13, lambdas_21, lambdas_22, lambdas_23, lambdas_31, lambdas_32, lambdas_33, kernel_11, kernel_12, kernel_13, kernel_22, kernel_23, kernel_33, NbaseN, NbaseD, related, Np_loc, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz):
    ...
    # ====================================
    # $ omp parallel
    # $ omp do reduction ( + : kernel_11, kernel_12, kernel_13, kernel_22, kernel_23, kernel_33, right1, right2, right3) private (mid1, mid2, mid3, ip, w, vol, width2, lambdas_11, lambdas_22, lambdas_33, lambdas_12, lambdas_13, lambdas_21, lambdas_23, lambdas_31, lambdas_32, cell_left, point_left, point_right, cell_number, compact, width, mat_11, mat_12, mat_13, mat_21, mat_22, mat_23, mat_31, mat_32, mat_33, i1, i2, i3, il1, il2, il3, index1, index2, index3, value_x, value_y, value_z, span1f, span2f, span3f, l1f, l2f, l3f, r1f, r2f, r3f, b1f, b2f, b3f, d1f, d2f, d3f, der1f, der2f, der3f, df, fx, dfinv, lambda_index1, lambda_index2, lambda_index3, global_i1, global_i2, global_i3, global_il1, global_il2, global_il3, jl1, jl2, jl3, eta1, eta2, eta3, final_index1, final_index2, final_index3, q1, q2, q3, basisvalue)
    for ip in range(Np_loc):
        ...
        mat_11  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_21  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_31  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)

        mat_12  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_22  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_32  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)

        mat_13  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_23  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
        mat_33  = zeros((cell_number[0], cell_number[1], cell_number[2], quad[0], quad[1], quad[2]), dtype=float)
            ...
            for i2 in range(cell_number[1]):
                ...
                for i3 in range(cell_number[2]):
                    ...
                    for jl1 in range(quad[0]):
                        eta1 = 1.0/Nel[0]*index1 + pts1[0, jl1]
                        value_x = bsp.piecewise(p_shape[0], p_size[0], abs(eta1 - particle[0, ip]))
                        span1f = int((eta1 % 1.0)*nelf[0]) + pf1
                        for jl2 in range(quad[1]):
                            eta2 = 1.0/Nel[1]*index2 + pts2[0, jl2]
                            value_y = bsp.piecewise(p_shape[1], p_size[1], abs(eta2 - particle[1, ip]))
                            span2f = int((eta2 % 1.0)*nelf[1]) + pf2
                            for jl3 in range(quad[2]):
                                eta3 = 1.0/Nel[2]*index3 + pts3[0, jl3]
                                value_z = bsp.piecewise(p_shape[2], p_size[2], abs(eta3 - particle[2, ip]))
                                span3f = int((eta3 % 1.0)*nelf[2]) + pf3
                                # ========= mapping evaluation =============
                                # evaluate Jacobian matrix
                                mapping_fast.df_all(
                                    kind_map, params_map, tf1, tf2, tf3, pf, nbasef, span1f, span2f, span3f, cx, cy, cz, l1f, l2f, l3f,
                                    r1f, r2f, r3f, b1f, b2f, b3f, d1f, d2f, d3f, der1f, der2f, der3f, eta1 % 1.0, eta2 % 1.0, eta3 % 1.0, df, fx, 0,
                                )
                                # evaluate inverse of Jacobian matrix
                                mapping_fast.df_inv_all(df, dfinv)

                                mat_11[i1, i2, i3, jl1, jl2, jl3] = dfinv[0, 0] * value_x * value_y * value_z * vol
                                mat_21[i1, i2, i3, jl1, jl2, jl3] = dfinv[0, 1] * value_x * value_y * value_z * vol
                                mat_31[i1, i2, i3, jl1, jl2, jl3] = dfinv[0, 2] * value_x * value_y * value_z * vol

                                mat_12[i1, i2, i3, jl1, jl2, jl3] = dfinv[1, 0] * value_x * value_y * value_z * vol
                                mat_22[i1, i2, i3, jl1, jl2, jl3] = dfinv[1, 1] * value_x * value_y * value_z * vol
                                mat_32[i1, i2, i3, jl1, jl2, jl3] = dfinv[1, 2] * value_x * value_y * value_z * vol

                                mat_13[i1, i2, i3, jl1, jl2, jl3] = dfinv[2, 0] * value_x * value_y * value_z * vol
                                mat_23[i1, i2, i3, jl1, jl2, jl3] = dfinv[2, 1] * value_x * value_y * value_z * vol
                                mat_33[i1, i2, i3, jl1, jl2, jl3] = dfinv[2, 2] * value_x * value_y * value_z * vol
            ...
            for i2 in range(cell_number[1]):
                ...
                for i3 in range(cell_number[2]):
                    ...
                    for lambda_index1 in range(p[0]):
                        final_index1 = iddx[index1 % Nel[0], lambda_index1]
                        for lambda_index2 in range(p[1]+1):
                            final_index2 = idny[index2 % Nel[1], lambda_index2]
                            for lambda_index3 in range(p[2]+1):
                                final_index3 = idnz[index3 % Nel[2], lambda_index3]
                                mid1 = 0.0
                                mid2 = 0.0
                                mid3 = 0.0
                                for q1 in range(quad[0]):
                                    for q2 in range(quad[1]):
                                        for q3 in range(quad[2]):

                                            basisvalue = bd1[index1 % Nel[0], lambda_index1, 0, q1] * bn2[index2 % Nel[1], lambda_index2, 0, q2] * bn3[
                                                index3 %
                                                Nel[2], lambda_index3, 0, q3,
                                            ] * wts1[index1 % Nel[0], q1]*wts2[index2 % Nel[1], q2]*wts3[index3 % Nel[2], q3]
                                            mid1 += mat_11[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid2 += mat_21[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid3 += mat_31[i1, i2, i3, q1, q2, q3] * basisvalue
                    ...
                    for lambda_index1 in range(p[0]+1):
                        final_index1 = idnx[index1 % Nel[0], lambda_index1]
                        for lambda_index2 in range(p[1]):
                            final_index2 = iddy[index2 % Nel[1], lambda_index2]
                            for lambda_index3 in range(p[2]+1):
                                final_index3 = idnz[index3 % Nel[2], lambda_index3]
                                mid1 = 0.0
                                mid2 = 0.0
                                ...
                                for q1 in range(quad[0]):
                                    for q2 in range(quad[1]):
                                        for q3 in range(quad[2]):
                                            basisvalue = bn1[index1 % Nel[0], lambda_index1, 0, q1] * bd2[index2 % Nel[1], lambda_index2, 0, q2] * bn3[
                                                index3 %
                                                Nel[2], lambda_index3, 0, q3,
                                            ] * wts1[index1 % Nel[0], q1]*wts2[index2 % Nel[1], q2]*wts3[index3 % Nel[2], q3]
                                            mid1 += mat_12[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid2 += mat_22[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid3 += mat_32[i1, i2, i3, q1, q2, q3] * basisvalue
                    ...
                    for lambda_index1 in range(p[0]+1):
                        ...
                        for lambda_index2 in range(p[1]+1):
                            ...
                            for lambda_index3 in range(p[2]):
                                final_index3 = iddz[index3 % Nel[2], lambda_index3]
                                mid1 = 0.0
                                mid2 = 0.0
                                mid3 = 0.0
                                for q1 in range(quad[0]):
                                    for q2 in range(quad[1]):
                                        for q3 in range(quad[2]):
                                            basisvalue = bn1[index1 % Nel[0], lambda_index1, 0, q1] * bn2[index2 % Nel[1], lambda_index2, 0, q2] * bd3[
                                                index3 %
                                                Nel[2], lambda_index3, 0, q3,
                                            ] * wts1[index1 % Nel[0], q1]*wts2[index2 % Nel[1], q2]*wts3[index3 % Nel[2], q3]
                                            mid1 += mat_13[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid2 += mat_23[i1, i2, i3, q1, q2, q3] * basisvalue
                                            mid3 += mat_33[i1, i2, i3, q1, q2, q3] * basisvalue
        ...
        for il1 in range(3):
            width[il1] = p[il1] + cell_number[il1]
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_11[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_11[global_i1, global_i2, global_i3] * lambdas_11[global_il1, global_il2, global_il3] + lambdas_21[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_21[global_il1, global_il2, global_il3] + lambdas_31[global_i1, global_i2, global_i3] * lambdas_31[global_il1, global_il2, global_il3]
                                )
                    # ===== 12 compoponent ==========
                    for il1 in range(3):
                        width2[il1] = 2*related[il1]+1
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_12[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_11[global_i1, global_i2, global_i3] * lambdas_12[global_il1, global_il2, global_il3] + lambdas_21[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_22[global_il1, global_il2, global_il3] + lambdas_31[global_i1, global_i2, global_i3] * lambdas_32[global_il1, global_il2, global_il3]
                                )
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_13[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_11[global_i1, global_i2, global_i3] * lambdas_13[global_il1, global_il2, global_il3] + lambdas_21[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_23[global_il1, global_il2, global_il3] + lambdas_31[global_i1, global_i2, global_i3] * lambdas_33[global_il1, global_il2, global_il3]
                                )
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_22[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_12[global_i1, global_i2, global_i3] * lambdas_12[global_il1, global_il2, global_il3] + lambdas_22[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_22[global_il1, global_il2, global_il3] + lambdas_32[global_i1, global_i2, global_i3] * lambdas_32[global_il1, global_il2, global_il3]
                                )
                    # ===== 23 compoponent ==========
                    for il1 in range(3):
                        width2[il1] = 2*related[il1]+1
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_23[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_12[global_i1, global_i2, global_i3] * lambdas_13[global_il1, global_il2, global_il3] + lambdas_22[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_23[global_il1, global_il2, global_il3] + lambdas_32[global_i1, global_i2, global_i3] * lambdas_33[global_il1, global_il2, global_il3]
                                )
                        ...
                        for il2 in range(width2[1]):
                            ...
                            for il3 in range(width2[2]):
                                ...
                                kernel_33[global_i1, global_i2, global_i3, il1, il2, il3] += w * (
                                    lambdas_13[global_i1, global_i2, global_i3] * lambdas_13[global_il1, global_il2, global_il3] + lambdas_23[
                                        global_i1, global_i2, global_i3,
                                    ] * lambdas_23[global_il1, global_il2, global_il3] + lambdas_33[global_i1, global_i2, global_i3] * lambdas_33[global_il1, global_il2, global_il3]
                                )
...
# ==============================================================================================
@types('int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'double', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'int', 'int[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:]', 'double[:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'int[:]', 'int[:]', 'int[:]', 'int', 'int', 'double[:]', 'double[:]', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]')
def bvpushltwo(idnx, idny, idnz, iddx, iddy, iddz, dt, bb1, bb2, bb3, bn1, bn2, bn3, bd1, bd2, bd3, pts1, pts2, pts3, wts1, wts2, wts3, Np, quad, p, Nel, p_shape, p_size, particle, lambdas_11, lambdas_12, lambdas_13, lambdas_21, lambdas_22, lambdas_23, lambdas_31, lambdas_32, lambdas_33, NbaseN, NbaseD, related, Np_loc, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz):
...
# ==============================================================================================
@types('double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'double[:,:,:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'int[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'int', 'int[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:]', 'double[:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'int[:]', 'int[:]', 'int', 'int', 'double[:]', 'double[:]', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]')
def kernel_1_heavy(bn1, bn2, bn3, bd1, bd2, bd3, idnx, idny, idnz, iddx, iddy, iddz, pts1, pts2, pts3, wts1, wts2, wts3, out1, out2, out3, in1, in2, in3, Np, quad, p, Nel, p_shape, p_size, particle, lambdas_11, lambdas_12, lambdas_13, lambdas_21, lambdas_22, lambdas_23, lambdas_31, lambdas_32, lambdas_33, NbaseN, NbaseD, Np_loc, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz):
    ...
    # $ omp parallel
    # $ omp do reduction ( + : out1, out2, out3) private (value1, value2, value3, mid1, mid2, mid3, ip, w, vol, lambdas_11, lambdas_12, lambdas_13, lambdas_21, lambdas_22, lambdas_23, lambdas_31, lambdas_32, lambdas_33, cell_left, point_left, point_right, cell_number, compact, mat_11, mat_12, mat_13, mat_21, mat_22, mat_23, mat_31, mat_32, mat_33, i1, i2, i3, il1, il2, il3, index1, index2, index3, value_x, value_y, value_z, span1f, span2f, span3f, l1f, l2f, l3f, r1f, r2f, r3f, b1f, b2f, b3f, d1f, d2f, d3f, der1f, der2f, der3f, df, fx, dfinv, lambda_index1, lambda_index2, lambda_index3, eta1, eta2, eta3, final_index1, final_index2, final_index3, jl1, jl2, jl3, q1, q2, q3, basisvalue)
    for ip in range(Np_loc):
        ...
        for il1 in range(NbaseN[0]):
            for il2 in range(NbaseN[1]):
                for il3 in range(NbaseN[2]):
                    out1[il1, il2, il3] += w * value1 * \
                        (lambdas_11[il1, il2, il3] + lambdas_21[il1, il2, il3] + lambdas_31[il1, il2, il3])
                    out2[il1, il2, il3] += w * value2 * \
                        (lambdas_12[il1, il2, il3] + lambdas_22[il1, il2, il3] + lambdas_32[il1, il2, il3])
                    out3[il1, il2, il3] += w * value3 * \
                        (lambdas_13[il1, il2, il3] + lambdas_23[il1, il2, il3] + lambdas_33[il1, il2, il3])
    ...
    for ip in range(Np_loc):
        ...
        mat_11  = zeros(
            (
                ...
                2, num_cell[1], num_cell[2], quad[0],
            ), dtype=float,
        )
        mat_21  = zeros(
            (
        ...
        mat_12  = zeros(
            (
                cell_number[0], cell_number[1], cell_number[2],
                num_cell[0], 2, num_cell[2], quad[1],
            ), dtype=float,
        )
        mat_22  = zeros(
            (
        ...
        mat_13  = zeros(
            (
                cell_number[0], cell_number[1], cell_number[2],
                num_cell[0], num_cell[1], 2, quad[2],
            ), dtype=float,
        )
        mat_23  = zeros(
            (
                        ...
                        for il2 in range(num_cell[1]):
                            ...
                            for il3 in range(num_cell[2]):
                                ...
                                for jl1 in range(quad[0]):
                                    ...
                                    mat_11[i1, i2, i3, il1, il2, il3, jl1] = dft[0, 0] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_21[i1, i2, i3, il1, il2, il3, jl1] = dft[0, 1] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_31[i1, i2, i3, il1, il2, il3, jl1] = dft[0, 2] * \
                                        value_x * value_y * value_z / det_df * vol
                        ...
                        for il2 in range(2):
                            for il3 in range(num_cell[2]):
                                ...
                                for jl1 in range(quad[1]):
                                    ...
                                    mat_12[i1, i2, i3, il1, il2, il3, jl1] = dft[1, 0] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_22[i1, i2, i3, il1, il2, il3, jl1] = dft[1, 1] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_32[i1, i2, i3, il1, il2, il3, jl1] = dft[1, 2] * \
                                        value_x * value_y * value_z / det_df * vol
                    for il1 in range(num_cell[0]):
                        eta1 = 1.0/Nel[0]*index1 + 1.0/Nel[0]/num_cell[0]*il1
                        span1f = int((eta1 % 1.0)*nelf[0]) + pf1
                        ...
                        for il2 in range(num_cell[1]):
                            ...
                            for il3 in range(2):
                                for jl1 in range(quad[2]):
                                    ...
                                    mat_13[i1, i2, i3, il1, il2, il3, jl1] = dft[2, 0] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_23[i1, i2, i3, il1, il2, il3, jl1] = dft[2, 1] * \
                                        value_x * value_y * value_z / det_df * vol
                                    mat_33[i1, i2, i3, il1, il2, il3, jl1] = dft[2, 2] * \
                                        value_x * value_y * value_z / det_df * vol
                                ...
                                for il1 in range(2):
                                    for il2 in range(num_cell[1]):
                                        for il3 in range(num_cell[2]):
                                            ...
                                            mid1 = coeff_h_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            f_int = 0.
                                            for jl1 in range(quad[0]):
                                                f_int += wts1[jl1] * mat_21[i1, i2, i3, il1, il2, il3, jl1]
                                            mid2 = coeff_h_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            ...
                                            mid3 = coeff_h_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            right1[final_index1, final_index2, final_index3]     += w * (
                                                (particle[3, ip]+ddt*mid_particle[0, ip])*mid1 + (
                                                    particle[4, ip]+ddt*mid_particle[1, ip]
                                                )*mid2 + (particle[5, ip]+ddt*mid_particle[2, ip])*mid3
                                            )
                                ...
                                for il1 in range(num_cell[0]):
                                    for il2 in range(2):
                                        for il3 in range(num_cell[2]):
                                            ...
                                            mid1 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_h_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            ...
                                            mid2 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_h_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            ...
                                            mid3 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_h_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_i_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                ...
                                for il1 in range(num_cell[0]):
                                    for il2 in range(num_cell[1]):
                                        for il3 in range(2):
                                            ...
                                            mid1 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_h_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            f_int = 0.
                                            for jl1 in range(quad[2]):
                                                f_int += wts3[jl1] * mat_23[i1, i2, i3, il1, il2, il3, jl1]
                                            mid2 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_h_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
                                            ...
                                            mid3 = coeff_i_x[2*(p[0]-1) - 2 * lambda_index1 + il1] * coeff_i_y[
                                                2*(
                                                    p[1]-1
                                                ) - 2 * lambda_index2 + il2
                                            ] * coeff_h_z[2*(p[2]-1) - 2 * lambda_index3 + il3] * f_int
