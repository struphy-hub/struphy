...
class terms_control_variate:
    ...
    def __init__(self, tensor_space_FEM, domain, basis_u):
        ...
        self.mat_jh1 = np.empty(
            (
                self.space.Nel[0], self.space.n_quad[0], self.space.Nel[1],
                self.space.n_quad[1], self.space.Nel[2], self.space.n_quad[2],
            ), dtype=float,
        )
        ...
        ker_cv.kernel_evaluation_quad(
            self.space.Nel, self.space.n_quad, self.space.pts[0], self.space.pts[1], self.space.pts[2], self.mat_jh1, kind_fun_eq[
                0
            ], domain.kind_map, domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ker_cv.kernel_evaluation_quad(
            self.space.Nel, self.space.n_quad, self.space.pts[0], self.space.pts[1], self.space.pts[2], self.mat_jh2, kind_fun_eq[
                1
            ], domain.kind_map, domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ker_cv.kernel_evaluation_quad(
            self.space.Nel, self.space.n_quad, self.space.pts[0], self.space.pts[1], self.space.pts[2], self.mat_jh3, kind_fun_eq[
                2
            ], domain.kind_map, domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        ker_cv.kernel_evaluation_quad(
            self.space.Nel, self.space.n_quad, self.space.pts[0], self.space.pts[1], self.space.pts[2], self.mat_nh, kind_fun_eq[
                3
            ], domain.kind_map, domain.params_map, domain.T[0], domain.T[1], domain.T[2], domain.p, domain.NbaseN, domain.cx, domain.cy, domain.cz,
        )
        ...
        if self.basis_u == 0:
            self.M12 = np.empty(
                (
                    self.space.NbaseN[0], self.space.NbaseN[1], self.space.NbaseN[2],
                    2*self.space.p[0] + 1, 2*self.space.p[1] + 1, 2*self.space.p[2] + 1,
                ), dtype=float,
            )
            self.M13 = np.empty(
                (
            ...
            self.M12 = np.empty(
                (
                    self.space.NbaseN[0], self.space.NbaseD[1], self.space.NbaseD[2],
                    2*self.space.p[0] + 1, 2*self.space.p[1] + 1, 2*self.space.p[2] + 1,
                ), dtype=float,
            )
            self.M13 = np.empty(
            ...
            self.M23 = np.empty(
                (
                    self.space.NbaseD[0], self.space.NbaseN[1], self.space.NbaseD[2],
                    2*self.space.p[0] + 1, 2*self.space.p[1] + 1, 2*self.space.p[2] + 1,
                ), dtype=float,
            )
    ...
    def inner_prod_jh_eq(self, b1, b2, b3):
        ...
        ker.kernel_evaluate_2form(
            self.space.Nel, self.space.p, [
                0, 1, 1,
            ], self.space.n_quad, b1, self.space.Nbase_2form[0], self.space.basisN[0], self.space.basisD[1], self.space.basisD[2], self.B2_1,
        )
        ker.kernel_evaluate_2form(
            self.space.Nel, self.space.p, [
                1, 0, 1,
            ], self.space.n_quad, b2, self.space.Nbase_2form[1], self.space.basisD[0], self.space.basisN[1], self.space.basisD[2], self.B2_2,
        )
        ker.kernel_evaluate_2form(
            self.space.Nel, self.space.p, [
                1, 1, 0,
            ], self.space.n_quad, b3, self.space.Nbase_2form[2], self.space.basisD[0], self.space.basisD[1], self.space.basisN[2], self.B2_3,
        )
