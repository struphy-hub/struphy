...
@types('int[:]', 'double[:,:,:]', 'int', 'double[:]', 'double[:]', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]')
    ...
    for ie1 in range(Nel[0]):
        for ie2 in range(Nel[1]):
            for ie3 in range(Nel[2]):
                # ========= physical domain =============
                x1 = map3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 1, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
                x2 = map3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 2, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
                x3 = map3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 3, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
...
@types('double',
       ...
       'double[:,:,:]',
       'double[:,:,:]')
def vv(tol, out_vector, Jeq, bb1, bb2, bb3, n, Np_loc, NbaseN, NbaseD, Nel, p, t1, t2, t3, particles, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz):
    ...
    # $ omp parallel
    # $ omp do private (tt, eta1, eta2, eta3, span1, span2, span3, ip, l1, l2, l3, r1, r2, r3, b1, b2, b3, d1, d2, d3, bn1, bn2, bn3, bd1, bd2, bd3, vel, vel2, dfinv, U_value, span1f, span2f, span3f, l1f, l2f, l3f, r1f, r2f, r3f, b1f, b2f, b3f, d1f, d2f, d3f, der1f, der2f, der3f, df, fx, x1, x2, x3, Jeq)
    for ip in range(Np_loc):
        ...
        tt = eva.evaluation_kernel(pn1, pn2, pn3, bn1, bn2, bn3, span1, span2, span3, NbaseN[0], NbaseN[1], NbaseN[2], n)
        if abs(tt) > tol:
            U_value = 1.0/tt
        else:
            U_value = 0.0
                            ...
                            for il1 in range(p1 + 1):
                                for il2 in range(p2 + 1):
                                    for il3 in range(p3 + 1):
                                        value += bn1[ie1, il1, 0, q1] * bn2[ie2, il2, 0, q2] * bn3[ie3, il3, 0, q3] * n[idnx[ie1, il1], idny[ie2, il2], idnz[ie3, il3]]
                            if abs(value) > tol:
