...
@types('int[:]', 'double[:,:,:]', 'int', 'double[:]', 'double[:]', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]')
    ...
    for ie1 in range(Nel[0]):
        for ie2 in range(Nel[1]):
            for ie3 in range(Nel[2]):
                # ========= physical domain =============
                x1 = mapping3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 1, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
                x2 = mapping3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 2, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
                x3 = mapping3d.f(ie1/Nel[0], ie2/Nel[2], ie3/Nel[2], 3, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
...
@types('double[:,:,:,:,:,:]',
       ...
       'double[:,:,:,:]',
       'double[:,:,:,:]')
def bvright1(G_inv_11, G_inv_12, G_inv_13, G_inv_22, G_inv_23, G_inv_33, idnx, idny, idnz, iddx, iddy, iddz, nel1, nel2, nel3, nq1, nq2, nq3, p1,
             p2, p3, d1, d2, d3, b1value, b2value, b3value, b1, b2, b3, dft, generate_weight1, generate_weight3, Jeq, bn1, bn2, bn3, bd1, bd2, bd3):
...
@types('double[:,:,:,:,:,:]',
       ...
       'double[:,:]',
       'double[:,:]')
def bvright2(DFI_11, DFI_12, DFI_13, DFI_21, DFI_22, DFI_23, DFI_31, DFI_32, DFI_33, df_det, Jeqx, Jeqy, Jeqz, nel1, nel2, nel3, nq1, nq2, nq3, p1,
             p2, p3, d1, d2, d3, b1value, b2value, b3value, uvalue, dft, generate_weight1, generate_weight3, Jeq, pts1, pts2, pts3, wts1, wts2, wts3):
                ...
                for q1 in range(nq1):
                    for q2 in range(nq2):
                        for q3 in range(nq3):
                            ...
                            b1value[ie1, ie2, ie3, q1, q2, q3] = detdet * uvalue[ie1, ie2, ie3, q1, q2,
                                                                                 q3] * (generate_weight3[1]*generate_weight1[2] - generate_weight3[2]*generate_weight1[1])
                            b2value[ie1, ie2, ie3, q1, q2, q3] = detdet * uvalue[ie1, ie2, ie3, q1, q2,
                                                                                 q3] * (generate_weight3[2]*generate_weight1[0] - generate_weight3[0]*generate_weight1[2])
                            b3value[ie1, ie2, ie3, q1, q2, q3] = detdet * uvalue[ie1, ie2, ie3, q1, q2,
                                                                                 q3] * (generate_weight3[0]*generate_weight1[1] - generate_weight3[1]*generate_weight1[0])
...
@types('int[:,:]',
       ...
       'double[:,:,:]',
       'double[:,:,:]')
def bvfinal(idnx, idny, idnz, iddx, iddy, iddz, nel1, nel2, nel3, nq1, nq2, nq3, p1, p2, p3, d1, d2, d3, b1value,
            b2value, b3value, uvalue, bn1, bn2, bn3, bd1, bd2, bd3, temp_final_1, temp_final_2, temp_final_3):
...
@types('double',
       ...
       'double[:,:,:]',
       'double[:,:,:]')
def vv(tol, out_vector, Jeq, bb1, bb2, bb3, gather_grid, Np_loc, NbaseN, NbaseD, Nel, p, t1, t2, t3, particles, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz):
    ...
    for ip in range(Np_loc):
        ...
        ie2 = int(eta2*Nel[1])
        ie3 = int(eta3*Nel[2])
        disx = (eta1 - ie1 / Nel[0])*Nel[0]
        disy = (eta2 - ie2 / Nel[1])*Nel[1]
        disz = (eta3 - ie3 / Nel[2])*Nel[2]

        U_value = gather_grid[ie1, ie2, ie3] * (1 - disx)*(1 - disy)*(1 - disz)
        U_value += gather_grid[(ie1 + 1) % Nel[0], ie2, ie3] * disx*(1 - disy)*(1 - disz)
        U_value += gather_grid[ie1, (ie2 + 1) % Nel[1], ie3] * (1 - disx)*disy*(1 - disz)
        U_value += gather_grid[ie1, ie2, (ie3 + 1) % Nel[2]] * (1 - disx)*(1 - disy)*disz
        U_value += gather_grid[ie1, (ie2 + 1) % Nel[1], (ie3 + 1) % Nel[2]] * (1 - disx)*disy*disz
        U_value += gather_grid[(ie1 + 1) % Nel[0], ie2, (ie3 + 1) % Nel[2]] * disx*(1 - disy)*disz
        U_value += gather_grid[(ie1 + 1) % Nel[0], (ie2 + 1) % Nel[1], ie3] * disx*disy*(1 - disz)
        U_value += gather_grid[(ie1 + 1) % Nel[0], (ie2 + 1) % Nel[1], (ie3 + 1) % Nel[2]] * disx*disy*disz
        ...
        x1 = mapping3d.f(eta1, eta2, eta3, 1, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
        x2 = mapping3d.f(eta1, eta2, eta3, 2, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
        x3 = mapping3d.f(eta1, eta2, eta3, 3, kind_map, params_map, tf1, tf2, tf3, pf, nbasef, cx, cy, cz)
