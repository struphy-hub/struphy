...
class EMW_operators:
    ...
    def __weight_1(self, eta1, eta2, eta3):
        ...
        det_g   = 1./abs(self.DOMAIN.evaluate(eta1, eta2, eta3, 'det_df'))
    ...
    def __assemble_M1_cross(self, weight):
        ...
        ind     = [[indD[0], indN[1], indN[2]],         # DNN
                   [indN[0], indD[1], indN[2]],            # NDN
                   [indN[0], indN[1], indD[2]]]            # NND

        basis   = [[basisD[0], basisN[1], basisN[2]],   # DNN
                   [basisN[0], basisD[1], basisN[2]],      # DNN
                   [basisN[0], basisN[1], basisD[2]]]      # NND
        ...
        for a in range(3):
            for b in range(3):
                ...
                if a != b:
                    ...
                    ker.kernel_mass(Nel[0], Nel[1], Nel[2],
                                    p[0], p[1], p[2],
                                    n_quad[0], n_quad[1], n_quad[2],
                                    ns[a][0], ns[a][1], ns[a][2], ns[b][0], ns[b][1], ns[b][2],
                                    wts[0], wts[1], wts[2],
                                    basis[a][0], basis[a][1], basis[a][2], basis[b][0], basis[b][1], basis[b][2],
                                    ind[a][0], ind[a][1], ind[a][2],
                                    M[a][b],
                                    mat_w)
    ...
    def set_Preconditioners(self, solver, drop_tol=1e-4, fill_fac=10.):
        """
        TODO
        """
