...
class Tensor_spline_space:
    ...
    def __init__(self, spline_spaces, ck=-1, cx=None, cy=None, n_tor=0, basis_tor='r'):
        ...
        self.E1 = spa.bmat(
            [
                [spa.kron(self.E1_pol, self.E0_tor), None],
                [None, spa.kron(self.E0_pol, self.E1_tor)],
            ], format='csr',
        )
        self.E2 = spa.bmat(
            [
                [spa.kron(self.E2_pol, self.E1_tor), None],
                [None, spa.kron(self.E3_pol, self.E0_tor)],
            ], format='csr',
        )
        self.E3 = spa.kron(self.E3_pol, self.E1_tor, format='csr')
        self.Ev = spa.bmat(
            [
                [spa.kron(self.Ev_pol, self.E0_tor), None],
                [None, spa.kron(self.E0_pol, self.E0_tor)],
            ], format='csr',
        )
        ...
        self.B1 = spa.bmat(
            [
                [spa.kron(self.B1_pol, self.B0_tor), None],
                [None, spa.kron(self.B0_pol, self.B1_tor)],
            ], format='csr',
        )
        self.B2 = spa.bmat(
            [
                [spa.kron(self.B2_pol, self.B1_tor), None],
                [None, spa.kron(self.B3_pol, self.B0_tor)],
            ], format='csr',
        )
        self.B3 = spa.kron(self.B3_pol, self.B1_tor, format='csr')
        self.Bv = spa.bmat(
            [
                [spa.kron(self.Bv_pol, self.E0_tor), None],
                [None, spa.kron(Bv3, self.B0_tor)],
            ], format='csr',
        )
    ...
    def apply_M1_0_ten(self, x, mats):
        ...
        out1 = self.B0_tor.dot(
            mats[0][1].dot(
                self.B0_tor.T.dot(
                    self.B1_pol.dot(mats[0][0].dot(self.B1_pol.T.dot(x1))).T,
                ),
            ),
        ).T
        out2 = self.B1_tor.dot(
            mats[1][1].dot(
                self.B1_tor.T.dot(
                    self.B0_pol.dot(mats[1][0].dot(self.B0_pol.T.dot(x2))).T,
                ),
            ),
        ).T
    ...
    def apply_M2_0_ten(self, x, mats):
        ...
        out1 = self.B1_tor.dot(
            mats[0][1].dot(
                self.B1_tor.T.dot(
                    self.B2_pol.dot(mats[0][0].dot(self.B2_pol.T.dot(x1))).T,
                ),
            ),
        ).T
        out2 = self.B0_tor.dot(
            mats[1][1].dot(
                self.B0_tor.T.dot(
                    self.B3_pol.dot(mats[1][0].dot(self.B3_pol.T.dot(x2))).T,
                ),
            ),
        ).T
        ...
        # 3D
            ...
            if self.dim == 2:
                ...
                self.M1_mat = spa.bmat(
                    [
                        [spa.kron(M11, self.M0_tor), None], [
                            None, spa.kron(M22, self.M1_tor),
                        ],
                    ], format='csr',
                )
            else:
        ...
        # 3D
            ...
            if self.dim == 2:
                ...
                self.M2_mat = spa.bmat(
                    [
                        [spa.kron(M11, self.M1_tor), None], [
                            None, spa.kron(M22, self.M0_tor),
                        ],
                    ], format='csr',
                )
            else:
    ...
    def reshape_pol_1(self, coeff):
        ...
        if c_size == self.E1.shape[0]:
            ...
            ].reshape(self.E1_pol.shape[0], self.E0_tor.shape[0])
            coeff1_pol_3 = coeff[
                self.E1_pol.shape[0]*self.E0_tor.shape[0]
                :
            ].reshape(self.E0_pol.shape[0], self.E1_tor.shape[0])
        else:
            coeff1_pol_1 = coeff[
                :self.E1_pol_0.shape[0]*self.E0_tor_0.shape[0]
            ].reshape(self.E1_pol_0.shape[0], self.E0_tor_0.shape[0])
            coeff1_pol_3 = coeff[
                self.E1_pol_0.shape[0]*self.E0_tor_0.shape[0]
                :
            ].reshape(self.E0_pol_0.shape[0], self.E1_tor_0.shape[0])
    ...
    def reshape_pol_2(self, coeff):
        ...
        if c_size == self.E2.shape[0]:
            ...
            ].reshape(self.E2_pol.shape[0], self.E1_tor.shape[0])
            coeff2_pol_3 = coeff[
                self.E2_pol.shape[0]*self.E1_tor.shape[0]
                :
            ].reshape(self.E3_pol.shape[0], self.E0_tor.shape[0])
        else:
            coeff2_pol_1 = coeff[
                :self.E2_pol_0.shape[0]*self.E1_tor_0.shape[0]
            ].reshape(self.E2_pol_0.shape[0], self.E1_tor_0.shape[0])
            coeff2_pol_3 = coeff[
                self.E2_pol_0.shape[0]*self.E1_tor_0.shape[0]
                :
            ].reshape(self.E3_pol_0.shape[0], self.E0_tor_0.shape[0])
    ...
    def reshape_pol_v(self, coeff):
        ...
        if c_size == self.Ev.shape[0]:
            ...
            ].reshape(self.Ev_pol.shape[0], self.E0_tor.shape[0])
            coeffv_pol_3 = coeff[
                self.Ev_pol.shape[0]*self.E0_tor.shape[0]
                :
            ].reshape(self.E0_pol.shape[0], self.E0_tor.shape[0])

        else:
            coeffv_pol_1 = coeff[
                :self.Ev_pol_0.shape[0]*self.E0_tor.shape[0]
            ].reshape(self.Ev_pol_0.shape[0], self.E0_tor.shape[0])
            coeffv_pol_3 = coeff[
                self.Ev_pol_0.shape[0]*self.E0_tor.shape[0]
                :
            ].reshape(self.E0_pol.shape[0], self.E0_tor_0.shape[0])
    ...
    def evaluate_NN(self, eta1, eta2, eta3, coeff, which='V0', part='r'):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            # matrix evaluation
                ...
                eva_2d.evaluate_matrix_2d(
                    self.T[0], self.T[1], self.p[0], self.p[1], self.indN[0],
                    self.indN[1], coeff_r[:, :, 0].copy(), eta1, eta2, values_r_1, 0,
                )
                eva_2d.evaluate_matrix_2d(
                    self.T[0], self.T[1], self.p[0], self.p[1], self.indN[0],
                    self.indN[1], coeff_i[:, :, 0].copy(), eta1, eta2, values_i_1, 0,
                )
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_matrix_2d(
                        self.T[0], self.T[1], self.p[0], self.p[1],
                        self.indN[0], self.indN[1], coeff_r[:, :, 1], eta1, eta2, values_r_2, 0,
                    )
                    eva_2d.evaluate_matrix_2d(
                        self.T[0], self.T[1], self.p[0], self.p[1],
                        self.indN[0], self.indN[1], coeff_i[:, :, 1], eta1, eta2, values_i_2, 0,
                    )
        ...
        # --------- evaluate FEM field at given point -------
            ...
            real_1 = eva_2d.evaluate_n_n(
                self.T[0], self.T[1], self.p[0], self.p[1],
                self.indN[0], self.indN[1], coeff_r[:, :, 0].copy(), eta1, eta2,
            )
            imag_1 = eva_2d.evaluate_n_n(
                self.T[0], self.T[1], self.p[0], self.p[1],
                self.indN[0], self.indN[1], coeff_i[:, :, 0].copy(), eta1, eta2,
            )
            ...
            if self.n_tor != 0 and self.basis_tor == 'r':
                ...
                real_2 = eva_2d.evaluate_n_n(
                    self.T[0], self.T[1], self.p[0], self.p[1],
                    self.indN[0], self.indN[1], coeff_r[:, :, 1], eta1, eta2,
                )
                imag_2 = eva_2d.evaluate_n_n(
                    self.T[0], self.T[1], self.p[0], self.p[1],
                    self.indN[0], self.indN[1], coeff_i[:, :, 1], eta1, eta2,
                )
    ...
    def evaluate_DN(self, eta1, eta2, eta3, coeff, which='V1', part='r'):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_2d.evaluate_tensor_product_2d(
                    self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                    self.indD[0], self.indN[1], coeff_r[:, :, 0].copy(), eta1, eta2, values_r_1, 11,
                )
                eva_2d.evaluate_tensor_product_2d(
                    self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                    self.indD[0], self.indN[1], coeff_i[:, :, 0].copy(), eta1, eta2, values_i_1, 11,
                )
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_tensor_product_2d(
                        self.t[0], self.T[1], self.p[0] -
                        1, self.p[1], self.indD[0], self.indN[1], coeff_r[:, :, 1], eta1, eta2, values_r_2, 11,
                    )
                    eva_2d.evaluate_tensor_product_2d(
                        self.t[0], self.T[1], self.p[0] -
                        1, self.p[1], self.indD[0], self.indN[1], coeff_i[:, :, 1], eta1, eta2, values_i_2, 11,
                    )
            ...
            # matrix evaluation
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_matrix_2d(
                        self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                        self.indD[0], self.indN[1], coeff_r[:, :, 1], eta1, eta2, values_r_2, 11,
                    )
                    eva_2d.evaluate_matrix_2d(
                        self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                        self.indD[0], self.indN[1], coeff_i[:, :, 1], eta1, eta2, values_i_2, 11,
                    )
        ...
        # --------- evaluate FEM field at given point -------
            ...
            real_1 = eva_2d.evaluate_d_n(
                self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                self.indD[0], self.indN[1], coeff_r[:, :, 0].copy(), eta1, eta2,
            )
            imag_1 = eva_2d.evaluate_d_n(
                self.t[0], self.T[1], self.p[0] - 1, self.p[1],
                self.indD[0], self.indN[1], coeff_i[:, :, 0].copy(), eta1, eta2,
            )

            if self.n_tor != 0 and self.basis_tor == 'r':

                real_2 = eva_2d.evaluate_d_n(
                    self.t[0], self.T[1], self.p[0] - 1,
                    self.p[1], self.indD[0], self.indN[1], coeff_r[:, :, 1], eta1, eta2,
                )
                imag_2 = eva_2d.evaluate_d_n(
                    self.t[0], self.T[1], self.p[0] - 1,
                    self.p[1], self.indD[0], self.indN[1], coeff_i[:, :, 1], eta1, eta2,
                )
    ...
    def evaluate_ND(self, eta1, eta2, eta3, coeff, which='V2', part='r'):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_2d.evaluate_tensor_product_2d(
                    self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                    self.indN[0], self.indD[1], coeff_r[:, :, 0].copy(), eta1, eta2, values_r_1, 12,
                )
                eva_2d.evaluate_tensor_product_2d(
                    self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                    self.indN[0], self.indD[1], coeff_i[:, :, 0].copy(), eta1, eta2, values_i_1, 12,
                )
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_tensor_product_2d(
                        self.T[0], self.t[1], self.p[0], self.p[1] -
                        1, self.indN[0], self.indD[1], coeff_r[:, :, 1].copy(), eta1, eta2, values_r_2, 12,
                    )
                    eva_2d.evaluate_tensor_product_2d(
                        self.T[0], self.t[1], self.p[0], self.p[1] -
                        1, self.indN[0], self.indD[1], coeff_i[:, :, 1].copy(), eta1, eta2, values_i_2, 12,
                    )
            ...
            # matrix evaluation
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_matrix_2d(
                        self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                        self.indN[0], self.indD[1], coeff_r[:, :, 1].copy(), eta1, eta2, values_r_2, 12,
                    )
                    eva_2d.evaluate_matrix_2d(
                        self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                        self.indN[0], self.indD[1], coeff_i[:, :, 1].copy(), eta1, eta2, values_i_2, 12,
                    )
        ...
        # --------- evaluate FEM field at given point -------
            ...
            real_1 = eva_2d.evaluate_n_d(
                self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                self.indN[0], self.indD[1], coeff_r[:, :, 0].copy(), eta1, eta2,
            )
            imag_1 = eva_2d.evaluate_n_d(
                self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                self.indN[0], self.indD[1], coeff_i[:, :, 0].copy(), eta1, eta2,
            )
            ...
            if self.n_tor != 0 and self.basis_tor == 'r':
                ...
                real_2 = eva_2d.evaluate_n_d(
                    self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                    self.indN[0], self.indD[1], coeff_r[:, :, 1].copy(), eta1, eta2,
                )
                imag_2 = eva_2d.evaluate_n_d(
                    self.T[0], self.t[1], self.p[0], self.p[1] - 1,
                    self.indN[0], self.indD[1], coeff_i[:, :, 1].copy(), eta1, eta2,
                )
    ...
    def evaluate_DD(self, eta1, eta2, eta3, coeff, which='V3', part='r'):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_2d.evaluate_tensor_product_2d(
                    self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                    self.indD[0], self.indD[1], coeff_r[:, :, 0].copy(), eta1, eta2, values_r_1, 2,
                )
                eva_2d.evaluate_tensor_product_2d(
                    self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                    self.indD[0], self.indD[1], coeff_i[:, :, 0].copy(), eta1, eta2, values_i_1, 2,
                )
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_tensor_product_2d(
                        self.t[0], self.t[1], self.p[0] - 1, self.p[1] -
                        1, self.indD[0], self.indD[1], coeff_r[:, :, 1], eta1, eta2, values_r_2, 2,
                    )
                    eva_2d.evaluate_tensor_product_2d(
                        self.t[0], self.t[1], self.p[0] - 1, self.p[1] -
                        1, self.indD[0], self.indD[1], coeff_i[:, :, 1], eta1, eta2, values_i_2, 2,
                    )
            ...
            # matrix evaluation
                ...
                if self.n_tor != 0 and self.basis_tor == 'r':
                    ...
                    eva_2d.evaluate_matrix_2d(
                        self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                        self.indD[0], self.indD[1], coeff_r[:, :, 1], eta1, eta2, values_r_2, 2,
                    )
                    eva_2d.evaluate_matrix_2d(
                        self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                        self.indD[0], self.indD[1], coeff_i[:, :, 1], eta1, eta2, values_i_2, 2,
                    )
        ...
        # --------- evaluate FEM field at given point -------
            ...
            real_1 = eva_2d.evaluate_d_d(
                self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                self.indD[0], self.indD[1], coeff_r[:, :, 0].copy(), eta1, eta2,
            )
            imag_1 = eva_2d.evaluate_d_d(
                self.t[0], self.t[1], self.p[0] - 1, self.p[1] - 1,
                self.indD[0], self.indD[1], coeff_i[:, :, 0].copy(), eta1, eta2,
            )
            ...
            if self.n_tor != 0 and self.basis_tor == 'r':
                ...
                real_2 = eva_2d.evaluate_d_d(
                    self.t[0], self.t[1], self.p[0] - 1,
                    self.p[1] - 1, self.indD[0], self.indD[1], coeff_r[:, :, 1], eta1, eta2,
                )
                imag_2 = eva_2d.evaluate_d_d(
                    self.t[0], self.t[1], self.p[0] - 1,
                    self.p[1] - 1, self.indD[0], self.indD[1], coeff_i[:, :, 1], eta1, eta2,
                )
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_3d.evaluate_tensor_product(
                    self.T[0], self.T[1], self.T[2], self.p[0], self.p[1],
                    self.p[2], self.indN[0], self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, values, 0,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.T[0], self.T[1], self.T[2], self.p[0], self.p[1], self.p[2], self.indN[0],
                        self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 0,
                    )

                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
                        self.T[0], self.T[1], self.T[2], self.p[0], self.p[1], self.p[2],
                        self.indN[0], self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, values, 0,
                    )
    ...
    def evaluate_DNN(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                values = np.empty((eta1.size, eta2.size, eta3.size), dtype=float)
                eva_3d.evaluate_tensor_product(
                    self.t[0], self.T[1], self.T[2], self.p[0] - 1, self.p[1],
                    self.p[2], self.indD[0], self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, values, 11,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.t[0], self.T[1], self.T[2], self.p[0] - 1, self.p[1], self.p[2], self.indD[0],
                        self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 11,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
                        self.t[0], self.T[1], self.T[2], self.p[0] - 1, self.p[1], self.p[2],
                        self.indD[0], self.indN[1], self.indN[2], coeff, eta1, eta2, eta3, values, 11,
                    )
    ...
    def evaluate_NDN(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                values = np.empty((eta1.size, eta2.size, eta3.size), dtype=float)
                eva_3d.evaluate_tensor_product(
                    self.T[0], self.t[1], self.T[2], self.p[0], self.p[1] - 1,
                    self.p[2], self.indN[0], self.indD[1], self.indN[2], coeff, eta1, eta2, eta3, values, 12,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.T[0], self.t[1], self.T[2], self.p[0], self.p[1] - 1, self.p[2], self.indN[0],
                        self.indD[1], self.indN[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 12,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
                        self.T[0], self.t[1], self.T[2], self.p[0], self.p[1] - 1, self.p[2],
                        self.indN[0], self.indD[1], self.indN[2], coeff, eta1, eta2, eta3, values, 12,
                    )
    ...
    def evaluate_NND(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                values = np.empty((eta1.size, eta2.size, eta3.size), dtype=float)
                eva_3d.evaluate_tensor_product(
                    self.T[0], self.T[1], self.t[2], self.p[0], self.p[1], self.p[2] - 1,
                    self.indN[0], self.indN[1], self.indD[2], coeff, eta1, eta2, eta3, values, 13,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.T[0], self.T[1], self.t[2], self.p[0], self.p[1], self.p[2] - 1, self.indN[0],
                        self.indN[1], self.indD[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 13,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
    ...
    def evaluate_NDD(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_3d.evaluate_tensor_product(
                    self.T[0], self.t[1], self.t[2], self.p[0], self.p[1] - 1,
                    self.p[2] - 1, self.indN[0], self.indD[1], self.indD[2], coeff, eta1, eta2, eta3, values, 21,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.T[0], self.t[1], self.t[2], self.p[0], self.p[1] - 1, self.p[2] - 1, self.indN[0],
                        self.indD[1], self.indD[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 21,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
                        self.T[0], self.t[1], self.t[2], self.p[0], self.p[1] - 1, self.p[2] - 1,
    ...
    def evaluate_DND(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_3d.evaluate_tensor_product(
                    self.t[0], self.T[1], self.t[2], self.p[0] - 1, self.p[1],
                    self.p[2] - 1, self.indD[0], self.indN[1], self.indD[2], coeff, eta1, eta2, eta3, values, 22,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.t[0], self.T[1], self.t[2], self.p[0] - 1, self.p[1], self.p[2] - 1, self.indD[0],
                        self.indN[1], self.indD[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 22,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
                        self.t[0], self.T[1], self.t[2], self.p[0] - 1, self.p[1], self.p[2] - 1,
    ...
    def evaluate_DDN(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                values = np.empty((eta1.size, eta2.size, eta3.size), dtype=float)
                eva_3d.evaluate_tensor_product(
                    self.t[0], self.t[1], self.T[2], self.p[0] - 1, self.p[1] - 1,
                    self.p[2], self.indD[0], self.indD[1], self.indN[2], coeff, eta1, eta2, eta3, values, 23,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.t[0], self.t[1], self.T[2], self.p[0] - 1, self.p[1] - 1, self.p[2], self.indD[0],
                        self.indD[1], self.indN[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 23,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
    ...
    def evaluate_DDD(self, eta1, eta2, eta3, coeff):
        ...
        if isinstance(eta1, np.ndarray):
            ...
            if eta1.ndim == 1:
                ...
                eva_3d.evaluate_tensor_product(
                    self.t[0], self.t[1], self.t[2], self.p[0] - 1, self.p[1] - 1,
                    self.p[2] - 1, self.indD[0], self.indD[1], self.indD[2], coeff, eta1, eta2, eta3, values, 3,
                )
            ...
            # matrix evaluation
                ...
                if max(eta1.shape) == eta1.size:
                    eva_3d.evaluate_sparse(
                        self.t[0], self.t[1], self.t[2], self.p[0] - 1, self.p[1] - 1, self.p[2] - 1, self.indD[0],
                        self.indD[1], self.indD[2], coeff, eta1, eta2, eta3, eta1.shape[0], eta2.shape[1], eta3.shape[2], values, 3,
                    )
                # `eta1` is a dense meshgrid. Process each point as default.
                else:
                    eva_3d.evaluate_matrix(
