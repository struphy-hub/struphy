...
def get_M0(tensor_space_FEM, domain, apply_boundary_ops=False, weight=None):
    ...
    ker.kernel_mass(
        np.array(Nel), np.array(p), np.array(n_quad), np.array([0, 0]), np.array(
            [0, 0],
        ), wts[0], wts[1], basisN[0], basisN[1], basisN[0], basisN[1], indN[0], indN[1], M, mat_w*det_df,
    )
...
def get_M1(tensor_space_FEM, domain, apply_boundary_ops=False, weight=None):
    ...
    g_inv = domain.metric_inv(pts[0].flatten(), pts[1].flatten(), 0.)
    ...
    for a in range(3):
        for b in range(3):
            ...
            if np.any(mat_w):
                ker.kernel_mass(
                    np.array(Nel), np.array(p), np.array(n_quad), np.array(ns[a]), np.array(
                        ns[b],
                    ), wts[0], wts[1], basis[a][0], basis[a][1], basis[b][0], basis[b][1], ind[a][0], ind[a][1], M[a][b], mat_w*det_df,
                )
...
def get_M2(tensor_space_FEM, domain, apply_boundary_ops=False, weight=None):
    ...
    g = domain.metric(pts[0].flatten(), pts[1].flatten(), 0.)
...
def get_M3(tensor_space_FEM, domain, apply_boundary_ops=False, weight=None):
    ...
    ker.kernel_mass(
        np.array(Nel), np.array(p), np.array(n_quad), np.array([1, 1]), np.array(
            [1, 1],
        ), wts[0], wts[1], basisD[0], basisD[1], basisD[0], basisD[1], indD[0], indD[1], M, mat_w/det_df,
    )
