...
def rhs1_1d(row1 : 'int[:]', col1 : 'int[:]', subs1 : 'int[:]', subs_cum1 : 'int[:]', wts1 : 'float[:,:]', bsp1 : 'float[:,:,:]', mat_eq : 'float[:,:]', rhs : 'float[:]'):
    ...
    for i1 in range(n_rows_1):
        ...
        for j1 in range(subs1[row1[i1]]):
            for q1 in range(nq1):
                value1 += wts1[row1[i1] + j1 + subs_cum1[row1[i1]], q1] * bsp1[
                    row1[i1] + j1 +
                    subs_cum1[row1[i1]], q1, col1[i1],
...
def rhs0_f_1d(indices1 : 'int[:,:]', bsp11 : 'float[:,:]', bsp12 : 'float[:,:]', mat_eq : 'float[:]', f : 'float[:]', rhs : 'float[:]', row : 'int[:]', col : 'int[:]'):
    ...
    for i1 in range(len(indices1[0])):
        ...
        rhs[i] += f[indices1[0, i1]] * bsp11[indices1[0, i1], indices1[1, i1]] * \
            bsp12[indices1[0, i1], indices1[2, i1]] * mat_eq[indices1[0, i1]]
...
def rhs1_f_1d(indices1 : 'int[:,:]', subs1 : 'int[:]', subs_cum1 : 'int[:]', wts1 : 'float[:,:]', bsp11 : 'float[:,:,:]', bsp12 : 'float[:,:,:]', mat_eq : 'float[:,:]', f : 'float[:]', rhs : 'float[:]', row : 'int[:]', col : 'int[:]'):
    ...
    for i1 in range(len(indices1[0])):
        ...
        for j1 in range(subs1[indices1[0, i1]]):
            for q1 in range(nq1):
                value1 += wts1[indices1[0, i1] + j1 + subs_cum1[indices1[0, i1]], q1] * bsp11[
                    indices1[0, i1] + j1 + subs_cum1[indices1[0, i1]], q1, indices1[1, i1],
    ...
    for i1 in range(n_rows_1):
        for i2 in range(n_rows_2):
            ...
            for j2 in range(subs2[row2[i2]]):
                for q2 in range(nq2):
                    value1 += wts2[row2[i2] + j2 + subs_cum2[row2[i2]], q2] * bsp2[
                        row2[i2] + j2 +
                        subs_cum2[row2[i2]], q2, col2[i2],
                ...
                for j2 in range(subs2[row2[i2]]):
                    for q1 in range(nq1):
                        for q2 in range(nq2):
                            ...
                            w_vol  = wts1[row1[i1] + j1 + subs_cum1[row1[i1]], q1] * \
                                wts2[row2[i2] + j2 + subs_cum2[row2[i2]], q2]

                            basis  = bsp1[row1[i1] + j1 + subs_cum1[row1[i1]], q1, col1[i1]] * \
                                bsp2[row2[i2] + j2 + subs_cum2[row2[i2]], q2, col2[i2]]

                            value1 += w_vol * basis * mat_eq[
                                row1[i1] + j1 +
                                subs_cum1[row1[i1]], q1, row2[i2] + j2 + subs_cum2[row2[i2]], q2,
                            ]
...
def rhs0_f_2d(indices1 : 'int[:,:]', indices2 : 'int[:,:]', bsp11 : 'float[:,:]', bsp12 : 'float[:,:]', bsp21 : 'float[:,:]', bsp22 : 'float[:,:]', mat_eq : 'float[:,:]', f : 'complex[:,:]', rhs : 'complex[:]', row : 'int[:]', col : 'int[:]'):
    ...
    for i1 in range(len(indices1[0])):
        for i2 in range(len(indices2[0])):
            ...
            rhs[i] += f[indices1[0, i1], indices2[0, i2]] * mat_eq[indices1[0, i1], indices2[0, i2]] * bsp11[
                indices1[0, i1], indices1[1, i1],
...
def rhs11_f_2d(indices1 : 'int[:,:]', indices2 : 'int[:,:]', subs1 : 'int[:]', subs_cum1 : 'int[:]', wts1 : 'float[:,:]', bsp11 : 'float[:,:,:]', bsp12 : 'float[:,:,:]', bsp21 : 'float[:,:]', bsp22 : 'float[:,:]', mat_eq : 'float[:,:,:]', f : 'complex[:,:]', rhs : 'complex[:]', row : 'int[:]', col : 'int[:]'):
    ...
    for i1 in range(len(indices1[0])):
        for i2 in range(len(indices2[0])):
            ...
            for j1 in range(subs1[indices1[0, i1]]):
                for q1 in range(nq1):
                    value1 += wts1[indices1[0, i1] + j1 + subs_cum1[indices1[0, i1]], q1] * bsp11[indices1[0, i1] + j1 + subs_cum1[indices1[0, i1]], q1, indices1[1, i1]] * \
            ...
            rhs[i] += f[indices1[0, i1], indices2[0, i2]] * value1 * \
                bsp21[indices2[0, i2], indices2[1, i2]] * bsp22[indices2[0, i2], indices2[2, i2]]
    ...
    for i1 in range(len(indices1[0])):
        for i2 in range(len(indices2[0])):
            ...
            for j2 in range(subs2[indices2[0, i2]]):
                for q2 in range(nq2):
                    value1 += wts2[indices2[0, i2] + j2 + subs_cum2[indices2[0, i2]], q2] * bsp21[indices2[0, i2] + j2 + subs_cum2[indices2[0, i2]], q2, indices2[1, i2]] * \
            ...
            rhs[i] += f[indices1[0, i1], indices2[0, i2]] * value1 * \
                bsp11[indices1[0, i1], indices1[1, i1]] * bsp12[indices1[0, i1], indices1[2, i1]]
        ...
        for i2 in range(n_rows_2):
            for i3 in range(n_rows_3):
                ...
                for j3 in range(subs3[row3[i3]]):
                    for q3 in range(nq3):
                        value1 += wts3[row3[i3] + j3 + subs_cum3[row3[i3]], q3] * bsp3[
                    ...
                    for q2 in range(nq2):
                        for j3 in range(subs3[row3[i3]]):
                            for q3 in range(nq3):
                                ...
                                w_vol  = wts2[row2[i2] + j2 + subs_cum2[row2[i2]], q2] * \
                                    wts3[row3[i3] + j3 + subs_cum3[row3[i3]], q3]

                                basis  = bsp2[row2[i2] + j2 + subs_cum2[row2[i2]], q2, col2[i2]] * \
                                    bsp3[row3[i3] + j3 + subs_cum3[row3[i3]], q3, col3[i3]]

                                value1 += w_vol * basis * mat_eq[
                                    row1[i1], row2[i2] + j2 +
                                    subs_cum2[row2[i2]], q2, row3[i3] + j3 + subs_cum3[row3[i3]], q3,
                                ]
                    ...
                    for q1 in range(nq1):
                        for j3 in range(subs3[row3[i3]]):
                            for q3 in range(nq3):
                                ...
                                value1 += w_vol * basis * \
                                    mat_eq[
                                        row1[i1] + j1 + subs_cum1[row1[i1]], q1,
                                        row2[i2], row3[i3] + j3 + subs_cum3[row3[i3]], q3,
                                    ]
                        ...
                        for j2 in range(subs2[row2[i2]]):
                            for q2 in range(nq2):
                                ...
                                value1 += w_vol * basis * \
                                    mat_eq[
                                        row1[i1] + j1 + subs_cum1[row1[i1]], q1,
                                        row2[i2] + j2 + subs_cum2[row2[i2]], q2, row3[i3],
                                    ]
                            ...
                            for q2 in range(nq2):
                                for j3 in range(subs3[row3[i3]]):
                                    for q3 in range(nq3):
                                        ...
                                        w_vol  = wts1[row1[i1] + j1 + subs_cum1[row1[i1]], q1] * wts2[
                                            row2[i2] +
                                            j2 + subs_cum2[row2[i2]], q2,
                                        ] * wts3[row3[i3] + j3 + subs_cum3[row3[i3]], q3]

                                        basis  = bsp1[row1[i1] + j1 + subs_cum1[row1[i1]], q1, col1[i1]] * bsp2[
                                            row2[i2] + j2 +
                                            subs_cum2[row2[i2]], q2, col2[i2],
                                        ] * bsp3[row3[i3] + j3 + subs_cum3[row3[i3]], q3, col3[i3]]
                                        ...
                                        value1 += w_vol * basis * \
                                            mat_eq[
                                                row1[i1] + j1 + subs_cum1[row1[i1]], q1, row2[i2] + j2 +
                                                subs_cum2[row2[i2]], q2, row3[i3] + j3 + subs_cum3[row3[i3]], q3,
                                            ]
        ...
        for i2 in range(len(indices2[0])):
            for i3 in range(len(indices3[0])):
                ...
                for j1 in range(n_row_sub1[indices1[0, i1]]):
                    for q1 in range(nq1):
                        value1 += wts1[indices1[0, i1] + j1 + counter1, q1] * bsp11[indices1[0, i1] + j1 + counter1, q1, indices1[1, i1]] * bsp12[
                            indices1[
                                0,
                                i1,
                            ] + j1 + counter1, q1, indices1[2, i1],
                        ] * mat_eq[indices1[0, i1] + j1 + counter1, q1, indices2[0, i2], indices3[0, i3]]

                i = nv2*nv3*indices1[3, i1] + nv3*indices2[3, i2] + indices3[3, i3]

                rhs[i] += f[indices1[0, i1], indices2[0, i2], indices3[0, i3]] * value1 * bsp21[indices2[0, i2], indices2[1, i2]] * \
                    bsp22[indices2[0, i2], indices2[2, i2]] * bsp31[
                        indices3[0, i3],
                        indices3[1, i3],
        ...
        for i2 in range(len(indices2[0])):
            for i3 in range(len(indices3[0])):
                ...
                for j2 in range(n_row_sub2[indices2[0, i2]]):
                    for q2 in range(nq2):
                        value1 += wts2[indices2[0, i2] + j2 + counter2, q2] * bsp21[indices2[0, i2] + j2 + counter2, q2, indices2[1, i2]] * bsp22[
                            indices2[
                                0,
                                i2,
                            ] + j2 + counter2, q2, indices2[2, i2],
                        ] * mat_eq[indices1[0, i1], indices2[0, i2] + j2 + counter2, q2, indices3[0, i3]]

                i = nv2*nv3*indices1[3, i1] + nv3*indices2[3, i2] + indices3[3, i3]

                rhs[i] += f[indices1[0, i1], indices2[0, i2], indices3[0, i3]] * value1 * bsp11[indices1[0, i1], indices1[1, i1]] * \
                    bsp12[indices1[0, i1], indices1[2, i1]] * bsp31[
                        indices3[0, i3],
                        indices3[1, i3],
                ] * bsp32[indices3[0, i3], indices3[2, i3]]
        ...
        for i2 in range(len(indices2[0])):
            for i3 in range(len(indices3[0])):
                ...
                for j3 in range(n_row_sub3[indices3[0, i3]]):
                    for q3 in range(nq3):
                        value1 += wts3[indices3[0, i3] + j3 + counter3, q3] * bsp31[indices3[0, i3] + j3 + counter3, q3, indices3[1, i3]] * bsp32[
                            indices3[
                                0,
                                i3,
                            ] + j3 + counter3, q3, indices3[2, i3],
                        ] * mat_eq[indices1[0, i1], indices2[0, i2], indices3[0, i3] + j3 + counter3, q3]

                i = nv2*nv3*indices1[3, i1] + nv3*indices2[3, i2] + indices3[3, i3]

                rhs[i] += f[indices1[0, i1], indices2[0, i2], indices3[0, i3]] * value1 * bsp11[indices1[0, i1], indices1[1, i1]] * \
                    bsp12[indices1[0, i1], indices1[2, i1]] * bsp21[
                        indices2[0, i2],
                        indices2[1, i2],
