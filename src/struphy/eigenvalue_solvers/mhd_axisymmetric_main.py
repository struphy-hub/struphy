def solve_mhd_ev_problem_2d(num_params, eq_mhd, n_tor, basis_tor='i', path_out=None):
    ...
    MAT = spa.linalg.inv(mhd_ops.Mn_mat.tocsc()).dot(
        EF.T.dot(
            space_2d.C0.conjugate().T.dot(
                M2_0.dot(
                    space_2d.C0.dot(
                        EF,
                    ),
        ...
        ) + mhd_ops.MJ_mat.dot(space_2d.C0.dot(EF)) - space_2d.D0.conjugate().T.dot(M3_0.dot(L)),
    ).toarray()
