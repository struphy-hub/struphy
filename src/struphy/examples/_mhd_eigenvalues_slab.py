...
def main():
    ...
    a = 1.
    R0 = 3.
    dom_params = {'l1': 0., 'r1': a, 'l2': 0.,
                  'r2': 2*np.pi*a, 'l3': 0., 'r3': 2*np.pi*R0}
    ...
    num_params = {'Nel': Nel, 'p': p, 'spl_kind': spl_kind,
                  'nq_el': nq_el, 'nq_pr': nq_pr, 'bc': bc, 'polar_ck': polar_ck}
    ...
    # safety factor q(x) = q0 + (q1 - q0)*(x/a)^2
    params_mhd['q0'] = 1.05
    params_mhd['q1'] = 1.85

    # number density n(x) = (1 - na)*(1 - (x/a)^n1)^n2 + na
    params_mhd['n1'] = 0.
    params_mhd['n2'] = 0.
    params_mhd['na'] = 1.
    ...
    fem_2d = Tensor_spline_space(
        [fem_1d_1, fem_1d_2], n_tor=n_tor, basis_tor=b_tor)
    ...
    omega2_eig, U_eig, MAT = solve_mhd_ev_problem_2d(
        num_params, eq_mhd, n_tor, b_tor)

    # analyze spectrum and plot continua
    A, S = get_mhd_continua_2d(fem_2d, domain, omega2_eig, U_eig,
                               [0, 12], params_mhd['B0']/params_mhd['R0'], 1e-2, 3)
    ...
    ax[1, 0].set_ylabel('y')
    ax[1, 0].set_title(
        r'Grid : $N_\mathrm{el}=$' + str(fem_2d.Nel[:2]), pad=10)

    # analytical continuous spectra
    spec_calc = MhdContinousSpectraShearedSlab(a=params_mhd['a'],
                                               R0=params_mhd['R0'],
                                               Bz=lambda x: params_mhd['B0'] - 0*x,
                                               q=eq_mhd.q, rho=eq_mhd.nx,
                                               p=eq_mhd.px, gamma=5/3)
    ...
    for m in ms_plot:
        ax[1, 1].plot(A[m][0]*a,
                      (np.sqrt(A[m][1])/norm) ** exponent_A,
                      '+', label='m = ' + str(m))
        ax[1, 1].plot(etaplot[0]*a, (spec_calc(etaplot[0]*a, m, n_tor)
                      ['shear_Alfvén']/norm)**exponent_A,
                      'k--', linewidth=0.5)
    ...
    for m in ms_plot:
        ax[1, 2].plot(S[m][0]*a,
                      (np.sqrt(S[m][1])/norm) ** exponent_S,
                      '+', label='m = ' + str(m))
        ax[1, 2].plot(etaplot[0]*a, (spec_calc(etaplot[0]*a, m, n_tor)
                                     ['slow_sound']/norm)**exponent_A,
                      'k--', linewidth=0.5)
