...
class PolarExtractionBlocksC1:
    ...
    def __init__(self, domain, derham):
        ...
        self._tau = max([((self.cx[1] - self.pole[0])*(-2)).max(),
                         ((self.cx[1] - self.pole[0]) - np.sqrt(3)
                          * (self.cy[1] - self.pole[1])).max(),
        ...
        self._xi_1[1, :] = 1/3 - 1/(3*self.tau)*(self.cx[1] - self.pole[0]) + \
            np.sqrt(3)/(3*self.tau)*(self.cy[1] - self.pole[1])
        self._xi_1[2, :] = 1/3 - 1/(3*self.tau)*(self.cx[1] - self.pole[0]) - \
            np.sqrt(3)/(3*self.tau)*(self.cy[1] - self.pole[1])
        ...
        # first n_rings tp rings --> "polar coeffs"
        e1_11_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][0], self.n_rings[1][0]*self.n1), dtype=float)
        e1_12_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][0], self.n_rings[1][1]*self.d1), dtype=float)

        e1_21_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][1], self.n_rings[1][0]*self.n1), dtype=float)
        e1_22_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][1], self.n_rings[1][1]*self.d1), dtype=float)

        # 1st component
        for l in range(2):
            for j in range(self.n1):
                e1_21_blocks_ten_to_pol[l, j] = self.xi_1[l +
                                                          1, j] - self.xi_0[l + 1, j]

        # 2nd component
        for l in range(2):
            for j in range(1*self.d1, 2*self.d1):
                e1_22_blocks_ten_to_pol[l, j] = self.xi_1[l + 1,
                                                          (j - self.d1 + 1) % self.d1] - self.xi_1[l + 1, j - self.d1]

        self._e_ten_to_pol['1'] = [[csr(e1_11_blocks_ten_to_pol), csr(e1_12_blocks_ten_to_pol), None],
                                   [csr(e1_21_blocks_ten_to_pol), csr(
                                       e1_22_blocks_ten_to_pol), None],
                                   [None, None, csr(e0_blocks_ten_to_pol)]]

        # =============== basis extraction operator for discrete 1-forms (Hdiv) =========

        # first n_rings tp rings --> "polar coeffs"
        e3_blocks_ten_to_pol = np.zeros(
            (self.n_polar[3][0], self.n_rings[3][0]*self.d1), dtype=float)

        self._e_ten_to_pol['2'] = [[csr(e1_22_blocks_ten_to_pol), csr(-e1_21_blocks_ten_to_pol), None],
                                   [csr(e1_12_blocks_ten_to_pol), csr(
                                       e1_11_blocks_ten_to_pol), None],
        ...
        self._e_ten_to_pol['v'] = [[csr(e0_blocks_ten_to_pol), None, None],
        ...
        # first n_rings tp rings --> "polar coeffs"
        p0_blocks_ten_to_pol = np.zeros(
            (self.n_polar[0][0], self.n_rings[0][0]*self.n1), dtype=float)
        ...
        p0_blocks_ten_to_ten = np.block(
            [0*np.identity(self.n1)]*self.n_rings[0][0] + [np.identity(self.n1)])
        ...
        # first n_rings tp rings --> "polar coeffs"
        p1_11_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][0], self.n_rings[1][0]*self.n1), dtype=float)
        p1_22_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][1], self.n_rings[1][1]*self.d1), dtype=float)
        ...
        p1_12_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][0], self.n_rings[1][1]*self.d1), dtype=float)
        p1_21_blocks_ten_to_pol = np.zeros(
            (self.n_polar[1][1], self.n_rings[1][0]*self.d1), dtype=float)

        self._p_ten_to_pol['1'] = [[csr(p1_11_blocks_ten_to_pol), csr(p1_12_blocks_ten_to_pol), None],
                                   [csr(p1_21_blocks_ten_to_pol), csr(
                                       p1_22_blocks_ten_to_pol), None],
        ...
        # !! NOTE: for odd spline degrees and periodic splines the first Greville point sometimes does NOT start at zero!!
        if domain.p[1] % 2 != 0 and not (abs(derham.Vh_fem['0'].spaces[1].interpolation_grid[0]) < 1e-14):
            p1_11_blocks_ten_to_ten[:, 3*self.n1 //
                                    3 - 1] = -np.roll(self.xi_1[0], -1)
            p1_11_blocks_ten_to_ten[:, 1*self.n1 //
                                    3 - 1] = -np.roll(self.xi_1[1], -1)
            p1_11_blocks_ten_to_ten[:, 2*self.n1 //
                                    3 - 1] = -np.roll(self.xi_1[2], -1)
        else:
        ...
        p1_11_blocks_ten_to_ten = np.block(
            [p1_11_blocks_ten_to_ten, np.identity(self.n1)])

        p1_22_blocks_ten_to_ten = np.block(
            [0*np.identity(self.d1)]*self.n_rings[1][1] + [np.identity(self.d1)])

        p1_12_blocks_ten_to_ten = np.zeros(
            (self.d1, (self.n_rings[1][1] + 1)*self.d1), dtype=float)
        p1_21_blocks_ten_to_ten = np.zeros(
            (self.n1, (self.n_rings[1][0] + 1)*self.n1), dtype=float)

        self._p_ten_to_ten['1'] = [[csr(p1_11_blocks_ten_to_ten), csr(p1_12_blocks_ten_to_ten), None],
                                   [csr(p1_21_blocks_ten_to_ten), csr(
                                       p1_22_blocks_ten_to_ten), None],
                                   [None, None, csr(p0_blocks_ten_to_ten)]]

        # ========== projection extraction operator for discrete 1-forms (Hdiv) ==========

        # first n_rings tp rings --> "polar coeffs"
        p3_blocks_ten_to_pol = np.zeros(
            (self.n_polar[3][0], self.n_rings[3][0]*self.d1), dtype=float)

        self._p_ten_to_pol['2'] = [[csr(p1_22_blocks_ten_to_pol), csr(p1_21_blocks_ten_to_pol), None],
                                   [csr(p1_12_blocks_ten_to_pol), csr(
                                       p1_11_blocks_ten_to_pol), None],
        ...
        if derham.Vh_fem['3'].spaces[1].histopolation_grid[0] < -1e-14:
            ...
            p3_blocks_ten_to_ten[:, 0*self.n1//3:1 *
                                 self.n1//3] = -a0[:, None] - a1[:, None]
            p3_blocks_ten_to_ten[:, 1*self.n1//3:2*self.n1//3] = -a1[:, None]

        p3_blocks_ten_to_ten += np.identity(self.d1)

        p3_blocks_ten_to_ten = np.block(
            [p3_blocks_ten_to_ten, np.identity(self.d1)])

        self._p_ten_to_ten['2'] = [[csr(p1_22_blocks_ten_to_ten), csr(p1_21_blocks_ten_to_ten), None],
                                   [csr(p1_12_blocks_ten_to_ten), csr(
                                       p1_11_blocks_ten_to_ten), None],
        ...
        self._p_ten_to_pol['v'] = [[csr(p0_blocks_ten_to_pol), None, None],
                                   [None, csr(p0_blocks_ten_to_pol), None],
                                   [None, None, csr(p0_blocks_ten_to_pol)]]

        self._p_ten_to_ten['v'] = [[csr(p0_blocks_ten_to_ten), None, None],
        ...
        grad_pol_to_pol_1 = np.zeros(
            (self.n_polar[1][0], self.n_polar[0][0]), dtype=float)
        grad_pol_to_pol_2 = np.array([[-1., 1., 0.], [-1., 0., 1.]])
        grad_pol_to_pol_3 = np.identity(self.n_polar[0][0], dtype=float)

        self._grad_pol_to_pol = [[csr(grad_pol_to_pol_1)],
                                 [csr(grad_pol_to_pol_2)],
                                 [csr(grad_pol_to_pol_3)]]

        # "polar coeffs" to "first tp ring"
        grad_pol_to_ten_1 = np.zeros(
            ((self.n_rings[1][0] + 1)*self.n1, self.n_polar[0][0]))
        grad_pol_to_ten_2 = np.zeros(
            ((self.n_rings[1][1] + 1)*self.d1, self.n_polar[0][0]))
        grad_pol_to_ten_3 = np.zeros(
            ((self.n_rings[0][0] + 1)*self.n1, self.n_polar[0][0]))

        grad_pol_to_ten_1[-self.n1:, :] = -self.xi_1.T

        self._grad_pol_to_ten = [[csr(grad_pol_to_ten_1)],
                                 [csr(grad_pol_to_ten_2)],
                                 [csr(grad_pol_to_ten_3)]]
        ...
        curl_pol_to_pol_13 = np.array([[-1., 1., 0.], [-1., 0., 1.]])

        curl_pol_to_pol_21 = np.identity(self.n_polar[1][0], dtype=float)
        curl_pol_to_pol_23 = np.zeros(
            (self.n_polar[2][1], self.n_polar[0][0]), dtype=float)

        curl_pol_to_pol_31 = np.zeros(
            (self.n_polar[3][0], self.n_polar[1][0]), dtype=float)
        curl_pol_to_pol_32 = np.zeros(
            (self.n_polar[3][0], self.n_polar[1][1]), dtype=float)

        self._curl_pol_to_pol = [[None, csr(-curl_pol_to_pol_12), csr(curl_pol_to_pol_13)],
                                 [csr(curl_pol_to_pol_21), None,
                                  csr(-curl_pol_to_pol_23)],
                                 [csr(-curl_pol_to_pol_31), csr(curl_pol_to_pol_32), None]]

        # "polar coeffs" to "first tp ring"
        curl_pol_to_ten_12 = np.zeros(
            ((self.n_rings[2][0] + 1)*self.d1, self.n_polar[1][1]))
        curl_pol_to_ten_13 = np.zeros(
            ((self.n_rings[2][0] + 1)*self.d1, self.n_polar[0][0]))

        curl_pol_to_ten_21 = np.zeros(
            ((self.n_rings[2][1] + 1)*self.n1, self.n_polar[1][0]))
        curl_pol_to_ten_23 = np.zeros(
            ((self.n_rings[2][1] + 1)*self.n1, self.n_polar[0][0]))

        curl_pol_to_ten_31 = np.zeros(
            ((self.n_rings[3][0] + 1)*self.n1, self.n_polar[1][0]))
        curl_pol_to_ten_32 = np.zeros(
            ((self.n_rings[3][0] + 1)*self.d1, self.n_polar[1][1]))

        curl_pol_to_ten_23[-self.n1:, :] = -self.xi_1.T

        for l in range(2):
            for j in range(self.d1, 2*self.d1):
                curl_pol_to_ten_32[j, l] = -(self.xi_1[l + 1, (j - self.d1 + 1) %
                                             self.d1] - self.xi_1[l + 1, j - self.d1])

        self._curl_pol_to_ten = [[None, csr(-curl_pol_to_ten_12), csr(curl_pol_to_ten_13)],
                                 [csr(curl_pol_to_ten_21), None,
                                  csr(-curl_pol_to_ten_23)],
        ...
        self._curl_e3 = [[None, csr(curl_e3_12), csr(curl_e3_13)],
        ...
        # "polar coeffs" to "polar coeffs"
        div_pol_to_pol_1 = np.zeros(
            (self.n_polar[3][0], self.n_polar[2][0]), dtype=float)
        div_pol_to_pol_2 = np.zeros(
            (self.n_polar[3][0], self.n_polar[2][1]), dtype=float)
        div_pol_to_pol_3 = np.identity(self.n_polar[3][0], dtype=float)

        self._div_pol_to_pol = [[csr(div_pol_to_pol_1),
                                 csr(div_pol_to_pol_2),
                                 csr(div_pol_to_pol_3)]]

        # "polar coeffs" to "first tp ring"
        div_pol_to_ten_1 = np.zeros(
            ((self.n_rings[3][0] + 1)*self.d1, self.n_polar[2][0]))
        div_pol_to_ten_2 = np.zeros(
            ((self.n_rings[3][0] + 1)*self.d1, self.n_polar[2][1]))
        div_pol_to_ten_3 = np.zeros(
            ((self.n_rings[3][0] + 1)*self.d1, self.n_polar[3][0]))

        for l in range(2):
            for j in range(self.d1, 2*self.d1):
                div_pol_to_ten_1[j, l] = -(self.xi_1[l + 1, (j - self.d1 + 1) %
                                           self.d1] - self.xi_1[l + 1, j - self.d1])

        self._div_pol_to_ten = [[csr(div_pol_to_ten_1),
                                 csr(div_pol_to_ten_2),
                                 csr(div_pol_to_ten_3)]]
...
class PolarSplines_C0_2D:
    ...
    def __init__(self, n0, n1):
        ...
        self.E0 = spa.bmat(
            [[self.E0_11, None], [None, self.E0_22]], format='csr')
        ...
        self.P0 = spa.bmat(
            [[self.P0_11, None], [None, self.P0_22]], format='csr')
        # =======================================================================
        ...
        self.E1C = spa.bmat(
            [[self.E1C_1, None], [None, self.E1C_2]], format='csr')
        ...
        # combined first and second component
        self.P1C = spa.bmat(
            [[self.P1C_1, None], [None, self.P1C_2]], format='csr')
        # ========================================================================
        ...
        # combined first and second component
        self.E1D = spa.bmat(
            [[self.E1D_1, None], [None, self.E1D_2]], format='csr')
        ...
        # combined first and second component
        self.P1D = spa.bmat(
            [[self.P1D_1, None], [None, self.P1D_2]], format='csr')
        # ========================================================================
...
class PolarSplines_C1_2D:
    ...
    def __init__(self, cx, cy):
        ...
        # size of control triangle
        self.tau = np.array([(-2*(cx[1] - self.x0)).max(), ((cx[1] - self.x0) - np.sqrt(3)*(
            cy[1] - self.y0)).max(), ((cx[1] - self.x0) + np.sqrt(3)*(cy[1] - self.y0)).max()]).max()
        ...
        self.Xi_1[1, :] = 1/3 - 1/(3*self.tau)*(cx[1] - self.x0) + \
            np.sqrt(3)/(3*self.tau)*(cy[1] - self.y0)
        self.Xi_1[2, :] = 1/3 - 1/(3*self.tau)*(cx[1] - self.x0) - \
            np.sqrt(3)/(3*self.tau)*(cy[1] - self.y0)
        ...
        # combined first and second component
        self.E1C = spa.bmat([[None, self.E1C_12, None, None], [
                            self.E1C_21, None, self.E1C_23, None], [None, None, None, self.E1C_34]], format='csr')
        ...
        self.P1C_34[0, (d1 + 0*d1//3):(d1 + 1*d1//3)
                    ] = np.ones(d1//3, dtype=float)
        self.P1C_34[1, (d1 + 0*d1//3):(d1 + 1*d1//3)
                    ] = np.ones(d1//3, dtype=float)
        self.P1C_34[1, (d1 + 1*d1//3):(d1 + 2*d1//3)
                    ] = np.ones(d1//3, dtype=float)
        ...
        for s in range(2):
            for j in range(n1):
                self.E1D_13[s, j] = - \
                    (self.Xi_1[s + 1, j] - self.Xi_0[s + 1, j])

        # combined first and second component
        self.E1D = spa.bmat([[self.E1D_11, None, self.E1D_13, None], [
                            None, self.E1D_22, None, None], [None, None, None, self.E1C_34]], format='csr')
        ...
        for i in range(d1):
            ...
            # block B
            self.P2_11[i, 1*n1//3:2*n1//3] = - \
                (self.Xi_1[2, (i + 1) % n1] - self.Xi_1[2, i])

        self.P2_11 += np.identity(d1)

        self.P2 = spa.bmat([[self.P2_11, self.P2_12, None], [
                           None, None, self.P2_23]], format='csr')
        # =========================================================================
        ...
        self.G2 = spa.bmat(
            [[self.G2_11, None], [None, self.G2_22]], format='csr')
        ...
        self.VC1 = spa.bmat(
            [[self.VC1_11, None], [None, self.VC1_22]], format='csr')
        ...
        for s in range(2):
            for j in range(d1):
                self.SC2_1[j, s] = -(self.Xi_1[s + 1, (j + 1) %
                                     n1] - self.Xi_1[s + 1, j])
        ...
        for s in range(2):
            for j in range(d1):
                self.D1_1[j, s] = -(self.Xi_1[s + 1, (j + 1) %
                                    n1] - self.Xi_1[s + 1, j])
...
class PolarSplines:
    ...
    def __init__(self, tensor_space, cx, cy):
        ...
        # size of control triangle
        self.tau = np.array([(-2*cx[1]).max(), (cx[1] - np.sqrt(3)
                            * cy[1]).max(), (cx[1] + np.sqrt(3)*cy[1]).max()]).max()
        ...
        self.Xi_1[1, :] = 1/3 - 1 / \
            (3*self.tau)*cx[1, :, 0] + np.sqrt(3)/(3*self.tau)*cy[1, :, 0]
        self.Xi_1[2, :] = 1/3 - 1 / \
            (3*self.tau)*cx[1, :, 0] - np.sqrt(3)/(3*self.tau)*cy[1, :, 0]

        # =========== extraction operators for discrete 0-forms ==================
        # extraction operator for basis functions
        self.E0_pol = spa.bmat([[np.hstack((self.Xi_0, self.Xi_1)), None], [
                               None, spa.identity((n0 - 2)*n1)]], format='csr')
        ...
        self.E1 = spa.bmat([[spa.kron(self.E1_pol, spa.identity(n2)), None], [
                           None, spa.kron(self.E1_3_pol, spa.identity(d2))]], format='csr')
        ...
        # 2nd component
        self.P1_2_pol[0, (n1 + 0*n1//3):(n1 + 1*n1//3)
                      ] = np.ones((1, n1//3), dtype=float)
        self.P1_2_pol[1, (n1 + 0*n1//3):(n1 + 1*n1//3)
                      ] = np.ones((1, n1//3), dtype=float)
        self.P1_2_pol[1, (n1 + 1*n1//3):(n1 + 2*n1//3)
                      ] = np.ones((1, n1//3), dtype=float)
        ...
        # combined first and second component
        self.P1_pol = spa.bmat(
            [[self.P1_1_pol, None], [None, self.P1_2_pol]], format='csr')

        # expansion in third dimension
        self.P1 = spa.bmat([[spa.kron(self.P1_pol, spa.identity(n2)), None], [
                           None, spa.kron(self.P1_3_pol, spa.identity(d2))]], format='csr')
        # =========================================================================
        ...
        # expansion in third dimension
        self.E2 = spa.bmat([[spa.kron(self.E2_pol, spa.identity(d2)), None], [
                           None, spa.kron(self.E2_3_pol, spa.identity(n2))]], format='csr')
        ...
        for i2 in range(d1):
            ...
            # block B
            self.P2_3_pol[i2, 1*n1//3:2*n1//3] = - \
                (self.Xi_1[2, (i2 + 1) % n1] - self.Xi_1[2, i2])
        ...
        # combined first and second component
        self.P2_pol = spa.bmat(
            [[self.P2_1_pol, None], [None, self.P2_2_pol]], format='csr')

        # expansion in third dimension
        self.P2 = spa.bmat([[spa.kron(self.P2_pol, spa.identity(d2)), None], [
                           None, spa.kron(self.P2_3_pol, spa.identity(n2))]], format='csr')
        # =========================================================================
        ...
        grad_2 = spa.lil_matrix(
            ((n0 - 2)*d1 + 2, self.Nbase0_pol), dtype=float)
        ...
        self.GRAD = spa.bmat([[spa.kron(self.grad_pol, spa.identity(n2))], [
                             spa.kron(spa.identity(self.Nbase0_pol), grad_1d_3)]], format='csr')
        # =======================================================================

        # ========= discrete polar curl matrix ===================================
        # 2D vector curl
        vector_curl_1 = spa.lil_matrix(
            ((n0 - 2)*d1 + 2, self.Nbase0_pol), dtype=float)
        vector_curl_2 = spa.lil_matrix(
            ((d0 - 1)*n1, self.Nbase0_pol), dtype=float)
        ...
        # combined 1st and 2nd component
        self.vector_curl_pol = spa.bmat(
            [[vector_curl_1], [vector_curl_2]], format='csr')

        # 2D scalar curl
        self.scalar_curl_pol = spa.lil_matrix(
            (self.Nbase3_pol, self.Nbase2_pol), dtype=float)

        # radial dofs (D N)
        self.scalar_curl_pol[:, :(d0 - 1)*n1] = - \
            spa.kron(spa.identity(d0 - 1), grad_1d_2)

        # angular dofs (N D)
        for s in range(2):
            for j in range(n1):
                self.scalar_curl_pol[j, (d0 - 1)*n1 + s] = - \
                    (self.Xi_1[s + 1, (j + 1) % n1] - self.Xi_1[s + 1, j])
        ...
        self.div_pol[:, 2:((d0 - 1)*n1 + 2)
                     ] = spa.kron(grad_1d_1[1:, 2:], spa.identity(d1))
        ...
        self.DIV = spa.bmat([[spa.kron(self.div_pol, spa.identity(d2)), spa.kron(
            spa.identity(self.Nbase3_pol), grad_1d_3)]], format='csr')
