    ...
    def __init__(self, domain, derham):
        ...
        for l in range(2):
            for j in range(self.n1):
                e1_21_blocks_ten_to_pol[l, j] = self.xi_1[
                    l +
                    1, j,
                ] - self.xi_0[l + 1, j]
        ...
        # !! NOTE: for odd spline degrees and periodic splines the first Greville point sometimes does NOT start at zero!!
        if domain.p[1] % 2 != 0 and not (abs(derham.Vh_fem['0'].spaces[1].interpolation_grid[0]) < 1e-14):
            p1_11_blocks_ten_to_ten[
                :, 3*self.n1 //
                3 - 1,
            ] = -np.roll(self.xi_1[0], -1)
            p1_11_blocks_ten_to_ten[
                :, 1*self.n1 //
                3 - 1,
            ] = -np.roll(self.xi_1[1], -1)
            p1_11_blocks_ten_to_ten[
                :, 2*self.n1 //
                3 - 1,
            ] = -np.roll(self.xi_1[2], -1)
        else:
        ...
        self._grad_pol_to_pol = [
            [csr(grad_pol_to_pol_1)],
        ...
        self._grad_pol_to_ten = [
            [csr(grad_pol_to_ten_1)],
        ...
        self._div_pol_to_pol = [[
            csr(div_pol_to_pol_1),
        ...
        self._div_pol_to_ten = [[
            csr(div_pol_to_ten_1),
...
class PolarSplines_C1_2D:
    ...
    def __init__(self, cx, cy):
        ...
        # size of control triangle
        self.tau = np.array([
            (-2*(cx[1] - self.x0)).max(), ((cx[1] - self.x0) - np.sqrt(3)*(
                cy[1] - self.y0
            )).max(), ((cx[1] - self.x0) + np.sqrt(3)*(cy[1] - self.y0)).max(),
        ]).max()
        ...
        self.E1C = spa.bmat(
            [
                [None, self.E1C_12, None, None], [
                    self.E1C_21, None, self.E1C_23, None,
                ], [None, None, None, self.E1C_34],
            ], format='csr',
        )
        ...
        self.P1C_34[
            0, (d1 + 0*d1//3):(d1 + 1*d1//3),
        ] = np.ones(d1//3, dtype=float)
        self.P1C_34[
            1, (d1 + 0*d1//3):(d1 + 1*d1//3),
        ] = np.ones(d1//3, dtype=float)
        self.P1C_34[
            1, (d1 + 1*d1//3):(d1 + 2*d1//3),
        ] = np.ones(d1//3, dtype=float)
        ...
        self.E1D = spa.bmat(
            [
                [self.E1D_11, None, self.E1D_13, None], [
                    None, self.E1D_22, None, None,
                ], [None, None, None, self.E1C_34],
            ], format='csr',
        )
        ...
        self.P2 = spa.bmat(
            [
                [self.P2_11, self.P2_12, None], [
                    None, None, self.P2_23,
                ],
            ], format='csr',
        )
        # =========================================================================
...
class PolarSplines:
    ...
    def __init__(self, tensor_space, cx, cy):
        ...
        self.E0_pol = spa.bmat(
            [
                [np.hstack((self.Xi_0, self.Xi_1)), None], [
                    None, spa.identity((n0 - 2)*n1),
                ],
            ], format='csr',
        )
        ...
        self.E1 = spa.bmat(
            [
                [spa.kron(self.E1_pol, spa.identity(n2)), None], [
                    None, spa.kron(self.E1_3_pol, spa.identity(d2)),
                ],
            ], format='csr',
        )
        ...
        # 2nd component
        self.P1_2_pol[
            0, (n1 + 0*n1//3):(n1 + 1*n1//3),
        ] = np.ones((1, n1//3), dtype=float)
        self.P1_2_pol[
            1, (n1 + 0*n1//3):(n1 + 1*n1//3),
        ] = np.ones((1, n1//3), dtype=float)
        self.P1_2_pol[
            1, (n1 + 1*n1//3):(n1 + 2*n1//3),
        ] = np.ones((1, n1//3), dtype=float)
        ...
        self.P1 = spa.bmat(
            [
                [spa.kron(self.P1_pol, spa.identity(n2)), None], [
                    None, spa.kron(self.P1_3_pol, spa.identity(d2)),
                ],
            ], format='csr',
        )
        # =========================================================================
        ...
        self.E2 = spa.bmat(
            [
                [spa.kron(self.E2_pol, spa.identity(d2)), None], [
                    None, spa.kron(self.E2_3_pol, spa.identity(n2)),
                ],
            ], format='csr',
        )
        ...
        self.P2 = spa.bmat(
            [
                [spa.kron(self.P2_pol, spa.identity(d2)), None], [
                    None, spa.kron(self.P2_3_pol, spa.identity(n2)),
                ],
            ], format='csr',
        )
        # =========================================================================
        ...
        self.GRAD = spa.bmat(
            [
                [spa.kron(self.grad_pol, spa.identity(n2))], [
                    spa.kron(spa.identity(self.Nbase0_pol), grad_1d_3),
                ],
            ], format='csr',
        )
        # =======================================================================
        ...
        self.DIV = spa.bmat(
            [[
                spa.kron(self.div_pol, spa.identity(d2)), spa.kron(
                    spa.identity(self.Nbase3_pol), grad_1d_3,
                ),
            ]], format='csr',
        )
