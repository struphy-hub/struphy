...
def test_accum_poisson(Nel, p, spl_kind, mapping, Np=1000, Nclones=1):
    ...
    params = {"grid": {"Nclones": Nclones}, "kinetic": {"test_particles": {"markers": {"Np": Np}}}}

    params, inter_comm, sub_comm = setup_domain_cloning(mpi_comm, copy.deepcopy(params), params["grid"]["Nclones"])

    # DeRham object
    derham = Derham(Nel, p, spl_kind, comm=sub_comm, inter_comm=inter_comm)
    ...
    params_markers = {
        "Np": params["kinetic"]["test_particles"]["markers"]["Np"],
        "eps": 0.25,
        "loading": {
            "type": "pseudo_random",
            "seed": 1607,
    ...
    acc = AccumulatorVector(particles, "H1", accum_kernels.charge_density_0form, derham, domain.args_domain)
...
if __name__ == "__main__":
    for Nclones in [1, 2]:
        test_accum_poisson(
            ...
            [2, 2, 3],
            [True] * 3,
            ["Cuboid", {"l1": 0.0, "r1": 1.0, "l2": 0.0, "r2": 2.0, "l3": 0.0, "r3": 1.0}],
            Np=1000,
            Nclones=Nclones,
        )
