...
def test_evaluation_SPH_Np_convergence_1d(boxes_per_dim, bc_x, eval_pts, tesselation, show_plot=False):
    ...
    if tesselation:
        ...
        xp. = [(2**k) * 10**3 for k in range(-2, 9)]
        ppbs = [None] * len(xp.)
    ...
    # loop
    err_vec = []
    for xp. ppb in zip(xp., ppbs):
        if tesselation:
            loading_params = LoadingParameters(ppb=ppb, loading="tesselation")
        else:
            loading_params = LoadingParameters(xp.xp. seed=1607)
        ...
        if show_plot and rank == 0:
            ...
            plt.plot(ee1.squeeze(), all_eval.squeeze(), "--.", label="eval_sph")
            plt.title(f"{xp.= }, {ppb = }")
            # plt.savefig(f"fun_{xp._{ppb}.png")
    ...
    if tesselation:
        ...
        fit = xp.polyfit(xp.log(xp.), xp.log(err_vec), 1)
        xvec = xp.
...
def test_evaluation_SPH_h_convergence_1d(boxes_per_dim, bc_x, eval_pts, tesselation, show_plot=False):
    ...
    if tesselation:
        ...
        xp.= 160000
        ppb = None
        loading_params = LoadingParameters(xp.xp. ppb=ppb, seed=1607)
...
def test_evaluation_mc_Np_and_h_convergence_1d(boxes_per_dim, bc_x, eval_pts, tesselation, show_plot=False):
    ...
    for h in h_arr:
        ...
        for xp. ppb in zip(xp., ppbs):
            ...
            if rank == 0:
                ...
                #     plt.plot(ee1.squeeze(), all_eval.squeeze(), "--.", label="eval_sph")
                #     plt.title(f"{h = }, {xp.= }")
                #     # plt.savefig(f"fun_h{h}_N{xp._ppb{ppb}.png")
    ...
    if show_plot and rank == 0:
        ...
        if not tesselation:
            h_mesh, n_mesh = xp.meshgrid(xp.log10(h_arr), xp.log10(xp.), indexing="ij")
        ...
        if tesselation:
            ...
            log_particles = xp.log10(xp.)
        plt.plot(min_h_values, log_particles, "r-", label="Min error h for each xp., linewidth=2)
        plt.legend()
    ...
    for xp. ppb in zip(xp., ppbs):
        ...
        if rank == 0:
            ...
            if show_plot:
                ...
                ax.set_xlabel("ee1")
                ax.set_ylabel("ee2")
                ax.set_title(f"{xp._{ppb = }")
                # fig.savefig(f"2d_sph_{xp._{ppb}.png")
