...
@pytest.mark.parametrize("eval_pts", [11, 16])
@pytest.mark.parametrize("tesselation", [False, True])
def test_sph_evaluation_2d(boxes_per_dim, kernel, derivative, bc_x, bc_y, eval_pts, tesselation, show_plot=False,):
    comm = MPI.COMM_WORLD
    ...
    if tesselation:
        loading = "tesselation"
        loading_params = {"n_quad": 1}
        if kernel == "trigonometric_2d" and derivative == 1:
            ppb = 100
        else:
            ppb = 16
    else:
    ...
    if derivative == 0:
        ...
        mode_params = {"given_in_basis": "0", "ls": [1], "ms": [1], "amps": [-2 * np.pi]}
