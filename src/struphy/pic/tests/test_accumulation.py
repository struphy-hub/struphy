...
def pc_lin_mhd_6d_step_ph_full(Nel, p, spl_kind, mapping, Np, verbose=False):
    ...
    params_markers = {
        ...
        "loading": {
            "type": "pseudo_random",
            "seed": 1607,
            "moments": [0.0, 0.0, 0.0, 1.0, 2.0, 3.0],
            "spatial": "uniform",
        },
    }

    particles = Particles6D("test_particles", **params_markers, derham=derham, bckgr_params=None)
    particles.draw_markers()

    # set random weights on each process
    particles.markers[~particles.holes, 6] = np.random.rand(particles.n_mks_loc)
    ...
    particles_leg = np.zeros((sum(marker_shapes), particles.markers.shape[1]), dtype=float)
    ...
    if rank == 0:
        ...
        for i in range(1, mpi_size):
            arr_recv = np.zeros((marker_shapes[i], particles.markers.shape[1]), dtype=float)
    ...
    for a in range(3):
        ...
        for b in range(3):
            mat[a][b] = np.zeros(
                (Ni[0], Ni[1], Ni[2], 2 * SPACES.p[0] + 1, 2 * SPACES.p[1] + 1, 2 * SPACES.p[2] + 1, 3, 3), dtype=float
            )
    ...
    compare_arrays(
        ACC.operators[0].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_11 passed test")
    compare_arrays(
        ACC.operators[0].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_11 passed test")
    compare_arrays(
        ACC.operators[0].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_11 passed test")
    compare_arrays(
        ACC.operators[0].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_11 passed test")
    compare_arrays(
        ACC.operators[0].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_11 passed test")
    compare_arrays(
        ACC.operators[0].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 0, 0], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat33_11 passed test")

    compare_arrays(
        ACC.operators[1].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_12 passed test")
    compare_arrays(
        ACC.operators[1].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_12 passed test")
    compare_arrays(
        ACC.operators[1].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_12 passed test")
    compare_arrays(
        ACC.operators[1].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_12 passed test")
    compare_arrays(
        ACC.operators[1].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_12 passed test")
    compare_arrays(
        ACC.operators[1].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 0, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat33_12 passed test")

    compare_arrays(
        ACC.operators[2].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_13 passed test")
    compare_arrays(
        ACC.operators[2].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_13 passed test")
    compare_arrays(
        ACC.operators[2].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_13 passed test")
    compare_arrays(
        ACC.operators[2].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_13 passed test")
    compare_arrays(
        ACC.operators[2].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_13 passed test")
    compare_arrays(
        ACC.operators[2].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 0, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat33_13 passed test")

    compare_arrays(
        ACC.operators[3].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_22 passed test")
    compare_arrays(
        ACC.operators[3].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_22 passed test")
    compare_arrays(
        ACC.operators[3].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_22 passed test")
    compare_arrays(
        ACC.operators[3].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_22 passed test")
    compare_arrays(
        ACC.operators[3].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_22 passed test")
    compare_arrays(
        ACC.operators[3].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 1, 1], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat33_22 passed test")

    compare_arrays(
        ACC.operators[4].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_23 passed test")
    compare_arrays(
        ACC.operators[4].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_23 passed test")
    compare_arrays(
        ACC.operators[4].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_23 passed test")
    compare_arrays(
        ACC.operators[4].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_23 passed test")
    compare_arrays(
        ACC.operators[4].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_23 passed test")
    compare_arrays(
        ACC.operators[4].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 1, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat33_23 passed test")

    compare_arrays(
        ACC.operators[5].matrix.blocks[0][0], mat[0][0][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat11_33 passed test")
    compare_arrays(
        ACC.operators[5].matrix.blocks[0][1], mat[0][1][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat12_33 passed test")
    compare_arrays(
        ACC.operators[5].matrix.blocks[0][2], mat[0][2][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat13_33 passed test")
    compare_arrays(
        ACC.operators[5].matrix.blocks[1][1], mat[1][1][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat22_33 passed test")
    compare_arrays(
        ACC.operators[5].matrix.blocks[1][2], mat[1][2][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
        print("mat23_33 passed test")
    compare_arrays(
        ACC.operators[5].matrix.blocks[2][2], mat[2][2][:, :, :, :, :, :, 2, 2], rank, atol=atol, verbose=verbose
    )
    if verbose:
