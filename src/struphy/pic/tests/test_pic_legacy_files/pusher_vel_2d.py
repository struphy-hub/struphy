...
# ==========================================================================================================
@types('double[:,:]', 'double', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'int[:]', 'int[:]', 'int', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'int', 'int', 'double[:]', 'double[:]', 'double[:]', 'double[:]', 'int[:]', 'int[:]', 'int[:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:,:,:]', 'double[:]', 'double[:]', 'int')
def pusher_step3(particles, dt, t1, t2, p, nel, nbase_n, nbase_d, np, b_eq_1, b_eq_2, b_eq_3, b_p_1, b_p_2, b_p_3, b_norm, u1, u2, u3, basis_u, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz, mu, power, n_tor):
    ...
    for ip in range(np):
        ...
        if basis_u == 0:
            ...
            for i in range(nbase_n[2]):
                ...
                u[0] += eva2.evaluation_kernel_2d(
                    pn1, pn2, bn1, bn2, span1, span2,
                    nbase_n[0], nbase_n[1], u1[:, :, i],
                )*cs[i]
                u[1] += eva2.evaluation_kernel_2d(
                    pn1, pn2, bn1, bn2, span1, span2,
                    nbase_n[0], nbase_n[1], u2[:, :, i],
                )*cs[i]
                u[2] += eva2.evaluation_kernel_2d(
                    pn1, pn2, bn1, bn2, span1, span2,
                    nbase_n[0], nbase_n[1], u3[:, :, i],
                )*cs[i]
        ...
        # velocity field (1-form, push forward with df^(-T))
            ...
            for i in range(nbase_n[2]):
                ...
                u[0] += eva2.evaluation_kernel_2d(
                    pd1, pn2, bd1, bn2, span1 - 1, span2 - 0,
                    nbase_d[0], nbase_n[1], u1[:, :, i],
                )*cs[i]
                u[1] += eva2.evaluation_kernel_2d(
                    pn1, pd2, bn1, bd2, span1 - 0, span2 - 1,
                    nbase_n[0], nbase_d[1], u2[:, :, i],
                )*cs[i]
                u[2] += eva2.evaluation_kernel_2d(
                    pn1, pn2, bn1, bn2, span1 - 0, span2 - 0,
                    nbase_n[0], nbase_n[1], u3[:, :, i],
                )*cs[i]
        ...
        # velocity field (2-form, push forward with df/|det df|)
            ...
            for i in range(nbase_n[2]):
                ...
                u[0] += eva2.evaluation_kernel_2d(
                    pn1, pd2, bn1, bd2, span1 - 0, span2 - 1,
                    nbase_n[0], nbase_d[1], u1[:, :, i],
                )*cs[i]
                u[1] += eva2.evaluation_kernel_2d(
                    pd1, pn2, bd1, bn2, span1 - 1, span2 - 0,
                    nbase_d[0], nbase_n[1], u2[:, :, i],
                )*cs[i]
                u[2] += eva2.evaluation_kernel_2d(
                    pd1, pd2, bd1, bd2, span1 - 1, span2 - 1,
                    nbase_d[0], nbase_d[1], u3[:, :, i],
                )*cs[i]
        ...
        b[0] = eva2.evaluation_kernel_2d(
            pn1, pd2, bn1, bd2, span1 - 0, span2 - 1,
            nbase_n[0], nbase_d[1], b_eq_1[:, :, 0],
        )
        b[1] = eva2.evaluation_kernel_2d(
            pd1, pn2, bd1, bn2, span1 - 1, span2 - 0,
            nbase_d[0], nbase_n[1], b_eq_2[:, :, 0],
        )
        b[2] = eva2.evaluation_kernel_2d(
            pd1, pd2, bd1, bd2, span1 - 1, span2 - 1,
            nbase_d[0], nbase_d[1], b_eq_3[:, :, 0],
        )
        ...
        for i in range(nbase_n[2]):
            ...
            b[0] += eva2.evaluation_kernel_2d(
                pn1, pd2, bn1, bd2, span1 - 0, span2 - 1,
                nbase_n[0], nbase_d[1], b_p_1[:, :, i],
            )*cs[i]
            b[1] += eva2.evaluation_kernel_2d(
                pd1, pn2, bd1, bn2, span1 - 1, span2 - 0,
                nbase_d[0], nbase_n[1], b_p_2[:, :, i],
            )*cs[i]
            b[2] += eva2.evaluation_kernel_2d(
                pd1, pd2, bd1, bd2, span1 - 1, span2 - 1,
                nbase_d[0], nbase_d[1], b_p_3[:, :, i],
            )*cs[i]
        ...
        b_grad[0] = eva2.evaluation_kernel_2d(
            pn1, pn2, der1, bn2, span1, span2,
            nbase_n[0], nbase_n[1], b_norm[:, :, 0],
        )
        b_grad[1] = eva2.evaluation_kernel_2d(
            pn1, pn2, bn1, der2, span1, span2,
            nbase_n[0], nbase_n[1], b_norm[:, :, 0],
        )
        b_grad[2] = 0.
...
def pusher_step5(particles, dt, t1, t2, p, nel, nbase_n, nbase_d, np, b_eq_1, b_eq_2, b_eq_3, b_p_1, b_p_2, b_p_3, kind_map, params_map, tf1, tf2, tf3, pf, nelf, nbasef, cx, cy, cz, n_tor):
