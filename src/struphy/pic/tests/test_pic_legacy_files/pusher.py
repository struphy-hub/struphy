...
class Pusher:
    ...
    def push_step4(self, particles, dt):
        ...
        if self.domain.pole:
            ...
            push_pos.pusher_step4_pcart(
                particles, dt, particles.shape[1], self.domain.kind_map, self.domain.params_numpy, self.domain.T[0], self.domain.T[1],
                self.domain.T[2], self.domain.p, self.domain.Nel, self.domain.NbaseN, self.domain.cx, self.domain.cy, self.domain.cz, self.map_pseudo, self.R0_pseudo,
            )
        ...
        # standard pusher in logical coordinates (for domains without a pole)
            ...
            push_pos.pusher_step4(
                particles, dt, particles.shape[1], self.domain.kind_map, self.domain.params_numpy, self.domain.T[0], self.domain.T[1],
                self.domain.T[2], self.domain.p, self.domain.Nel, self.domain.NbaseN, self.domain.cx, self.domain.cy, self.domain.cz, self.bc_pos,
            )
    ...
    def push_eta_pc_full(self, particles, dt, up):
        ...
        push_pos.pusher_rk4_pc_full(
            particles, dt, self.fem_space.T[0], self.fem_space.T[1], self.fem_space.T[2],
