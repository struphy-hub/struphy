    ...
    if kind_map == 0:
        ...
        if mat_or_vec == 1 or mat_or_vec == 2:
            ...
            vec_out[0] = evaluation_kernel_3d(
                pn[0], pn[1], pn[2], b1[pn[0]], b2[pn[1]],
                b3[pn[2]], span_n1, span_n2, span_n3, nbase_n[0], nbase_n[1], nbase_n[2], cx,
            )
            vec_out[1] = evaluation_kernel_3d(
                pn[0], pn[1], pn[2], b1[pn[0]], b2[pn[1]],
                b3[pn[2]], span_n1, span_n2, span_n3, nbase_n[0], nbase_n[1], nbase_n[2], cy,
            )
            vec_out[2] = evaluation_kernel_3d(
                pn[0], pn[1], pn[2], b1[pn[0]], b2[pn[1]],
                b3[pn[2]], span_n1, span_n2, span_n3, nbase_n[0], nbase_n[1], nbase_n[2], cz,
            )
    ...
    # discrete cylinder
        ...
        if mat_or_vec == 1 or mat_or_vec == 2:
            ...
            vec_out[0] = evaluation_kernel_2d(
                pn[0], pn[1], b1[pn[0]], b2[pn[1]], span_n1,
    ...
    # analytical mapping
        ...
        if mat_or_vec == 0 or mat_or_vec == 2:
            ...
            mat_out[0, 0] = mapping.df(
                eta1, eta2, eta3, 11, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[0, 1] = mapping.df(
                eta1, eta2, eta3, 12, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[0, 2] = mapping.df(
                eta1, eta2, eta3, 13, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )

            mat_out[1, 0] = mapping.df(
                eta1, eta2, eta3, 21, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[1, 1] = mapping.df(
                eta1, eta2, eta3, 22, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[1, 2] = mapping.df(
                eta1, eta2, eta3, 23, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )

            mat_out[2, 0] = mapping.df(
                eta1, eta2, eta3, 31, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[2, 1] = mapping.df(
                eta1, eta2, eta3, 32, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
            mat_out[2, 2] = mapping.df(
                eta1, eta2, eta3, 33, kind_map, params_map,
                tn1, tn2, tn3, pn, nbase_n, cx, cy, cz,
            )
...
def df_inv_all(mat_in : 'float[:,:]', mat_out : 'float[:,:]'):
    ...
    over_det_df = 1.0 / (
        mat_in[0, 0]*(mat_in[1, 1]*mat_in[2, 2] - mat_in[2, 1]*mat_in[1, 2]) + mat_in[1, 0]*(
            mat_in[2, 1]*mat_in[0, 2] - mat_in[0, 1]*mat_in[2, 2]
