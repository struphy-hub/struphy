...
def _dg_launch(
    name, markers, n_cols, first_init_idx, first_shift_idx, residual_idx,
    first_free_idx, mu_idx, epsilon, pn, tn1_dev, tn2_dev, tn3_dev, starts,
    grad_b_full, e_field, evaluate_e_field, dt,
):
    import cupy as cp
    import numpy as np
    ...
    _get_dg_kernel(name)(
        ...
        (threads,),
        (
            markers, np.int32(n_cols), np.int32(n_markers),
...
def _j1_launch(
    name, markers, kind_map, params_dev, epsilon,
    pn, tn1_dev, tn2_dev, tn3_dev, starts,
...
def _j2_stage_launch(
    name, markers, first_init_idx, first_free_idx, kind_map, params_dev, epsilon,
    pn, tn1_dev, tn2_dev, tn3_dev, starts,
    b2, norm_b1, curl_norm_b, u, dt_a, dt_b, last,
):
    import cupy as cp
    import numpy as np
    ...
    _get_j2_stage_kernel(name)(
        ...
        (
            markers, np.int32(markers.shape[1]), np.int32(n_markers),
            np.int32(first_init_idx), np.int32(first_free_idx),
            np.float64(dt_a), np.float64(dt_b), np.float64(last),
            np.int32(kind_map), params_dev,
            np.float64(epsilon),
            np.int32(pn[0]), np.int32(pn[1]), np.int32(pn[2]),
