...
@stack_array('dfm', 'df_inv', 'v', 'df_inv_v', 'e_vec', 'update')
def push_velocities_with_efield_delta_f_va(markers: 'float[:,:]',
                                           ...
                                           e1_1: 'float[:,:,:]', e1_2: 'float[:,:,:]', e1_3: 'float[:,:,:]',
                                           n0: 'float', vth: 'float',
                                           f0_values: 'float[:]', kappa: 'float',
                                           tol: 'float', substep: 'int'):
    r"""
    ...
    for ip in range(n_markers):
        ...
        if substep == 0:
            pusher_utilities_kernels.iterative_helper_e_v_dfva(
                df_inv_t_e[:], markers[ip, 3:6], update[:],
                markers[ip, 6], markers[ip, 7],
                n0, vth, 
                dt, kappa, tol,
            )

            markers[ip, 9:12] = update[:]

            markers[ip, 12:15] = markers[ip, 3:6] + dt * update[:]

        elif substep == 1:
            pusher_utilities_kernels.iterative_helper_e_v_dfva(
                df_inv_t_e, markers[ip, 3:6], update,
                markers[ip, 6], markers[ip, 7],
                n0, vth, dt, kappa, tol*2
            )

            markers[ip, 3:6] += (markers[ip, 9:12] + update[:]) * dt / 2.
