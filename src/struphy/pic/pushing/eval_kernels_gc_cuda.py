...
def driftkinetic_hamiltonian_gpu(
    markers, alpha, column_nr, first_init_idx, first_shift_idx, mu_idx,
    args_derham, epsilon, B_dot_b_coeffs, phi_coeffs, evaluate_e_field,
):
    """GPU replacement for
    ...
    _get_dk_kernel()(
        ...
        (
            ...
            np.float64(a[0]), np.float64(a[1]), np.float64(a[2]), np.float64(a[3]),
            np.float64(epsilon),
            ...
            bdb, np.int32(bdb.shape[1]), np.int32(bdb.shape[2]),
            phi, np.int32(phi.shape[1]), np.int32(phi.shape[2]),
