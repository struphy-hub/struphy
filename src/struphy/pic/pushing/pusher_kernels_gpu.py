...
#         # metric coeffs
#         det_df_val = linalg_kernels.det(dfm)
...
#         # metric coeffs
#         det_df_val = linalg_kernels.det(dfm)
#         linalg_kernels.matrix_inv_with_det(dfm, det_df_val, dfinv)
#         linalg_kernels.transpose(dfinv, dfinv_t)
...
#     N: int = A.shape[0]
#     s: float = 0.0
#     # -- removed omp: #$ omp target teams distribute parallel for collapse(2)
#     for i in range(N):
#         for j in range(N):
#             s = 0.0
#             for k in range(N):
...
#     dfm = empty((3, 3), dtype=float)
#     df_inv_1 = empty((3, 3), dtype=float)
#     v = empty(3, dtype=float)
...
#         linalg_kernels.matrix_inv(dfm, df_inv_1)

#         # compute DF^{-1} v
#         linalg_kernels.matrix_vector(df_inv_1, v, df_inv_v)
...
#     plus = a[0, 0]*det_inline(tmp2) + a[2, 0]*det_inline(tmp1)

#     tmp1[2] = a[2, 1:]

#     tmp2[:] = a[:3, 1:]

#     minus = a[1, 0]*det_inline(tmp1) + a[3, 0]*det_inline(tmp2)
...
# def _tmp_floor_division_evaluation_kernels_2123(x: int):
#     y = zeros(10)
#     z = copy(y)
#     return x // 2
