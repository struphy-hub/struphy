...
class Pusher:
    ...
    * MPI sorting is done automatically before kernel calls according to the specified
    ...
    def __call__(self, dt: float):
        ...
        markers[:, buffer_idx + 3 + vdim : buffer_idx + 3 + vdim + 3] = 0.0
        ...
        for stage in range(self.n_stages):
            ...
            if self.verbose and self.maxiter > 1:
                max_res = 1.0
                print(f"rank {rank}: {k = }, tol: {self._tol}, {n_not_converged[0] = }, {max_res = }")
                self.particles.derham.comm.Barrier()
            ...
            while True:
                ...
                for ker_args in self.eval_kernels:
                    ...
                    self.particles.mpi_sort_markers(apply_bc=False, alpha=alpha[:3])
                    # evaluate
                    ker(
                        alpha,
                        column_nr,
                ...
                # sort according to alpha-weighted average
                self.particles.mpi_sort_markers(apply_bc=False, alpha=self._alpha_in_kernel)

                # push markers
                self.kernel(
                    dt, stage, self.particles.args_markers, self._args_derham, self._args_domain, *self._args_kernel
                )
                ...
                if self.maxiter > 1:
                    ...
                    n_not_converged[0] = np.count_nonzero(self._not_converged_loc)

                    if self.verbose:
                        print(f"rank {rank}: {k = }, tol: {self._tol}, {n_not_converged[0] = }, {max_res = }")
                        self.particles.derham.comm.Barrier()

                    self.particles.derham.comm.Allreduce(self._mpi_in_place, n_not_converged, op=self._mpi_sum)

                    # take converged markers out of the loop
                    markers[self._converged_loc, buffer_idx] = -1.0
            ...
            # print stage info
            if self.verbose:
                print(f"rank {rank}: stage {stage + 1} of {self.n_stages} done.")
                self.particles.derham.comm.Barrier()
