...
def _sph_marker_column_launch(
    name, markers, valid_mks, column_nr, weight_idx,
    boxes, neighbours, holes, periodic, kernel_type, h,
    *, first_free_idx=None, mu=None,
):
    import cupy as cp
    import numpy as np
    ...
    args = [
        ...
        np.int32(column_nr), np.int32(weight_idx),
    ]
    if first_free_idx is not None:
        args.append(np.int32(first_free_idx))
    args += [
        dev_valid,
        dev_boxes, np.int32(dev_boxes.shape[1]),
        dev_neigh, dev_holes,
        np.int32(bool(periodic[0])), np.int32(bool(periodic[1])), np.int32(bool(periodic[2])),
        np.int32(kernel_type),
        np.float64(h[0]), np.float64(h[1]), np.float64(h[2]),
    ]
    if mu is not None:
...
def sph_pressure_coeffs_gpu(markers, valid_mks, column_nr, weight_idx, boxes, neighbours, holes, periodic, kernel_type, h):
    """GPU replacement for one call of
    :func:`~struphy.pic.pushing.eval_kernels_sph.sph_pressure_coeffs`."""
    _sph_marker_column_launch(
        "sph_pressure_coeffs_cuda", markers, valid_mks, column_nr, weight_idx,
        boxes, neighbours, holes, periodic, kernel_type, h,
    )
...
def sph_mean_velocity_coeffs_gpu(
    ...
    _sph_marker_column_launch(
        "sph_mean_velocity_coeffs_cuda", markers, valid_mks, column_nr, weight_idx,
        boxes, neighbours, holes, periodic, kernel_type, h,
    )


def sph_viscosity_tensor_gpu(
    markers, valid_mks, column_nr, weight_idx, first_free_idx,
    boxes, neighbours, holes, periodic, kernel_type, h, mu,
):
    """GPU replacement for one call of
    :func:`~struphy.pic.pushing.eval_kernels_sph.sph_viscosity_tensor`."""
    _sph_marker_column_launch(
        "sph_viscosity_tensor_cuda", markers, valid_mks, column_nr, weight_idx,
        boxes, neighbours, holes, periodic, kernel_type, h,
        first_free_idx=first_free_idx, mu=mu,
    )
