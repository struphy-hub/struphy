...
class Particles(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        **marker_params,
        ...
        self._marker_params = set_defaults(marker_params, marker_params_default)
        ...
        self._control_variate = self.marker_params["type"] == "control_variate"
        ...
        self._args_markers = MarkerArguments(self.markers, self.vdim, self.bufferindex)
    ...
    def create_marker_array(self):
        ...
        n_cells_loc = np.prod(self._domain_decomp[self._mpi_rank, 2::3], dtype=int)
        ...
        n_rows = round(n_mks_load_loc * (1 + 1 / np.sqrt(n_mks_load_loc) + self.marker_params["eps"]))
    ...
    def draw_markers(self):
        ...
        if self.marker_params["loading"]["type"] == "external":
            ...
            if self.mpi_rank == 0:
                file = h5py.File(self.marker_params["loading"]["dir_external"], "r")
                print("Loading markers from file: ".ljust(25), file)

                self._markers[: n_mks_load_cum_sum[0], :] = file["markers"][: n_mks_load_cum_sum[0], :]
                ...
                recvbuf = np.zeros((n_mks_load_loc, self.markers.shape[1]), dtype=float)
        ...
        # load markers from restart .hdf5 file
            ...
            if self.marker_params["loading"]["dir_particles_abs"] is None:
                data_path = os.path.join(o_path, self.marker_params["loading"]["dir_particles"])
            else:
                data_path = self.marker_params["loading"]["dir_particles_abs"]

            data = DataContainer(data_path, comm=self.comm)

            self.markers[:, :] = data.file["restart/" + self.marker_params["loading"]["key"]][-1, :, :]
        ...
        # load fresh markers
            ...
            # 3. symmetric sobol numbers in all 6 dimensions with skip of first 1000 numbers
                ...
                assert self.vdim == 3, NotImplementedError('"sobol_antithetic" requires vdim=3 at the moment.')
                ...
                sampling_kernels.set_particles_symmetric_3d_3v(temp_markers, self.markers)
            ...
            u_mean = np.array(self.marker_params["loading"]["moments"][: self.vdim])
            v_th = np.array(self.marker_params["loading"]["moments"][self.vdim :])
            ...
            # Particles5D: (1d Maxwellian, polar Maxwellian as volume-form)
                ...
                self.markers[:n_mks_load_loc, 3] = (
                    sp.erfinv(2 * self.velocities[:, 0] - 1) * np.sqrt(2) * v_th[0] + u_mean[0]
                )

                self.markers[:n_mks_load_loc, 4] = (
                    np.sqrt(-1 * np.log(1 - self.velocities[:, 1])) * np.sqrt(2) * v_th[1] + u_mean[1]
                )
            elif self.vdim == 0:
                pass
            else:
            ...
            if self._spatial == "disc":
                self._markers[:n_mks_load_loc, 0] = np.sqrt(self.markers[:n_mks_load_loc, 0])
            else:
            ...
            assert np.all(~self._holes[:n_mks_load_loc]) and np.all(self._holes[n_mks_load_loc:])

    def mpi_sort_markers(self, apply_bc: bool = True, alpha: tuple | list | int | float = 1.0, do_test=False):
        """
        ...
        markers_to_be_sent, hole_inds_after_send, sorting_etas = sendrecv_determine_mtbs(
            self._markers, self._holes, self.domain_decomp, self.mpi_rank, self.vdim, self.bufferindex, alpha=alpha
        )
        ...
        sendrecv_markers(send_list, recv_info, hole_inds_after_send, self._markers, self.comm)
    ...
    def binning(self, components, bin_edges):
        ...
        f_slice = np.histogramdd(self.markers_wo_holes[:, slicing], bins=bin_edges, weights=_weights0)[0]

        df_slice = np.histogramdd(self.markers_wo_holes[:, slicing], bins=bin_edges, weights=_weights)[0]
    ...
    def show_distribution_function(self, components, bin_edges):
        ...
        labels = {0: r"$\eta_1$", 1: r"$\eta_2$", 2: r"$\eta_3$", 3: "$v_1$", 4: "$v_2$", 5: "$v_3$"}
    ...
    def apply_kinetic_bc(self, newton=False):
        ...
        for axis, bc in enumerate(self.marker_params["bc"]["type"]):
            ...
            self._is_outside[:] = np.logical_or(self._is_outside_right, self._is_outside_left)
            ...
            if bc == "remove":
                ...
                self._markers[outside_inds, :-1] = -1.0
                ...
                if newton:
                    self.markers[outside_right_inds, self.bufferindex + 3 + self.vdim + axis] += 1.0
                    self.markers[outside_left_inds, self.bufferindex + 3 + self.vdim + axis] += -1.0
                else:
                    self.markers[:, self.bufferindex + 3 + self.vdim + axis] = 0.0
                    self.markers[outside_right_inds, self.bufferindex + 3 + self.vdim + axis] = 1.0
                    self.markers[outside_left_inds, self.bufferindex + 3 + self.vdim + axis] = -1.0
    ...
    def boundary_transfer(self, is_outside):
        ...
        smaller_than_rmin = self.markers[:, 0] < 0.0
        ...
        r_loss = self._markers[transfer_inds, 0] * (1.0 - self._domain.params_map["a1"]) + self._domain.params_map["a1"]
        ...
        self._markers[transfer_inds, 1] = 1.0 - self.markers[transfer_inds, 1]

        # mark the particle as done for multiple step pushers
        self._markers[transfer_inds, 11] = -1.0
    ...
    def particle_refilling(self, is_outside):
        ...
        smaller_than_rmin = self.markers[:, 0] > 1.0
        ...
        self._markers[transfer_inds, 0] = 1.0 - 1e-8
...
def sendrecv_determine_mtbs(
    markers, holes, domain_decomp, mpi_rank, vdim, buffer_index, alpha: list | tuple | np.ndarray = (1.0, 1.0, 1.0)
):
    """
    ...
    is_on_proc_domain = np.logical_and(
        sorting_etas > domain_decomp[mpi_rank, 0::3], sorting_etas < domain_decomp[mpi_rank, 1::3]
    )
...
def sendrecv_get_destinations(markers_to_be_sent, sorting_etas, domain_decomp, mpi_size):
    ...
    for i in range(mpi_size):
        ...
        conds = np.logical_and(sorting_etas > domain_decomp[i, 0::3], sorting_etas < domain_decomp[i, 1::3])
