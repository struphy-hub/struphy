    ...
    * ``-1``: particle ID
    ...
    def __init__(
        ...
        if self.boxes_per_dim is None:
            ...
            assert all([nboxes >= xp.oc for nboxes, xp.oc in zip(self.boxes_per_dim, self.xp.ocs)]), (
                f"There must be at least one box {self.boxes_per_dim = } on each process {self.xp.ocs = } in each direction."
            )
            assert all([nboxes % xp.oc == 0 for nboxes, xp.oc in zip(self.boxes_per_dim, self.xp.ocs)]), (
                f"Number of boxes {self.boxes_per_dim = } must be divisible by number of processes {self.xp.ocs = } in each direction."
            )
        ...
        # total number of markers (xp. and particles per cell (ppc)
        xp.= self.loading_params.Np
        ppc = self.loading_params.ppc
        ppb = self.loading_params.ppb
        if xp.is not None:
            self._Np = int(xp.
            self._ppc = self.xp./ n_cells
            self._ppb = self.xp./ n_boxes
    ...
    @property
    def ppc(self):
        """Particles per cell (=xp.if no grid is present)."""
        return self._ppc
    ...
    def _get_domain_decomp(self, mpi_dims_mask: tuple | list = None):
        ...
        assert xp.prod(xp.ocs) == self.mpi_size

        # domain decomposition
        breaks = [xp.linspace(0.0, 1.0, xp.oc + 1) for xp.oc in xp.ocs]

        # fill domain array
        for n in range(self.mpi_size):
            # determine (ijk box index) corresponding to n (inverse flattening)
            i = n // (xp.ocs[1] * xp.ocs[2])
            nn = n % (xp.ocs[1] * xp.ocs[2])
            j = nn // xp.ocs[2]
            k = nn % xp.ocs[2]
    ...
    def _n_mks_load_and_Np_per_clone(self):
        ...
        if self.clone_config is not None:
            ...
            _n_mks_load_tot = self.clone_config.get_Np_clone(self.xp.
        ...
        # add deviation from xp.to rank 0
        ...
        # xp.on each clone
        xp.per_clone = self._gather_scalar_in_intercomm_array(_n_mks_load_tot)
        assert xp.sum(xp.per_clone) == self.Np

        return n_mks_load, xp.per_clone
    ...
    def _initialize_sorting_boxes(self):
        ...
        if self.boxes_per_dim is not None:
            ...
            nboxes = [nboxes // xp.oc for nboxes, xp.oc in zip(self.boxes_per_dim, self.xp.ocs)]
    ...
    def draw_markers(
        ...
        # xp.per_clone_loc = self.xp.per_clone[self.clone_id]
        ...
        xp.per_clone_cum_sum = xp.cumsum(self.xp.per_clone)
        _first_marker_id = (xp.per_clone_cum_sum - self.xp.per_clone)[self.clone_id] + (
        ...
        if self.loading == "external":
            ...
            if self.loading == "pseudo_random":
                ...
                while num_loaded_particles_glob < int(self.xp.:
                    # Generate a chunk of random particles
                    num_to_add_glob = min(chunk_size, int(self.xp. - num_loaded_particles_glob)
                ...
                assert self.xp.== int(num_loaded_particles_glob), f"{self.xp.= }, {int(num_loaded_particles_glob) = }"
    ...
    def binning(
        ...
        f_slice /= self.xp.* bin_vol
        df_slice /= self.xp.* bin_vol
