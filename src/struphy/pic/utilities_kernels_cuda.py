...
def eval_canonical_toroidal_moment_5d_gpu(
    ...
    _get_kernel("eval_canonical_toroidal_moment_5d_cuda")(
        ...
        (
            ...
            np.int32(first_diagnostics_idx), np.int32(mu_idx), np.int32(idx_can_momentum),
            np.float64(epsilon), np.float64(B0), np.float64(R0),
            ...
            absB, np.int32(absB.shape[1]), np.int32(absB.shape[2]),
        ),
    )


def eval_canonical_toroidal_moment_6d_gpu(
    markers, args_derham, first_diagnostics_idx, epsilon, B0, R0, absB
):
    """GPU replacement for
...
def eval_magnetic_energy_PBb_gpu(markers, args_derham, first_diagnostics_idx, mu_idx, abs_B0, PBb):
    ...
    _get_kernel("eval_magnetic_energy_PBb_cuda")(
        ...
        (
            markers, np.int32(markers.shape[1]), np.int32(n_markers),
            np.int32(first_diagnostics_idx), np.int32(mu_idx),
            np.int32(args_derham.pn[0]), np.int32(args_derham.pn[1]), np.int32(args_derham.pn[2]),
            tn1, np.int32(tn1.shape[0]),
            tn2, np.int32(tn2.shape[0]),
            tn3, np.int32(tn3.shape[0]),
            np.int32(args_derham.starts[0]), np.int32(args_derham.starts[1]), np.int32(args_derham.starts[2]),
            abs_B0, np.int32(abs_B0.shape[1]), np.int32(abs_B0.shape[2]),
            PBb, np.int32(PBb.shape[1]), np.int32(PBb.shape[2]),
        ),
    )
...
def eval_guiding_center_from_6d_gpu(
    ...
    _get_gc_from_6d_kernel()(
        ...
        (
            ...
            tn3, np.int32(tn3.shape[0]),
            np.int32(args_derham.starts[0]), np.int32(args_derham.starts[1]), np.int32(args_derham.starts[2]),
            b21, np.int32(b21.shape[1]), np.int32(b21.shape[2]),
            b22, np.int32(b22.shape[1]), np.int32(b22.shape[2]),
            b23, np.int32(b23.shape[1]), np.int32(b23.shape[2]),
            absB, np.int32(absB.shape[1]), np.int32(absB.shape[2]),
        ),
    )
