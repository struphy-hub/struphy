...
def sort_by_num_threads(bm):
    ...
    for key, value in sorted_arrays.items():
        sorted_indices = sorted(range(len(value['num_threads'])), key=lambda k: value['num_threads'][k])
...
def add_plot_flop(mfig,gflops,label = None, linestyle='-', color=None, theoretical_max_gflops=3072.0,xmax=1e3):
...
def add_plot_diagonal(mfig,bandwidth_GBps,label="",ymax=1e4,operational_intensity_FLOPpMB=np.arange(0,1000,1)):
        ...
        specific_x = np.interp(specific_y, max_performance_GFLOP, operational_intensity_FLOPpMB)
        mfig.axs.text(specific_x, specific_y, f"{round(bandwidth_GBps)} GB/s {label}", ha='left', va='bottom',rotation=45,color=line.get_color(),fontsize=8)
    
def get_likwid_value(filename, df_index, metric = 'Operational intensity [FLOP/Byte]', column_name = 'HWThread 0'):
    table_dict = {}; xvec = []; yvec = []
    dfs = read_likwid_output(filename)
    if len(dfs) >= 3:
        return get_metric_value(dfs[df_index], metric = metric, column_name = column_name)
    return 0
def get_average_val(path, metric1 = 'Operational intensity [FLOP/Byte]', metric2 = 'DP [MFLOP/s]', column_name = 'HWThread 0'):
    table_dict = {}; xvec = []; yvec = []
    ...
    for name,dfs in sorted(table_dict.items()):
        ...
        if len(dfs) >= 3:
            ...
            if x * y == 0:
                break
            xvec.append(x); yvec.append(y)
    #print('xvec', xvec, 'yvec', yvec)
    xvec = np.array(xvec); yvec = np.array(yvec)
    #print('xvec', xvec, 'yvec', yvec)
...
def get_maximum(path,df_index=-1,metric='DP [MFLOP/s] STAT', column_name = 'Sum'):
    ...
    for filepath in glob.glob(path):
        #print(filepath)
        val = max(val, get_likwid_value(filepath, df_index=-1, metric = metric, column_name = 'Sum'))
    return val
...
def get_roofline_point(path):
    #print(path)
    dp_MFLOPps     = get_maximum(path,df_index=-1,metric='DP [MFLOP/s] STAT', column_name = 'Sum')
    dp_GFLOPps     = dp_MFLOPps * 1e-3
    
    bandwidth_MBps = get_maximum(path,df_index=-1,metric='Memory bandwidth [MBytes/s] STAT', column_name = 'Sum')
