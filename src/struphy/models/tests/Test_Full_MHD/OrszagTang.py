...
def check_mhd_scalar_diagnostics(
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No HDF5 files found in {os.path.join(output_path, 'data')!r}.",
        )
    ...
    if hdf5_path is None:
        raise FileNotFoundError(
            "No HDF5 file containing time and scalar diagnostics was found.",
        )

    print(
        f"Reading scalar diagnostics from:\n{hdf5_path}",
    )
    ...
    for name, values in histories.items():
        assert values.size == expected_size, (
            f"{name} has {values.size} entries, but the time history has {expected_size} entries."
        )

        assert np.all(np.isfinite(values)), f"Non-finite values found in {name}."
    ...
    print(
        f"Div-div alpha                  : {ALPHA_DIVDIV:.12e}",
    )
    ...
    print(
        f"First saved time              : {time_history[0]:.12e}",
    )

    print(
        f"Last saved time               : {time_history[-1]:.12e}",
    )

    print("\nDiscrete regularized energy:")

    print(
        f"Initial total energy          : {total_energy[0]:.16e}",
    )

    print(
        f"Final total energy            : {total_energy[-1]:.16e}",
    )

    print(
        f"Maximum relative energy drift : {energy_drift:.12e}",
    )

    print(
        f"Signed final energy change    : {final_energy_change:.12e}",
    )

    print("\nEnergy-definition consistency:")

    print(
        f"Maximum component-sum error   : {maximum_component_sum_error:.12e}",
    )

    print(
        f"Relative component-sum error  : {relative_component_sum_error:.12e}",
    )

    print("\nConserved quantities:")

    print(
        f"Maximum relative mass drift   : {mass_drift:.12e}",
    )

    print(
        f"Signed final mass change      : {final_mass_change:.12e}",
    )

    print(
        f"Maximum relative entropy drift: {entropy_drift:.12e}",
    )

    print(
        f"Signed final entropy change   : {final_entropy_change:.12e}",
    )

    print(
        f"Maximum div(B) diagnostic     : {maximum_div_b:.12e}",
    )

    print("\nEnergy exchange:")

    print(
        f"Relative kinetic variation    : {kinetic_variation:.12e}",
    )

    print(
        f"Relative magnetic variation   : {magnetic_variation:.12e}",
    )

    print(
        f"Thermodynamic variation       : {thermodynamic_variation:.12e}",
    )

    # Structural invariants of the nondissipative formulation.
    assert mass_drift < 1.0e-10, f"Relative mass drift is too large: {mass_drift:.12e}."

    assert entropy_drift < 1.0e-10, f"Relative total-entropy drift is too large: {entropy_drift:.12e}."

    assert maximum_div_b < 1.0e-10, f"The magnetic-divergence diagnostic is too large: {maximum_div_b:.12e}."

    if energy_tolerance is not None:
        assert energy_drift < energy_tolerance, (
            f"Relative energy drift {energy_drift:.6e} exceeds the tolerance {energy_tolerance:.6e}."
        )
    ...
    *,
    ...
    if with_viscosity:
        model.propagators.variat_viscous.options = model.propagators.variat_viscous.Options(
            model="full", with_regularization=False, mu_a=2 * (1 / 32) ** 2
        )
        model.propagators.variat_resist.options = model.propagators.variat_resist.Options(eta_a=2 * (1 / 32) ** 2)
    ...
    try:
        ...
        if rank == 0:
            ...
            jacobian = simulation.domain.jacobian_det(
            ...
            print(
                f"Density range : [{minimum_density:.12e}, {maximum_density:.12e}]",
            )

            print(
                f"Pressure range: [{minimum_pressure:.12e}, {maximum_pressure:.12e}]",
            )
            ...
            assert diagnostics["kinetic_variation"] > 1.0e-3

            assert diagnostics["magnetic_variation"] > 1.0e-3
            ...
            if do_plot:
                x = simulation.grids_phy[0][:, :, 0]

                y = simulation.grids_phy[1][:, :, 0]
        ...
        if rank == 0 and cleanup and os.path.exists(test_folder):
            shutil.rmtree(test_folder)
...
if __name__ == "__main__":
    _run_orszag_tang(do_plot=True, cleanup=False, with_viscosity=False)
