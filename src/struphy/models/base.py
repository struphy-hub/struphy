...
class StruphyModel(metaclass=ABCMeta):
    ...
    def __init__(self, params, comm=None, inter_comm=None):
        ...
        # compute model units
        self._units, self._equation_params = self.model_units(self.params, verbose=True, comm=self._comm)

        # create domain, equilibrium
        self._domain, self._mhd_equil = setup_domain_and_equil(params, units=self.units)
        ...
        if comm.Get_rank() == 0:
            ...
            print(
                ...
                "{0} ({1:4.2e} s)".format(params["time"]["dt"], params["time"]["dt"] * self.units["t"]),
            )
            print(
                f"final time:".ljust(25),
                "{0} ({1:4.2e} s)".format(params["time"]["Tend"], params["time"]["Tend"] * self.units["t"]),
            )
        ...
        self._mass_ops = WeightedMassOperators(
            self.derham, self.domain, eq_mhd=self.mhd_equil, eq_braginskii=self.braginskii_equil
        )
        ...
        Propagator.basis_ops = BasisProjectionOperators(self.derham, self.domain, eq_mhd=self.mhd_equil)
    ...
    def options(cls):
        ...
        for prop, vars in cls.propagators_dct().items():
            ...
            cls.add_option(species=species, option=prop, dct=dct)

        return dct

    @classmethod
    def add_option(cls, species: str | list, option, dct: dict, *, key=None):
    ...
    def update_markers_to_be_saved(self):
        ...
        for val in self.kinetic.values():
            ...
            if n_mks_save > 0:
                markers_on_proc = np.logical_and(obj.markers[:, -1] >= 0.0, obj.markers[:, -1] < n_mks_save)
            ...
            with ProfileRegion("initialize_em_fields"):
                for key, val in self.em_fields.items():
                    ...
                    if "params" in key:
                        ...
                        obj.initialize_coeffs(domain=self.domain, mhd_equil=self.mhd_equil)
                ...
                for species, val in self.fluid.items():
                    ...
                    for variable, subval in val.items():
                        ...
                        if "params" in variable:
                            ...
                            obj = subval["obj"]
                            assert isinstance(obj, Derham.Field)
                            obj.initialize_coeffs(domain=self.domain, mhd_equil=self.mhd_equil, species=species)
                    ...
                    if self._comm_world_rank == 0:
                        ...
                        if "background" in val["params"]:
                            ...
                            if bckgr_type is None:
                                ...
                                raise NotImplemented(f"The type of initial perturbation must be null or str or list.")
                    ...
                    if not val["params"]["markers"]["loading"]["type"] == "restart":
                        ...
                        if obj.coords == "vpara_mu":
                            ...
                            obj.save_constants_of_motion(
                                epsilon=self.equation_params[species]["epsilon"], f_coords=obj.f0.coords, initial=True
                            )
            ...
            for species, val in self.fluid.items():
                ...
                for variable, subval in val.items():
                    ...
                    if "params" in variable:
                        ...
                        obj = subval["obj"]
                        assert isinstance(obj, Derham.Field)
                        obj.initialize_coeffs_from_restart_file(data.file, species)
            ...
            if "params" in key:
                ...
                if val["save_data"]:
                    ...
                    if isinstance(obj.vector_stencil, StencilVector):
                        data.add_data({key_field: obj.vector_stencil._data})

                    else:
                        for n in range(3):
                            key_component = key_field + "/" + str(n + 1)
                            data.add_data({key_component: obj.vector_stencil[n]._data})
                ...
                if isinstance(obj.vector_stencil, StencilVector):
                    data.add_data({key_field_restart: obj.vector_stencil._data})
                else:
                    for n in range(3):
                        key_component_restart = key_field_restart + "/" + str(n + 1)
                        data.add_data({key_component_restart: obj.vector_stencil[n]._data})
    ...
    def model_units(cls, params, verbose=False, comm=None):
        ...
        if verbose and rank == 0:
            ...
            print("\nUNITS:")
            print(f"Unit of length:".ljust(25), "{:4.3e}".format(units["x"]) + " m")
            print(f"Unit of time:".ljust(25), "{:4.3e}".format(units["t"]) + " s")
            print(f"Unit of velocity:".ljust(25), "{:4.3e}".format(units["v"]) + " m/s")
            print(f"Unit of magnetic field:".ljust(25), "{:4.3e}".format(units["B"]) + " T")

            if A_bulk is not None:
                print(f"Unit of particle density:".ljust(25), "{:4.3e}".format(units["n"]) + " m⁻³")
                print(f"Unit of mass density:".ljust(25), "{:4.3e}".format(units["rho"]) + " kg/m³")
                print(f"Unit of pressure:".ljust(25), "{:4.3e}".format(units["p"] * 1e-5) + " bar")
                print(f"Unit of current density:".ljust(25), "{:4.3e}".format(units["j"]) + " A/m²")
...
Available options stand in lists as dict values.\nThe first entry of a list denotes the default value.'
    ...
    @classmethod
    def generate_default_parameter_file(cls, file: str = None, save: bool = True, prompt: bool = True):
        ...
        parameters["kinetic"]["species_name"]["markers"]["loading"].pop("moments")
        ...
        for keys in bckgr_params_scalar:
            ...
            parameters["fluid"]["species_name"]["background"][bckgr_type].pop(keys)
        ...
        for keys in pert_params_scalar:
            ...
            parameters["fluid"]["species_name"]["perturbation"][pert_type].pop(keys)
            parameters["fluid"]["species_name"]["perturbation"][pert_type][keys] = {}
            parameters["kinetic"]["species_name"]["perturbation"][pert_type].pop(keys)
            parameters["kinetic"]["species_name"]["perturbation"][pert_type][keys] = {}

        # standard Maxwellians
        maxw_name = {"6D": "Maxwellian3D", "5D": "GyroMaxwellian2D", "4D": "Maxwellian1D", "3D": "Constant"}

        moms = {"6D": [0.0, 0.0, 0.0, 1.0, 1.0, 1.0], "5D": [0.0, 0.0, 1.0, 1.0], "4D": [0.0, 1.0], "3D": []}
        ...
        if "options" in cls.options()["em_fields"]:
            ...
            d_default = descend_options_dict(cls.options()["em_fields"]["options"], d_opts["em_fields"])
        ...
        if len(cls.species()["fluid"]) > 0:
            ...
            for name, dct in cls.species()["fluid"].items():
                ...
                if name in cls.options()["fluid"]:
                    ...
                    # create the default options parameters
                    d_default = descend_options_dict(cls.options()["fluid"][name]["options"], d_opts["fluid"][name])
        ...
        if len(cls.species()["kinetic"]) > 0:
            ...
            for name, space in cls.species()["kinetic"].items():
                ...
                if name in cls.options()["kinetic"]:
                    ...
                    # create the default options parameters
                    d_default = descend_options_dict(cls.options()["kinetic"][name]["options"], d_opts["kinetic"][name])

                    parameters["kinetic"][name]["options"] = d_default

                # set the correct names in the parameter file
                dim = space[-2:]
                parameters["kinetic"][name]["background"] = {"type": maxw_name[dim], maxw_name[dim]: {"n": 0.05}}
    ...
    def _allocate_variables(self):
            ...
            for key, val in self.em_fields.items():
                ...
                if "params" in key:
                    ...
                    val["obj"] = self.derham.create_field(
                        key, val["space"], bckgr_params=bckgr_params, pert_params=pert_params
                    )
                ...
                for variable, subval in val.items():
                    ...
                    if "params" in variable:
                        ...
                        subval["obj"] = self.derham.create_field(
                            variable, subval["space"], bckgr_params=bckgr_params, pert_params=pert_params
                        )
        ...
        if "kinetic" in self.params:
            ...
            for species, val in self.kinetic.items():
                ...
                if n_markers > 0:
                    val["kinetic_data"] = {}
                    val["kinetic_data"]["markers"] = np.zeros((n_markers, obj.markers.shape[1]), dtype=float)
                ...
                if "f" in val["params"]["save_data"]:
                    ...
                    if len(slices) > 0:
                        for i, sli in enumerate(slices):
                            ...
                            for j in range(dims):
                                val["bin_edges"][sli] += [
                                    np.linspace(ranges[i][j][0], ranges[i][j][1], n_bins[i][j] + 1)
                                ]
                            val["kinetic_data"]["f"][sli] = np.zeros(n_bins[i], dtype=float)
                            val["kinetic_data"]["df"][sli] = np.zeros(n_bins[i], dtype=float)
        ...
        if "diagnostics" in self.params:
            ...
            for key, val in self.diagnostics.items():
                ...
                if "params" in key:
                    ...
                    val["obj"] = self.derham.create_field(key, val["space"], bckgr_params=None, pert_params=None)
    ...
    def _print_plasma_params(self, verbose=True):
        ...
        # compute model units
        units, equation_params = self.model_units(self.params, verbose=False, comm=self.comm)
        ...
        if verbose:
            print("\nPLASMA PARAMETERS:")
            print(f"Plasma volume:".ljust(25), "{:4.3e}".format(plasma_volume) + units_affix["plasma volume"])
            print(f"Transit length:".ljust(25), "{:4.3e}".format(transit_length) + units_affix["transit length"])
            print(f"Avg. magnetic field:".ljust(25), "{:4.3e}".format(magnetic_field) + units_affix["magnetic field"])
            print(f"Max magnetic field:".ljust(25), "{:4.3e}".format(B_max) + units_affix["magnetic field"])
            print(f"Min magnetic field:".ljust(25), "{:4.3e}".format(B_min) + units_affix["magnetic field"])
        ...
        if len(self.fluid) > 0:
            ...
            for species, val in self.fluid.items():
                ...
                pparams[species]["density"] = (
                    np.mean(self.mhd_equil.n0(eta1, eta2, eta3) * np.abs(det_tmp))
                    * units["x"] ** 3
                    / plasma_volume
                    * units["n"]
                )
                ...
                pparams[species]["pressure"] = (
                    np.mean(self.mhd_equil.p0(eta1, eta2, eta3) * np.abs(det_tmp))
                    * units["x"] ** 3
                    / plasma_volume
                    * units["p"]
                    * 1e-5
        ...
        if len(self.kinetic) > 0:
            ...
            eta1mg, eta2mg, eta3mg = np.meshgrid(eta1, eta2, eta3, indexing="ij")
            ...
            for species, val in self.kinetic.items():
                ...
                if tmp.coords == "constants_of_motion":
                    ...
                    for k in range(len(vths)):
                        vth += [vths[k] * np.abs(det_tmp) * units["x"] ** 3 / plasma_volume * units["v"]]
                    thermal_speed = 0.0
        ...
        for species in pparams:
            ...
            # thermal speed (m/s)
            pparams[species]["v_th"] = np.sqrt(pparams[species]["kBT"] * 1e3 * e / pparams[species]["mass"])
        ...
        if verbose:
            ...
            for species, ch in pparams.items():
                ...
                for kinds, vals in ch.items():
                    print(kinds.ljust(25), "{:+4.3e}".format(vals), units_affix[kinds])
                print("------------------------------------")
