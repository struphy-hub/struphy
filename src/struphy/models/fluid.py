...
class ViscoresistiveMHD(StruphyModel):
    ...
    def __init__(self, params, comm, clone_config=None):
        ...
        from struphy.feec.variational_utilities import MassMatrix_density, InternalEnergyEvaluator
        from struphy.polar.basic import PolarVector

        # initialize base class
        super().__init__(params, comm=comm, clone_config=clone_config)

        self.WMM = MassMatrix_density(self.derham, self.mass_ops, self.domain, "H1vec")
...
class ViscoresistiveMHD_with_p(StruphyModel):
    ...
    def __init__(self, params, comm, clone_config=None):
        from struphy.feec.projectors import L2Projector
        from struphy.feec.variational_utilities import MassMatrix_density
        from struphy.polar.basic import PolarVector
...
class ViscoresistiveMHD_with_q(StruphyModel):
    ...
    def __init__(self, params, comm, clone_config=None):
        ...
        self._tmp_m1 = self.derham.Vh_pol["v"].zeros()
        self._tmp_wb2 = self.derham.Vh_pol["2"].zeros()
        self._tmp_wq3 = self.derham.Vh_pol["3"].zeros()
        self._tmp_div_B = self.derham.Vh_pol["3"].zeros()
        self._tmp_w_div_B = self.derham.Vh_pol["3"].zeros()
        tmp_dof = self.derham.Vh_pol["3"].zeros()
        projV3 = L2Projector("L2", self.mass_ops)
...
class ViscoresistiveLinearMHD_Hdiv(StruphyModel):
    r"""Linear visco-resistive MHD equations, with the q variable (square root of the pressure), discretized with a variational method.
    ...
    def species():
        dct = {"em_fields": {}, "fluid": {}, "kinetic": {}}
        dct["em_fields"]["b2"] = "Hdiv"
        dct["fluid"]["mhd"] = {"rho3": "L2", "q3": "L2", "u2": "Hdiv"}
        return dct
    ...
    def propagators_dct():
        return {
            propagators_fields.VariationalDensityEvolve: ["mhd_rho3", "mhd_u2"],
            propagators_fields.VariationalQBEvolve: ["mhd_q3", "b2", "mhd_u2"],
            # propagators_fields.VariationalViscosity: ["mhd_q3", "mhd_uv"],
            # propagators_fields.VariationalResistivity: ["mhd_q3", "b2"],
        }
    ...
    def __init__(self, params, comm, clone_config=None):
        ...
        self.WMM = MassMatrix_density(self.derham, self.mass_ops, self.domain, "Hdiv")
        ...
        # temporary vectors for scalar quantities
        self._tmp_m1 = self.derham.Vh_pol["2"].zeros()
        self._tmp_wb2 = self.derham.Vh_pol["2"].zeros()
        self._tmp_wq3 = self.derham.Vh_pol["3"].zeros()
        self._tmp_div_B = self.derham.Vh_pol["3"].zeros()
        self._tmp_w_div_B = self.derham.Vh_pol["3"].zeros()
    ...
    def update_scalar_quantities(self):
        # Update mass matrix
        m1 = self.WMM.massop.dot(self.pointer["mhd_u2"], out=self._tmp_m1)

        en_U = self.pointer["mhd_u2"].dot(m1) / 2
        self.update_scalar("en_U", en_U)
        ...
        wb2 = self._mass_ops.M2.dot(self.pointer["bt2"], out=self._tmp_wb2)
        en_mag2 = wb2.dot(self.derham.extraction_ops['2'].dot(self.projected_equil.b2))
        # self.update_scalar("en_mag_2", en_mag2)
        ...
        wq3 = self._mass_ops.M3.dot(self.pointer["qt3"], out=self._tmp_wq3)
        en_th_2 = wq3.dot(self.derham.extraction_ops['3'].dot(self.projected_equil.q3)) * 2 / (self._gamma - 1)
        # self.update_scalar("en_thermo_2", en_th_2)
