...
class Maxwellian3D(Maxwellian):
    ...
    def default_maxw_params(cls):
        ...
        return {"n": 1.0, "u1": 0.0, "u2": 0.0, "u3": 0.0, "vth1": 1.0, "vth2": 1.0, "vth3": 1.0}

    def __init__(
        self,
        maxw_params: dict = {"n": 1.0, "u1": 0.0, "u2": 0.0, "u3": 0.0, "vth1": 1.0, "vth2": 1.0, "vth3": 1.0},
        ...
        if maxw_params is not None:
            ...
            self._maxw_params = set_defaults(maxw_params, self.default_maxw_params())

        # check if mhd or braginskii is needed
        for key, val in self.maxw_params.items():
            if val == "mhd":
                assert isinstance(mhd_equil, MHDequilibrium), f"MHD equilibrium must be passed to compute {key}."
        ...
        self._moment_factors = {"n": 1.0, "u": [1.0, 1.0, 1.0], "vth": [1.0, 1.0, 1.0]}
    ...
    def n(self, eta1, eta2, eta3):
        ...
        if eta1.ndim == 1:
            etas = [np.concatenate((eta1[:, None], eta2[:, None], eta3[:, None]), axis=1)]
        # assuming that input comes from meshgrid.
        elif eta1.ndim == 6:
            etas = (eta1[:, :, :, 0, 0, 0], eta2[:, :, :, 0, 0, 0], eta3[:, :, :, 0, 0, 0])
        else:
            etas = (eta1, eta2, eta3)
        ...
        if self.pert_params is not None and "n" in self.pert_params[self._pert_type]["comps"]:
            ...
            perturbation = getattr(perturbations, self._pert_type)(**n_pert_params)
        ...
        if self.pert_params is not None:
            ...
            for i, comp in enumerate(comps):
                if comp in self.pert_params[self._pert_type]["comps"]:
                    ...
                    perturbation = getattr(perturbations, self._pert_type)(**u_pert_params)
    ...
    def vth(self, eta1, eta2, eta3):
        ...
        if (
            ...
            tmp2 = np.sqrt(self.braginskii_equil.p0(*etas) / self.braginskii_equil.n0(*etas))
        ...
        if self.pert_params is not None:
            ...
            for i, comp in enumerate(comps):
                if comp in self.pert_params[self._pert_type]["comps"]:
                    ...
                    perturbation = getattr(perturbations, self._pert_type)(**vth_pert_params)
...
class GyroMaxwellian2D(Maxwellian):
    ...
    def __init__(
        self,
        maxw_params: dict = {"n": 1.0, "u_para": 0.0, "u_perp": 0.0, "vth_para": 1.0, "vth_perp": 1.0},
        pert_params: dict = None,
        ...
        # factors multiplied onto the defined moments n, u and vth (can be set via setter)
        self._moment_factors = {"n": 1.0, "u": [1.0, 1.0], "vth": [1.0, 1.0]}
    ...
    def n(self, eta1, eta2, eta3):
        ...
        # assuming that input comes from meshgrid.
            ...
            etas = (eta1[:, :, :, 0, 0], eta2[:, :, :, 0, 0], eta3[:, :, :, 0, 0])
        else:
            etas = (eta1, eta2, eta3)
    ...
    def u(self, eta1, eta2, eta3):
        ...
        if self.maxw_params["u_para"] == "braginskii" or self.maxw_params["u_perp"] == "braginskii":
            ...
            tmp_uv = self.braginskii_equil.uv(*etas) / self.braginskii_equil.n0(*etas)
        ...
        if self.maxw_params["u_perp"] == "mhd":
            raise NotImplementedError("A shift in v_perp is not yet implemented.")
...
class CanonicalMaxwellian(CanonicalMaxwellian):
    ...
    def __init__(
        ...
        self._moment_factors = {"n": 1.0, "vth": 1.0}
    ...
    def velocity_jacobian_det(self, eta1, eta2, eta3, energy):
        ...
        jacobian_det = np.sqrt(energy) * 2.0 * np.sqrt(2.0) / absB0
