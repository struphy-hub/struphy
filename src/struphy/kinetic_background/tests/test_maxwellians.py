...
def test_maxwellian_3d_uniform(Nel, show_plot=False):
    ...
    assert np.allclose(res, 2.0 / (2 * np.pi) ** (3 / 2) + 0 * e1, atol=10e-10), f"{res=},\n {2. / (2 * np.pi)**(3/2)}"
...
def test_maxwellian_3d_perturbed(Nel, show_plot=False):
    ...
    maxw_params = {"n": 2.0}
    pert_params = {"type": "ModesCos", "ModesCos": {"comps": {"n": "0"}, "ls": {"n": [mode]}, "amps": {"n": [amp]}}}
    ...
    pert_params = {"type": "ModesCos", "ModesCos": {"comps": {"u1": "0"}, "ls": {"u1": [mode]}, "amps": {"u1": [amp]}}}
    ...
    pert_params = {
        "type": "ModesCos",
        "ModesCos": {"comps": {"vth1": "0"}, "ls": {"vth1": [mode]}, "amps": {"vth1": [amp]}},
    }
    ...
    maxw_params = {"n": 0.0}
    pert_params = {"type": "ITPA_density", "ITPA_density": {"comps": {"n": "0"}, "n0": {"n": n0}, "c": {"n": c}}}
...
def test_maxwellian_2d_uniform(Nel, show_plot=False):
    ...
    res = maxwellian(*meshgrids).squeeze()
    assert np.allclose(
        res, 2.0 / (2 * np.pi) ** (1 / 2) * np.exp(-(0.01**2)) + 0 * e1, atol=10e-10
    ), f"{res=},\n {2. / (2 * np.pi)**(3/2)}"
...
def test_maxwellian_2d_perturbed(Nel, show_plot=False):
    ...
    pert_params = {
        "type": "ModesCos",
        "ModesCos": {"comps": {"u_para": "0"}, "ls": {"u_para": [mode]}, "amps": {"u_para": [amp]}},
    }
    ...
    pert_params = {
        "type": "ModesCos",
        "ModesCos": {"comps": {"u_perp": "0"}, "ls": {"u_perp": [mode]}, "amps": {"u_perp": [amp]}},
    }
    ...
    pert_params = {
        "type": "ModesCos",
        "ModesCos": {"comps": {"vth_para": "0"}, "ls": {"vth_para": [mode]}, "amps": {"vth_para": [amp]}},
    }
    ...
    pert_params = {
        "type": "ModesCos",
        "ModesCos": {"comps": {"vth_perp": "0"}, "ls": {"vth_perp": [mode]}, "amps": {"vth_perp": [amp]}},
    }
                ...
                for key_2, val_2 in inspect.getmembers(perturbations):
                    if inspect.isclass(val_2):
                        ...
                        pert_params = {
                            "type": key_2,
                            key_2: {
                                "comps": {"n": "0", "u_para": "0", "u_perp": "0", "vth_para": "0", "vth_perp": "0"},
                            },
                        }
...
def test_canonical_maxwellian_uniform(Nel, show_plot=False):
    ...
    maxw_params = {"n": {"type": "ITPA_density", "ITPA_density": {"n0": n0, "c": c}}, "vth": 1.0}
