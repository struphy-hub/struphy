    ...
    def __call__(self, P_init=None, out=None):
        ...
        for iteration in range(self._max_iter):
            ...
            if residual_norm < self._tol:
                return self._P, self._U, self._solverA._info

            # Step 3: Update potential P <- P - rho * R
            P -= self._rho * R

        # Return with info if maximum iterations reached
        print(f"P shape: {P.shape}, P type: {type(P)}")
        print(f"U shape: {U.shape}, U type: {type(U)}")
        print(f"R shape: {R.shape}, R type: {type(R)}")
        return U, P, {'converged': False, 'iterations': self._max_iter, 'residual_norm': residual_norm}
