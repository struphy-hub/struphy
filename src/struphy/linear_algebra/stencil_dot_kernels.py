def matvec_1d_kernel(mat: 'float[:, :]',
    ...
    for i1 in range(s_out, e_out):  # global row index
        ...
        for d1 in range(2*p_in + 1):
            val += mat[p_out*sh_out + i1_loc, d1] * \
                x[i1 + d1 - s_in + p_in*sh_in - p_in]

        out[p_out*sh_out + i1_loc] = val
                ...
                for d1 in range(2*p_in[0] + 1):
                    for d2 in range(2*p_in[1] + 1):
                        for d3 in range(2*p_in[2] + 1):
                            ...
                            val += mat[p_out[0]*sh_out[0] + i1_loc,
                                       p_out[1]*sh_out[1] + i2_loc,
                                       p_out[2]*sh_out[2] + i3_loc,
                                       d1, d2, d3] * x[i1 + d1 - s_in[0] + p_in[0]*sh_in[0] - p_in[0],
                                                       i2 + d2 - s_in[1] + p_in[1]*sh_in[1] - p_in[1],
                                                       i3 + d3 - s_in[2] + p_in[2]*sh_in[2] - p_in[2]]
                out[p_out[0]*sh_out[0] + i1_loc,
                    p_out[1]*sh_out[1] + i2_loc,
                    p_out[2]*sh_out[2] + i3_loc] = val
