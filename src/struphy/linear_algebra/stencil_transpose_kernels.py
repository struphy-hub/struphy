def transpose_1d_kernel(mat: 'float[:, :]',
    ...
    for i1 in range(s_out, e_out):  # global row index of matT = global column index of mat
        ...
        for d1 in range(2*p_in + 1):
            ...
            matT[p_out*sh_out + i1_loc, d1] = mat[p_in *
                                                  sh_in + j1_loc, p_out + i1 - j1]
...
def transpose_3d_kernel(mat: 'float[:, :, :, :, :, :]',
    ...
    #####################################
    #####################################
    # global row index of matT = global column index of mat
        ...
        for i2 in range(s_out[1], e_out[1]):
            ...
            for i3 in range(s_out[2], e_out[2]):
                ...
                for d1 in range(2*p_in[0] + 1):
                    ...
                    # local column index of matT = local row index of mat
                    j1_loc = j1 - s_in[0]
                    for d2 in range(2*p_in[1] + 1):
                        ...
                        for d3 in range(2*p_in[2] + 1):
                            # global column index of matT
                            j3 = i3 - p_in[2] + d3
                            # local column index of matT = local row index of mat
                            j3_loc = j3 - s_in[2]

                            matT[p_out[0]*sh_out[0] + i1_loc,
                                 p_out[1]*sh_out[1] + i2_loc,
                                 p_out[2]*sh_out[2] + i3_loc,
                                 d1, d2, d3] = mat[p_in[0]*sh_in[0] + j1_loc,
                                                   p_in[1]*sh_in[1] + j2_loc,
                                                   p_in[2]*sh_in[2] + j3_loc,
                                                   p_out[0] + i1 - j1,
                                                   p_out[1] + i2 - j2,
                                                   p_out[2] + i3 - j3]
