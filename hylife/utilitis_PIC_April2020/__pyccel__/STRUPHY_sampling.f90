...
use interface, only: inter_fqe2j2_fh_eq => fh_eq
use interface, only: inter_fqe2j2_g_sampling => g_sampling
use interface, only: inter_fqe2j2_fh_ini => fh_ini
implicit none
...
subroutine compute_weights_ini(particles, w0, g0, kind_map, params_map) 
  ...
  do ip = 0, np - 1_8, 1
    ...
    g0(ip) = inter_fqe2j2_g_sampling(xi1, xi2, xi3, vx, vy, vz)
    w0(ip) = inter_fqe2j2_fh_ini(xi1, xi2, xi3, vx, vy, vz, kind_map, &
      params_map)/g0(ip)
...
subroutine update_weights(particles, w0, g0, kind_map, params_map) 
  ...
  do ip = 0, np - 1_8, 1
    ...
    particles(ip, 6_8) = (-inter_fqe2j2_fh_eq(xi1, xi2, xi3, vx, vy, vz, &
