...
class sBase:
    ...
    def __init__(self, Nel, el_b, p, bc=False, use_hylife=True, use_cache=False):
        ...
        self.hylife_sbase = spline_space_1d(self.knots_N, p, bc) # Reverted.
        # Class constructor signature CHANGED in hylife's `devel_standard` branch!
        # The first parameter is now `Nel`, instead of the knot vector!
        # Now reverted.
        # self.hylife_sbase = spline_space_1d(Nel, p, bc)

        # Cache evaluations in memory.
        self.use_cache = use_cache
        self.cache = {
            'eval_s_hylife': {},
            'eval_s_1st_der_hylife': {},
        }
        if self.use_cache:
    ...
    def eval_s(self, s, coef):
        ...
        Notes
        -----
        Not implemented. Passthrough directly to `eval_s_hylife()`.
        """

        # logger.debug('sBase.eval_s()')

        return self.eval_s_hylife(s, coef)

    def eval_s_hylife(self, s, coef):
        """Evaluate B-spline at point `s` given control points `coef` using `hylife` class `spline_space_1d`.
        ...
        # logger.debug('sBase.eval_s_hylife()')

        if self.use_cache:
            param_hash = hash(str(s) + str(coef))
            if param_hash in self.cache['eval_s_hylife']:
                return self.cache['eval_s_hylife'][param_hash]

        # After `hylife` update, if `s` is a numpy array, `spline_space` will only process 1D numpy array!
        ...
        if isinstance(s, np.ndarray):
            ...
            if s.ndim == 1:
                ...
                #     N_eval_at_s[i] = self.hylife_sbase.evaluate_N(si, coef)

                # New implementation:
                # Transform (s, theta, zeta) into Cartesian (x, y, z) coordinates.
                N_eval_at_s = self.hylife_sbase.evaluate_N(s, coef)
            ...
            # `s` is a mesh grid. 
                ...
                if max(s.shape) == s.size:
                    ...
                    N_eval_at_s = self.hylife_sbase.evaluate_N(s.flatten(), coef).reshape(s.shape)
                    ...
                    if s.ndim == 2:
                        for i in range(s.shape[0]):
                            for j in range(s.shape[1]):
                                N_eval_at_s[i, j] = self.hylife_sbase.evaluate_N(s[i, j], coef)

                    elif s.ndim == 3:
                        for i in range(s.shape[0]):
                            for j in range(s.shape[1]):
                                for k in range(s.shape[2]):
                                    N_eval_at_s[i, j, k] = self.hylife_sbase.evaluate_N(s[i, j, k], coef)
        ...
        if self.use_cache:
            self.cache['eval_s_hylife'][param_hash] = N_eval_at_s
        return N_eval_at_s



    def eval_s_1st_der(self, s, coef):
        """Evaluate the first derivative of B-spline at point `s` given control points `coef` using `hylife` class `spline_space_1d`.
        ...
        Notes
        -----
        Not implemented. Passthrough directly to `eval_s_1st_der_hylife()`.
        """

        # logger.debug('sBase.eval_s_1st_der()')

        return self.eval_s_1st_der_hylife(s, coef)

    def eval_s_1st_der_hylife(self, s, coef):
        """Evaluate the first derivative of B-spline at point `s` given control points `coef` using `hylife` class `spline_space_1d`.
        ...
        # logger.debug('sBase.eval_s_1st_der_hylife()')

        if self.use_cache:
            param_hash = hash(str(s) + str(coef))
            if param_hash in self.cache['eval_s_1st_der_hylife']:
                return self.cache['eval_s_1st_der_hylife'][param_hash]
        ...
        if isinstance(s, np.ndarray):
            ...
            if s.ndim == 1:
                ...
                # for i, si in enumerate(s):
                #     dN_eval_at_s[i] = self.hylife_sbase.evaluate_dN(si, coef)

                # New implementation:
                # Transform (s, theta, zeta) into Cartesian (x, y, z) coordinates?
                dN_eval_at_s = self.hylife_sbase.evaluate_dN(s, coef)
            ...
            # `s` is a mesh grid. 
                ...
                if max(s.shape) == s.size:
                    ...
                    dN_eval_at_s = self.hylife_sbase.evaluate_dN(s.flatten(), coef).reshape(s.shape)
                ...
                # `s` is a dense meshgrid. Process each point as a scalar.
                    ...
                    if s.ndim == 2:
                        for i in range(s.shape[0]):
                            for j in range(s.shape[1]):
                                dN_eval_at_s[i, j] = self.hylife_sbase.evaluate_dN(s[i, j], coef)

                    elif s.ndim == 3:
                        for i in range(s.shape[0]):
                            for j in range(s.shape[1]):
                                for k in range(s.shape[2]):
                                    dN_eval_at_s[i, j, k] = self.hylife_sbase.evaluate_dN(s[i, j, k], coef)
        ...
        if self.use_cache:
            self.cache['eval_s_1st_der_hylife'][param_hash] = dN_eval_at_s
        return dN_eval_at_s
