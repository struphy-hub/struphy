...
import hylife.utilitis_FEEC.kernels_projectors_local as ker_loc
...
class projectors_local_1d():
    ...
    def __init__(self, T, p, bc):
        ...
        self.NbaseN    = self.Nel + self.p - self.bc*self.p             # number of basis functions (N)
        self.NbaseD    = self.NbaseN - (1 - self.bc)                    # number of basis functions (D)
        self.delta     = 1/self.Nel                                     # element length
        self.n_quad    = self.p + 1                                     # degree of quadrature rule
        self.quad_loc  = np.polynomial.legendre.leggauss(self.n_quad)   # quadrature points and weights in (-1, 1)
        
        
        # set interpolation and histopolation coefficients
        if self.bc == True:
            self.coeff_i = np.zeros((1, 2*(self.p - 1) + 1), dtype=float)
            ...
            self.coeff_i = np.zeros((2*(self.p - 1) + 1, 2*(self.p - 1) + 1), dtype=float)
            self.coeff_h = np.zeros((2*(self.p - 1) + 1, 2*self.p),           dtype=float)
        ...
        self.n_int          = 2*self.p - 1       # number of interpolation points
        ...
        self.int_loccof_N   = np.zeros((n_lambda_int, self.n_int_locbf_N), dtype=int) # index of non-vanishing coefficient (N)
        self.int_loccof_D   = np.zeros((n_lambda_int, self.n_int_locbf_D), dtype=int) # index of non-vanishing coefficient (D)
        ...
        if self.bc == False:
            ...
            self.pts, self.wts = bsp.quadrature_grid(np.unique(self.x_his.flatten()), self.quad_loc[0], self.quad_loc[1])
            ...
            # quadrature points and weights
            self.pts, self.wts = bsp.quadrature_grid(np.append(np.unique(self.x_his.flatten()%1.), 1.), self.quad_loc[0], self.quad_loc[1])
...
class projectors_local_3d:
    ...
    def PI_0(self, fun, *args):
        ...
        if fun == None:
            ...
            ker_loc_ini.kernel_eva(n_unique, x_int1, x_int2, x_int3, mat_f, kind_fun=args[0], kind_map=args[1], params=args[2])
        else:
    ...
    # projector on space V1 ([h, i, i], [i, h, i], [i, i, h])
    ...
    def PI_3(self, fun, *args):
        ...
        if fun == None:
            mat_f = np.zeros((n_unique[0], n_unique[1], n_unique[2]), dtype=float, order='F')
            ker_loc_ini.kernel_eva(n_unique, self.pts[0].flatten(), self.pts[1].flatten(), self.pts[2].flatten(), mat_f, kind_fun=args[0], kind_map=args[1], params=args[2])
        else:
            xx, yy, zz = np.meshgrid(self.pts[0].flatten(), self.pts[1].flatten(), self.pts[2].flatten(), indexing='ij')
            mat_f      = np.asfortranarray(fun(xx, yy, zz))  
            
        ker_loc.kernel_pi3_3d(self.NbaseD, self.p, self.coeff_h[0], self.coeff_h[1], self.coeff_h[2], self.coeffh_indices[0], self.coeffh_indices[1], self.coeffh_indices[2], self.x_his_indices[0], self.x_his_indices[1], self.x_his_indices[2], self.wts[0], self.wts[1], self.wts[2], mat_f.reshape(self.pts[0][:, 0].size, self.pts[0][0, :].size, self.pts[1][:, 0].size, self.pts[1][0, :].size, self.pts[2][:, 0].size, self.pts[2][0, :].size), lambdas)
