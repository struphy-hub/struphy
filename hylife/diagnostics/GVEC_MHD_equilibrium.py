...
# fig.subplots_adjust(top=0.8, wspace=0, hspace=None)
...
ax3_3D = fig.add_subplot(gs[1, 0:2], projection='3d')
...
IOTA = gvec.IOTA(eta1, eta2, eta3)
Q = 2 * np.pi / IOTA

img1 = ax1_3D.scatter(xs, ys, zs, c= P   , cmap=plt.cm.plasma, marker='.')
img2 = ax2_3D.scatter(xs, ys, zs, c=-IOTA, cmap=plt.cm.plasma, marker='.')
img4 = ax4_3D.scatter(xs, ys, zs, c=-1/Q , cmap=plt.cm.plasma, marker='.')

cbar1 = fig.colorbar(img1, ax=ax1_3D, shrink=0.5, pad=0.1, label='Pressure $P(s)$')
cbar2 = fig.colorbar(img2, ax=ax2_3D, shrink=0.5, pad=0.1, label='$[-1 \\times]$ Iota $\iota(s)$')
cbar4 = fig.colorbar(img4, ax=ax4_3D, shrink=0.5, pad=0.1, label='$[-1 \\times]$ Safety factor $q(s)$')

ax1_1D.plot(eta1[:,0,0], P[:,0,0]      , marker='.')
ax2_1D.plot(eta1[:,0,0], -IOTA[:,0,0]  , marker='.')
ax4_1D.plot(eta1[:,0,0], -1/IOTA[:,0,0], marker='.')

ax1_1D.set_ylabel('Pressure $P(s)$')
ax2_1D.set_ylabel('$[-1 \\times]$ Iota $\iota(s)$')
ax4_1D.set_ylabel('$[-1 \\times]$ Safety factor $q(s)$')
