import numpy as np
from numba import jit


def find_spans(T, p, x):
    """
    Determines the knot span index at locations x, given the B-Splines' knot
    vector and polynomial degree.

    For a degree p, the knot span index i identifies the indices [i-p:i] of all
    p+1 non-zero basis functions at given locations x.

    Parameters
    ----------
    T : np.array
        Knot vector.

    p : int
        Spline degree.

    x : float
        Locations of interest.

    Returns
    -------
    span : int
        Knot span index.

    """
    
    # Knot index at left/right boundary
    low  = p
    high = len(T) - 1 - degree

    # Check if point is exactly on left/right boundary, or outside domain
    if x <= T[low ]: return low
    if x >= T[high]: return high - 1

    # Perform binary search
    span = (low + high)//2
    while x < knots[span] or x >= knots[span + 1]:
        if x < knots[span]:
            high = span
        else:
            low  = span
        span = (low + high)//2

    return span



@jit(nopython=True, parallel=True)
def basis_funs(T, p, x, span):
    """
    Compute the non-vanishing B-splines at location x, given the knot sequence,
    polynomial degree and knot span. See Algorithm A2.2 in [1].

    Parameters
    ----------
    knots : array_like
        Knots sequence.

    degree : int
        Polynomial degree of B-splines.

    x : float
        Evaluation point.

    span : int
        Knot span index.

    Results
    -------
    values : numpy.ndarray
        Values of p+1 non-vanishing B-Splines at location x.

    Notes
    -----
    The original Algorithm A2.2 in The NURBS Book [1] is here slightly improved
    by using 'left' and 'right' temporary arrays that are one element shorter.

    """
    left   = np.empty((p, len(x)))
    right  = np.empty((p, len(x)))
    values = np.empty((p + 1, len(x)))

    values[0] = 1.0
    
    for j in range(p):
        left [j] = x - T[span - j]
        right[j] = T[span + 1 + j] - x
        saved    = np.zeros(len(x))
        for r in range(j + 1):
            temp      = values[r]/(right[r] + left[j - r])
            values[r] = saved + right[r]*temp
            saved     = left[j - r] * temp
        values[j + 1] = saved

    return values